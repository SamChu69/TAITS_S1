...
1. 資料層蒐集（News/Fundamental/Technical/Derivatives/Flows）
2. Explore：雙軌並行生成候選池（消息/基本面軌、技術面軌）
3. Cross-Market & Regime：期貨/選擇權作為指標生成市場狀態
4. Veto Stack：Regime → Risk → Manipulation 依序否決或降級
5. Exploit：生成可執行的進出場計畫（含倉位、止損、停利、時間窗口）
6. Execution（v0.1 模擬）：成交品質/滑價/撤單原因寫入日誌
7. Post-Trade：績效與否決回饋，週期性調整策略線權重與門檻
