...
# Addendum 2025-12-28-E｜Only-Add：Latest Mapping Override Patch（doc_key → Latest Artifact）× Freeze v1.0

> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220.md（doc_key：DOCUMENT_INDEX）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 稽核對位：VERSION_AUDIT:VA-METADATA_FIX-20251227-0021（建議同步補登 VERSION_AUDIT｜Appendix A｜METADATA_FIX Ledger）  
> 目的：針對 2025-12-28 新增之 machine-readable mapping（Addendum 2025-12-28-D）與 DEPLOY_OPS Trace/Evidence Contract（Addendum 2025-12-28），以 append-only 方式追加「latest 映射覆寫補丁」，供工具鏈在載入時以本 patch 覆蓋先前映射，降低選錯檔風險。
...
2) 工具鏈套用順序：先讀 Authoritative Index → 再讀 mapping（C/D）→ 最後套用本 override patch（E）。  
3) 若 override patch 與 repo 落盤仍不一致：以 repo 落盤為準，並以 Only-Add 追加下一版 patch。

---

## E2. Override Table（Human-readable）

| doc_key | latest_artifact_filename | change_summary | audit_anchor_fix_id |
|---|---|---|---|
| `DOCUMENT_INDEX` | `TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228D_FINAL.md` | Only-Add｜Latest Mapping 機器可讀附錄（YAML/JSON）已納入（本檔即為 latest） | `VA-METADATA_FIX-20251227-0019` |
| `DEPLOY_OPS` | `TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219__ADDENDUM_20251228_FINAL.md` | Only-Add｜Trace ID / Evidence Chain 欄位貫穿規範已納入 | `VA-METADATA_FIX-20251227-0020` |
| `VERSION_AUDIT` | `TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219__ADDENDUM_20251228B_FINAL.md` | Only-Add｜Ledger 補登（0019/0020）延伸批次 | `VA-METADATA_FIX-20251227-0021` |

---

## E3. YAML Override Patch（machine-readable）
...
override:
  - doc_key: DOCUMENT_INDEX
    latest_artifact: "TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228D_FINAL.md"
    change_summary: "Only-Add｜Latest Mapping 機器可讀附錄（YAML/JSON）已納入（本檔即為 latest）"
    fix_id: VA-METADATA_FIX-20251227-0019
  - doc_key: DEPLOY_OPS
    latest_artifact: "TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219__ADDENDUM_20251228_FINAL.md"
    change_summary: "Only-Add｜Trace ID / Evidence Chain 欄位貫穿規範已納入"
    fix_id: VA-METADATA_FIX-20251227-0020
  - doc_key: VERSION_AUDIT
    latest_artifact: "TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219__ADDENDUM_20251228B_FINAL.md"
    change_summary: "Only-Add｜Ledger 補登（0019/0020）延伸批次"
    fix_id: VA-METADATA_FIX-20251227-0021
```

---

## E4. JSON Override Patch（machine-readable）
...
（Addendum 2025-12-28-E｜Only-Add｜Freeze v1.0 完）
