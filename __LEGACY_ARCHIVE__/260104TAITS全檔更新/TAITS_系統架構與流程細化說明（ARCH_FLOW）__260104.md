# TAITS_系統架構與流程細化說明（ARCH_FLOW）__260104

doc_key：ARCH_FLOW  
治理等級：B（治理／制度級｜架構與流程細化說明）  
...
平行參照：FULL_ARCH／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／VERSION_AUDIT／UI_SPEC／DATA_SOURCES／LOCAL_ENV／DEPLOY_OPS  
...
- scope_files_output: TAITS_系統架構與流程細化說明（ARCH_FLOW）__260104.md
- scope_files_source: TAITS_系統架構與流程細化說明（ARCH_FLOW）__260102.md
...
3) 置頂補強全局定錨（人類主權＋L9/L10/L11 口徑），用於跨文件一致化（不改寫原流程細化內容，只固定入口口徑）。  
4) 若原文件尾端存在既有稽核留痕段落，已以本檔末端之單一稽核區塊取代，避免多份留痕並存。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 5256d7884304e55c7602eb2a4274185afdb00477a0fcc0009a29edfee58d67fa

## D. Audit Hand-off（裁決承接）
- change_id: AF-FUSION-260104-0010
- authority_basis: HFI（人類最高決策者明確命令｜scope=ARCH_FLOW｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（並對齊 MASTER_CANON）
- downstream_notes:
  - 本文件需與 FULL_ARCH 的模組邊界一致；若後續 FULL_ARCH 更新模組責任分工，ARCH_FLOW 必須同步對齊以避免矛盾。
  - 若 DOCUMENT_INDEX 之 ARCH_FLOW 條目仍指向舊日期版本（__260102），需同步更新映射至 __260104，以避免載入混讀。
- rationale: 以單一正文消除補片式混讀來源，同時維持流程細化規格之最大完備與可追溯性，作為工程落地與 L11 回放閉環的一致化基線。
