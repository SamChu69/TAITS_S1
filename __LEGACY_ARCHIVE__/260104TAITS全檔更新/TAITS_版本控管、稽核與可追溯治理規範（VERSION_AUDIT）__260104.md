# TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260104
...
> 已移除 補充條目/補充資料集/最大完備＋累積式更新 等差分式標籤，以避免新舊混讀；其內容語義由本文件正文與 DOCUMENT_INDEX 裁決承接。
...
# 補充條目 2025-12-28｜台帳新增：GLOBAL_ALIGNMENT_PATCH（MASTER_CANON 對齊全域差分）｜（歷史狀態記錄）
...
| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0002 | 2025-12-31 | BATCH-20251231-P1-REFERENCE-ALIAS-EXPANSION | Reference Alias Map（legacy refs → physical） | 補登 90 筆 legacy 引用檔名解析（含舊版版本戳、README.md、工程路徑、舊 HISTORY 檔名、樣板檔名）。對應 DOCUMENT_INDEX｜補充條目 2025-12-31｜P1（P1.1–P1.3）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
> 生效狀態：GOVERNANCE_STATE = （歷史狀態記錄）  
> 上位裁決：DOCUMENT_INDEX（A+）→ MASTER_ARCH（A）→ MASTER_CANON（A）＋AI_GOV（A+）  
> 目的：補登 P1B 之 Reference Alias Extension 事件，使 Postcheck v4 新發現之 legacy 引用（多為 HISTORY/inline歷史參考/工程路徑）可回放解析。
...
| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0003 | 2025-12-31 | BATCH-20251231-P1B-REFERENCE-ALIAS-EXTENSION | Reference Alias Extension（v4 補登） | 補登 21 筆 legacy 引用解析（多為 HISTORY/inline歷史參考/工程路徑）。對應 DOCUMENT_INDEX｜補充條目 2025-12-31｜P1B（P1B.1–P1B.3）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
- scope_files_modified: TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260104.md
...
4) 重新排版整序：移除多次生成之「治理補強（已整合為正文）」與「版本戳記」差分式段落；將 A1–A4、J1–J3、G1–G5、Ledger 等零散章節整併為連續章節（第 15–19 章），提升引用定位穩定性。  
5) 融合收錄 S1 歷史台帳條目資料集：將 S1 檔案之 補充資料集 台帳批次收錄為本檔 §20，並移除 補充條目/補充資料集/最大完備＋累積式更新 等差分式標籤以避免新舊混讀（內容保留、語義由正文與 DOCUMENT_INDEX 承接）。
...
> 為避免新舊混讀：本區塊不參與 VERSION_AUDIT 正文裁決；正文以本檔案開頭至本區塊前之內容為準。

## A. Scope（適用範圍）

- scope_doc_key: VERSION_AUDIT
- scope_files_output: TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260104.md
- scope_files_source:
  - TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260102.md
...
- anchors_enforced:
  ...
  - 留痕與正文分離（本次更新之 Changelog/Hash/Scope/Audit Hand-off 不混入正文）

## B. Changelog（變更清單）

1) 版本日期與檔名統一更新為 2026-01-04（Asia/Taipei）／__260104。  
2) 形成「單一正確正文」：將正文中可能造成新舊混讀之舊式補片標籤與附錄式標記，改寫為「歷史參考／HISTORY」之中性承接措辭，避免讀者誤將歷史補片視為現行正文入口。  
3) 保留既有制度內容之最大完備性：未刪減任何未被新內容明確取代之有效條款；僅進行必要的語義去混讀與字串層級的標記清理（不做摘要縮水）。  
4) 本次更新之留痕（Changelog／Hash Manifest／Scope／Audit Hand-off）獨立於正文末端，以符合「留痕不得混入正文」之規範。

## C. Hash Manifest（指紋清單）

- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: c7fb437a0f3e787465495c5429eab7e3c8a44cdc7cfaecaad54e6c9cf2ade8f8

## D. Audit Hand-off（裁決承接）

- change_id: VA-FUSION-260104-0003
- authority_basis: HFI（人類最高決策者明確命令｜scope=VERSION_AUDIT｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（並對齊 MASTER_CANON）
- downstream_notes:
  - 若 DOCUMENT_INDEX 的 VERSION_AUDIT 條目仍指向舊日期版本（__260102），需同步更新映射至 __260104，以避免新對話載入混讀。
  - 本文件為 L11 稽核回放制度核心，後續更新其他 L9–L11 重要檔案時，其「留痕欄位」與「最小引用欄位」應持續對齊本文件之規格，以維持可回放閉環。
- rationale: 以單一正文消除補片式混讀來源，同時維持版本治理制度的最大完備與可追溯性，確保後續工程落地與稽核回放可依此文件一致承接。
