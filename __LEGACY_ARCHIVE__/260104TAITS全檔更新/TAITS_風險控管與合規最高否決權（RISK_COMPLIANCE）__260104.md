# TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104
...
平行參照：GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／UI_SPEC／VERSION_AUDIT／FULL_ARCH／ARCH_FLOW／TWSE_RULES  
...
- scope_files_output: TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104.md
- scope_files_source: TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260102.md
...
2) 移除舊版文件內之「稽核留痕區塊」與任何會造成新舊混讀之補丁式段落，改以本檔末端之單一稽核區塊承接（正文不混入留痕）。  
3) 對正文中會造成混讀的治理狀態字樣進行中性化處理：一律以「治理狀態（以 GOVERNANCE_STATE 為準）」作為解讀口徑，不保留補丁式標記語氣。  
4) 保留舊版正文之最大完備內容；僅進行必要的重排與去混讀字串整理（不做摘要縮水；非重複內容不得減少）。  
5) 新增 Level-2 目錄以強化可讀性與可稽核定位（不改變既有制度語義）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 24455a9162a44a29606cf6f2279e55241563a0d65220bc775b92a24aaafd0bd9

## D. Audit Hand-off（裁決承接）
- change_id: RC-FUSION-260104-0005
- authority_basis: HFI（人類最高決策者明確命令｜scope=RISK_COMPLIANCE｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（並對齊 MASTER_CANON）
- downstream_notes:
  - 後續若更新 GOVERNANCE_GATE_SPEC、EXECUTION_CONTROL、UI_SPEC，須保持 PASS/VETO/RETURN、reason_code、最小引用欄位與「L11 非下單層」之口徑一致，以避免跨文件混讀。
  - 若 DOCUMENT_INDEX 之 RISK_COMPLIANCE 條目仍指向舊日期版本（__260102），需同步更新映射至 __260104，以避免新對話載入混讀。
- rationale: 以單一正文消除新舊混讀來源，同時維持風險與合規最高否決制度之最大完備與可追溯性，確保可落地執行與可回放稽核閉環。
