...
|---|---|---|---|---|---|
| TAITS_AI_行為與決策治理最終規則全集__260102.md | AI_GOV | A+ | ACTIVE | 2026-01-04 | 全系統 AI 行為與決策治理母法 |
...
| TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260102__單一檔.md | GOVERNANCE_STATE_FREEZE_V1 | A | ACTIVE | 2026-01-04 | 治理狀態宣告：治理狀態 狀態（以 GOVERNANCE_STATE 為準） 生效（對變更施加門檻） |
...
| TAITS_資料來源全集（DATA_SOURCES）__260102.md | DATA_SOURCES | B |  | ACTIVE | 2026-01-04 | 官方優先、來源追溯、Fallback、Provenance |
...
- scope_files_source: TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260104__單一正確正文版.md（前一版同檔名｜內文映射修整）
...
## B. Changelog（變更清單）
1) **索引映射修整（P0）**：修正正文中仍指向 `__260101` 的檔名快照／列表，統一回填為現行版本集合（以 `__260102` 為基線）。  
2) **核心檔案對齊 `__260104`**：將已於本輪完成融合更新之核心檔案（MASTER_ARCH／MASTER_CANON／UI_SPEC／VERSION_AUDIT／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／FULL_ARCH／ARCH_FLOW／DOCUMENT_INDEX）在索引內的檔名映射同步更新至 `__260104`，以避免新對話載入混讀。  
3) **不做摘要縮水**：除檔名映射修整外，不變更任何治理序位、文件等級 bucket、或既有條款語義。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 6d5f21bc9e67efcf9a526c25b9ca2b5e7c8495be075aec89feaa91740227a236

## D. Audit Hand-off（裁決承接）
- change_id: DI-FIXMAP-260104-0011
- authority_basis: HFI（人類最高決策者明確命令｜scope=DOCUMENT_INDEX｜索引映射修整）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - PROJECT_PROMPT 與其他含「載入清單快照」之文件，若仍引用舊版檔名（__260101/__260102），需在下一輪依本索引映射同步更新，避免操作載入錯檔。
- rationale: DOCUMENT_INDEX 為最高裁決索引；檔名映射若不一致將導致治理裁決與引用定位失真。本次僅修整映射並維持正文最大完備。
