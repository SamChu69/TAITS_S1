...
適用範圍：TAITS 交易執行、下單控制、風控檢核、事件驅動執行、回報與失敗處理（不含策略決策）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／AI_GOV（A+）／MASTER_CANON（A）  
平行參照：RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／UI_SPEC／VERSION_AUDIT／FULL_ARCH／ARCH_FLOW  
...
- 例：`doc_key=RISK_COMPLIANCE, ver=依引用檔版本, §X.Y, constraint=PASS_TOKEN_REQUIRED`
...
- scope_files_source: TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260104.md（上版）
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **示例版號去耦合**：將正文示例中固定寫死的 `ver=YYYY-MM-DD` 統一改為 `ver=依引用檔版本`，避免 L11 稽核回放時誤以為必須引用舊版日期（不改變流程語義）。  
2) **版本日期一致化**：確保檔頭與正文殘留日期引用皆為 2026-01-04（Asia/Taipei），並同步修正舊版號標記（不刪減有效內容）。  
3) **稽核段落單一化**：移除既有稽核段落（若存在），以本檔末端單一稽核區塊承接（留痕與正文分離）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 9dc430e51fe2f80e7426fc254406c064ba86897de3045945e4500dd8189710e9

## D. Audit Hand-off（裁決承接）
- change_id: EC-FIX-260104-0029
- authority_basis: HFI（人類最高決策者明確命令｜scope=EXECUTION_CONTROL｜Final QA 示例版號去耦合/一致化）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - UI_SPEC / GOVERNANCE_GATE_SPEC 若對 EXECUTION_CONTROL 有跨文件引用（例如 ver/章節索引），建議後續以 ref_key+hash 方式避免對「日期字串」產生耦合。
