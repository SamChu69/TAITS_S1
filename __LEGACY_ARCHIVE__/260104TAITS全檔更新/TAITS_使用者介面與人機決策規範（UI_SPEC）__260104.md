# TAITS_使用者介面與人機決策規範（UI_SPEC）__260104
doc_key：UI_SPEC  
治理等級：B（人機互動制度級｜使用者介面與人類裁決邊界）  
...
平行參照：RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／VERSION_AUDIT／LOCAL_ENV／DEPLOY_OPS  
變更原則：最大完備＋累積式更新（允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要化縮水；未被新版本明確更新之有效內容一律保留並持續累積；已被新版本明確取代者可自正文移除但必須由稽核留痕承接）  
語義定錨：L9＝投資報告層（含數據/價格/圖形/條件式進出場建議〔非指令〕/風險敘述/可追蹤更新欄位）；L10＝人類裁決與交易決策層；L11＝全層工程稽核回放層（L1–L11 全留痕），且 L11 非下單層  
UI 分層定錨：L9 UI＝投資報告呈現與追蹤更新介面；L10 UI＝人類裁決、風控覆核、下單確認介面；L11 UI＝全層稽核回放與證據鏈檢視介面（不可下單）  
風控優先：Risk / Compliance 具最高否決權；任何介面流程不得提供繞過 Gate/Token/審計之捷徑  
...
- scope_files_source: TAITS_使用者介面與人機決策規範（UI_SPEC）__260102.md
...
- downstream_notes:
  - 若 DOCUMENT_INDEX 之 UI_SPEC 條目仍指向舊日期版本（__260102），需同步更新映射至 __260104 以避免新對話載入混讀。
