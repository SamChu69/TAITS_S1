# TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260104

doc_key：GOVERNANCE_GATE_SPEC  
治理等級：B（治理／制度級｜閘門與裁決規範）  
...
平行參照：RISK_COMPLIANCE／EXECUTION_CONTROL／UI_SPEC／VERSION_AUDIT／FULL_ARCH／ARCH_FLOW  
...
## 0. 文件定位（Gate & Arbitration Charter）

本文件定義 TAITS 在 Canonical Flow 中的「治理閘門（Gate）」與「裁決（Arbitration）」的制度規格，確保：
- 任何流程推進皆以 Gate 結果為合法邊界，不可繞過
- 任何衝突處理皆可追溯、可回放（L11）
- Risk/Compliance 最高否決權可被正確承接（L7/L8 相關）
- L9 報告與 L10 人類裁決具備一致的引用與留痕規格

---

doc_key：GOVERNANCE_GATE_SPEC  
適用範圍：TAITS 全系統（Research / Backtest / Simulation / Paper / Live）  
治理等級：以 DOCUMENT_INDEX（Authoritative Index）裁決為準（本檔頭不具覆蓋裁決效力）  
版本狀態：ACTIVE（治理裁決核心文件｜最大完備＋累積式更新｜單一正確正文版）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：AI_GOV（A+）／DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／MASTER_CANON（A）  
平行參照：FULL_ARCH／ARCH_FLOW／RISK_COMPLIANCE／EXECUTION_CONTROL／UI_SPEC／VERSION_AUDIT／DEPLOY_OPS／LOCAL_ENV／DATA_SOURCES／TWSE_RULES  
...
本文件為 TAITS 的「治理 Gate（閘門）裁決規範」核心文件，用於定義：
...
> 本章為跨文件一致之「語義定錨」。若任一文件出現不同口徑，一律以本章為準，並由 DOCUMENT_INDEX 的衝突裁決程序承接。
...
注意：子 Gate 不得改寫 L9 主 Gate 的裁決語義；只提供可稽核的檢核結果。

### 3.2 主 Gate（L9）輸入（Inputs｜不可缺）
...
- 本文件實體檔名（本次正文版）：`TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260104.md`
...
timestamp = <ISO-8601>
doc_key = GOVERNANCE_GATE_SPEC
doc_title = TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260104.md
ref_version_date = 2026-01-04
ref_section = <章節/段落路徑>
...
- L7 若 VETO：
  - L9 不得以任何理由放行；
  - L9 僅能完成完整記錄、回放包封存、reason codes 與版本引用的治理義務。
- L7 若 PASS：
  - L9 仍可因治理缺陷 BLOCK/RETURN（例如缺版本、缺審計、引用非法、UI 遮蔽）。
...
```mermaid
flowchart TB
  A[L8 Strategy Proposal<br/>(Non-binding)] --> B[L9 Governance Gate]

  B -->|PASS| C[L10 Human Decision]
  B -->|RETURN| R[Return to Required Layer(s)<br/>+ Required Items List]
  B -->|BLOCK| X[STOP<br/>Preserve Evidence + Audit Artifact]

  R --> D[L4/L5/L6補齊<br/>Evidence/Regime/Artifacts]
  D --> B

  C -->|NO_ACTION| Y[STOP + Audit]
  C -->|BACKTEST/SIMULATION/PAPER/LIVE| E[L11 Audit & Replay<br/>(Not an execution approval gate)]
```
...
## A. Scope（適用範圍）
- scope_doc_key: GOVERNANCE_GATE_SPEC
- scope_files_output: TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260104.md
- scope_files_source: TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102.md
...
2) 以「單一正文」方式承接原文件最大完備內容：未刪減任何未被本次更新明確取代之有效條款；僅進行必要的標記字串去混讀（例如融合承接/附錄/治理狀態等補片式字樣之中性化），避免新舊混讀。  
3) 若原文件尾端存在既有稽核留痕段落，已以本檔末端之單一稽核區塊取代（舊稽核留痕不再混入正文）。  
4) 補強跨文件一致化入口：置頂加入 L9/L10/L11 最終語義定錨與人類主權條款（不改寫原制度語義，只固定對齊口徑）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: ff383c5c64ab2e8fa2052a6dbb1c981ae5c25010abdf0f6855de6718c9b1468e

## D. Audit Hand-off（裁決承接）
- change_id: GGS-FUSION-260104-0006
- authority_basis: HFI（人類最高決策者明確命令｜scope=GOVERNANCE_GATE_SPEC｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（並對齊 MASTER_CANON）
- downstream_notes:
  - 若 DOCUMENT_INDEX 之 GOVERNANCE_GATE_SPEC 條目仍指向舊日期版本（__260102），需同步更新映射至 __260104，以避免載入混讀。
  - 後續若更新 RISK_COMPLIANCE / EXECUTION_CONTROL / UI_SPEC / VERSION_AUDIT，需保持 Gate 結果（PASS/VETO/RETURN）、reason_code、最小引用欄位與「L11 非下單層」之口徑一致。
- rationale: 以單一正文消除補片式混讀來源，同時維持治理閘門與裁決制度之最大完備與可追溯性，確保 L9→L10→（執行）與 L11 回放鏈路可依同一口徑承接。
