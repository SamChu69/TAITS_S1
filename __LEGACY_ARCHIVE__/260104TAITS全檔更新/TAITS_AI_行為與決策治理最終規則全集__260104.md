# TAITS_AI_行為與決策治理最終規則全集__260104
...
平行參照：MASTER_CANON／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／VERSION_AUDIT／UI_SPEC／FULL_ARCH／ARCH_FLOW  
...
### 2. L9–L11 最終語義（跨文件一致）
- L9（投資報告層）：可追蹤、可更新、可標的化投資報告；必含數據、價格、圖形、條件式進出場價格建議（區間＋觸發條件，非指令）、風險敘述、追蹤欄位（狀態/更新時間/依據來源），並支援事件驅動滾動更新。
...
- scope_files_output: TAITS_AI_行為與決策治理最終規則全集__260104.md
- scope_files_source: TAITS_AI_行為與決策治理最終規則全集__260102__融合更新完整版.md
...
2) 移除會造成新舊混讀之補丁式標記語氣（Addendum/Appendix/Freeze 類字樣），改為中性承接用語（內容不刪減）。  
3) 置頂補強全局定錨（人類主權＋L9/L10/L11 口徑），以固定跨文件一致解讀（不改寫既有治理條文，只固定入口語義）。  
4) 若原文件尾端存在既有稽核留痕段落，已以本檔末端之單一稽核區塊取代，避免多份留痕並存。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 98c3fcdae6919bdb80a02e6809d126bbefffec98cc589de1eddbbd6fcedf1d7d

## D. Audit Hand-off（裁決承接）
- change_id: AIG-FUSION-260104-0013
- authority_basis: HFI（人類最高決策者明確命令｜scope=AI_GOV｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 需同步更新 AI_GOV 的 ref_file 由 `__260102__融合更新完整版` 改為本檔 `__260104`，以完成 Single Correct Load Set。
  - PROJECT_PROMPT / Unified Process Anchor 若含載入快照，需同步更新 AI_GOV 檔名與版本日期，避免操作載入混讀。
