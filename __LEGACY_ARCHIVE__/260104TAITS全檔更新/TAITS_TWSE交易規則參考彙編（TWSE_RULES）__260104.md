...
適用範圍：TAITS 台股現貨（TWSE）相關之 Research / Backtest / Simulation / Paper / Live / Live-Shadow  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／AI_GOV（A+）／MASTER_CANON（A）  
平行參照：RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／UI_SPEC／VERSION_AUDIT／DATA_SOURCES  
...
- scope_files_source: TAITS_TWSE交易規則參考彙編（TWSE_RULES）__260102.md
...
3) 置頂補強全局定錨（人類主權＋L9/L10/L11 口徑）以確保規則引用可被 L11 回放一致承接。  
4) 若原文件存在舊式留痕段落，已以本檔末端單一稽核區塊取代（留痕與正文分離）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 6d73908a73dd990dd136e6737c21a9a2300d24312f554eca06b43a98de698b31

## D. Audit Hand-off（裁決承接）
- change_id: TR-FUSION-260104-0017
- authority_basis: HFI（人類最高決策者明確命令｜scope=TWSE_RULES｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 需同步更新 TWSE_RULES 的 ref_file 為本檔 `...__260104.md`，以完成 Single Correct Load Set。
