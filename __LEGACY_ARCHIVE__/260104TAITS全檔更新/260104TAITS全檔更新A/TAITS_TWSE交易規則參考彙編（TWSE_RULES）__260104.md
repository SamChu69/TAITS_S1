# TAITS_TWSE交易規則參考彙編（TWSE_RULES）__260104

doc_key：TWSE_RULES  
治理等級：B（治理／制度級｜TWSE 交易規則參考彙編）  
適用範圍：TAITS 對 TWSE 交易規則之參考整理（合規檢核/限制條件/交易流程參照；不構成法律意見）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／AI_GOV（A+）／MASTER_CANON（A）  
平行參照：RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／VERSION_AUDIT／DATA_SOURCES  
...
- scope_files_output: TAITS_TWSE交易規則參考彙編（TWSE_RULES）__260104.md
- scope_files_source: TAITS_TWSE交易規則參考彙編（TWSE_RULES）__260104.md（上版）
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **目錄去重**：移除目錄/清單區塊中的完全重複條目（例如重複的「治理一致化條款（正文）」等），避免同名條目散落導致混讀；正文內容不刪減。  
2) **章節去重（不刪內容）**：若同名章節重複出現，將第二次起改標「（補充）」以滿足章節唯一性要求並保留全部原文。  
3) **版本日期一致化**：確保檔頭與正文殘留日期引用皆為 2026-01-04（Asia/Taipei）（不刪減有效內容）。  
4) **稽核段落單一化**：移除既有稽核段落（若存在），以本檔末端單一稽核區塊承接（留痕與正文分離）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: a0240fd393fda38ab8eca0830c8cfbdfe18ce9ebf8614f47328efbdc25fe5f2b

## D. Audit Hand-off（裁決承接）
- change_id: TR-FIX-260104-0030
- authority_basis: HFI（人類最高決策者明確命令｜scope=TWSE_RULES｜Final QA 目錄去重/一致化）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - 若你後續要補充「規則來源/日期」的嚴格對齊（例如公告日期/修正沿革），建議以 DATA_SOURCES 的 source_id 與 VERSION_AUDIT 的變更流程納入（本次不做外部規則事實更新）。
