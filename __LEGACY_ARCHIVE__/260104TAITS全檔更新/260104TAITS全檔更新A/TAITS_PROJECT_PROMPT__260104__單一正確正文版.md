# TAITS_PROJECT_PROMPT__260104（單一正確正文版）

doc_key：TAITS_PROJECT_PROMPT  
治理等級：C（操作／啟動級｜專案專屬啟動 Prompt／對話約束）  
適用範圍：TAITS 專案所有新對話與工作對話（角色/權責/語義定錨/交付格式/禁止事項）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV → TAITS_PROJECT_PROMPT（本檔）  
平行參照：DOCUMENT_INDEX／MASTER_ARCH／MASTER_CANON／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／VERSION_AUDIT／UI_SPEC  
...
- 本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV／TAITS_PROJECT_PROMPT 為準）。  
- L9＝投資報告層；L10＝人類裁決與交易決策層；L11＝全層稽核回放層（非下單層）。  
- Risk/Compliance 最高否決權與治理裁決序位不得被對話指令覆寫。  
...
- scope_files_output: TAITS_PROJECT_PROMPT__260104__單一正確正文版.md
- scope_files_source: TAITS_PROJECT_PROMPT__260104.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **去混讀修正**：將正文中作為補丁標記的字樣（Addendum/Appendix/Only-Add/Freeze 等）中性化為「融合承接/附錄/累積式更新/治理狀態」等不造成新舊混讀的表述；不刪減任何有效啟動規範。  
...
- hash_value_sha256: 53ebe860550a9ed38ca6184fd66d0c4141ea7011682473364c649ca79380d2ac

## D. Audit Hand-off（裁決承接）
- change_id: PP-FIX-260104-0042
- authority_basis: HFI（人類最高決策者明確命令｜scope=TAITS_PROJECT_PROMPT｜Final QA 去混讀/一致化）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - README／BEGINNER_GUIDE 若內嵌舊版啟動語，建議後續同步對齊本檔的「L9/L10/L11 定錨」與「留痕分離」規則。
