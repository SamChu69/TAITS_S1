# TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260104

doc_key：UNIFIED_PROCESS_ANCHOR  
治理等級：Engineering Support（工程支援文件｜非 21 份核心治理文件，但必須與其一致）  
適用範圍：TAITS 工程開發、Cursor 工單拆解、Phase 0–5 與 L1–L11 對照、交付/驗收節奏  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／AI_GOV（A+）／MASTER_CANON（A）  
平行參照：VERSION_AUDIT／GOVERNANCE_GATE_SPEC／RISK_COMPLIANCE／EXECUTION_CONTROL／UI_SPEC  
...
- scope_files_output: TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260104.md
- scope_files_source: TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260104.md（上版）
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **去重修正**：移除目錄/清單中的重複條目（例如重複的「治理一致化條款（正文）」等），避免章節散落與混讀。  
2) **格式修正**：修補融合重排版遺留的括號未閉合等格式問題（不刪減有效內容）。  
3) **稽核段落單一化**：移除既有稽核段落（若存在），以本檔末端單一稽核區塊承接（留痕與正文分離）。  

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: c2a5b855ed61b9c4b931fa739bcac9956a7e2025ff2fd2daabb8b075a3aca931

## D. Audit Hand-off（裁決承接）
- change_id: UPA-FIX-260104-0025
- authority_basis: HFI（人類最高決策者明確命令｜scope=UNIFIED_PROCESS_ANCHOR｜Final QA 去重/格式修正）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - 本檔為工程支援文件；若後續要納入 DOCUMENT_INDEX 的「Support Docs」區塊，需以 DOCUMENT_INDEX 的變更流程處理（本次不做結構性擴充）。
