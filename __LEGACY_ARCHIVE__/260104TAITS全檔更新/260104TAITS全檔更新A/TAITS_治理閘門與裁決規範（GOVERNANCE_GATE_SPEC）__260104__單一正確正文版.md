# TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260104（單一正確正文版）

doc_key：GOVERNANCE_GATE_SPEC  
治理等級：B（治理／制度級｜治理閘門與裁決規範）  
適用範圍：TAITS 全系統（治理閘門 Gate 定義／裁決序位／通過/阻擋條件／升級路徑／裁決留痕）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV → GOVERNANCE_GATE_SPEC（本檔）  
平行參照：RISK_COMPLIANCE／VERSION_AUDIT／EXECUTION_CONTROL／UI_SPEC／MASTER_CANON  
...
- 本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV／GOVERNANCE_GATE_SPEC 為準）。  
...
- scope_files_output: TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260104__單一正確正文版.md
- scope_files_source: TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260104.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **去混讀修正**：將正文中作為補丁標記的字樣（Addendum/Appendix/Only-Add/Freeze 等）中性化為「融合承接/附錄/累積式更新/治理狀態」等不造成新舊混讀的表述；不刪減任何有效閘門規則。  
...
- hash_value_sha256: 5f3f5cc8672ef6a8524acfcc3afc169a7aec27e59f8733165713f18e15a0a07b

## D. Audit Hand-off（裁決承接）
- change_id: GG-FIX-260104-0036
- authority_basis: HFI（人類最高決策者明確命令｜scope=GOVERNANCE_GATE_SPEC｜Final QA 去混讀/一致化）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 若仍指向 `...GOVERNANCE_GATE_SPEC）__260104.md`，建議改指向本檔 `...__單一正確正文版.md` 以避免載入集合出現兩份 GOVERNANCE_GATE_SPEC。
