# TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104（單一正確正文版）

doc_key：RISK_COMPLIANCE  
治理等級：B（治理／制度級｜風險控管與合規最高否決權）  
適用範圍：TAITS 全系統（風險控管/合規檢核/最高否決權/停機條件/例外處理與復歸）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV → RISK_COMPLIANCE（本檔）  
平行參照：GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／VERSION_AUDIT／MASTER_CANON／UI_SPEC／TWSE_RULES  
...
- 本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV／RISK_COMPLIANCE 為準）。  
- **Risk/Compliance 具最高否決權**：可否決策略、AI、Agent、執行與任何自動化流程。  
...
- scope_files_output: TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104__單一正確正文版.md
- scope_files_source: TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **去混讀修正**：將正文中作為補丁標記的字樣（Addendum/Appendix/Only-Add/Freeze 等）中性化為「融合承接/附錄/累積式更新/治理狀態」等不造成新舊混讀的表述；不刪減任何有效風控/合規規則。  
...
- hash_value_sha256: 709b03a86464c2604811913f96ed86d08b74c7b3e8b84dcac71945f24fa72d6d

## D. Audit Hand-off（裁決承接）
- change_id: RC-FIX-260104-0034
- authority_basis: HFI（人類最高決策者明確命令｜scope=RISK_COMPLIANCE｜Final QA 去混讀/一致化）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 若仍指向 `...RISK_COMPLIANCE）__260104.md`，建議改指向本檔 `...__單一正確正文版.md` 以避免載入集合出現兩份 RISK_COMPLIANCE。
