# TAITS_使用者介面與人機決策規範（UI_SPEC）__260104（單一正確正文版）

doc_key：UI_SPEC  
治理等級：B（治理／制度級｜使用者介面與人機決策規範）  
適用範圍：TAITS 全系統（UI/UX、提示/呈現、權限、決策介面、審批流程、人機協作邊界）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV → UI_SPEC（本檔）  
平行參照：MASTER_CANON／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／VERSION_AUDIT  
...
- 本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV／UI_SPEC 為準）。  
- **UI 只呈現與收斂，不授權交易**；交易授權唯一入口為 L10 人類裁決。  
...
## A. Scope（適用範圍）
- scope_doc_key: UI_SPEC
- scope_files_output: TAITS_使用者介面與人機決策規範（UI_SPEC）__260104.md
- scope_files_source: TAITS_使用者介面與人機決策規範（UI_SPEC）__260104.md
...
- anchors_enforced:
  ...
  - L10＝人類裁決與交易決策層
  - L11＝全層工程稽核回放層（L11 非下單層）

## B. Changelog（變更清單）
1) 版本日期與檔名統一更新為 2026-01-04（Asia/Taipei）／__260104，並保持「單一正確正文版｜最大完備＋累積式更新」。  
2) 全文移除可能造成新舊混讀之治理狀態舊式標記字樣，改以「治理狀態以 GOVERNANCE_STATE 為準」之單一口徑呈現（正文無補丁式語句）。  
3) 依 MASTER_CANON 的 L9 定錨補強：將 L9 定義明確納入「價格」與「條件式進出場建議（非指令）」與「追蹤欄位」，並於 UI 必備欄位補上 L9 投資報告摘要之最小呈現要求（含價格/關鍵價位與圖形引用）。  
4) 補齊原文件中「最終宣告」空段落（G5）：以不新增制度、僅固定解讀口徑之方式，明確化 UI 對 GOVERNANCE_STATE 與 DOCUMENT_INDEX 的依從責任。  
5) 保留既有所有有效制度內容；僅進行語義對齊、去混讀字樣與必要補強（不做摘要縮水）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: 361bc37de034ac2e6c189bcb1c092a52e9ab4589979532b16c6132200c4e4d85

## D. Audit Hand-off（裁決承接）
- change_id: UI-FUSION-260104-0002
- authority_basis: HFI（人類最高決策者明確命令｜scope=UI_SPEC｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（並對齊 MASTER_CANON）
- downstream_notes:
  - 若 DOCUMENT_INDEX 之 UI_SPEC 條目仍指向舊日期版本（__260104），需同步更新映射至 __260104 以避免新對話載入混讀。
  - 若 EXECUTION_CONTROL / VERSION_AUDIT / GOVERNANCE_GATE_SPEC 仍沿用舊式 L9 定錨口徑（未含價格/條件式建議/追蹤欄位），後續融合更新時需一致化以維持跨文件語義定錨。
...
## A. Scope（適用範圍）
- scope_doc_key: UI_SPEC
- scope_files_output: TAITS_使用者介面與人機決策規範（UI_SPEC）__260104__單一正確正文版.md
- scope_files_source: TAITS_使用者介面與人機決策規範（UI_SPEC）__260104.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **去混讀修正**：將正文中作為補丁標記的字樣（Addendum/Appendix/Only-Add/Freeze 等）中性化為「融合承接/附錄/累積式更新/治理狀態」等不造成新舊混讀的表述；不刪減任何有效 UI/決策規範。  
2) **版本/檔名一致化**：將正文內殘留的 `__260102` 與舊日期引用統一為 `__260104`／2026-01-04（Asia/Taipei），避免跨文件引用錯亂。  
3) **定錨補齊**：若原文缺少 L9/L10/L11 定錨口徑，於檔頭定錨區補齊；不改動原正文其他內容。  
4) **稽核段落單一化**：以本檔末端單一稽核區塊承接（留痕與正文分離）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 60a4852249e9bda54504e2a7bb38fa896a47f21435a4a8441f8efafc3c53b7a4

## D. Audit Hand-off（裁決承接）
- change_id: UI-FIX-260104-0038
- authority_basis: HFI（人類最高決策者明確命令｜scope=UI_SPEC｜Final QA 去混讀/一致化/定錨補齊）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 若仍指向 `...UI_SPEC）__260104.md`，建議改指向本檔 `...__單一正確正文版.md` 以避免載入集合出現兩份 UI_SPEC。
