# TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260104

doc_key：DEPLOY_OPS  
治理等級：B（治理／制度級｜部署、營運與日常運作規範）  
適用範圍：TAITS 全系統（部署／環境／運維／監控／值班／事故處理／變更流程）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／AI_GOV（A+）／MASTER_CANON（A）  
平行參照：LOCAL_ENV／VERSION_AUDIT／GOVERNANCE_GATE_SPEC／RISK_COMPLIANCE／EXECUTION_CONTROL／DATA_SOURCES／FULL_ARCH／ARCH_FLOW  
...
> 以下為歷史檔頭欄位承接，僅作為資訊保留；正文裁決以本檔案開頭之檔頭為準。
...
- scope_files_output: TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260104.md
- scope_files_source: TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260104.md（上版）
...
- hash_value_sha256: 4da8665bab5e58ec1c229f3131e23dc954ba44ae2eb4b069bdf85dea0532139b

## D. Audit Hand-off（裁決承接）
- change_id: DO-FIX-260104-0028
- authority_basis: HFI（人類最高決策者明確命令｜scope=DEPLOY_OPS｜Final QA 一致化修正）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - 若 DOCUMENT_INDEX 或 PROJECT_PROMPT 仍列出 DEPLOY_OPS 其他版本檔名，需於 DOCUMENT_INDEX 進行 final sync 以完成引用收斂。
