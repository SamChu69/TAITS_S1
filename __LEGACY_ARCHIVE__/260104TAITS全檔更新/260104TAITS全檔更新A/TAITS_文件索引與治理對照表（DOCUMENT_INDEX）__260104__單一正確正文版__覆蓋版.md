...
|---|---|---|---|---|---|
| TAITS_AI_行為與決策治理最終規則全集__260104.md | AI_GOV | A+ | ACTIVE | 2026-01-04 | 全系統 AI 行為與決策治理母法 |
...
| TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__260104.md | RISK_COMPLIANCE | A | ACTIVE | 2026-01-04 | 最高否決權（PASS/VETO）、理由碼、合規保守處置 |
...
| TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260104__單一檔.md | GOVERNANCE_STATE_FREEZE_V1 | A | ACTIVE | 2026-01-04 | 治理狀態宣告：治理狀態 狀態（以 GOVERNANCE_STATE 為準） 生效（對變更施加門檻） |
...
| TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260104.md | STRATEGY_UNIVERSE | B |  | ACTIVE | 2026-01-04 | 策略白名單、生命週期、輸出契約；策略≠下單 |
| TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260104.md | STRATEGY_FEATURE_INDEX | B |  | ACTIVE | 2026-01-04 | 特徵/因子定義與審計；禁止信號化偷換 |
...
- doc_key：DOCUMENT_INDEX
- 版本日期：2026-01-04（Asia/Taipei）
- 變更類型：FUSION_UPDATE（融合更新／整合重排版／最後檢視）
- 影響層級：A+（Authoritative Index）
- 影響範圍：跨文件引用合法性、檔名版本對位、doc_key 對位（不改寫治理分桶與覆蓋規則）

### B. 變更清單（Changelog）
1. 將索引表格中仍指向 `__260102`（與 `__260101`）之檔名快照，統一修正為現行版本 `__260104`（以避免載入錯檔）。
2. 修正 GOVERNANCE_STATE 之索引列：檔名對位為 `TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260104.md`，doc_key 對位為 `GOVERNANCE_STATE`（與檔頭一致）。
3. 修正本文件（DOCUMENT_INDEX）於索引表格之檔名列：與本檔實際檔名一致（含 `__單一正確正文版`）。
4. 更新「索引映射修整」與「不入 §5」之工程提示：統一使用 `__260104` 作為現行基線。

### C. 指紋清單（Hash Manifest）
- BODY_ONLY_SHA256：606715c5d348791b9190c3f9ed161c9b4e9b7fd474c54784505d526c757fce94

### D. 裁決承接（Audit Hand-off）
- 本次變更屬「索引對位修整」，不改寫治理裁決序位與分桶規則。
- 後續落帳要求：請於 `VERSION_AUDIT` 追加一筆變更帳本（建議事件鍵：`VA-INDEXMAP-20260104-0001`），承接本次修正之檔名/對位調整與本指紋。
- 下一優先待檢與待融合更新（依治理優先序與 L9–L11 定錨風險）：AI_GOV → MASTER_ARCH → MASTER_CANON → EXECUTION_CONTROL → RISK_COMPLIANCE → GOVERNANCE_GATE_SPEC → ARCH_FLOW → FULL_ARCH → UI_SPEC → VERSION_AUDIT。
