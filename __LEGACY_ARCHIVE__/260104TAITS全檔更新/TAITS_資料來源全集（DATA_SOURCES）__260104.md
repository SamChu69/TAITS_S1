# TAITS_資料來源全集（DATA_SOURCES）__260104
...
平行參照：ARCH_FLOW／FULL_ARCH／VERSION_AUDIT／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／LOCAL_ENV／DEPLOY_OPS  
...
source_doc_name = TAITS_資料來源全集（DATA_SOURCES）__260104.md
...
```text
ref_doc = TAITS_資料來源全集（DATA_SOURCES）__260104.md
ref_doc_key = DATA_SOURCES
...
- canonical_filename：`TAITS_資料來源全集（DATA_SOURCES）__260104.md`
...
- canonical_filename（Index 裁決檔名）：`TAITS_資料來源全集（DATA_SOURCES）__260104.md`
...
- 任何跨文件引用本文件時，必須使用：`doc_key=DATA_SOURCES` + `canonical_filename=TAITS_資料來源全集（DATA_SOURCES）__260104.md`。  
...
- scope_files_output: TAITS_資料來源全集（DATA_SOURCES）__260104.md
- scope_files_source: TAITS_資料來源全集（DATA_SOURCES）__260102 (2).md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **檔名與版本統一**：輸出單一正確正文版檔名 `TAITS_資料來源全集（DATA_SOURCES）__260104.md`，消除來源檔 `(2)` 造成的引用不一致風險。  
2) **正文最大完備承接**：保留來源檔全部有效內容；不進行摘要縮水。  
3) **去混讀處理**：將會造成新舊混讀之補丁式標記語氣（Addendum/Appendix/Freeze 類）以中性承接用語替代（內容不刪減）。  
4) **置頂全局定錨**：補上人類主權與 L9/L10/L11 最終語義口徑，確保資料引用鏈可被 L11 回放一致承接。  
5) **稽核留痕分離**：以本檔末端單一稽核區塊承接留痕要求，避免留痕混入正文。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 0154e2e7089ed3f352ba00c336f8d850c5f506b2170c5d333454a33c0b317750

## D. Audit Hand-off（裁決承接）
- change_id: DS-FUSION-260104-0015
- authority_basis: HFI（人類最高決策者明確命令｜scope=DATA_SOURCES｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 需同步更新 DATA_SOURCES 的 ref_file 為本檔 `TAITS_資料來源全集（DATA_SOURCES）__260104.md`，並移除/封存任何含 `(2)` 的歷史檔名映射，避免載入混讀。
  - 若其他文件（PROJECT_PROMPT/Unified Process Anchor/README）含載入快照，需同步更新 DATA_SOURCES 檔名。
