...
|---|---|---|---|---|---|
| TAITS_AI_行為與決策治理最終規則全集__260102__融合更新完整版.md | AI_GOV | A+ | ACTIVE | 2026-01-02 | 全系統 AI 行為與決策治理母法 |
...
| TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260102.md | GOVERNANCE_STATE_FREEZE_V1 | A | ACTIVE | 2026-01-04 | 治理狀態宣告：治理狀態 狀態（以 GOVERNANCE_STATE 為準） 生效（對變更施加門檻） |
...
| TAITS_資料來源全集（DATA_SOURCES）__260102 (2).md | DATA_SOURCES | B |  | ACTIVE | 2026-01-04 | 官方優先、來源追溯、Fallback、Provenance |
| TAITS_TWSE交易規則參考彙編（TWSE_RULES）__260102.md | TWSE_RULES | B |  | ACTIVE | 2026-01-04 | 交易規則參考、觸發映射、制度快照 |
...
| TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260102.md | DEPLOY_OPS | B |  | ACTIVE | 2026-01-04 | 部署、回滾、停機、Runbook、Live 運作制度 |

### 5.4 C（Operational / Guide｜不具治理裁決力）
| 文件名稱（檔名） | doc_key | 治理等級 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|
| TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__260102.md | BEGINNER_GUIDE | C | ACTIVE | 2026-01-04 | 新手操作引導；不得新增治理權力 |
| README__260102.md | README | C | ACTIVE | 2026-01-04 | 專案總覽與使用方式；不得越權 |
| TAITS_PROJECT_PROMPT__260102.md | PROJECT_PROMPT | C | ACTIVE | 2026-01-04 | 專案對話啟動與行為規範（操作級） |
| TAITS_本地執行與運算環境規範（LOCAL_ENV）__260102.md | LOCAL_ENV | C | ACTIVE | 2026-01-04 | 金鑰隔離、環境檢核、敏感資料禁止入 repo |
...
### 10.1 常見工程支援文件（示例）
- TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260102.md（建議 doc_key：PROCESS_ANCHOR；**不入 §5**）
...
- scope_files_source: TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260104__單一正確正文版.md（同檔名｜索引映射一致化修整）
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **修正 AI_GOV 檔名映射**：將索引中 AI_GOV 的 `ref_file` 由 `...__260102.md` 修正為實體檔名 `...__260102__融合更新完整版.md`，避免引用失真。  
2) **修正 AI_GOV 版本日期宣告**：索引內 AI_GOV 條目之版本日期若標示為 2026-01-04，統一修正回 2026-01-02（與實體檔一致），避免同一母法在不同文件被宣告成不同版本日期。  
3) **修正 RISK_COMPLIANCE 檔名映射**：索引內若仍指向 `__260102`，統一修正為 `__260104`（與本輪已交付版本一致）。  
4) **修正 GOVERNANCE_STATE 檔名映射**：將索引中帶有 `__單一檔.md` 的欄位修正為實體檔名 `TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260102.md`。  
5) **修正 DATA_SOURCES 檔名映射**：將索引中 `TAITS_資料來源全集（DATA_SOURCES）__260102.md` 修正為目前實體檔名 `TAITS_資料來源全集（DATA_SOURCES）__260102 (2).md`（待後續進行 DATA_SOURCES 單一正文與檔名一致化時再回補）。  
6) **不做摘要縮水**：本次僅做索引映射一致化修整，不變更任何治理序位、文件等級 bucket、或既有條款語義。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: d03c7d55781968b8a8053f9b52a0a0945bb4e97b68056a4f64f9686c78f42a52

## D. Audit Hand-off（裁決承接）
- change_id: DI-FIXMAP-260104-0012
- authority_basis: HFI（人類最高決策者明確命令｜scope=DOCUMENT_INDEX｜索引映射一致化修整）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - 後續進行 AI_GOV / GOVERNANCE_STATE / DATA_SOURCES 的融合更新（含檔名/版本日期/單一正文化）後，DOCUMENT_INDEX 需再次同步映射，才能形成最終單一正確載入集合（Single Correct Load Set）。
