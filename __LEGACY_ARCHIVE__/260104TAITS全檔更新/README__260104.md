...
- 版本狀態：ACTIVE（Freeze v1.0；最大完備＋累積式更新（原 累積式更新 口徑已淘汰））
...
- scope_files_source: README__260102.md
...
2) L9 入口定錨補齊「價格」：將入口描述中「條件式進出場建議」統一為「條件式進出場價格建議」，避免與核心治理的 L9 口徑不一致。  
3) 去除會造成新舊混讀的補丁式提示語（Freeze 等），改為中性「單一正文」宣告；內容不刪減。  
4) 若原文件存在舊式留痕段落，已以本檔末端單一稽核區塊取代（留痕與正文分離）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: f2ff29af6753fe04985fa3e9ea109676c226c9944ba86dba772bdf908a4cd1a4

## D. Audit Hand-off（裁決承接）
- change_id: RD-FUSION-260104-0021
- authority_basis: HFI（人類最高決策者明確命令｜scope=README｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 需同步更新 README 的 ref_file 為本檔 `README__260104.md`，以完成 Single Correct Load Set。
