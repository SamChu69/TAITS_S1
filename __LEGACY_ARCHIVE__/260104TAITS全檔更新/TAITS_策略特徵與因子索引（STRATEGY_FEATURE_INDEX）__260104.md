# TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260104

doc_key：STRATEGY_FEATURE_INDEX  
治理等級：B（治理／制度級｜策略特徵與因子索引）  
適用範圍：TAITS 策略研究、特徵工程、報告輸出規格（不含下單）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／AI_GOV（A+）／MASTER_CANON（A）  
平行參照：STRATEGY_UNIVERSE／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／UI_SPEC／VERSION_AUDIT／DATA_SOURCES  
...
- scope_files_output: TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260104.md
- scope_files_source: TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260102__融合更新完整版.md
...
2) 不做摘要縮水：保留原文件最大完備內容；僅進行必要的去混讀處理與置頂全局定錨（人類主權＋L9/L10/L11 口徑）。  
3) 若原文件存在舊式留痕段落，已以本檔末端單一稽核區塊取代（留痕與正文分離）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 3274ffefd457f0e70a566b05d302206d26238e7ec1d4b1ea1419840344736679

## D. Audit Hand-off（裁決承接）
- change_id: SFI-FUSION-260104-0019
- authority_basis: HFI（人類最高決策者明確命令｜scope=STRATEGY_FEATURE_INDEX｜融合更新形成單一正文）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - DOCUMENT_INDEX 需同步更新 STRATEGY_FEATURE_INDEX 的 ref_file 為本檔 `...__260104.md`，以完成 Single Correct Load Set。
