...
適用範圍：TAITS 全系統（模組總覽／邊界／依賴關係／資料流與事件流總覽）  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
對齊母法：DOCUMENT_INDEX（A+）／MASTER_ARCH（A）／AI_GOV（A+）／MASTER_CANON（A）  
平行參照：ARCH_FLOW／VERSION_AUDIT／RISK_COMPLIANCE／GOVERNANCE_GATE_SPEC／EXECUTION_CONTROL／UI_SPEC／DATA_SOURCES／LOCAL_ENV／DEPLOY_OPS  
...
- scope_files_source: TAITS_全系統架構總覽（FULL_ARCH）__260104.md（上版）
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **章節去重（不刪內容）**：針對同名章節（例如「文件定位」「目錄」等）重複散落問題，保留全部原文內容，並將第 2 次起之同名章節標題改為「（補充）」以避免混讀與違反章節唯一性要求。  
2) **檔頭承接隔離**：若正文中出現重複的 doc_key 檔頭欄位，改置於「文件定位（檔頭承接）」區塊並以引用格式保留，避免被誤讀為第二份正文檔頭。  
3) **稽核段落單一化**：移除既有稽核段落（若存在），以本檔末端單一稽核區塊承接（留痕與正文分離）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: fd02d8b7a4e502bd2b33eb96bdadca029aac4ebd863a8308f02465248fbf986f

## D. Audit Hand-off（裁決承接）
- change_id: FA-FIX-260104-0026
- authority_basis: HFI（人類最高決策者明確命令｜scope=FULL_ARCH｜Final QA 章節去重/去混讀修正）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - ARCH_FLOW 若引用 FULL_ARCH 章節錨點（heading），需留意本次「（補充）」標題調整可能影響內部連結；建議後續統一改為以 anchor_id 或 ref_key 方式引用。
