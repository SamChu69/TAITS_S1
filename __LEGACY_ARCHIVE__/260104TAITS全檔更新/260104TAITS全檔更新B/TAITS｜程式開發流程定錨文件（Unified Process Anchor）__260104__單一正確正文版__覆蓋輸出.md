# TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260104（單一正確正文版｜覆蓋輸出）

doc_key：PROCESS_ANCHOR  
文件屬性：SUPPORT / Non-Authoritative（工程流程定錨／操作手冊；不具治理裁決效力）  
治理裁決序位（最終）：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV  
版本日期：2026-01-04（Asia/Taipei）  
適用範圍：TAITS 工程實作（Phase 0–5）、新對話續編定錨、Cursor 工單拆解與交付一致性  
平行參照：MASTER_CANON／ARCH_FLOW／EXECUTION_CONTROL／RISK_COMPLIANCE／VERSION_AUDIT／README／BEGINNER_GUIDE  

---

## 全局定錨｜本檔在治理體系中的正確位置（S1）

- 本檔為「工程流程定錨／操作手冊」，**不具治理裁決效力**，不得被用於裁決：ACTIVE 文件集合、doc_key 合法性、版本有效性、覆蓋順位、或任何風控/合規判定。  
- 所有治理裁決（含衝突裁決）一律依：**DOCUMENT_INDEX → MASTER_ARCH → AI_GOV**。  
- Canonical Flow（L1–L11）之流程定義以 MASTER_CANON／ARCH_FLOW 為主要參照來源；但其引用合法性與生效狀態仍受最終裁決序位約束。  
- 強制語義定錨（跨文件一致）：
  - L9＝投資報告層（Investment Report；含數據/圖形/條件式進出場建議〔非指令〕/風險敘述/可追蹤更新欄位）
  - L10＝人類裁決與交易決策層（唯一交易授權入口）
  - L11＝全層工程稽核回放層（L1–L11 全留痕；L11 非下單層）
  - Governance Gate＝non-layer 檢核機制（主要作用於 L9 → L10）
  - Execution Control＝交易執行控制模組（僅能在 L10 授權後啟動；其留痕由 L11 回放承接）
...
| L9 | Investment Report（投資報告） | Phase 4 |
| L10 | Human Decision | Phase 5 |
| L11 | Audit Replay（全層稽核回放） | Phase 5 |
...
- GOVERNANCE_STATE = 以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
### 5.6 Phase 5｜人類裁決與稽核回放封存（L10–L11）
...
# 【最大完備＋累積式更新｜新增】All-in-One 內嵌附錄（為減檔與新手便利而設）

新增日期：2025-12-28  
生效前提：GOVERNANCE_STATE = 以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
# TAITS｜工程操作總手冊（合併版：GPT 續編 × Cursor 落地 × 雙錨）

文件定位：Master Operating Runbook（合併版總手冊；供 GPT 專案與 Cursor 工程共同使用）  
文件屬性：SUPPORT / Non-Governance（工程/操作支援文件；**不具治理裁決效力**）  
版本日期：2026-01-04  
合併目的：在不違反 以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。

---

## B1. Scope Lock（必讀｜以 GOVERNANCE_STATE 現況為準）
1) **保留既有文件不動**（不刪、不覆寫）  
2) **新增本合併手冊**作為日常入口（你之後只看這一份）  
3) 只在「入口文件」做 **最大完備＋累積式更新 指向**（可選，但推薦）：  
   - 在 Unified Process Anchor 的附錄區 最大完備＋累積式更新 一段「日常操作以本合併手冊為主」  
   - 在 README 的操作區 最大完備＋累積式更新 一段「新手從本合併手冊開始」

這樣你得到「實務上只用一份」的效果，同時不違反 最大完備＋累積式更新。
...
嚴格對齊模式：以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
〔TAITS 最小引用標頭｜以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
ref_file = <完整檔名>
ref_doc_key = <DOC_KEY>
...
```text
〔TAITS Cursor 工程任務｜以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
硬性約束：
1) 僅能依 docs/governance 內 ACTIVE 文件作依據；引用必帶 ref_file/ref_doc_key/ref_version_date/ref_section/audit_anchor。
2) 最大完備＋累積式更新：不可刪減、不可覆寫、不可偷換語義。
...
- governance_state_ref: GOVERNANCE_STATE（狀態由 GOVERNANCE_STATE 準據；本檔正文不內嵌狀態字串）
...
（把本文件 A3 的續編包整段貼在這裡，並在每次工作包後 最大完備＋累積式更新 一段歷史快照）
```
...
## 補充章節 B｜Cursor 工程落地 SOP（雙錨整合）全文內嵌

# TAITS｜Cursor 工程落地 SOP（雙錨整合：PROCESS_ANCHOR × 工程轉譯錨定）
...
```text
請進入嚴格對齊模式（以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
## 8. 常見失敗模式與硬性修正方式（以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
# TAITS｜GPT 新對話續編編輯 SOP（以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。

文件定位：GPT Project Editing Runbook / SOP（新手可照做）  
文件屬性：SUPPORT / Non-Governance（工程/操作支援文件；不具治理裁決效力）  
版本日期：2026-01-04  
核心目的：讓你在 ChatGPT 專案中「頻繁開新對話」也能穩定續編，且不踩 TAITS 治理紅線（Index Gate / 引用欄位 / 最大完備＋累積式更新 / 不跳層）。  
...
4) **目標治理文件（doc_key 對應檔）**（你真正要 最大完備＋累積式更新 的那份文件）
...
工作包的典型範例：
- 只針對某個 **doc_key** 進行一次 最大完備＋累積式更新：新增一段文字到指定章節末
- 只做一次「盤點/審視」報告（只報告、不修改）
- 只做一次 Gate 檢核（PASS / RETURN 缺口清單 / BLOCK 原因碼）

### 2.2 為什麼必須這樣做？
- 對話變長時，容易引用漏欄位、容易漂移、容易跨 doc_key
- 以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
目標：doc_key=<...>
輸出：1) 你觀察到的缺口清單 2) 建議的 最大完備＋累積式更新 增補方向（只列方向，不產生可貼入原文的段落）
```

### 5.2 ONLY_ADD_EDIT（最常用：只新增）
```text
嚴格對齊模式。
任務：最大完備＋累積式更新 編輯（不得刪減/覆寫/偷換語義）。
doc_key=<...>
目的=<你要補什麼>
...
### 7.3 我貼上去後發現有錯字或寫錯位置
- 不要去修改舊內容（避免覆寫/改寫爭議）。  
- 新增一個「更正工作包」最大完備＋累積式更新：補一段「更正聲明」或「勘誤」段落（視你文件風格），並在 Ledger 新增一行標註 CORR。
...
- change_summary：一句話摘要（最大完備＋累積式更新）
...
| ecr_id | created_at | work_type | target_doc_key | target_file | insert_point | change_summary | ref_min_citation | gate_result | output_artifact | next_step |
|---|---|---|---|---|---|---|---|---|---|---|
| ECR-20251228-0001 | 2025-12-28 00:00 | ONLY_ADD_EDIT | (EXAMPLE) DOCUMENT_INDEX | (EXAMPLE) <file>.md | (EXAMPLE) §X 末 | (EXAMPLE) 新增：引用格式補強段 | ref_file=...; ref_doc_key=...; ref_version_date=...; ref_section=...; audit_anchor=... | N/A | (EXAMPLE) 段落標題/產出檔 | (EXAMPLE) 下一包：最大完備＋累積式更新 增補 MASTER_ARCH |
| ECR-20251228-0002 | 2025-12-28 00:00 | REPORT_ONLY | (EXAMPLE) MASTER_CANON | (EXAMPLE) <file>.md | N/A | (EXAMPLE) 盤點：L1–L11 邊界缺口 | ref_file=...; ref_doc_key=...; ref_version_date=...; ref_section=...; audit_anchor=... | N/A | (EXAMPLE) 報告摘要 | (EXAMPLE) 下一包：最大完備＋累積式更新 補一段於 §Y 末 |
...
文件定位：GPT Editing Worklog（工程支援；非治理裁決文件）  
規則：最大完備＋累積式更新 / Append-only（新增與追加；不覆寫歷史條目）  
用途：回答「目前做到哪」「下一步是什麼」，避免新對話斷線。  
...
文件定位：GPT New Chat Context Pack（工程支援；非治理裁決文件）  
規則：最大完備＋累積式更新（可融合更新/覆寫修正新的「最新續編包」區塊；保留舊包作歷史）  
用途：每次開新對話，把上下文一次搬回來（不靠 AI 記憶）。  
...
# TAITS｜專案檔案更新包（最大完備＋累積式更新 Migration Pack：合併入口×不刪檔）
...
3) 專案入口（README / Unified Process Anchor）用 最大完備＋累積式更新 加上「從此以合併版為入口」的指引段。
...
## 2. 更新入口文件：README（最大完備＋累積式更新 貼上段）
...
## 【最大完備＋累積式更新｜新增】工程操作入口（合併版總手冊）

本段為 以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
## 3. 更新主流程錨點：Unified Process Anchor（最大完備＋累積式更新 貼上段）
...
## 4. 更新既有 SOP 檔：加上「已併入合併版」導流段（最大完備＋累積式更新）
...
## 【最大完備＋累積式更新｜新增】導流聲明（合併版入口）

本文件內容已納入下列「合併版總手冊」作為日常唯一入口；本文件仍保留作歷史快照與追溯參考（以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
- 完全符合 以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
...
## A. Scope（適用範圍）
- doc_key: PROCESS_ANCHOR
- file_role: SUPPORT / Non-Authoritative
- change_type: Final QA（去混讀／語義定錨一致化／結構收斂）
- scope_notes:
  - 本次僅針對本檔內部一致性、跨文件語義定錨（L9/L10/L11）、與去混讀要求進行收斂；不新增治理制度、不變更上位治理文件語義。

## B. Changelog（變更清單）
- PA-FIX-260104-0001
  1) **裁決序位口徑收斂**：移除「將本檔納入裁決鏈」之表述；統一為最終裁決序位 `DOCUMENT_INDEX → MASTER_ARCH → AI_GOV`，並明確本檔不具治理裁決力。  
  2) **doc_key 一致化**：修正正文內混用 `DATA_UNIVERSE`／`UNIFIED_PROCESS_ANCHOR` 之 doc_key；統一為 `PROCESS_ANCHOR`（並保留檔名/別稱 Unified Process Anchor 於標題）。  
  3) **去混讀收斂**：移除正文中 `ADDENDUM/Appendix/FREEZE` 類標記（改以引用/承接方式保留於稽核留痕）；正文不再殘留補丁式語句。  
  4) **L9/L10/L11 強制定錨一致化**：修正本檔內映射表與 Phase 命名中將 L9 誤作 Governance Gate、L11 誤作 Execution 的殘留；並補強 Governance Gate（non-layer）與 Execution Control（模組）之正確位置。  
  5) **章節結構收斂**：保留有效內容但整理為單一路徑，避免重複目錄/重複檔頭造成引用與閱讀混讀。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- BODY_ONLY:
  - scope: 本檔正文（不含本稽核區塊）
  - hash_value_sha256: 3ad71377b79f5a29a949f67ea31920b763988271f6b25a2c74744f6465021139
- FULL_EXCLUDING(hash_value_sha256_full_excluding):
  - scope: 全檔（含稽核區塊；但不含本欄位之 hash_value 行）
  - hash_value_sha256_full_excluding: 22b8b462aca988f3ab9b87bd2714c9b815d05bf54ba6fe0de0e83f9b022b6d0d

## D. Audit Hand-off（裁決承接）
- change_id: PA-FIX-260104-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=PROCESS_ANCHOR｜Final QA MODE）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- handoff_notes:
  - 本檔屬 SUPPORT / Non-Authoritative；不得被 DOCUMENT_INDEX 視為治理裁決文件。
  - 若需將本檔納入索引，建議以「工程流程定錨（支援文件）」標註並清楚區隔於治理有效集合之外。

## Z. Archive（原始片段留痕｜非正文）
> 目的：保留本次收斂前之原始文字片段，以避免「刪除造成資訊遺失」；本區不構成正文裁決依據。

### Z1. 原始檔頭與前置區塊快照（pre-core）
```text
# TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260104（單一正確正文版）

doc_key：UNIFIED_PROCESS_ANCHOR  
治理等級：C（操作／啟動級｜工程流程定錨）  
適用範圍：TAITS 工程實作（Phase 0–5）、每次新對話進度定錨、工單拆解與交付一致性  
版本狀態：ACTIVE（單一正確正文版｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV → UNIFIED_PROCESS_ANCHOR（本檔）  
平行參照：MASTER_CANON／ARCH_FLOW／VERSION_AUDIT／README／BEGINNER_GUIDE／TAITS_PROJECT_PROMPT  
...
- 本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV／UNIFIED_PROCESS_ANCHOR 為準）。  
- 本文件用於「工程流程定錨」與「進度一致化」，不構成策略/交易授權。  
...
- 6. 工程進度狀態標示（流程座標）
- 附錄 A｜工程操作總手冊（合併版）全文內嵌
- 0. Scope Lock（以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
  - 0.1 本合併手冊可以做什麼
  - 0.2 本合併手冊不得做什麼
- 1. 你現在的問題：檔案太多，要合併——最佳作法（合規且最省事）
  - 1.1 以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
  - 1.2 最佳落地策略（推薦）
- 2. 雙錨整合（你要記住的核心）
  - 2.1 兩個錨點各司其職（不可互相取代）
  - 2.2 本合併手冊的定位
...
- 0. Scope Lock（必讀｜以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
  - 0.1 本文件**可以**做什麼
  - 0.2 本文件**不得**做什麼（硬禁止）
...
適用範圍：TAITS 全系統（Research / Backtest / Simulation / Paper / Live）  
治理狀態：以 GOVERNANCE_STATE 現況為準（本文件為單一正確正文版；任何歷史狀態標記不構成正文裁決依據（以 DOCUMENT_INDEX／MASTER_ARCH／AI_GOV 為準）。
最後更新：2025-12-29  
