...
- scope_files_output: TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260104__單一正確正文版__覆蓋輸出.md
...
3) **版本/檔名一致化**：維持版本日期為 2026-01-04（Asia/Taipei）；輸出為可直接覆蓋之 `__覆蓋輸出.md` 版本，避免跨文件引用錯亂。
4) **稽核段落單一化**：維持檔尾單一稽核區塊（Scope／Changelog／Hash Manifest／Audit Hand-off），留痕與正文分離。
## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 77871a94fdb312a3b6f6a9a7b7fb223d7cdf3a3f771e12b8b1b1181a8490857c
