# TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104（單一正確正文版｜覆蓋版）
...
- L9＝投資報告層（含數據/圖形/條件式進出場價格建議〔非指令〕/風險敘述/可追蹤更新欄位）  
- L10＝人類裁決與交易決策層（唯一交易授權入口，仍受本檔最高否決權約束）  
...
- scope_files_output: TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104__單一正確正文版__覆蓋版__覆蓋輸出.md
- scope_files_source: TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104__單一正確正文版__覆蓋版.md
...
5) **版本一致化**：版本日期維持 2026-01-04（Asia/Taipei）。
## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope_body: BODY_ONLY（不含本稽核區塊）
- hash_body_sha256: 35223a5ad3e2363ce00cd7f081509b2a377dabb02cabd33e3cbc3115fbccb366
- scope_full: FULL_FILE_EXCLUDING(hash_full_line)（含稽核區塊，但不含下列 `hash_full_sha256` 這一行）
- hash_full_sha256: 177f056b9199d8291ae445ad11fcf21835573e91bfa8d4eed22672985e643c8c
## D. Audit Hand-off（裁決承接）
- change_id: RC-FIX-260104-0061
...
- notes:
  - 建議 ACTIVE 集合僅保留本覆蓋版；原檔 `TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260104__單一正確正文版.md` 封存以避免混讀。
