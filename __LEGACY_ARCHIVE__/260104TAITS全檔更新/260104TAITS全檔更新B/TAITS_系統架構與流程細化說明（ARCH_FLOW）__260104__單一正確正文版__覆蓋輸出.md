# TAITS_系統架構與流程細化說明（ARCH_FLOW）__260104（單一正確正文版）
...
- scope_files_output: TAITS_系統架構與流程細化說明（ARCH_FLOW）__260104__單一正確正文版.md
- scope_files_source: TAITS_系統架構與流程細化說明（ARCH_FLOW）__260104.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **語義定錨一致化（L9/L10/L11）**：移除同檔內將 L9 描述為「Governance Gate」之殘留，統一為 `L9＝投資報告層（Investment Report）`；同時將 L11 之定位統一為 `全層工程稽核回放（Audit Replay）`（L11 非下單層）；並明確 `Execution Control（受控執行）` 為模組（非 Canonical Layer），僅可在 L10 HumanApprove 授權後啟動。  
2) **流程圖/狀態矩陣對齊**：將 State Transition Matrix 之 L9/L10/L11 狀態命名收斂為 `ReportReady/ReportInvalid`、`HumanApprove/HumanReject`、`ReplayReady/AuditFail`；同步修正流程圖節點與箭頭標示，避免 L9/L11 被誤讀為治理閘門或送單層。  
3) **去混讀維持**：正文維持「去混讀」要求（補丁式語句不得混入正文）；留痕僅存在於本檔尾稽核區塊，不參與正文裁決。  
...
- manifests:
  - name: BODY_ONLY
    ...
    hash_value_sha256: 6ee8f8717d4fa45e3d441df423a18e6665bd944ccb42e95b85dd9885bab18e30
  - name: FULL_EXCLUDING_HASH_LINE
    scope: FULL（含稽核區塊；但不含本段中之 hash_value_sha256 行，以避免自指）
    hash_value_sha256: 7e8a817fb0aa861ad8853eebee4e6359c67bd9a4208c56f52a50493f0c20d473

## D. Audit Hand-off（裁決承接）
- change_id: AF-FIX-260104-ARCH_FLOW-SEMANTIC_ANCHOR-0001
...
- downstream_notes:
  - DOCUMENT_INDEX 若仍指向 `...ARCH_FLOW）__260104.md`，建議改指向本檔 `...__單一正確正文版.md` 以避免載入集合出現兩份 ARCH_FLOW。
