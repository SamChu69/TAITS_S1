# TAITS_本地執行與運算環境規範（LOCAL_ENV）__260104（單一正確正文版｜覆蓋版）
...
- scope_files_output: TAITS_本地執行與運算環境規範（LOCAL_ENV）__260104__單一正確正文版__覆蓋版.md
- scope_files_source: TAITS_本地執行與運算環境規範（LOCAL_ENV）__260104__單一正確正文版.md
...
3) **版本一致化**：統一日期為 2026-01-04（Asia/Taipei）。
...
- notes:
  - 建議 ACTIVE 集合僅保留本覆蓋版；原檔 `TAITS_本地執行與運算環境規範（LOCAL_ENV）__260104__單一正確正文版.md` 封存以避免混讀。
