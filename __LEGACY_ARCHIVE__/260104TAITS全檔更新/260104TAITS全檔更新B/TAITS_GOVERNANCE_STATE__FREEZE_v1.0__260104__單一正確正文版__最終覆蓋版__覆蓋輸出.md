# TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260104（單一正確正文版｜最終覆蓋版）
...
- scope_files_output: TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260104__單一正確正文版__最終覆蓋版.md
- scope_files_source: TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260104__單一正確正文版__覆蓋版.md
- scope_mode: FILE UPDATE MODE（doc_key 去歧義定錨／去混讀／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **裁決序位口徑修正**：將本檔檔頭「裁決序位」收斂為最終裁決序位 `DOCUMENT_INDEX → MASTER_ARCH → AI_GOV`，並明示本檔僅為治理狀態宣告，不得凌駕最終裁決序位。  
...
- hash_value_sha256: c7713a51c7733dfc8db40f99c5b9dc0672671b215c2b1ab49d358d9e076dbaf9

## D. Audit Hand-off（裁決承接）
- change_id: GS-FIX-260104-0060
- authority_basis: HFI（人類最高決策者明確命令｜scope=GOVERNANCE_STATE｜Final QA doc_key 去歧義定錨）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- notes:
  - 建議 ACTIVE 集合僅保留本最終覆蓋版；前一版 `TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260104__單一正確正文版__覆蓋版.md` 封存以避免混讀。
  - 本次覆蓋輸出僅修正檔頭裁決序位口徑與對應 Hash；正文規則未改寫。
