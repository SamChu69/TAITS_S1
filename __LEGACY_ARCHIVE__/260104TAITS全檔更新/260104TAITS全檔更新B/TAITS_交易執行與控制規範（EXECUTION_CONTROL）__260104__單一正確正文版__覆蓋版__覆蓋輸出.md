# TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260104（單一正確正文版｜覆蓋版）
...
- 例：`doc_key=RISK_COMPLIANCE, ver=2026-01-04, §X.Y, constraint=PASS_TOKEN_REQUIRED`
...
- scope_files_output: TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260104__單一正確正文版__覆蓋版.md
- scope_files_source: TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260104__單一正確正文版.md
- scope_mode: FILE UPDATE MODE（去混讀／留痕分離／Final QA）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）

1) **去混讀收斂**：移除正文中提及「補丁/附錄」之補丁式描述；必要的歷史承接說明改由稽核區塊承接，避免新舊混讀。  
2) **語義定錨一致化**：將章節標題「執行層在 Canonical Flow 的定位（L11）」更名為「…（與 L11 的關係）」；避免將 L11 誤讀為執行/下單層。  
3) **治理裁決序位對齊**：稽核區塊之 `governance_order_applied` 依 Final QA MODE 統一為 `DOCUMENT_INDEX → MASTER_ARCH → AI_GOV`。  
4) **留痕分離**：若原正文含 Scope/Changelog/Hash/Audit Hand-off（A/B/C/D）段落，已自正文移除並統一置於本檔稽核區塊。
5) **去混讀修正**：中性化補丁式字樣（Addendum/Appendix/Only-Add/POSTCHECK）為不造成裁決混讀的表述（歷史承接/附錄/累積式更新/稽核檢核）。
6) **版本一致化**：統一日期為 2026-01-04（Asia/Taipei）。


## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: a928da48ea52f9bd9118da426bbdd69545df72cffe12eef4f94712405058537b

## D. Audit Hand-off（裁決承接）
- change_id: EC-FIX-260104-0056
- authority_basis: HFI（人類最高決策者明確命令｜scope=EXECUTION_CONTROL｜Final QA 留痕分離/去混讀）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- notes:
  - 建議 ACTIVE 集合僅保留本覆蓋版；原檔 `TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260104__單一正確正文版.md` 封存以避免混讀。
