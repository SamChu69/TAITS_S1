...
- scope_files_output: TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260104__單一正確正文版__覆蓋版__覆蓋輸出.md
- scope_files_source: TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260104__單一正確正文版__覆蓋版.md
- scope_mode: FINAL QA MODE（去混讀／語義定錨一致化／融合更新收斂／單一正確正文）
- version_date: 2026-01-04（Asia/Taipei）

## B. Changelog（變更清單）
1) **結構去重**：移除正文內重複的文件頭 metadata 區塊，保留單一權威文件頭，避免新舊混讀。  
2) **排版修復**：修正分隔線（`---`）與標題黏連（`---###`）之格式錯誤，確保章節層級與分隔線可被穩定解析。  
3) **交叉引用修復**：修正「全局語義定錨（L9–L11）」重導向段落的錯誤章節引用，改指向本檔實際定義段落（`### 2.` 與 `#### 2.A`）。  
4) **指紋重算**：重算 BODY_ONLY SHA256 指紋以對齊本次覆蓋輸出內容。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: 3d97ff80ad05c86446b081392f1c84841fe7342df8eda22dd0f2f40bc992bac0

## D. Audit Hand-off（裁決承接）
- change_id: MA-FQA-260104-0001
