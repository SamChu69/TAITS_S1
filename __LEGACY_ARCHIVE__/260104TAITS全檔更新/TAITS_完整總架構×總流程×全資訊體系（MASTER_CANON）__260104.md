# TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__260104.md

doc_key：MASTER_CANON  
治理等級：A（Supreme Canon｜最高裁決母體）  
適用範圍：TAITS 全系統（Research／Backtest／Simulation／Paper／Live／Live-Shadow）  
版本狀態：ACTIVE（Single Correct 正文｜最大完備＋累積式更新）  
版本日期：2026-01-04（Asia/Taipei）  
變更原則：最大完備＋累積式更新（允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要化縮水；未被新版本明確取代之有效內容一律保留並持續累積；已被新版本明確取代者可自正文移除，但必須以稽核留痕承接。）

---

## 目錄

1. SOVEREIGNTY｜人類最高決策者唯一主權  
2. L9–L11｜最終語義定錨（跨文件一致）  
3. HFI｜人類明確命令（可執行觸發工件）  
4. 0. 文件定位（Canonical Master｜最大完備整合版）  
5. Canonical 前言與使用規則（第 1 章）  
6. TAITS Canonical Flow（L1–L11）（第 2 章）  
7. Gate／Evidence／Veto／Agent／Audit（第 3–10 章）  
8. 稽核區塊（非正文：Changelog／Hash Manifest／Scope／Audit Hand-off）
...
- 作為跨文件語義定錨之最終裁決依據（若與下位文件衝突，一律以本文件裁決）
...
> 為避免新舊混讀：本區塊不參與 Canonical 正文敘述裁決；正文以本檔案開頭至本區塊前之內容為準。

## A. Scope（適用範圍）

- scope_doc_key: MASTER_CANON
- scope_files_output: TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__260104.md
- scope_files_source:
  - TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化／Final QA）
- version_date: 2026-01-04（Asia/Taipei）
- anchors_enforced:
  - 最大完備＋累積式更新（允許融合更新／覆寫修正／重排版；禁止摘要化縮水；未被新內容明確取代者一律保留並持續累積；被取代者以稽核留痕承接）
  - L9＝投資報告層（含數據與價格/圖形/條件式進出場建議〔非指令〕/風險敘述/可追蹤更新欄位）
  - L10＝人類裁決與交易授權層
  - L11＝全層工程稽核回放層（L11 非下單層；禁止作為下單/執行入口）

## B. Changelog（變更清單）

1) 版本日期統一更新為 2026-01-04（Asia/Taipei），並輸出為單一正文檔名後綴 __260104。  
2) 移除並消弭所有可能造成新舊混讀之「舊式附錄／舊稽核留痕正文混入」段落，確保本檔正文為 Single Correct 正文。  
3) 將 L9/L10/L11 之最終語義定錨置於文件前段，並補強 L9 必含「數據與價格、圖形、條件式進出場建議（非指令）、風險敘述、追蹤欄位」之要求，以符合跨文件一致化定錨。  
4) 保留既有正文所有有效內容；僅移除重複標題與重複之稽核留痕段落（屬重複或混讀來源），不進行摘要化縮水。  
5) 新增目錄與閱讀指引以提升可讀性與可稽核性（不改變 Canonical Flow 定義）。

## C. Hash Manifest（指紋清單）

- hash_alg: sha256
- scope: BODY_ONLY（不含本稽核區塊）
- hash_value_sha256: b433e4025d3200e3eb7add792fd367dacc9bc298cfc97959604c6a9dab25321c

## D. Audit Hand-off（裁決承接）

- change_id: MC-FUSION-260104-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=MASTER_CANON｜單一正文融合更新）
- governance_order_applied: DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- downstream_notes:
  - 若 DOCUMENT_INDEX 仍引用舊版 MASTER_CANON 檔名/日期，需同步更新其版本映射條目（以避免跨文件混讀）。
  - VERSION_AUDIT 若承接本次更新，應建立對應 ledger 條目並引用本檔 Hash Manifest（BODY_ONLY）。
- rationale: 以單一正文消除混讀來源，維持最大完備且可演進之 Canonical 定義，同步固化 L9/L10/L11 跨文件語義定錨，確保可稽核、可回放、可落地。
