...
### 1) Changelog（變更紀錄）
- FINAL_QA_NORMALIZE：正文乾淨（移除對話/補丁/占位符）、去重收斂、結構重排、裁決序位對齊。
- 去重收斂：合併重複之「全局定錨」為單一版本，並移除任何對層級之重複定義。
- 正文清理：移除「標籤/助記/順位字串法律定位」等非必要正文段落，避免混讀。
- 版本更新：版本日期與基線日期更新為 2026-01-10（Asia/Taipei）。
- 稽核同步：更新 Hash Manifest 以維持可追溯性。

### 2) Hash Manifest（指紋清單）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SCOPE：全文正文（不含本稽核區塊）
- BODY_SHA256：30eac5293556736d40760e2afbf2436ab4fb70ef71c0401a4396109d8cd813b3
...
- change_id：RC-FINALQA-260110-0001
- authority_basis：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV
- prepared_by：TAITS FINAL QA MODE｜NORMALIZE ALL
