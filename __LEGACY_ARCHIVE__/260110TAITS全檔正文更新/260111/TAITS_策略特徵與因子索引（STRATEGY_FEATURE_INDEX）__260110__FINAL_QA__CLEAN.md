# TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260110__FINAL_QA__CLEAN
doc_key：STRATEGY_FEATURE_INDEX  
治理等級：B（治理／制度級：Feature Governance & Semantic Boundary Charter｜最大完備覆蓋版）  
...
本文件是 TAITS 的 **「策略特徵與因子索引」**，其定位不是列清單而已，而是提供一份**即使後續使用情境只讀這一份，也能合法展開策略研究所需的「語義憲法＋知識索引」**。

本文件同時滿足兩個目標：

1) **治理封頂（Hard Governance）**  
- 明確定義 Feature 的法定意義  
- 明確定義 Feature 的合法使用路徑（Feature 與 Evidence、Regime、Risk、Strategy、Human、Execution 之銜接路徑（依 MASTER_CANON））  
...
- 提供「備註」讓後續使用情境能清楚理解每類 Feature 常用到的資料、指標、結構語義與常見陷阱（lookahead、資料延遲、過度信號化）
...
- 對應 Evidence（L5）
- 對應 Regime（L6）
- 對應 Risk/Compliance Gate（L7）
- 對應 Strategy Proposal（L8）
- 對應 Governance Gate（L9）
- 對應 Human Decision（L10）
- 對應 Execution（L11）
...
- 對應 用 `__implX` 記錄，不可用 `v#` 假裝語義有變。

---

## A6. FeatureMeta 必備欄位（用於後續使用情境可展開）
...
## Changelog
- 2026-01-10：FINAL QA NORMALIZE（正文整編）— 移除 Git 合併衝突標記、重複檔頭／重複定位段、對話痕跡與箭頭順位式表達；移除 Part 完結註記；補齊稽核四件套。僅進行去重收斂與結構重排，不更動制度條文語義。

## Hash Manifest
- scope_doc_key：STRATEGY_FEATURE_INDEX
- baseline_date：2026-01-10
- body_sha256：512f108211fe713f856d9bf77ce1786ea4867b027dba4512354d5ac26a2690ba
- file_sha256：a35e7a3f2d0893ee1af3fdad18752b5450f72b2de5ca187ceb64adab4ef62c3d

## Scope
- 本檔範圍：特徵（Feature）層之治理定義、語義邊界、標準資料結構（schema）、Evidence 組裝規則、治理檢核（Checklist）與落地模板／範例（附錄 A–C）。
- 本檔不涵蓋：Canonical Flow（L1–L11）正文定義（以 doc_key=MASTER_CANON 為唯一正文來源）；Risk/Compliance 否決理由碼全集（以 doc_key=RISK_COMPLIANCE 為準）；UI 呈現樣式（以 doc_key=UI_SPEC 為準）。

## Audit Hand-off
- 交付項：可直接覆蓋之單一 md 正文（最大完備覆蓋版）＋稽核四件套
- 應驗收要點：
  1) Feature ≠ Signal ≠ Strategy ≠ Order 之邊界是否完整、無偷換語義
  2) FeatureMeta / FeatureOutput / Provenance / Replay / No-lookahead 欄位是否齊備且一致
  3) 與 MASTER_CANON、RISK_COMPLIANCE、GOVERNANCE_GATE_SPEC、VERSION_AUDIT 之引用關係是否越權或重複定義
- 狀態：READY_FOR_REVIEW
