...
文件性質：Strategy Universe｜白名單 × 生命週期 × 審計可回放 × Regime-First
適用範圍：TAITS 全系統（Research / Backtest / Simulation / Paper / Live）
版本狀態：ACTIVE（策略宇宙可 僅允許新增 擴充；不得刪減既有治理欄位與否決鏈）
版本日期：2026-01-10
對齊母法：TAITS_AI_行為與決策治理最終規則全集__251217（最高母法）
上位約束：MASTER_ARCH / MASTER_CANON / DOCUMENT_INDEX / VERSION_AUDIT / RISK_COMPLIANCE
平行參照：ARCH_FLOW / FULL_ARCH / DATA_SOURCES / TWSE_RULES / EXECUTION_CONTROL / UI_SPEC / DEPLOY_OPS / LOCAL_ENV
變更原則：僅允許新增（只可新增，不可覆寫/刪除/弱化；策略版本只可追加；任何啟用需可追溯）
...
> 每個策略是一筆可版本化記錄（Strategy Record），僅允許新增。
...
11. 策略宇宙演進規則（增補原則）（STRATEGY_UNIVERSE 專屬）
允許新增：
...
文件性質：Strategy Universe｜白名單 × Regime-First × 僅允許新增
版本狀態：ACTIVE
版本日期：2026-01-10
...
變更原則：
- 增補原則：僅允許新增（僅新增 Annotation，不刪除、不合併、不改 strategy_id）
...
文件性質：Strategy Universe｜白名單 × Regime-First × Risk-Strict × 僅允許新增
版本狀態：ACTIVE
版本日期：2026-01-10
對齊母法：TAITS_AI_行為與決策治理最終規則全集__251217
上位約束：
...
文件性質：Strategy Universe｜白名單 × Official-First × Evidence-First × Regime-First × 僅允許新增
版本狀態：ACTIVE
版本日期：2026-01-10
對齊母法：TAITS_AI_行為與決策治理最終規則全集__251217
上位約束：
...
變更原則：
- 增補原則：僅允許新增（僅新增 Annotation，不刪除、不合併、不改 strategy_id、不改策略邏輯）
...
文件性質：Strategy Universe｜Regime-First × Risk-First × Compliance-Supreme × 僅允許新增
版本狀態：ACTIVE
版本日期：2026-01-10
對齊母法：TAITS_AI_行為與決策治理最終規則全集__251217
上位約束：
...
文件性質：Structure System｜結構體系為判斷框架，不等於策略；白名單 × Evidence-First × Regime-First × 僅允許新增
版本狀態：ACTIVE
版本日期：2026-01-10
對齊母法：TAITS_AI_行為與決策治理最終規則全集__251217
上位約束：
...
變更原則：
- 增補原則：僅允許新增（僅新增 Annotation，不刪除、不合併、不改 strategy_id、不改結構定義）
...
文件性質：Cross-Market Strategy Universe｜Regime-First × Official-First × Evidence-First × Risk-First × 僅允許新增
版本狀態：ACTIVE
版本日期：2026-01-10
對齊母法：TAITS_AI_行為與決策治理最終規則全集__251217
上位約束：
...
### 1. Changelog
- 2026-01-10｜FINAL QA NORMALIZE：移除合併衝突標記、同檔重複區塊與非正文完結註記；清理對話痕跡與交付流程敘述；用一致措辭承載「增補原則」並保留原制度語義；同步引用名稱為 DATA_SOURCES；補齊稽核四件套。

### 2. Hash Manifest
- doc_key：STRATEGY_UNIVERSE
- hash_scope：正文（本節〈稽核四件套〉之前之全部內容）
- hash_alg：SHA-256
- hash：eb4a2a2878694927382a2942a33218b8c89248a069411d2d9d9cde65a4363714
- bytes：75714
- lines：2424

### 3. Scope
- 允許變更：去重收斂、結構重排、措辭正規化（不改變制度語義）、移除非正文註記（合併衝突標記／完結字樣／交付流程敘述／對話痕跡）。
- 禁止變更：新增或刪除策略制度條文之實質要求；重寫或特別化 Canonical Flow（L1–L11）；削弱 Risk / Compliance 最高否決權；改變策略輸出契約之邊界。

### 4. Audit Hand-off
- 文件狀態：可直接覆蓋（最大完備覆蓋版）
- 驗收要點：
  - 正文不含任何版本合併衝突殘留（例如 git merge conflict markers）
  - 正文不含對話痕跡、交付流程敘述、或任何非制度之完結註記
  - Canonical Flow（L1–L11）僅引用 MASTER_CANON，不於本文件另立定義
  - 檔尾保留且僅保留稽核四件套（不含 Legacy Snapshot）
