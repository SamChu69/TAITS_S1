...
### 1) 變更清單（Changelog）
- FINAL_QA_NORMALIZE：移除子章節錯誤編號（僅保留頂層章節編號），避免跳號與引用錯位。
- FINAL_QA_NORMALIZE：移除 AI 允許/禁止清單與跨文件治理重述；AI 權限邊界由 doc_key=AI_GOV 承載，本檔僅保留架構層可落地的 Agent Registry/Binding 與 RBAC 約束。
- FINAL_QA_NORMALIZE：正文去補丁、去重、去混讀；修正章節編號與結構一致性。
- FINAL_QA_NORMALIZE：刪除「資料宇宙/別名/助記順位」類治理補強段落；改為架構層引用口徑（doc_key=DATA_SOURCES）。
- FINAL_QA_NORMALIZE：L1–L11 層級定義與權級說明不在本檔重複；本檔僅保留「架構對位」並以 MASTER_CANON / UI_SPEC / VERSION_AUDIT 作為細節引用依據。
- FINAL_QA_NORMALIZE：修正 Mermaid 圖塊為標準 ` ```mermaid ` 區塊，避免渲染失敗。
- 稽核：重算 BODY_SHA256。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：b00667535e2ea180f31a0bafd07570f659b85044ee3983c470aaa46cb4b4666a
### 3) 覆蓋範圍（Scope）
