...
---
## 0. 文件定位（最大完備｜全知識自洽）
本文件是 TAITS 的 **「策略特徵與因子索引」**，其定位不是列清單而已，而是提供一份**即使僅閱讀此文件，也能合法展開策略研究所需的「語義憲法＋知識索引」**。
本文件同時滿足兩個目標：
1) **治理封頂（Hard Governance）**
- 明確定義 Feature 的法定意義
- 明確定義 Feature 的合法使用路徑（Feature到Evidence到Regime到Risk到Strategy到Human到Execution）
- 明確列出禁止事項與違規判定（可被審計、可被阻斷）
2) **全知識自洽（Knowledge Self-Contained）**
- 將常用方法論（GMMA、顧比倒數線、威科夫、鮑迪克纏論等）落在 Feature 層，並以「描述性輸出」表達
- 提供 Feature 到 Evidence 組裝方式 到 Regime 對應 到 Strategy 適用邏輯 的展開規則
- 提供「備註」讓新使用情境能清楚理解每類 Feature 常用到的資料、指標、結構語義與常見陷阱（lookahead、資料延遲、過度信號化）
📌 本文件不負責（避免越權）：
...
到 Evidence
到 Regime
到 Risk/Compliance Gate
到 Strategy Proposal
到 Investment Report
到 Governance Gate（non-layer｜對 報告做檢核）
到 Human Decision
到 Execution Control（non-layer｜僅於 授權後可啟動）
到 Audit Replay
任何違反此路徑的行為，均屬治理違規：
...
- `feature_id`：唯一 ID（建議：`FTR.（填寫：domain）.（填寫：family）.（填寫：name）.（填寫：version）`）
- `feature_name_zh`：中文名
- `feature_name_en`：英文名（如有）
- `domain`：資料面向（TECH/FUND/FLOW/SENT/MACRO/RISK/STRUCT/EXEC）
- `family`：家族（Trend/Momentum/Volatility/Structure/Valuation/Quality/Growth/Liquidity/Event（以擴充方式新增））
- `methodology_tags`：方法論標籤（GMMA/Wyckoff/ChanLun_Bodick（以擴充方式新增）可多選）
- `timeframe`：適用週期（Tick/1m/5m/1h/1D/1W/1M（以擴充方式新增））
...
---
## 6. Feature 到 Evidence 組裝規則（Evidence Assembly Rules｜最大完備）
...
### 7.2 Feature 到 Regime 常見映射（描述性）
- 趨勢結構完整度較高、均線群組呈有序、波動穩定 到 Trend 的候選描述
- 波動收斂、價格在區間內均值回歸、量能相對偏低 到 Range 的候選描述
- 波動明顯上升、跳空、點差擴大、深度不足 到 Volatility Shock / Liquidity Stress 候選描述
- 事件密度高、公告窗口內、政策時點 到 Event Risk 候選描述
- 價量嚴重背離、撮合異常、長時間無量 到 Abnormal 候選描述
...
- Trend Evidence：GMMA 群組呈有序＋Flow/Liquidity 正常＋波動不失真
- Range Evidence：GMMA 收斂＋波動收斂＋量能相對偏低
🚫 GMMA 禁止用法（明文）
- 「短群上穿長群＝買」
- 「群組擴張＝追」
- 「顏色變化＝做多做空」
...
- `FTR.STRUCT.WYCKOFF.EVENT_FLAGS.v1`（PS/SC/AR/ST/SOS/LPS（以擴充方式新增）事件旗標）
...
- 陷阱：用宏觀單一指標對短線交易下結論
---
## 10. Feature 到 Strategy 的「展開備註」規則（本文件要求的「清楚」）
本節提供「新使用情境如何展開」：不是告訴你買賣，而是告訴你每種策略家族通常會用到哪些 Feature、怎麼組裝 Evidence、怎麼對位 Regime。
---
...
`FTR.MARKET_DATA.VOLUME.VOL_IMPACT_5D.v1`
- `FTR`：固定前綴（Feature）
- `DOMAIN`：資料面向域（見 A2）
- `FAMILY`：家族分類（見 A3）
- `NAME`：特徵名稱（描述性、無方向性）
...
`FTR.MARKET_DATA.VOLUME.VOL_IMPACT_5D.v1__PY__1D__TWSE`
...
到 用 `__implX` 記錄，不可用 `v#` 假裝語義有變。
---
## A6. FeatureMeta 必備欄位（用於新使用情境可展開）
本節把 Part 1 的 schema 具體化成「建檔模板」。
任何新增 Feature，必須能寫出下面這段（可直接貼到 repo / 文檔）。
### A6.1 FeatureMeta Template（可貼用）
```yaml
feature_id: "FTR.MARKET_DATA.VOLUME.VOL_IMPACT_5D.v1"
feature_name_zh: "範例：成交量衝擊指標（5日）"
feature_name_en: "Example: Volume Impact Indicator (5D)"
domain: "MARKET_DATA"
family: "VOLUME"
methodology_tags: ["（填寫：GMMA|Wyckoff|ChanLun_Bodick|可擴充）"]
timeframe: "（填寫：Tick|1m|5m|1D|1W|可擴充）"
inputs:
 - "（填寫：canonical_field_or_datasource_field_1）"
 - "（填寫：可擴充）"
lookahead_policy: "NO_LOOKAHEAD"
data_latency: "（填寫：REALTIME|DELAYED|ANNOUNCEMENT_LAG|SPARSE）"
output_type: "（填寫：Scalar|Vector|Label|State|Distribution）"
output_semantics: "（填寫：描述性語義，不含方向）"
quality_checks:
 - "（填寫：missing_value_handling）"
 - "（填寫：halt/suspension handling）"
 - "（填寫：outlier handling）"
audit_requirements:
 must_have:
 - correlation_id
...
FTR.TECH.TREND.GMMA_BAND_STRUCTURE.v1
zh：GMMA 群組結構呈有序度
output_semantics：群組排列與分層的呈有序程度（0~1）
...
zh：威科夫事件旗標集合
output_semantics：事件集合（PS/SC/AR/ST/SOS/LPS（以擴充方式新增））的存在標記（非方向）
...
---
## B3. Feature 到 Evidence 的納入規則（Evidence Ingestion Rules）
...
---
## B4. Feature 到 Regime 的使用邊界（Regime Input Boundary）
Regime 是狀態判定，不是交易方向。
...
- `GOV-REG-PERF-DRIVEN`
---
## B5. Feature 到 Risk 的使用邊界（Risk Context Only）
...
- `SEMANTIC_CHANGE` 到 必升 `VER`
- `IMPLEMENTATION_CHANGE` 到 只能改 `__impl`
- `DATA_SOURCE_CHANGE` 到 可能升 `VER`（若影響語義）
...
flowchart TB
 ...
 G1 -->|FAIL| R1[RETURN: Fix Naming/Semantics（填寫：br/）Audit: GOV-FTR-SEM-*]
 G1 -->|PASS| G2{Gate-2 Provenance + Hash}
 G2 -->|FAIL| R2[RETURN: Fix Provenance/Hash（填寫：br/）Audit: GOV-FTR-PROV/HASH-*]
 G2 -->|PASS| G3{Gate-3 No Lookahead + Available_at}
 G3 -->|FAIL| R3[RETURN: Fix Lookahead/Available_at（填寫：br/）Audit: GOV-FTR-LOOKAHEAD]
 G3 -->|PASS| G4{Gate-4 Replay Determinism}
 G4 -->|FAIL| R4[RETURN: Snapshot/Determinism Required（填寫：br/）Audit: GOV-FTR-REPLAY-*]
 G4 -->|PASS| G5{Gate-5 Robust QA Handling}
 G5 -->|FAIL| R5[RETURN: Define QA/Halt/Outlier Handling（填寫：br/）Audit: GOV-FTR-QA-*]
 G5 -->|PASS| G6{Gate-6 Allowed/Forbidden Uses}
 G6 -->|FAIL| R6[RETURN: Fix Uses Whitelist/Blacklist（填寫：br/）Audit: GOV-FTR-USE-*]
 G6 -->|PASS| A1[APPROVE: Feature Eligible for Evidence/Regime Use（填寫：br/）Tier Controls Apply]
...
feature_meta:
 ...
 family: "TREND" # 族群分類：TREND/MOMENTUM/VOL/LIQ/STRUCT/VALUATION/QUALITY/（以擴充方式新增）
 subfamily: "GMMA" # 子族群：GMMA/WYCKOFF/CHANLUN_BODICK/（以擴充方式新增）
 description: >
 描述長短期均線群之間的擴散程度，用於刻畫趨勢呈有序度與動能狀態，
 僅輸出連續數值，不輸出買賣方向。
 ...
 display_hint: "可視化為趨勢呈有序度/擴散程度，禁止顯示買賣箭頭"
...
沒有 provenance_map_ref 到 Part 3 Gate-2 失敗。
json
複製程式碼
{
 ...
 "time_range": ["2025-10-01", "2025-12-19"],
 "raw_ref": "FTRSET-（以擴充方式新增）-MA",
 "transform_chain": ["ma_calc", "group_aggregate"]
 }
 ],
 "integrity": {
 "input_hashes": ["sha256:（以擴充方式新增）","sha256:（以擴充方式新增）"],
 "provenance_hash": "sha256:（以擴充方式新增）"
 }
...
語義：短群 vs 長群的擴散程度（越大代表趨勢呈有序度可能更強，但不等於做多/做空）
GMMA-B：群相對位階（短群在上/下）
...
category：WIDE_SPREAD_HIGH_VOL / NARROW_SPREAD_HIGH_VOL / （以擴充方式新增）
...
C8. Mermaid｜Feature 到 Evidence 到 Regime 的合法流（完整）
mermaid
複製程式碼
flowchart TB
 [Snapshot Ready] --> [Feature Engine（填寫：br/）Compute FeatureValueRecords]
 --> P[ProvenanceMap + Hash + QualityFlags]
 P --> G{Feature Governance Gates（填寫：br/）Part3 Gate-1..6}
 G -->|FAIL| R[RETURN: Fix Feature Meta/Provenance/Replay]
 G -->|PASS| [Evidence Builder（填寫：br/）Assemble Evidence Bundle]
 --> [Regime Engine（填寫：br/）State Classification]
 --> [Risk & Compliance Gate（填寫：br/）PASS/VETO]
...
- STRATEGY_FEATURE_INDEX：定義 （Feature）層之**語義邊界、計算輸出、品質旗標、來源映射（Provenance）、版本/雜湊（Hash）、可回放（Replay Determinism）**等工程與治理必備義務；並提供 Feature 到 Evidence 到 Regime 的合法流之規格化展開。
...
TAITS_策略特徵與因子索引（STRATEGY_FEATUR（以擴充方式新增）
...
TAITS_策略宇宙全集（STRATEGY_UNIVERSE）（以擴充方式新增）
任何下單參數、券商指令、API payload、立即執行語義
Y.4 新增/更新 Feature 的 Gate 統一要求（不得放寬）
本段為「本檔內部一致性」要求，並對位治理 Gate 流程：Semantic Boundary 到 Provenance/Hash 到 No Lookahead/Available_at 到 Replay Determinism 到 QA Handling 到 Allowed/Forbidden Uses。
TAITS_策略特徵與因子索引（STRATEGY_FEATUR（以擴充方式新增）
...
---
## 治理補強（已整合為正文）
上位裁決：DOCUMENT_INDEX（A+｜Authoritative Index）到 MASTER_ARCH（A）到 MASTER_CANON（A）＋AI_GOV（A+）

## 稽核區塊（Audit Section｜非正文）

### 1) 變更清單（Changelog）
- 2026-01-10：FINAL_QA_NORMALIZE（正文去重收斂、格式修復、移除占位符與對話語境字樣；不改寫治理語義。）
- 2026-01-10：收斂重複段落與命名口徑（維持最大完備；不新增/不刪減制度條文）。
- 2026-01-10：版面一致化（標題階層、清單、分隔線與可讀性修正）。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256
- BODY_SHA256：2b23c48563a32b005a06a5c6d4b2ffa5fc4722f5829dc934de9c635d7a329d20
