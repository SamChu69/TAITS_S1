...
9) FULL_ARCH（全系統架構總覽）  
10) 其他文件（UI_SPEC、DEPLOY_OPS、LOCAL_ENV、DATA_SOURCES、STRATEGY_*、TWSE_RULES、README 等）依 DOCUMENT_INDEX 所列治理等級與文件性質裁決
...
### 1) 變更清單（Changelog）
- 2026-01-10：FINAL_QA_NORMALIZE（基線日期對齊；移除正文中之標籤化定位字樣；衝突裁決秩序收斂並受「DOCUMENT_INDEX → MASTER_ARCH → AI_GOV」上位裁決序位約束；修正「執行（Execution）」語義承接為 L10 授權後經 RISK/Execution Control 硬門檻，並由 L11 以稽核回放承接；將「新對話」用語收斂為「新會話」以維持正文制度語境一致；移除 舊稽核快照；稽核四件套固定化；重算 BODY_SHA256。）

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：709f207e94de24597a2aecea89373479e6f6ea9ee0dbcc8d16ccaa8d2eceff0e

### 3) 適用範圍（Scope）
本次覆蓋版修正範圍僅限於 doc_key=MASTER_CANON（正文整編與驗收）：
- 基線日期／版本日期對齊（2026-01-10）
- 正文乾淨化：移除標籤化定位字樣（例如：S、B（含加號助記）等）與可造成混讀之流程性表述
- 衝突裁決秩序收斂：以「DOCUMENT_INDEX → MASTER_ARCH → AI_GOV」為上位裁決序位約束，並明確 MASTER_CANON 之承接定位
- Canonical Flow（L1–L11）層級語義維持單一口徑（L9=投資報告；L10=人類裁決；L11=全層稽核回放）
- 稽核區塊固定化：僅保留 Changelog／Hash Manifest／Scope／Audit Hand-off；不得保留 舊稽核快照
