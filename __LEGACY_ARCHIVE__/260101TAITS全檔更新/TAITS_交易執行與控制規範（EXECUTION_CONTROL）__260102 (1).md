# TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260102
...
- 實體檔名（本次單一正文版）：`TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260102.md`
...
## B. Changelog（變更清單）
1) 融合舊版（S1/251231）與新版（260102）內容：保留全部有效資訊，移除補丁標籤（Addendum/Appendix/Freeze 類型標記），以單一正文避免新舊混讀。  
2) 將「只能加／只增不減」舊式口徑，統一改為「最大完備＋累積式更新」：允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要縮水；未被新版本明確更新之有效內容必保留並持續累積；已被明確取代者以稽核留痕承接。  
3) 強制語義定錨一致：統一 L9/L10/L11 終局語義，並明確禁止 AI 自批與禁止把 L11 誤作批准/下單入口。  
4) 重新排版依序排列：依「定位→語義定錨→治理身份/引用→執行目標→I/O→Pre/In/Post→審計/回放→圖示→工程補充」之順序重整，並修復易混讀之段落組織。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: f330d11e96b0225be80e7e353a5436384eb7f97807e8b85828d1b5b4fd07c06b

## D. Audit Hand-off（裁決承接）
- change_id: EC-FUSION-260102-0002
...
- downstream_requirements:
  ...
  - 若其他文件仍沿用舊式補丁標籤或存在「L9=Gate」「L11=批准/下單」等錯誤口徑，需依同一模式逐檔融合更新以完成全局一致化。
