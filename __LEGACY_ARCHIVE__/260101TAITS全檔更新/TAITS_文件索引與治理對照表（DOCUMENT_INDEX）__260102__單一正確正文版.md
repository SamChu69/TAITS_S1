# TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260102.md
...
- 有效 HFI 存在時：FREEZE 狀態/更新規則/Gate **不得**阻擋 scope 範圍內之更新/覆寫/重排版；並必須同步產生稽核承接（VERSION_AUDIT 落帳、CHANGELOG、HASH_MANIFEST）。
...
|---|---|---|---|---|---|
| TAITS_AI_行為與決策治理最終規則全集__260101.md | AI_GOV | A+ | ACTIVE | 2026-01-02 | 全系統 AI 行為與決策治理母法 |
| TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260102.md | DOCUMENT_INDEX | A+ | ACTIVE | 2026-01-02 | 文件位階裁決最高層（本文件） |

### 5.2 A（Constitutional / State）
| 文件名稱（檔名） | doc_key | 治理等級 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|
| TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260101.md | MASTER_ARCH | A | ACTIVE | 2026-01-02 | 人類主權、鐵律、三權分立、邊界與否決鏈 |
| TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__260101.md | MASTER_CANON | A | ACTIVE | 2026-01-02 | L1–L11 Canonical Flow 最高總綱與全資訊體系 |
| TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__260101.md | RISK_COMPLIANCE | A | ACTIVE | 2026-01-02 | 最高否決權（PASS/VETO）、理由碼、合規保守處置 |
| TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260101.md | EXECUTION_CONTROL | A | ACTIVE | 2026-01-02 | 執行控制、Token、Kill Switch、Human-in-the-Loop |
| TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260101__單一檔.md | GOVERNANCE_STATE_FREEZE_V1 | A | ACTIVE | 2026-01-02 | 治理狀態宣告：FREEZE 狀態（以 GOVERNANCE_STATE 為準） 生效（對變更施加門檻） |

### 5.3 B（Governance / Spec）
| 文件名稱（檔名） | doc_key | 治理等級 | 顯示標籤 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|---|
| TAITS_全系統架構總覽（FULL_ARCH）__260101.md | FULL_ARCH | B |  | ACTIVE | 2026-01-02 | 橫向模組 × 縱向層級架構地圖 |
| TAITS_系統架構與流程細化說明（ARCH_FLOW）__260101.md | ARCH_FLOW | B | B+（顯示） | ACTIVE | 2026-01-02 | 流程層細化（承接 MASTER_CANON），不得改寫 Canonical |
| TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260101.md | GOVERNANCE_GATE_SPEC | B |  | ACTIVE | 2026-01-02 | Gate 規則：完整性、引用合法性、拒絕與退回語義 |
| TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260101.md | VERSION_AUDIT | B |  | ACTIVE | 2026-01-02 | 變更帳本、回放、稽核證據、可追溯治理 |
| TAITS_資料來源全集（DATA_SOURCES）__260101.md | DATA_SOURCES | B |  | ACTIVE | 2026-01-02 | 官方優先、來源追溯、Fallback、Provenance |
| TAITS_TWSE交易規則參考彙編（TWSE_RULES）__260101.md | TWSE_RULES | B |  | ACTIVE | 2026-01-02 | 交易規則參考、觸發映射、制度快照 |
| TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260101.md | STRATEGY_UNIVERSE | B |  | ACTIVE | 2026-01-02 | 策略白名單、生命週期、輸出契約；策略≠下單 |
| TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260101.md | STRATEGY_FEATURE_INDEX | B |  | ACTIVE | 2026-01-02 | 特徵/因子定義與審計；禁止信號化偷換 |
| TAITS_使用者介面與人機決策規範（UI_SPEC）__260101.md | UI_SPEC | B |  | ACTIVE | 2026-01-02 | UI 決策可視化、否決呈現、Decision Trace |
| TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260101.md | DEPLOY_OPS | B |  | ACTIVE | 2026-01-02 | 部署、回滾、停機、Runbook、Live 運作制度 |

### 5.4 C（Operational / Guide｜不具治理裁決力）
| 文件名稱（檔名） | doc_key | 治理等級 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|
| TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__260101.md | BEGINNER_GUIDE | C | ACTIVE | 2026-01-02 | 新手操作引導；不得新增治理權力 |
| README__260101.md | README | C | ACTIVE | 2026-01-02 | 專案總覽與使用方式；不得越權 |
| TAITS_PROJECT_PROMPT__260101.md | PROJECT_PROMPT | C | ACTIVE | 2026-01-02 | 專案對話啟動與行為規範（操作級） |
| TAITS_本地執行與運算環境規範（LOCAL_ENV）__260101.md | LOCAL_ENV | C | ACTIVE | 2026-01-02 | 金鑰隔離、環境檢核、敏感資料禁止入 repo |
...
- 任何狀態切換（例如 FREEZE 狀態 → Unfreeze）必須：
...
| 補充資料集 C | FREEZE 狀態（以 GOVERNANCE_STATE 為準） 下 Index 可行為清單 | DOCUMENT_INDEX | ACTIVE | FREEZE 狀態 期間 Index 層允許/禁止行為 |
...
| `TAITS｜GPT新對話續編編輯SOP（FREEZE 狀態（以 GOVERNANCE_STATE 為準）｜Index Gate First）__ADDENDUM_20251228R_FINAL.md` | `TAITS｜程式開發流程定錨文件（Unified Process Anchor）__251231.md` | `support_addendum_route` |
| `TAITS｜GPT新對話續編編輯SOP（FREEZE 狀態（以 GOVERNANCE_STATE 為準）｜Index Gate First）__ADDENDUM_20251228_FINAL.md` | `TAITS｜程式開發流程定錨文件（Unified Process Anchor）__251231.md` | `support_addendum_route` |
...
reference_alias_map:
  ...
  items:
    ...
    - legacy_ref_filename: "TAITS｜GPT新對話續編編輯SOP（FREEZE 狀態（以 GOVERNANCE_STATE 為準）｜Index Gate First）__ADDENDUM_20251228R_FINAL.md"
      resolve_to: "TAITS｜程式開發流程定錨文件（Unified Process Anchor）__251231.md"
      rule: "support_addendum_route"
    - legacy_ref_filename: "TAITS｜GPT新對話續編編輯SOP（FREEZE 狀態（以 GOVERNANCE_STATE 為準）｜Index Gate First）__ADDENDUM_20251228_FINAL.md"
      resolve_to: "TAITS｜程式開發流程定錨文件（Unified Process Anchor）__251231.md"
      rule: "support_addendum_route"
...
2) Authoritative Index（§5）歸一：以 2026-01-02 之現行檔名重建清單；治理分桶一致化為 A+/A/B/C，並明確定義 B+ 僅為顯示標籤。
...
6) 整合 2025-12-31（S1）版本之路由映射／解析表／機器可讀補丁工件：以「工程附錄（不屬正文）」形式保留最大完備與可回放相容性，並明示其不具治理裁決力，避免新舊混讀。

## C. Hash Manifest（指紋清單）
- hash_algo: SHA-256
- hash_target: 正文區段（§0–§11；不含工程附錄與本章「稽核留痕」）
- hash_value_sha256: 36e10694851a24f4f9972bd0b5e6d661322f9bd65265c25cff43ddb96f8eb2ae

## D. Audit Hand-off（裁決承接）
請將以下條目以「單一交易」落入 `VERSION_AUDIT`（doc_key=VERSION_AUDIT）：

- change_id: DI-FUSION-260102-0001
