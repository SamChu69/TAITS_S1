# TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260102.md
...
口徑對齊：
目的：在不改寫本文件任何既有治理條款與內容的前提下，消解「治理等級：B+」可能被誤讀為新治理層級之歧義；將本文件之治理身份、引用合法性、裁決序列字串全面對齊 DOCUMENT_INDEX；確保 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 下跨文件引用一致、可稽核、可回放。
---
## A0. B+ 的法律定位（Label ≠ Governance Grade｜不可誤讀）
### A0.1 統一裁決：B+ 僅為子級顯示標籤（Sub-Label）
...
# Block A｜文件開頭追加：MASTER_CANON 對位指引（最大完備＋累積式更新 · 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
（最大完備＋累積式更新 · Canonical Alignment Guideline · 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
## A.0 本段定位與效力聲明（不可省略）
...
- 版本狀態：ACTIVE（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）；最大完備＋累積式更新）
### A2. 本專案目錄中的實體檔案（Physical Artifact）
...
## A. Scope（適用範圍）
- doc_key：STRATEGY_FEATURE_INDEX
- 檔案：TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260102.md
- 治理等級：B+（Feature Governance & Semantic Boundary Charter）
- 適用狀態：ACTIVE
- 適用時間：2026-01-02（Asia/Taipei）
- 本次作業：融合更新（S1 口徑擴充落地：L9/L10/L11 詳細規格＋L11 雙料輸出；新增「禁止以歷史舊版作為口徑依據」定錨）、清理雜訊標記（移除 contentReference/oaicite 類殘留）、重排版（維持目錄依序排列）、稽核留痕分離（Scope/Changelog/Hash/Audit Hand-off）。

## B. Changelog（變更清單）
1) S1 定錨增強：擴充「L9–L11 最終語義」為可直接工程落地之詳細規格（含 L9 必備欄位、L10 最小輸出集合、L11 雙料輸出與覆蓋範圍/邊界）。
2) 反混讀治理：新增「禁止以歷史舊版作為口徑依據（Prevent Semantic Drift）」條款；工程端不得以歷史版本作解釋依據，歷史追溯僅能引用 VERSION_AUDIT 留痕鏈。
3) 雜訊清理：移除正文中殘留之 `:contentReference[...]{...}`／`[oaicite:...]{...}` 等非規格內容，避免混讀與污染可追溯性。
4) 口徑與狀態表述維持：保留既有「最大完備＋累積式更新」治理口徑與「以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）」之混讀防護表述。
5) 稽核留痕：更新本檔 Hash Manifest 與 Audit Hand-off，並維持與正文分離。

## C. Hash Manifest（指紋清單）
- hash_algo: sha256
- hash_scope: main_body_excluding_audit_section
- hash_value_sha256: 1c7ccfa3747471055019f46cef075cb9862c93823ef51ccff5dd6c2619ec8558

## D. Audit Hand-off（裁決承接）
- 裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（如有衝突以此序為準）。
- 承接聲明：本檔為策略特徵與因子索引（治理等級 B+），僅規範 Feature（特徵）之語義、品質與稽核輸出契約；不得越權覆寫 RISK_COMPLIANCE 之最高否決權與 MASTER_CANON 的 Canonical Flow（L1–L11）。
- L9/L10/L11 定錨承接：L9＝投資報告層（含數據/圖形/條件式進出場建議/可追蹤更新）；L10＝人類裁決與交易授權層；L11＝工程稽核回放層（L1–L11 全留痕、雙料輸出），且 L11 非下單入口/非決策層。
