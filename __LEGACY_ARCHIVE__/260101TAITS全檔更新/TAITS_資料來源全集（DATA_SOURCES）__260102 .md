...
source_doc_name = TAITS_資料來源全集（DATA_SOURCES）__260102.md
...
```text
ref_doc = TAITS_資料來源全集（DATA_SOURCES）__260102.md
ref_doc_key = DATA_SOURCES
...
- canonical_filename：`TAITS_資料來源全集（DATA_SOURCES）__260102.md`
...
- canonical_filename（Index 裁決檔名）：`TAITS_資料來源全集（DATA_SOURCES）__260102.md`
...
- 任何跨文件引用本文件時，必須使用：`doc_key=DATA_SOURCES` + `canonical_filename=TAITS_資料來源全集（DATA_SOURCES）__260102.md`。  
...
本次融合更新（__260102｜最大完備＋累積式更新）變更要點：
1. **章節編號去重與順序化**：將原重複之 `99/100/101`（宏觀/新聞/社群）重新編號為 `108/109/110`，並同步改寫其子章節編號，避免新舊混讀。
2. **新增機構報告與資金動向章（111）**：補齊「國內外銀行/投資銀行/投資機構研究報告」與「資金動向」可治理宇宙（BF1–BF24）與 ResearchFlowRegistry 欄位，並明確限制輸出邊界（僅 L5/L6/L9）。
3. **清理非正文追蹤參數**：將 OpenAPI/Swagger 參考連結去除 `utm_source=chatgpt.com`，改為可追溯之官方入口清單，避免污染正文與稽核鏈。
4. **治理身份定錨更新**：將 `canonical_filename` 統一為 `TAITS_資料來源全集（DATA_SOURCES）__260102.md` 以符合單一正確正文版的引用一致性。
5. **新增快速導覽目錄**：於正文前段加入「目錄（快速導覽）」以便人類與工具快速定位重要章節，不影響既有制度內容。

## C. Hash Manifest（指紋清單）

- algorithm: `SHA-256`
- artifact: `TAITS_資料來源全集（DATA_SOURCES）__260102.md`
- sha256: `6ba98efd79b301b84289d273eb2d7292ecc9d040c117c00a3932d212b13e9215`
