# TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__260102.md
doc_key：RISK_COMPLIANCE  
治理等級：A（Risk & Compliance Supreme Veto｜高於策略、高於績效、高於主觀）  
適用範圍：TAITS 全系統（Research / Backtest / Simulation / Paper / Live）  
版本狀態：ACTIVE（最高否決權文件，最大完備＋累積式更新 演進）  
版本日期：2026-01-02  
變更原則：最大完備＋累積式更新（只可新增，不可刪減/覆寫/弱化否決權/偷換定義）  
...
- GOV-SCOPE-01：Freeze 後越界變更（最大完備＋累積式更新 違規）
...
### 治理補強（已整合為正文）
（最大完備＋累積式更新 · Boundary Lock for L7 · Freeze v1.0）
...
### 治理補強（已整合為正文）
（最大完備＋累積式更新 · Audit/Replay Hardening for L7 · Freeze v1.0）
...
為確保 EXECUTION_CONTROL 可驗證且不可偽造，本文件主文 §10.2 之 Token 最小欄位在 Freeze v1.0 下補強如下（最大完備＋累積式更新）：
- `token_id`
- `correlation_id`
- `policy_version`
...
### 治理補強（已整合為正文）
（最大完備＋累積式更新 · Handoff Integrity · Freeze v1.0）
...
- scope_files_modified: TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__260102.md
...
- anchors_enforced:
  ...
  - HFI 承接：有 HFI 時不得程序性卡死命令（改為揭露/確認/留痕）

## B. Changelog（變更清單）
1) 將文件內「只能加／只增不減（歷史術語）/只能加/只增不減」等更新限制口徑，統一改為「最大完備＋累積式更新」：允許融合更新／覆寫修正／重排版以形成單一正文；禁止摘要化縮水；未更新舊資訊必保留並持續累積；已被取代者以稽核留痕承接。  
2) 補強並固定跨文件一致之 L9/L10/L11 語義定錨，並新增「禁止 AI 自批／禁止把 L11 當批准或下單層」之硬性條款，避免風控制度被誤讀為自動批准鏈。  
3) 新增「HFI 與最高否決權承接」章節：明確區分無 HFI/有 HFI 之處置義務，並要求以 L11/VERSION_AUDIT 留痕承接。
4) 重新排版並依序排列：保留所有既有有效內容不縮水，將主文章節（§0–§17）按數字序集中，並將分散的補強/對位內容集中至 §18（正文一部分），以避免主文序列被打斷與新舊混讀。  

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: e0bb5db32b7ee3ab65ff57526b131ab9762742bb2fba5ef1b0ef47bcf9e6047a

## D. Audit Hand-off（裁決承接）
- change_id: RC-FUSION-260102-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=RISK_COMPLIANCE 文件融合更新）
- downstream_requirements:
  - 請將本次變更承接落入 `VERSION_AUDIT`（doc_key=VERSION_AUDIT），並更新 `DOCUMENT_INDEX` 之版本日期/檔名（若有表列）。
- rationale: 以單一正文消除舊式「只能加」限制與新舊混讀風險，並以最大完備哲學承接風控制度細節，同步確立 L9/L10/L11 與 HFI 承接口徑，確保可稽核、可回放、可落地。
