...
> 你問的「國內外債券資訊」在 TAITS 中屬於 **利率（Rates）／債券（Bonds）／信用（Credit）** 的核心證據層。  
> 嚴格對齊的核心原則：
> - 債券資訊通常屬於 L6（Regime/Context）與 Risk 觀測的關鍵輸入，也可作為 L9 報告的可追溯引用。  
> - **不得直接觸發 L10 下單**；只能形成 Evidence Bundle 並由人類裁決。  
> - 必須顯式記錄 **期限（tenor）、品種（sovereign/corp/agency）、報價方式（yield/price/spread）、計息/日計數規則、流動性與延遲**。  
> - 任何非公開、未授權或不可重現之數據一律拒用（RESTRICTED）。

### 112.1 類型宇宙（BD1–BD18）
- BD1：主權債殖利率曲線（各期限點：1M/3M/6M/1Y/2Y/5Y/10Y/30Y 等）
- BD2：政策利率與走廊（央行政策利率、存放款便利等）
- BD3：國庫券/短票（Bills / T-Bills）
- BD4：公債現券價格與殖利率（價格/殖利率）
- BD5：公債期貨/利率期貨（若有；與 DV 互參）
- BD6：政府公債標售結果（Bid-to-cover、得標利率、投標分布）
- BD7：公司債殖利率與信用利差（Spread / OAS / Z-spread）
- BD8：投資級/高收益信用指標（IG/HY 指標、利差代理）
...
- BD16：期限利差與關鍵曲線形狀（2s10s、5s30s 等）
- BD17：信用評級與評級變動（rating actions）
- BD18：債券相關監理統計（銀行資本適足、利率風險揭露的代理）
...
## 113. 貴金屬與大宗商品資訊（Precious Metals / Commodities）類型宇宙（CM1–CM18）

> 你問的「貴金屬資訊」在 TAITS 中屬於 **商品（Commodities）** 的核心證據層之一（黃金/白銀/鉑/鈀等），常用於 Regime（風險偏好/通膨預期/避險情緒）與風險監控。  
> 嚴格對齊原則：
> - 商品資訊可用於 L6/L9，但不得直接觸發 L10。  
> - 必須記錄 **報價市場、幣別、合約規格、交割月份、現貨/期貨/遠期、庫存與供需資料**。  
> - 若資料涉及商業授權/再散布限制，必須以 license_flag 控管。

### 113.1 類型宇宙（CM1–CM18）
- CM1：黃金現貨（spot）價格（幣別、報價來源）
- CM2：白銀現貨價格
- CM3：鉑/鈀現貨價格
...
- CM7：LBMA/交易所定盤（若可得）
- CM8：交易所庫存（倉單/庫存；若公開可得）
- CM9：COT/持倉分類報告（若為公開監理統計）
- CM10：實體需求代理（珠寶/工業需求指標，若可得）
- CM11：供給代理（礦產供給、產量、罷工/停產事件）
- CM12：美元指數/匯率對商品影響代理（作交叉證據）
- CM13：通膨預期與實質利率（與 BD 互參）
- CM14：其他工業金屬（銅/鋁/鎳等，若你要納入可擴）
- CM15：能源（原油/天然氣，若你要納入可擴）
- CM16：農產品（玉米/黃豆/小麥等，若你要納入可擴）
- CM17：商品運輸/航運指標（供應鏈/運價）
- CM18：商品相關重大事件（制裁/關稅/天災/政策）
...
## B. Changelog（變更清單）
0.7) 貴金屬/商品資訊補齊：新增「貴金屬與大宗商品資訊（CM1–CM18）」類型宇宙與 CommoditiesRegistry（合約/幣別/規格/庫存/持倉/授權欄位必填）。  
0.6) 債券資訊補齊：新增「國內外債券資訊（BD1–BD18）」類型宇宙與 BondRatesRegistry（期限/報價/計息/延遲/流動性/授權欄位必填）。  
...
- hash_value_sha256: 813a0d8a897198b76e0566014b12e9c92036957f73d3ccbc929fb250685d56a4
