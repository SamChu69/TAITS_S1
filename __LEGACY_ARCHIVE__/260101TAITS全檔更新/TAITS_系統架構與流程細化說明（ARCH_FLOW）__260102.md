# TAITS_系統架構與流程細化說明（ARCH_FLOW）__260102
...
（最大完備＋累積式更新 · Rendering-Safe Copy · FREEZE 狀態（以 GOVERNANCE_STATE 為準））
...
flowchart TB
  ...
  L11 -->|SUCCESS| END[END]
  L11 -->|FAIL| EMERGENCY[EMERGENCY STOP]
（最大完備＋累積式更新 · Audit/Replay Hardening · FREEZE 狀態（以 GOVERNANCE_STATE 為準））
...
- scope_files_modified: TAITS_系統架構與流程細化說明（ARCH_FLOW）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）

## B. Changelog（變更清單）
1) 變更原則口徑統一：將原文中「僅允許追加/只可新增/不可覆寫」等措辭，統一為「最大完備＋累積式更新」之單一口徑；允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要縮水；未被新版本明確取代之有效內容必保留並持續累積；被取代者由留痕承接。  
2) 重新排版與依序排列：將原文「G0–G5 與重複治理補強段落」整併為單一正文結構（0–11），避免重複段落造成新舊混讀，並使每一章節具有穩定引用錨點。  
3) 語義定錨一致化：在檔頭明確鎖定 L9/L10/L11 角色邊界（L11 非下單/非批准），並修正任何可能造成「AI 越權批准」的錯誤敘述為禁止條款。  

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: 4f0d6470f1a93bbc8febcfe0d5aa7a2ba344c43dd6fe687333a32320ad27c0f1

## D. Audit Hand-off（裁決承接）
- change_id: AF-FUSION-260102-0002
- authority_basis: HFI（人類最高決策者明確命令｜scope=ARCH_FLOW 文件融合更新）
- downstream_requirements:
  - 若 DOCUMENT_INDEX §5 仍引用 ARCH_FLOW 舊版檔名（__260101），需更新為 __260102 以維持新對話載入一致性。  
  - 後續若要將 FULL_ARCH／UI_SPEC／DEPLOY_OPS／DATA_SOURCES 等逐檔一致化為同一「最大完備＋累積式更新」口徑，請沿用本次同一交付格式（正文單一口徑、留痕分離、hash manifest）。  
