...
（PROJECT_PROMPT｜單一正確正文版｜2026-01-02）
---

# 稽核留痕（Audit｜不屬正文｜避免新舊混讀）

> 本章僅作為本次融合更新之留痕承接；不新增治理規則、不覆蓋正文。
> 正文以本檔案上半部（至「（PROJECT_PROMPT｜單一正確正文版｜2026-01-02）」）為準。

## A. Scope（適用範圍）
- scope_doc_key: PROJECT_PROMPT
- scope_files_inputs:
  - TAITS_PROJECT_PROMPT__251231.md（歷史版本）
  - TAITS_PROJECT_PROMPT__260102.md（工作中版本）
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）

## B. Changelog（變更清單｜摘要）
1) **治理更新哲學統一**：將文件內殘留之「只能加／只可新增／只增不減」等口徑，統一改寫為「最大完備＋累積式更新（允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要化縮水；未被新版本明確取代之有效內容一律保留並持續累積；已被新版本明確取代者可自正文移除但必須由稽核留痕承接）」；並修正舊版中可能導致新舊混讀的敘述。
2) **移除補丁標記語境**：將歷史版本中以「附加/補丁」呈現之內容，整合為正文的連續章節；保留內容但移除會造成新舊混讀的標籤語境（例如加註版本字樣、凍結標記等）。
3) **L9–L11 強制定錨保留且上置**：保留並上置「L9 投資報告／L10 人類裁決／L11 全層稽核回放」之詳細定義，確保跨文件一致且可直接引用。
4) **文件清單改為 Index Gate 快照**：將舊版中容易被誤用為「固定 ACTIVE 集合/固定數量」的清單，改為「依 DOCUMENT_INDEX §5 的可操作快照」呈現，並強化「不得固化 ACTIVE 集合」之約束。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit 章）
- hash_value_sha256: 3740cc7a98a5d7b7f877254d005f3098eaf527fa4218d8b2e03aad14483e49cd

## D. Audit Hand-off（裁決承接）
- change_id: PROJECT_PROMPT-FUSION-20260102-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=PROJECT_PROMPT｜融合更新／重排版）
- downstream_requirements:
  1) 請於 DOCUMENT_INDEX（doc_key=DOCUMENT_INDEX）之 Authoritative Index 表格中，承接本文件版本日期與檔名（__260102）。
  2) 後續若需承載「本地環境／硬體規格／電腦規格」等具體環境資訊，應由 LOCAL_ENV（doc_key=LOCAL_ENV）正文承接；PROJECT_PROMPT 僅保留引用、邊界與操作契約，不保存具體規格。

## E. 歷史快照留存（供稽核回放）
> 下列為歷史版本中之「專案文件載入清單（19 份）」快照，已由正文第 9 章「依 DOCUMENT_INDEX §5 快照」明確取代；保留於此僅供稽核回放，不得作為 ACTIVE 裁決依據。
> 為避免新舊混讀，本快照已將歷史口徑（例如「只能加」）正規化為本文件現行之「最大完備＋累積式更新（允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要化縮水；未被新版本明確取代之有效內容一律保留並持續累積；已被新版本明確取代者可自正文移除但必須由稽核留痕承接）」表述；檔名與 doc_key 仍保留原樣供對照。
