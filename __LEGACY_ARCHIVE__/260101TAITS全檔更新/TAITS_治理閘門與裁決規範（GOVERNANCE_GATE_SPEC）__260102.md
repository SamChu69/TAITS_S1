# TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102

doc_key：GOVERNANCE_GATE_SPEC  
適用範圍：TAITS 全系統（Research / Backtest / Simulation / Paper / Live）  
版本狀態：ACTIVE（治理裁決核心文件｜最大完備＋累積式更新）  
...
### 0.1 適用範圍（Hard Boundary｜避免越權與混讀）
...
## 1. 全局定錨｜單一口徑（S1｜跨文件一致）

### 1.1 人類最高決策者主權（SOVEREIGNTY）
- TAITS 之唯一最高主權屬於人類最高決策者（產品負責人／架構裁決者）。
- 任何治理閘門、程序規則、Agent、文件等級不得凌駕人類主權；不得以程序性理由阻止人類明確命令之生效。
- 風險與合規（Risk/Compliance）在無人類明確命令時可否決；在有人類明確命令時必須輸出完整風險揭露與替代方案，並以「強制揭露＋確認＋全紀錄」承接，不得以程序性理由卡死更新。
...
- 有效 HFI 存在時：任何治理流程與限制口徑不得阻擋 scope 範圍內之**融合更新／覆寫修正／重排版**；並必須同步產生稽核承接（VERSION_AUDIT 留痕、HASH_MANIFEST、CHANGELOG）。

## 2. 核心鐵律（Hard Gates｜不可動搖）
...
## 3. Gate 在 Canonical Flow 的定位（L9 主 Gate + 全鏈路子 Gate）
...
## 5. Gate 優先序與衝突裁決（Priority & Conflict Resolution｜對齊 DOCUMENT_INDEX）
...
### 5.1 治理等級與 ACTIVE 身份之唯一裁決（Index Gate｜不可否認）
### 0.1 Index 表格裁決優先
...
### 5.2 裁決順序字串之法律定位（Mnemonic ≠ Override Rule）
...
### 5.3 本文件之衝突裁決「唯一可採用」程序（對齊 DOCUMENT_INDEX）
...
### 5.4 Index Gate 身份裁決、Label 解讀與資料 alias 封口（避免錯誤引用）
並統一裁決：
...
### 5.5 最小可稽核引用格式（Minimum Legal Citation Format）
...
### 5.6 文件身份與引用合法性（Canonical Identity & Citation Rules）
### A1. 本文件之唯一治理身份（Canonical Identity）
- canonical_filename（Index 裁決檔名）：`TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102.md`
- canonical_doc_key（Index 裁決識別碼）：`GOVERNANCE_GATE_SPEC`
- 版本狀態：ACTIVE（最大完備＋累積式更新）

### A2. 本專案目錄中的實體檔案（Physical Artifact）
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。

### A3. 引用合法性最小規則（不新增制度，只固定寫法）
- 任何跨文件引用本文件時，必須使用：`doc_key=GOVERNANCE_GATE_SPEC` + `canonical_filename=TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102.md`。  
...
## 6. Gate 裁決原因碼（Gate Reason Codes｜最大完備）
...
- GOV-SCOPE-01：（歷史術語：Freeze） 後越界變更（（歷史術語） 違規跡象）
...
## 8. Gate 與 Risk Gate（L7）關係（Two-Gate Model）
...
### 9.1 最小引用標頭與 Evidence Chain（可回放最低要求）
...
timestamp = <ISO-8601>
doc_key = GOVERNANCE_GATE_SPEC
doc_title = TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102.md
...
- scope_files_modified: TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）

## B. Changelog（變更清單）
1) 將檔頭「只能加／只增不減」限制口徑，統一改為「最大完備＋累積式更新」：允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要化縮水。  
2) 移除會造成新舊混讀的補丁式章節（例如格式修補宣告類內容），並將其「必要語義」改以正文結構承接（例如：Mnemonic 定位、引用合法性、最小可稽核引用格式）。  
3) 重新排版、依序排列：以「文件定位→全局語義定錨→核心鐵律→Flow 定位→邊界→衝突裁決→原因碼→規則全集→Two-Gate→審計輸出→UI→流程圖」建立單一路徑閱讀順序。  
4) 修復 Markdown 區塊邊界風險：補齊 EC1 的 code fence，避免後續章節被誤判為 code block。  
5) 更新文內引用之檔名日期碼：統一為本次正文版 TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102.md，並移除 Freeze/Only-Add 類狀態標記造成的誤讀風險。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: c0ca889e577d9de96cd93515fed2d23efd36c91a7498d104801b8f7e778066b6

## D. Audit Hand-off（裁決承接）
- change_id: GGS-FUSION-260102-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=GOVERNANCE_GATE_SPEC 文件融合更新）
- downstream_requirements:
  - 請將本次變更承接落入 `VERSION_AUDIT`（doc_key=VERSION_AUDIT），並更新 `DOCUMENT_INDEX` 之版本日期/檔名映射（若有表列）。
  - 若其他文件仍出現舊式「順位字串=覆蓋規則」或「缺引用欄位仍可 PASS」等口徑，需依同一模式逐檔融合更新以完成全局一致化。
