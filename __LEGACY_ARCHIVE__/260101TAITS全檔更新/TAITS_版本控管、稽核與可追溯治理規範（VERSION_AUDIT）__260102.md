...
治理等級：B（制度級｜最終以 DOCUMENT_INDEX 裁決為準）  
...
平行參照：GOVERNANCE_GATE_SPEC／RISK_COMPLIANCE／EXECUTION_CONTROL／LOCAL_ENV／DEPLOY_OPS／UI_SPEC  
...
### 0.1 HUMAN SOVEREIGNTY｜最高裁決置頂條款（Top-of-File）## 全局定錨｜單一口徑（S1）
...
### 6.3 Freeze 狀態（狀態約束，不等於禁止修正）
Freeze 的意義是「限制未授權之結構性變更與範圍擴張」，不是把系統鎖死在錯誤口徑。  
...
> 已移除 補充條目/補充資料集/最大完備＋累積式更新 等補丁式標籤，以避免新舊混讀；其內容語義由本文件正文與 DOCUMENT_INDEX 裁決承接。

### 20.1 台帳批次｜A｜METADATA_FIX Ledger（累積式）— Extension Batch 2025-12-28（P0-CLOSURE）

| fix_id | date | batch_id | target_doc_key | output_artifact | change_type |
|---|---|---|---|---|---|
| VA-METADATA_FIX-20251228-0029 | 2025-12-28 | BATCH-20251228-P0-CLOSURE | AI_GOV | `TAITS_AI_行為與決策治理最終規則全集__251217__ADDENDUM_20251228H_FINAL.md` | 台帳新增｜P0 全閉合：Index-First doc_key 解析鍵封口＋machine-readable override |
| VA-METADATA_FIX-20251228-0030 | 2025-12-28 | BATCH-20251228-P0-CLOSURE | GOVERNANCE_STATE_FREEZE_V1 | `TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251220__ADDENDUM_20251228H_FINAL.md` | 台帳新增｜P0 全閉合：舊模板作廢（保留歷史）＋唯一合法新模板固定＋alias 封口 |
| VA-METADATA_FIX-20251228-0031 | 2025-12-28 | BATCH-20251228-P0-CLOSURE | DOCUMENT_INDEX | `TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228H_FINAL.md` | 台帳新增｜P0 全閉合：GOVERNANCE_STATE 正名封口＋mapping override patch（delta） |
| VA-METADATA_FIX-20251228-0032 | 2025-12-28 | BATCH-20251228-P0-CLOSURE | VERSION_AUDIT | `TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219__ADDENDUM_20251228H_FINAL.md` | 台帳新增｜P0 全閉合：Ledger 補登（累積式） |
...
（補充條目 2025-12-28｜台帳新增｜VERSION_AUDIT｜P0 全閉合 完）
---

# 補充條目 2025-12-28｜台帳新增：P2 Completeness／Banner Ledger（稽核補登｜（歷史狀態記錄））
...
| fix_id | date | batch_id | target_doc_key | output_artifact | change_type |
|---|---|---|---|---|---|
| VA-METADATA_FIX-20251228-0036 | 2025-12-28 | BATCH-20251228-P2-COMPLETENESS | DOCUMENT_INDEX | `TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228K_FINAL.md` | 台帳新增｜v2 latest_mapping 全量補齊（ACTIVE 全覆蓋＋入口/定錨文件補登） |
| VA-METADATA_FIX-20251228-0037 | 2025-12-28 | BATCH-20251228-P2-COMPLETENESS | GOVERNANCE_STATE_FREEZE_V1 | `TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251220__ADDENDUM_20251228K_FINAL.md` | 台帳新增｜S2 Template Hard Banner（防誤抄封口） |
| VA-METADATA_FIX-20251228-0038 | 2025-12-28 | BATCH-20251228-P2-COMPLETENESS | VERSION_AUDIT | `TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219__ADDENDUM_20251228K_FINAL.md` | 台帳新增｜Ledger 補登（累積式） |
...
（補充條目 2025-12-28｜台帳新增｜VERSION_AUDIT｜P2 Completeness／Banner Ledger 完）
---

# 補充條目 2025-12-28｜台帳新增：P3 Canonical Bundle Ledger（稽核補登｜（歷史狀態記錄））
...
| fix_id | date | batch_id | target_doc_key | output_artifact | change_type |
|---|---|---|---|---|---|
| VA-METADATA_FIX-20251228-0039 | 2025-12-28 | BATCH-20251228-P3-CANONICAL-BUNDLE | DOCUMENT_INDEX | `TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228L_FINAL.md` | 台帳新增｜schema_version=3 Canonical Bundle（單一可載入束：legacy 解析＋完整 mapping） |
| VA-METADATA_FIX-20251228-0040 | 2025-12-28 | BATCH-20251228-P3-CANONICAL-BUNDLE | DOCUMENT_INDEX | `TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228L_FINAL.md` | 台帳新增｜PROJECT_PROMPT ACTIVE 補登（Authoritative Index Amendment，append-only） |
| VA-METADATA_FIX-20251228-0041 | 2025-12-28 | BATCH-20251228-P3-CANONICAL-BUNDLE | VERSION_AUDIT | `TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219__ADDENDUM_20251228L_FINAL.md` | 台帳新增｜Ledger 補登（累積式） |
...
（補充條目 2025-12-28｜台帳新增｜VERSION_AUDIT｜P3 Canonical Bundle Ledger 完）
---

# 補充條目 2025-12-28｜台帳新增：P4 Copy-Safe DocKey Ledger（稽核補登｜（歷史狀態記錄））
...
| fix_id | date | batch_id | target_doc_key | output_artifact | change_type |
|---|---|---|---|---|---|
| VA-METADATA_FIX-20251228-0042 | 2025-12-28 | BATCH-20251228-P4-COPYSAFE | DOCUMENT_INDEX | `TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228M_FINAL.md` | 台帳新增｜Copy-Safe DocKey List（人因防呆：表格省略禁用＋完整鍵清單） |
| VA-METADATA_FIX-20251228-0043 | 2025-12-28 | BATCH-20251228-P4-COPYSAFE | VERSION_AUDIT | `TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219__ADDENDUM_20251228M_FINAL.md` | 台帳新增｜Ledger 補登（累積式） |
...
（補充條目 2025-12-28｜台帳新增｜VERSION_AUDIT｜P4 Copy-Safe DocKey Ledger 完）
---
...
> 目的：在不改寫本文件既有任何章節內容與語義的前提下，補齊「引用合法性」所需的識別資訊與檔名映射規則；避免因 補充條目/下載副本檔名差異導致 Gate / 稽核引用失配。
...
# 補充條目 2025-12-28｜台帳新增：GLOBAL_ALIGNMENT_PATCH（MASTER_CANON 對齊全域補丁）｜（歷史狀態記錄）
...
- 本文件 `doc_key` 既有標示為 `VERSION_AUDIT`；本 補充條目 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
...
- 本 補充條目 為 台帳新增；不改寫本文件任何既有條款。  
- 本 補充條目 之解讀與適用，均以 DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON 為最終裁決順序；AI 行為仍以 AI_GOV 為最高約束。  
- 本 補充條目 目的僅在於消解引用歧義、避免 Gate 因 metadata/格式缺口而誤判，並確保全鏈路可稽核、可回放。

（補充條目 2025-12-28｜台帳新增｜（歷史狀態記錄） 完）

---

# 補充條目 2025-12-28｜台帳新增：METADATA_FIX Ledger 續增（0023–0025）｜（歷史狀態記錄）
...
# 補充條目 2025-12-31｜台帳新增：Bundle 251231 Physical Map × Hash Ledger 補登（（歷史狀態記錄））

> 補充性質：台帳新增（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219.md（doc_key：VERSION_AUDIT）  
> 生效狀態：GOVERNANCE_STATE = （歷史狀態記錄）  
> 上位裁決：DOCUMENT_INDEX（A+）→ MASTER_ARCH（A）→ MASTER_CANON（A）＋AI_GOV（A+）  
> 目的：將 2025-12-31（Bundle 251231）之「實體檔案映射（Physical Artifact Map）」與「sha256_12 hash ledger」以稽核條目形式補登，確保 repo 落盤檔案可被回放與追溯；本 補充條目 **不改寫**既有 Ledger/章節內容。
...
| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0001 | 2025-12-31 | BATCH-20251231-P0-PHYSICAL-ARTIFACT-MAP | Physical Artifact Map × Hash Ledger（__251231 落盤對齊） | 將 repo 內 22 份實體檔（含 21 ACTIVE + 1 SUPPORT）以 mapping + sha256_12 補登；供 Gate/Loader/稽核回放使用。對應 DOCUMENT_INDEX｜補充條目 2025-12-31（N1–N4）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
（補充條目 2025-12-31｜台帳新增｜（歷史狀態記錄） 完）

# 補充條目 2025-12-31｜台帳新增：P1｜Reference Alias Expansion 補登（（歷史狀態記錄））

> 補充性質：台帳新增（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219.md（doc_key：VERSION_AUDIT）  
> 生效狀態：GOVERNANCE_STATE = （歷史狀態記錄）  
> 上位裁決：DOCUMENT_INDEX（A+）→ MASTER_ARCH（A）→ MASTER_CANON（A）＋AI_GOV（A+）  
> 目的：補登 P1 之 legacy 引用解析（Reference Alias Expansion）事件，使 Gate/Loader/稽核回放在 （歷史狀態記錄） 期間可穩定解析舊檔名/工程路徑/舊 addendum/樣板引用。
...
| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0002 | 2025-12-31 | BATCH-20251231-P1-REFERENCE-ALIAS-EXPANSION | Reference Alias Map（legacy refs → physical） | 補登 90 筆 legacy 引用檔名解析（含舊版版本戳、README.md、工程路徑、舊 addendum 檔名、樣板檔名）。對應 DOCUMENT_INDEX｜補充條目 2025-12-31｜P1（P1.1–P1.3）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
（補充條目 2025-12-31｜台帳新增｜P1 完）

# 補充條目 2025-12-31｜台帳新增：P1B｜Reference Alias Extension 補登（（歷史狀態記錄））

> 補充性質：台帳新增（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219.md（doc_key：VERSION_AUDIT）  
> 生效狀態：GOVERNANCE_STATE = （歷史狀態記錄）  
> 上位裁決：DOCUMENT_INDEX（A+）→ MASTER_ARCH（A）→ MASTER_CANON（A）＋AI_GOV（A+）  
> 目的：補登 P1B 之 Reference Alias Extension 事件，使 Postcheck v4 新發現之 legacy 引用（多為 addendum/inlineappendix/工程路徑）可回放解析。
...
| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0003 | 2025-12-31 | BATCH-20251231-P1B-REFERENCE-ALIAS-EXTENSION | Reference Alias Extension（v4 補登） | 補登 21 筆 legacy 引用解析（多為 addendum/inlineappendix/工程路徑）。對應 DOCUMENT_INDEX｜補充條目 2025-12-31｜P1B（P1B.1–P1B.3）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
（補充條目 2025-12-31｜台帳新增｜P1B 完）

# 補充條目 2025-12-31｜台帳新增：P1C｜Canonical Ref Alias Map 補登（（歷史狀態記錄））
...
- scope_files_modified: TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）

## B. Changelog（變更清單）
1) 置頂標頭補齊「治理等級」：明確標示本文件屬 B（制度級），並宣告最終裁決以 DOCUMENT_INDEX 為準，避免文件自我宣稱造成混讀。  
2) 將更新哲學由「最大完備＋累積式更新/只能加/只增不減/不得覆寫」等口徑，統一改為「最大完備＋累積式更新」：允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要縮水；未被新版本明確更新之有效內容必保留並持續累積；被明確取代者由稽核留痕承接。  
3) 修正 VERSION_AUDIT 與「個人硬體規格」的邊界：本文件正文不保存個人電腦硬體備忘，改以 `env_fingerprint`/`env_fingerprint_ref` 指向 `doc_key=LOCAL_ENV`（環境治理主檔）；如需設備基線，應由 MASTER_ARCH/LOCAL_ENV 承接，以避免制度文件混讀。  
4) 重新排版整序：移除多次生成之「治理補強（已整合為正文）」與「版本戳記」補丁式段落；將 A1–A4、J1–J3、G1–G5、Ledger 等零散章節整併為連續章節（第 15–19 章），提升引用定位穩定性。  
5) 融合收錄 S1 歷史台帳條目資料集：將 S1 檔案之 補充資料集 台帳批次收錄為本檔 §20，並移除 補充條目/補充資料集/最大完備＋累積式更新 等補丁式標籤以避免新舊混讀（內容保留、語義由正文與 DOCUMENT_INDEX 承接）。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: 392c3940ef7c2e82622f1c09a13660e1e4e1d907cc3335b276403ba11d99c982

## D. Audit Hand-off（裁決承接）
- change_id: VA-FUSION-260102-0002
- authority_basis: HFI（人類最高決策者明確命令｜scope=VERSION_AUDIT 文件融合更新）
- downstream_requirements:
  - 請將本次版本日期/檔名映射更新承接至 DOCUMENT_INDEX（doc_key=DOCUMENT_INDEX）（若其表列/引用需更新）。  
  - 請同步檢查 LOCAL_ENV 是否已具備 `env_fingerprint`/`env_fingerprint_ref` 規範與（若需要）設備基線專章；VERSION_AUDIT 僅維持引用關係，不保存硬體備忘。  
  - 其餘仍採「只能加」口徑作為現行更新規則之文件，應依本文件 §6 的更新哲學逐檔融合更新以完成全局一致化。
