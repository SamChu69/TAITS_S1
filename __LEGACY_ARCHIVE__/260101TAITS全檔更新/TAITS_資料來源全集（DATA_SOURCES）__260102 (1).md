...
### 3.6 Macro（宏觀）
- **定位**：宏觀資訊是「由上而下（全球→區域→台灣）」的 **Regime/Context 證據層**，用於 L6（Regime/Context）與 L8（Risk），並在 L9 報告層以「可追溯、可更新」方式呈現（不得直接跳接 L10/L11 裁決／下單）。
- **核心範圍（最大完備）**：
  - **全球/跨境總量**：GDP、通膨、就業、金融情勢、信用/槓桿、國際收支、全球景氣領先指標（IMF / World Bank / OECD / BIS / UN / ADB 等）
  - **主要經濟體**（US/EU/JP/CA/UK 等）：政策利率、殖利率曲線、匯率、CPI/PPI、就業、工業/零售、領先/信心指標（FRED、ECB、Eurostat、BoE、Bundesbank、BoC 等）
  - **商品/能源/運輸**：油氣煤電、原物料、航運運價、供需庫存（IMF PCPS、World Bank Pink Sheet、EIA、IEA 等）
  - **全球風險/資金價格**：美元指數、全球利差、波動率/風險偏好代理指標（以官方或準官方來源為主，必要時由授權商業供應商補足）
- **台灣宏觀**：利率/匯率/金融統計、景氣循環、物價、就業、產業景氣、國際收支等（CBC/DGBAS/NDC 等官方來源優先；詳見 §20 宏觀資料）
...
## 20. 宏觀資料（Macro）
### 20.0 定位（Macro＝全球→區域→台灣之上行證據層）
- **宏觀資訊（Macro）不是「國內資訊」的同義詞**：它指的是會 **跨市場、跨資產、跨周期** 影響資金價格與風險溢酬的上層訊號。
- **TAITS 用法（嚴格對齊）**：
  - **L6（Regime/Context）**：用宏觀序列與政策事件建立「狀態」與「情境」；
  - **L8（Risk/Compliance）**：用宏觀衝擊（利率、匯率、流動性、信用）對部位/槓桿/品種曝險做約束；
  - **L9（投資報告層）**：必須把宏觀證據「數據化、來源可追溯、可更新」，並可回放；
  - **L10（人類裁決）**：宏觀只提供證據，不得替代裁決；
  - **L11（全留痕回放）**：保存宏觀資料快照、版本、取數參數與落地檔案指紋。
- **影響方向（不得硬編成鐵律）**：
  - **常態先驗**：全球（特別是美國/歐洲貨幣政策與流動性）→區域→台灣；
  - **例外但必須支援**：台灣重大產業/供應鏈事件可回傳影響全球風險偏好與特定資產（需以事件+數據佐證，且不得以直覺硬判）。

---

### 20.1 全球/跨境總量（Global Macro Aggregates）
> 目標：以「跨國可比較、長序列、可程式化取數」為優先，做為宏觀基底層。

- subcategory_id（建議）：
  - `MC_GLOBAL_IMF_SDMX`（IMF：WEO/IFS/GFS/BOP/PCPS 等，SDMX）
  - `MC_GLOBAL_WB_WDI`（World Bank：WDI/各主題指標，REST/JSON）
  - `MC_GLOBAL_OECD_SDMX`（OECD：各類總量與領先指標，SDMX）
  - `MC_GLOBAL_BIS_SDMX`（BIS：金融統計、信用/槓桿、有效匯率等，SDMX）
  - `MC_GLOBAL_UN_SDG_SDMX`（UN：SDG 統計，SDMX）
  - `MC_GLOBAL_ADB_SDMX`（ADB：Key Indicators Database，SDMX）
- 核心指標域（最大完備骨架）：
  - 成長：GDP（實質/名目）、工業生產、零售、出口/進口
  - 物價：CPI/PPI/核心通膨
  - 就業：失業率、就業人口、薪資/工時
  - 外部：經常帳、貿易收支、外匯存底
  - 金融：信用增速、槓桿、跨境資金流、金融情勢（Financial Conditions）
  - 領先：OECD CLI、各國景氣循環指標（官方/準官方）

- 主要官方介面（可程式化）：
  - IMF Data API（SDMX 2.1/3.0）— 以 SDMX Data + 結構（DSD）查詢落地取數。  
    參考：IMF API Resource Page（data.imf.org）。  
  - World Bank Indicators API（REST）— 指標/國別/期間可查詢，覆蓋大量長序列。  
  - OECD API（SDMX）— SDMX 標準介面。  
  - BIS Stats API（SDMX）— BIS 統計之 SDMX 介面。  
  - Eurostat/UN/ADB 等：優先採用 SDMX 介面（同樣走「結構→取數」模式）。

---

### 20.2 美國（US Macro：政策/利率/就業/物價/財政/能源）
- subcategory_id（建議）：
  - `MC_US_FRED`（FRED/ALFRED：美國與全球多來源序列匯聚）
  - `MC_US_TREASURY_FISCAL`（U.S. Treasury Fiscal Data：財政與債務）
  - `MC_US_EIA`（EIA：能源/油氣庫存/價格/產量）
- 主要官方介面：
  - FRED API（St. Louis Fed Web Services）：需要 API key，支援 series/release/category 等查詢。
  - U.S. Treasury Fiscal Data API：官方財政資料 API。
  - EIA Open Data API：需註冊 API key。
- 典型使用：
  - 利率路徑、殖利率曲線、通膨與預期、就業、美元流動性/財政供給、油氣供需與庫存。

---

### 20.3 歐洲（EU/Euro Area Macro：ECB＋Eurostat）
- subcategory_id（建議）：
  - `MC_EU_ECB_SDMX`（ECB Data Portal：SDMX）
  - `MC_EU_EUROSTAT_SDMX`（Eurostat：SDMX 2.1/3.0）
- 主要官方介面：
  - ECB Data Portal SDMX 2.1 RESTful web service。
  - Eurostat SDMX API（2.1/3.0）。
- 典型使用：
  - 歐元區政策/利率、匯率與金融統計、通膨/就業、產業/貿易等。

---

### 20.4 其他主要央行/統計機構（UK/CA/DE/FR/ES…）
> 原則：優先「央行/統計局官方 API」，不足再用次級匯聚（例如 FRED）。

- subcategory_id（建議）：
  - `MC_UK_BOE`（Bank of England：Database）
  - `MC_CA_BOC_VALET`（Bank of Canada：Valet API）
  - `MC_DE_BBK_SDMX`（Deutsche Bundesbank：SDMX Web Service）
  - `MC_FR_BDF_WEBSTAT`（Banque de France：Webstat API）
  - `MC_ES_BDE_API`（Banco de España：Statistics Web Service API）
- 主要官方介面（示例）：
  - BoE Database：官方資料庫（可匯出；若需程式化，採其官方可用介面/下載方式落地）
  - BoC Valet API：可程式化取得匯率與多種金融序列
  - Bundesbank SDMX Web Service：SDMX data/metadata 端點
  - Banque de France Webstat API：API key + SDMX 格式支援
  - Banco de España Statistics web service（API）

---

### 20.5 商品/能源/原物料（Commodities & Energy）
- subcategory_id（建議）：
  - `MC_COMMOD_IMF_PCPS`（IMF Primary Commodity Prices / PCPS）
  - `MC_COMMOD_WB_PINK_SHEET`（World Bank Pink Sheet：商品價格）
  - `MC_ENERGY_EIA`（EIA：能源）
  - `MC_ENERGY_IEA`（IEA：能源資料服務；需 API key／授權條款）
- 核心範圍（最大完備骨架）：
  - 原油/成品油/天然氣/煤/電力與庫存
  - 工業金屬/貴金屬/農產品/化肥等大宗價格與指數
  - 與通膨/製造成本高度相關之能源與運輸代理指標
- 官方/準官方來源示例：
  - IMF Commodity Data Portal（PCPS）— 68 項商品價格與指數，並有資料集頁面（PCPS）。
  - World Bank Commodity Markets / Pink Sheet — 提供月度價格（XLS）與報告。
  - EIA Open Data — 能源序列 API。
  - IEA API — 以 IEA 帳號/金鑰存取其 API（依授權等級）。

---

### 20.6 國際貿易/供應鏈（Trade & Supply Chain）
- subcategory_id（建議）：
  - `MC_TRADE_UN_COMTRADE`（UN Comtrade：全球貿易統計；需 key）
- 核心範圍：
  - 進出口金額/量、主要品項與夥伴國、月頻貿易動能（用於需求/景氣/外部衝擊判讀）

---

### 20.7 台灣宏觀（TW Macro：利率/匯率/景氣/物價/就業/外部）
- subcategory_id（既有/建議補齊）：
  - `MC_CBC_RATES`（央行：利率/匯率/金融統計）
  - `MC_DGBAS_ECON`（主計總處：GDP/CPI/就業等）
  - `MC_NDC_CYCLE`（國發會：景氣循環/領先/同時指標）
- Sources（官方入口；可追溯）：
  - CBC（中央銀行）官方統計入口
  - DGBAS（行政院主計總處）官方統計入口
  - NDC（國家發展委員會）景氣指標入口
- Provenance（強制）：
  - 必須保存 `series_id`/表號/期別、發布日期、修正公告與版本戳記；
  - 若同一指標存在多個來源（例如匯率），需指定 **Primary Source** 與 **Fallback Source**。

---

### 20.8 Macro Source Registry（註冊表範本｜可落地）
> 下列為「宏觀來源」在 §4 註冊表的建議欄位填法（示例級），供工程落地參照。

- 例：IMF SDMX（Global）
```yaml
source_id: SRC_IMF_SDMX
provider: IMF
jurisdiction: GLOBAL
category: Macro
subcategory_id: MC_GLOBAL_IMF_SDMX
access:
  type: SDMX_REST
  base_url: https://data.imf.org/en/Resource-Pages/IMF-API
  auth: NONE_OR_KEY_IF_REQUIRED
update:
  cadence: "dataset-dependent"
  revision_policy: "keep_release_and_revision_metadata"
provenance:
  required_fields: [dataflow_id, dsd_version, series_key, query_params, retrieved_at_utc]
```

- 例：World Bank Indicators API（Global）
```yaml
source_id: SRC_WB_INDICATORS
provider: World Bank
jurisdiction: GLOBAL
category: Macro
subcategory_id: MC_GLOBAL_WB_WDI
access:
  type: REST_JSON
...
update:
  ...
  revision_policy: "keep_last_updated_and_source_notes"
provenance:
  required_fields: [indicator_id, country_code, date_range, query_params, retrieved_at_utc]
```

- 例：FRED API（US + Global Aggregates）
```yaml
source_id: SRC_FRED
provider: Federal Reserve Bank of St. Louis
jurisdiction: US_GLOBAL
category: Macro
subcategory_id: MC_US_FRED
access:
  type: REST_JSON
...
update:
  ...
  revision_policy: "keep_vintage_if_using_alfred"
provenance:
  required_fields: [series_id, api_version, query_params, retrieved_at_utc]
```

---

### 20.9 與「新聞/事件」之邊界（避免新舊混讀）
- **政策決議/記者會/會議紀要**：
  - 本質屬「宏觀事件」，但其 **數據化落地（時間戳＋決議內容摘要＋原始來源）** 仍歸入 Macro，並在 News/Events 保留引用索引（不得重複存兩份不同語義版本）。
- **媒體解讀/評論**：
  - 一律歸入 News/Events（輔助），不得替代官方或宏觀序列。
...
2) 宏觀資料最大完備補齊：重寫 §20「宏觀資料（Macro）」為「全球→區域→台灣」分層宇宙，補齊跨境總量、主要央行/統計機構、商品/能源、貿易/供應鏈、以及可落地的 Macro Source Registry 範本；並同步補強 §3.6 Macro 之分類說明，避免「宏觀＝國內」混讀。  
3) 語義定錨一致化：於檔頭/正文鎖定 L9/L10/L11 定義與資料邊界，避免資料來源文件對決策/下單層產生混讀。  
4) 重新排版與依序排列：保留全部既有有效內容，僅做章節依序整理與編號連續化（0..N），避免章節跳號造成引用不穩定。  

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: dd5d84aa86b364f886e9c1454ca8d24cf43f0f02ef13d68f6533cffad65e7a9c

## D. Audit Hand-off（裁決承接）
- change_id: DS-FUSION-260102-0002
...
- downstream_requirements:
  ...
  - 後續若 UI_SPEC／ARCH_FLOW／DEPLOY_OPS 等仍含舊口徑或宏觀/新聞邊界混讀，應沿用同一交付規格逐檔一致化，以完成治理口徑與語義定錨的一致性。  
