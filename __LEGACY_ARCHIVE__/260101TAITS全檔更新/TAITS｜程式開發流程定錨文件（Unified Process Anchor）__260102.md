# TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260102.md
...
- 0. Scope Lock（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 下的硬邊界）
  - 0.1 本合併手冊可以做什麼
  - 0.2 本合併手冊不得做什麼
- 1. 你現在的問題：檔案太多，要合併——最佳作法（合規且最省事）
  - 1.1 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 下「合併」的正確定義
  - 1.2 最佳落地策略（推薦）
- 2. 雙錨整合（你要記住的核心）
  - 2.1 兩個錨點各司其職（不可互相取代）
  - 2.2 本合併手冊的定位
...
- 0. Scope Lock（必讀｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 下的硬邊界）
  - 0.1 本文件**可以**做什麼
  - 0.2 本文件**不得**做什麼（硬禁止）
...
- 7. 你每天照做的「最短正路」（推薦節奏）
- 8. 常見失敗模式與硬性修正方式（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
  - 8.1 失敗：換新對話接不起來
...
治理狀態：以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）  
最後更新：2025-12-29  
...
- GOVERNANCE_STATE = 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）
...
新增日期：2025-12-28  
生效前提：GOVERNANCE_STATE = 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）（最大完備＋累積式更新）  
...
版本日期：2026-01-01  
合併目的：在不違反 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） / 最大完備＋累積式更新 的前提下，把「日常操作流程」集中到**一份文件**，避免檔案過多造成新手困擾與新對話斷線。  
...
嚴格對齊模式：以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） / 最大完備＋累積式更新 / Index Gate First。
...
〔TAITS 最小引用標頭｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）｜最大完備＋累積式更新〕
ref_file = <完整檔名>
ref_doc_key = <DOC_KEY>
...
```text
〔TAITS Cursor 工程任務｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）｜最大完備＋累積式更新〕
硬性約束：
...
以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 下，一律 最大完備＋累積式更新：保留既有錨點、以新增並列方式演進。
...
```text
請進入嚴格對齊模式（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） / 最大完備＋累積式更新 / Index Gate First）。

〔TAITS 引用標頭｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）｜最大完備＋累積式更新〕
ref_file = <完整檔名>
ref_doc_key = <DOC_KEY>
...
# TAITS｜GPT 新對話續編編輯 SOP（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）｜Index Gate First）__ADDENDUM_20251228R_FINAL
...
- 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 下，「越做越像改寫制度」的風險會隨對話長度上升
...
本段為 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）（最大完備＋累積式更新）下之「工程操作導流」新增段落，不改寫任何治理文件語義。  
...
本段為 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）（最大完備＋累積式更新）下之導流新增段落，不取代本文件作為「主流程骨架」之定位。  
...
本文件內容已納入下列「合併版總手冊」作為日常唯一入口；本文件仍保留作歷史快照與追溯參考（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）：最大完備＋累積式更新，不刪檔不覆寫）。
...
- 完全符合 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）（最大完備＋累積式更新）與「不取代既有語義」要求
...
- 檔案：TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260102.md
- 適用狀態：ACTIVE
- 適用時間：2026-01-02（Asia/Taipei）
- 本次作業：融合更新（口徑統一：Only-Add → 最大完備＋累積式更新）、重排版（新增目錄依序排列）、稽核留痕分離（Scope/Changelog/Hash/Audit Hand-off）。

## B. Changelog（變更清單）
1) 口徑統一：將全文殘留之「Only-Add/只可新增/只增不減/只能加/不得覆寫」等流程約束口徑，統一為「最大完備＋累積式更新」治理口徑（允許融合更新/覆寫修正/重排版；禁止摘要縮水；未被新內容明確取代者必保留；被取代者可移除但須留痕承接）。
2) 混讀防護：將 Freeze/FREEZE 等標記改為「以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）」的表述，避免狀態混讀。
3) 重排版：新增「目錄（依序排列）」以提升定位效率；正文有效資訊不減少。
4) 稽核留痕：新增本檔 Scope／Changelog／Hash Manifest／Audit Hand-off，並與正文分離。

## C. Hash Manifest（指紋清單）
- file_name: TAITS｜程式開發流程定錨文件（Unified Process Anchor）__260102.md
- hash_algo: sha256
- hash_value_sha256: 73bdd60b05293c260b84ee10cb7c48a6dd5615161814b6fddef594a62049c732

## D. Audit Hand-off（裁決承接）
- 裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（如有衝突以此序為準）。
- 承接聲明：本檔為工程流程定錨（治理等級 C/工程操作級），不得越權覆寫 A+/A/B 級治理裁決；所有治理裁決以其上位文件為準。
