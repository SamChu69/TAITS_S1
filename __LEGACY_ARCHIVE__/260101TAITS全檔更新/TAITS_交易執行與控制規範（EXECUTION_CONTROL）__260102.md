# TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260102.md
doc_key：EXECUTION_CONTROL  
治理等級：A（Execution & Control Specification｜嚴格執行層最高規範）  
適用範圍：TAITS 全系統（Research / Backtest / Simulation / Paper / Live）  
版本狀態：ACTIVE（執行控制規範，最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語） 演進）  
版本日期：2026-01-02  
變更原則：最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語）（最大完備（可融合更新／覆寫修正，但禁止摘要化縮水），不可刪減/覆寫/弱化安全機制/偷換定義）  
...
- **L10（人類裁決與交易決策層）**：唯一授權層；決定是否進入回測/模擬/紙上/實盤與授權封套（authorization envelope）。
...
## 10. Kill Switch 的觸發條件（示例｜可 最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語） 擴充）
...
最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語） 演進規則（本文件專屬）
允許新增：
...
附錄 A｜最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語）：對位 MASTER_CANON 的「L10→L11 執行接面」補強（Freeze v1.0）
...
（附錄 A｜最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語）：MASTER_CANON 對位補強 v2025-12-27 · Freeze v1.0 生效）
---

## 99. 補充對位與一致性（承接既有補強內容｜正文的一部分）
> 本章承接原文件中分散之「補充聲明 / 對位映射 / 例外備註」內容，統一集中於文末，以維持主文序列一致性並避免新舊混讀。
...
- 有效 HFI 存在時：Freeze/最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語）/Gate 不得阻擋 scope 範圍內之更新/覆寫/重排版；並必須同步產生稽核承接（VERSION_AUDIT 留痕、HASH_MANIFEST、CHANGELOG）。
...
為維持 Freeze v1.0 之 最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語） 原則：
...
- 版本狀態：ACTIVE（Freeze v1.0；最大完備（可融合更新／覆寫修正，但禁止摘要化縮水）／最大完備＋累積式更新（歷史術語））
...
- scope_files_modified: TAITS_交易執行與控制規範（EXECUTION_CONTROL）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）
- anchors_enforced:
  - 最大完備＋累積式更新（允許融合更新/覆寫修正/重排版；禁止摘要化縮水；未更新舊資訊持續累積；被取代者以稽核留痕承接）
  - L9＝投資報告層；L10＝人類裁決層；L11＝全層稽核回放層（L11 非批准亦非下單）
  - 禁止 AI 自行批准；禁止把 L11 當批准/下單層
  - 主文依序排列（維持章節序列一致性）

## B. Changelog（變更清單）
1) 將文件內「只能加／只增不減（歷史術語）」之更新限制口徑，統一改為「最大完備＋累積式更新」：允許融合更新／覆寫修正／重排版以形成單一正文；禁止摘要化縮水；未更新舊資訊必保留並持續累積；已被取代者以稽核留痕承接。  
2) 補強並固定跨文件一致之 L9/L10/L11 語義定錨，並新增「禁止 AI 自批／禁止把 L11 當批准或下單層」之硬性條款，避免執行控制被誤讀為自動批准鏈。  
3) 重新排版並依序排列：保留所有既有有效內容不縮水，將主文章節依數字序排列，分散補充內容集中至文末（§99）以避免主文序列被打斷與新舊混讀。  

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: 1896be40382fe552670eb0eef26208782e5efd15e4601d6bd504301ab0ba898f

## D. Audit Hand-off（裁決承接）
- change_id: EC-FUSION-260102-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=EXECUTION_CONTROL 文件融合更新）
- downstream_requirements:
  - 請將本次變更承接落入 `VERSION_AUDIT`（doc_key=VERSION_AUDIT），並更新 `DOCUMENT_INDEX` 之版本日期/檔名（若有表列）。
- rationale: 以單一正文消除舊式「只能加」限制與新舊混讀風險，並以最大完備哲學承接執行控制細節，同步確立 L9/L10/L11 與 AI 越權禁止條文，確保可稽核、可回放、可落地。
