# TAITS_全系統架構總覽（FULL_ARCH）__260102
...
- 有效 HFI 存在時：任何治理狀態（含 Freeze）/最大完備＋累積式更新/Gate 不得阻擋 scope 範圍內之更新/覆寫/重排版；並必須同步產生稽核承接（VERSION_AUDIT 留痕、HASH_MANIFEST、CHANGELOG）。
...
本文件若出現類似語句（例：「`source_id` 需存在於 DATA_UNIVERSE」），在 FREEZE 狀態 下的唯一合法解讀為：
...
- canonical_filename（Index 裁決檔名）：`TAITS_全系統架構總覽（FULL_ARCH）__260102.md`
...
- 任何跨文件引用本文件時，必須使用：`doc_key=FULL_ARCH` + `canonical_filename=TAITS_全系統架構總覽（FULL_ARCH）__260102.md`。  
...
- 本文件為 doc_key=FULL_ARCH 之「單一正確正文」；任何舊版、片段、補丁、Addendum/Appendix、或內嵌舊標頭均不再具有正文效力。
- 本文件之**版本狀態**為 ACTIVE；專案之**治理狀態**（例如 Freeze v1.0）由 `TAITS_GOVERNANCE_STATE__FREEZE_v1.0` 單一文件宣告與承接；本文件不重複宣告治理狀態以避免新舊混讀。
...
> 本章節為強制留痕（Scope／Changelog／Hash Manifest／Audit Hand-off），不屬 FULL_ARCH 正文；不得被任何正文引用所誤讀為「舊版補丁」或「可選附錄」。

## A. Scope（適用範圍）

- doc_key：FULL_ARCH
- canonical_filename：TAITS_全系統架構總覽（FULL_ARCH）__260102.md
- 版本日期：2026-01-02（Asia/Taipei）
- 融合輸入來源（Inputs）：
  - `TAITS_全系統架構總覽（FULL_ARCH）__260102.md`
  - `TAITS_S1_全系統架構總覽（FULL_ARCH）__251231.md`
- 本次變更類型：
  - 融合更新（Single Correct 正文）
  - 重排版（去除內嵌舊標頭、Part 分段、重複段落；修正章節號碼衝突）
  - 語義定錨一致化（L9/L10/L11）
  - 嚴格對齊：允許融合更新／覆寫修正／重排版；禁止摘要縮水；未被新內容取代之舊內容保留累積；被取代者可省略但需留痕承接

## B. Changelog（變更清單）

1. 移除正文中之內嵌舊標頭（`__251219` 片段）與 Part 分段標記，整併為單一正文版本。
2. 補強並固定 L9/L10/L11 定錨（加入 L9 必備內容、Tracking 欄位、L10 授權包絡、L11 雙料輸出），確保跨文件一致。
3. 將正文內「不得覆寫原文（僅限縮解讀）」口徑，統一修正為「最大完備＋累積式更新」治理哲學（允許融合更新/覆寫修正/重排版，但禁止摘要縮水；舊內容未被取代者保留）。
4. 修正 §5 中因歷史合併造成的章節號碼衝突（事件匯流排/Agent/AI/Storage 段落已改為 §5.3–§5.6 子章節），避免與 §6–§9 產生混讀。
5. 將原置於 §5 末尾之安全/權限與部署拓樸段落，移動至 §10–§13 並改為標準 Markdown 章節結構，以保持章節序列一致。
6. 將 S1 版本中「本地運算環境基線」視為 LOCAL_ENV 範疇，**不置入正文**；改以本章節「Legacy 保留＋交辦」方式承接，避免 FULL_ARCH 越級承載硬體細節。

## C. Hash Manifest（指紋清單）

- hash_algo：SHA-256
- body_sha256（不含本稽核章節）：`b1341926414ab5373fc008e0432c970a447d524ce5ba4dc91f99ec1282cf128d`
- full_sha256（含稽核章節）：`5f4acf7f9485e75046d5cc413c2ced1b8672fdd6b92e666eb6afee6a2d4b3416`

## D. Audit Hand-off（裁決承接／交辦）

- VERSION_AUDIT：請新增一筆登記（建議鍵）：`VA-FULL_ARCH-20260102-0001`
  - 內容至少包含：doc_key、canonical_filename、body_sha256、full_sha256、變更摘要（Changelog）、承接人/時間戳。
- LOCAL_ENV：請於後續更新 `TAITS_本地執行與運算環境規範（LOCAL_ENV）` 時，承接並歸檔下列「Legacy 本地運算環境基線」內容（避免資訊遺失，並使歸屬正確）。

### D.1 Legacy｜本地運算環境基線（待移入 LOCAL_ENV）
