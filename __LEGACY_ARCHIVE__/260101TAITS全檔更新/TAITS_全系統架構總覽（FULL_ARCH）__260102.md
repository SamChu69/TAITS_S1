...
flowchart LR
  ...
  INFRA --- UI
  INFRA --- EXE
12. 最大完備＋累積式更新 演進規則（FULL_ARCH 專屬）
允許：
...
適用範圍：TAITS 全系統（Research / Backtest / Simulation / Paper / Live）  
版本狀態：ACTIVE（架構層總覽，可隨系統擴充 最大完備＋累積式更新）  
版本日期：2026-01-01  
變更原則：最大完備＋累積式更新（可融合更新（但必留痕），不可刪減/覆寫/弱化邊界與否決鏈/偷換模組職責）
...
口徑對齊：  
目的：封住本文件中「DATA_UNIVERSE」被誤當成可引用治理文件（doc_key）的風險；將 FULL_ARCH 對資料來源治理的引用口徑固定回歸 `DATA_SOURCES`；並對正文中「source_id 需存在於 DATA_UNIVERSE」等語句做限縮解讀（最大完備＋累積式更新，不改原文），避免 Gate/Audit/新對話載入出現引用歧義。
...
- 版本狀態：ACTIVE（FREEZE 狀態；最大完備＋累積式更新）
...
- scope_files_modified: TAITS_全系統架構總覽（FULL_ARCH）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）

## B. Changelog（變更清單）
1) 更新文件治理口徑：將原先「僅允許追加/不可覆寫」之更新措辭，統一改為「最大完備＋累積式更新」；允許融合更新／覆寫修正／重排版以形成單一正確正文，禁止摘要縮水；未被新版本明確取代之有效內容必保留並持續累積；被取代者由稽核留痕承接。  
2) 重新排版與依序排列：消除原文中多段落並列、重複編號與跳號（如 0–4 後跳至 13–15）之結構，使全文主章節連續（0–9），可穩定引用。  
3) 融合整併「資料與引用治理」補強段落：將原先獨立的補強章節整併為正文 §6，並將其中「DATA_UNIVERSE 僅為 alias、doc_key 以 DATA_SOURCES 為準」等語義修正納入單一正確口徑，避免混用概念別名造成治理衝突。  
4) 中性化狀態描述：將條文敘述中的特定版本標記措辭改為「FREEZE 狀態」等中性描述，狀態定義由 GOVERNANCE_STATE 文件承接，避免新舊混讀。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: ca812d5a61d0c9b0577cbb0b305d29ce5ebab587aed6ee4daf667484aa0339fd

## D. Audit Hand-off（裁決承接）
- change_id: FA-FUSION-260102-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=FULL_ARCH 文件融合更新）
- downstream_requirements:
  - 請將 FULL_ARCH 之版本日期/檔名映射承接更新至 DOCUMENT_INDEX（doc_key=DOCUMENT_INDEX）（若其表列/引用需更新）。  
  - 若 ARCH_FLOW／DATA_SOURCES／LOCAL_ENV 等文件仍存在「只能加」口徑，請依同規格逐檔融合更新以完成全域一致化（保留有效內容、禁止摘要縮水、留痕分離）。  
