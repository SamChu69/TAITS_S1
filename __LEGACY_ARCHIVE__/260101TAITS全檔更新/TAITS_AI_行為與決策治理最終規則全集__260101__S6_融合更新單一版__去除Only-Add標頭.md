# TAITS_AI_行為與決策治理最終規則全集__260101（S6｜融合更新單一版｜去除 Only-Add 標頭）

> canonical_doc_key：AI_GOV
> 治理等級：A+（最高｜不可推翻）
> 狀態：ACTIVE（Final Canon）
> 生成日期：2026-01-01（Asia/Taipei）
> 版本定位：S6（融合更新｜單一可用版）
> 來源基準：S5（融合更新單一版，sha256:94a154eec53f…）
> 合併策略：**融合編排（Reflow & Consolidate）**｜保留文字內容，允許標題層級/段落位置調整以提升可用性與可稽核性
...
### 0.1 本版本（S5）要解決的問題
本版本目標是提供一份「單一可用版」：在不更動既有規則語義之前提下，將下列內容**融合為同一份可直接引用與操作的治理文件**：
- S3 之條文主體（含核心主權與系統治理硬邊界）
- S3 之補丁／封口（Addenda）
- S4 之更新整理內容（引用模板彙整／身份對照／Gate-Audit 檢核清單）

### 0.2 融合更新的基本原則（必遵）
- **語句內容不改寫**：條文文字保持原意與原句；本次變更僅限於「重排與融合」。
- **可稽核優先**：提供統一引用模板、身份對照與檢核清單，以支援 Gate / Audit / Replay。
- **衝突處理不擅自裁決**：若補丁段落存在重複編號（例如 G0–Gx），本文件不做內容裁決；裁決流程仍回歸 DOCUMENT_INDEX / Index Gate 與既有治理規則。

---

## 1. 統一引用與稽核操作（Operational Annex｜彙整）

## 1. 統一引用模板（彙整｜非裁決性｜便於貼用）

> 本章僅「彙整」本文件既有之引用欄位要求與標頭範本（例如 G2/G4/E2/E3 等既有段落）。
> 本章 **不新增** 新欄位義務，不改變既有規則；若與主文任何要求不一致，以主文段落為準。

### 1.1 Minimum Legal Citation（彙整版）
```text
ref_doc_key = <DOC_KEY>
ref_file = <完整檔名>
ref_version_date = <YYYY-MM-DD 或 __yymmdd>
ref_section = <章節定位（§ / Heading Path）>
audit_anchor = <VERSION_AUDIT 稽核錨點；若本批次尚未登錄，填 PENDING 並於同批次補登>
...
ref_notes = <可選：alias/Label/legacy 對照備註>
```

### 1.2 可直接貼用之引用標頭（彙整版）
```text
〔TAITS 引用標頭｜AI_GOV｜Only-Add〕
ref_doc_key = <DOC_KEY>
ref_file = <完整檔名>
...
## 2. 版本／檔名／治理身份對照（彙整｜非裁決性）

> 本章目的：把「同一規範可能以不同檔名存在」的事實說清楚，避免引用端只抓檔名導致身份不合法。

| 類別 | 欄位 | 裁決口徑（以主文/Index Gate 為準） |
|---|---|---|
| 治理身份 | canonical_doc_key | `AI_GOV` |
| 治理等級 | governance_level | A+ |
| 版本日期 | version_date | `__260101`（本次整併/編排日期戳） |
| 內容基準 | base_text | S3（sha256:b26286852f70…） |
| 實體檔名 | physical_filename | 以 repo/分發載體為準，**不得**取代 doc_key 身份裁決 |

---

---

## 3. Gate / Audit 快速檢核清單（彙整｜非裁決性）

> 本清單僅將文件內既有要求整理為「檢核步驟」，不構成新增規則。

1) **Identity**：引用是否含 `ref_doc_key/ref_file/ref_version_date/ref_section/audit_anchor/index_gate`？  
2) **Index-First**：doc_key/ACTIVE/等級裁決是否回到 DOCUMENT_INDEX（Index Gate）？  
3) **Snapshot ≠ 裁決**：是否誤用「文件數量/清單」作為裁決依據？  
4) **Alias 封口**：是否把 legacy alias（例如 DATA_UNIVERSE 或歷史 doc_key 字串）當成治理身份？  
5) **Traceability**：是否能以 correlation_id / VERSION_AUDIT 錨點回放？（對應主文既有 Log/Audit 條款）  

---

---

## 2. 條文主體（Normative Core｜融合後正文）

> 本章為 S3 主文之融合重排呈現；除標題層級與段落位置之整理外，文字內容以原文為準。

# TAITS_AI_行為與決策治理最終規則全集__260101（S3｜整合重排版）

> canonical_doc_key：AI_GOV
> 治理等級：A+（最高｜不可推翻）
> 狀態：ACTIVE（Final Canon）
> 生成日期：2026-01-01（Asia/Taipei）
> 編排版本：S3（整合重排｜新文檔｜語義不變優先）
...
### 0.1 本版本（S3）之定位
本文件為「新文檔」：在不改變既有條文語義之前提下，將 S1 正文與歷次 Only-Add 補丁進行**結構化整合與排版重排**，使其更便於：

- 治理閘門（Gate）引用
- 稽核（Audit）與回放（Replay）
- 工具鏈（Tooling）doc_key 解析
- 新對話啟動（Startup）與人類 HFI（Human Final Instruction）承接

### 0.2 最高原則（語義不變優先）
- 本文件之「條文內容」皆採 **原文區塊**收錄；  
- S3 的新增僅限於：章節重排、導覽、索引、來源對照（Source Map）、以及「原文區塊」的清楚標示；  
- 若需實質修訂條文，必須建立後續版本（例如 S4）並依 VERSION_AUDIT 留痕。

### 0.3 快速導覽（TOC）
- 0. 文件定位與使用方式
- 1. 核心主權與最終語義（S1 Normative Core）
  - 1.1 SOVEREIGNTY｜人類最高決策者唯一主權
  - 1.2 L9–L11｜最終語義定錨
  - 1.3 HFI｜人類明確命令
- 2. 系統治理硬邊界（S1）
  - 2.1 0–16 條文（原文分章）
- 3. 補丁／封口（Addenda，Only-Add）
  - 3.1 入口快照不裁決 × Index Gate 唯一裁決 × 引用最小格式（2025-12-27）
  - 3.2 doc_key／工具鏈解析封口與機器可讀區塊（2025-12-28）
  - 3.3 GLOBAL_ALIGNMENT_PATCH（MASTER_CANON 對齊全域補丁）（2025-12-28）
- 4. 附錄
  - 4.1 來源與追溯（Source Map）
  - 4.2 原文保存聲明


---

## 1. 核心主權與最終語義（S1 Normative Core｜原文區塊）

### 1.1 SOVEREIGNTY｜人類最高決策者唯一主權（原文）
...
## 2. 系統治理硬邊界（S1 條文｜原文分章）

> 本章收錄 S1 之 0–16 章節，採原文區塊方式保留，僅調整為連續閱讀順序。
...
# 【B】AI_GOV｜Addendum 2025-12-28-E
...
## 3. 補丁／封口（Addenda｜融合收錄）

> 本章將 S3 之補丁／封口（Addenda）內容融合收錄於同一章節中。
> 為避免與主文章節層級衝突，本章對「原補丁段落」之標題層級做 **降階處理**（例如 `##` → `####`），文字內容不改寫。

#### 3. 補丁／封口（Addenda，原文區塊）

> 本章收錄歷次 補丁，並依主題分組呈現。
> 其中若存在重複 G0–Gx 編號，均以其原文中之「日期／段落脈絡」辨識；裁決衝突仍回歸 DOCUMENT_INDEX 之衝突裁決程序。

##### 3.1 入口快照不裁決 × Index Gate 唯一裁決 × 引用最小格式（2025-12-27｜原文）

#### 治理補強｜AI_GOV 入口快照（文件數/清單）不裁決 × Index Gate（DOCUMENT_INDEX）唯一裁決 × 引用最小格式強制
...
##### 3.2 doc_key／工具鏈解析封口與機器可讀區塊（2025-12-28｜原文）
...
#### 治理補強｜P0 全閉合（AI_GOV doc_key 解析鍵封口 × Tooling 規則固定）
...
#### 治理補強｜doc_key 別名（Legacy Alias）保留宣告
...
#### 治理補強｜治理識別（doc_key）× 實體檔名（Physical Filename）對齊宣告
...
##### 3.3 GLOBAL_ALIGNMENT_PATCH（MASTER_CANON 對齊全域補丁｜2025-12-28｜原文）

#### 治理補強｜GLOBAL_ALIGNMENT_PATCH（MASTER_CANON 對齊全域補丁）
...
### 4.1 來源與追溯（Source Map）
- S1 來源：本文件內所有「原文」章節，均可在 S2 之「原文正文（保持不變｜S1 全文原封收錄）」段落中對應找到。
- S2 來源：S2 之新增內容屬導讀/索引用途；S3 僅將其改為更適合「條文整合檢索」的結構。
- 若需以 Gate/Audit 進行嚴格比對，建議同時保存：
  - `TAITS_AI_行為與決策治理最終規則全集__260101__S1_單一正確版.md`
  - `TAITS_AI_行為與決策治理最終規則全集__260101__S2_整併編排版.md`
  - 本文件 `...__S3_整合重排版.md`

### 4.2 原文保存聲明
本文件為「整合重排」，不構成對 S1 條文語義之改寫。若發現任何段落缺失或疑似語義差異，應以 S1 / S2 原文為準，並依 VERSION_AUDIT 流程提出修訂版（S4+）。
...
- 變更型態：融合更新（Reflow & Consolidate），不改寫條文語義
- 來源：
  - S3：條文主體＋補丁＋附錄（原文）
  - S4：引用模板彙整／身份對照／Gate-Audit 檢核清單（彙整）
- 本次處理：
  1) 將 S4 操作型內容整併為 §1（Operational Annex）
  2) 將 S3 主文整併為 §2（Normative Core）
  3) 將 S3 補丁內容融合收錄為 §3，並以「標題降階」方式避免層級衝突
  4) 保留 S3 附錄作為 §4
- 注意：若你要求「實質修訂條文語義」，必須建立後續版本（S6+）並依 VERSION_AUDIT 留痕流程處理
