# TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__251231
...
<!-- =========================================================
TAITS｜STRATEGY_FEATURE_INDEX｜Only-Add Canonical Alignment 整合段落
...
# Block B｜文件最末端追加：整合段落 Y（Only-Add · Freeze v1.0）

# 補強規範
（Only-Add · Canonical + Template Sync 整合段落 · Freeze v1.0）
...
（整合段落 Y｜STRATEGY_FEATURE_INDEX × MASTER_CANON × STRATEGY_UNIVERSE 同步對位補充附錄 · Freeze v1.0 · Only-Add 完）
---

## 補強規範
### A1. 本文件之唯一治理身份（Canonical Identity）
- canonical_filename（Index 裁決檔名）：`TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__251231.md`
- canonical_doc_key（Index 裁決識別碼）：`STRATEGY_FEATURE_INDEX`
- 版本狀態：ACTIVE（Freeze v1.0；Only-Add）

### A2. 本專案目錄中的實體檔案（Physical Artifact）
- physical_filename（目前專案內實際檔名）：`TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__251231__ADDENDUM_20251227_FINAL.md`
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。

### A3. 引用合法性最小規則（不新增制度，只固定寫法）
- 任何跨文件引用本文件時，必須使用：`doc_key=STRATEGY_FEATURE_INDEX` + `canonical_filename=TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__251231.md`。  
...
- 本文件 `doc_key` 既有標示為 `STRATEGY_FEATURE_INDEX`；本 整合段落 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
