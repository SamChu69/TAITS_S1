# TAITS_系統架構與流程細化說明（ARCH_FLOW）__251231
doc_key：ARCH_FLOW  
...
本 整合段落 僅處理下列事項，**不改寫**本文件的架構模組定義、資料流、事件流、分層責任與工程規格內容：
...
ref_section = <例如：§3 / §6 / §7 / §9 / 整合段落 2025-12-27-B>
...
（ARCH_FLOW｜最大完備整合版 完）
整合段落 A｜ARCH_FLOW × MASTER_CANON 差異與對齊補充（Only-Add）
（Only-Add · Alignment 整合段落 · Freeze v1.0）

A.0 補充性質聲明（不可省略）
適用文件：ARCH_FLOW（doc_key=ARCH_FLOW）

補充類型：整合段落 / Guideline（對齊補充；不改寫 Canonical）
...
整合段落 B｜渲染安全版 Mermaid（Only-Add，不覆寫原文）
（Only-Add · Rendering-Safe Copy · Freeze v1.0）

B.0 補充性質聲明
適用文件：ARCH_FLOW

補充類型：整合段落（渲染修復副本；原文保留）
...
flowchart TB
  ...
  L11 -->|SUCCESS| END[END]
  L11 -->|FAIL| EMERGENCY[EMERGENCY STOP]
整合段落 C｜Reason Code / Artifact / Replay 最小集對齊補強（Only-Add）
...
（ARCH_FLOW｜Only-Add 最大完備補強：整合段落 A–C · Freeze v1.0）
---

## 補強規範
### A1. 本文件之唯一治理身份（Canonical Identity）
- canonical_filename（Index 裁決檔名）：`TAITS_系統架構與流程細化說明（ARCH_FLOW）__251231.md`
- canonical_doc_key（Index 裁決識別碼）：`ARCH_FLOW`
- 版本狀態：ACTIVE（Freeze v1.0；Only-Add）

### A2. 本專案目錄中的實體檔案（Physical Artifact）
- physical_filename（目前專案內實際檔名）：`TAITS_系統架構與流程細化說明（ARCH_FLOW）__251231__ADDENDUM_20251227_FINAL.md`
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。

### A3. 引用合法性最小規則（不新增制度，只固定寫法）
- 任何跨文件引用本文件時，必須使用：`doc_key=ARCH_FLOW` + `canonical_filename=TAITS_系統架構與流程細化說明（ARCH_FLOW）__251231.md`。  
...
- 本文件 `doc_key` 既有標示為 `ARCH_FLOW`；本 整合段落 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
