# TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__251231
...
ref_section = <例如：§3 / §6 / §7 / §9 / 整合段落 2025-12-27 / 整合段落 2025-12-27-B>
...
- 本 整合段落 為 Only-Add；不改寫任何既有正文。  
- 本文件的 VETO 條款、reason_codes、證據要求與保守停止規則仍維持原樣，且不因本 整合段落 而弱化。  
...
# 補強規範
（Only-Add · Alignment 整合段落 · Freeze v1.0）

## A.0 補充性質聲明（不可省略）
- 適用文件：RISK_COMPLIANCE（doc_key=RISK_COMPLIANCE）
- 補充類型：整合段落 / Alignment Guideline（對位補充；不改寫 Canonical）
...
## D.0 補充性質聲明
- 適用文件：RISK_COMPLIANCE
- 補充類型：Checklist / Contract 整合段落（交接契約附錄；不取代主文）
- 生效狀態：GOVERNANCE_STATE = Freeze v1.0
...
（RISK_COMPLIANCE｜Only-Add 整合段落 Pack：整合段落 A–D · Freeze v1.0 生效）
---
...
- physical_filename（目前專案內實際檔名）：`TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231__ADDENDUM_20251227_FINAL.md`
...
- 本文件 `doc_key` 既有標示為 `RISK_COMPLIANCE`；本 整合段落 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
