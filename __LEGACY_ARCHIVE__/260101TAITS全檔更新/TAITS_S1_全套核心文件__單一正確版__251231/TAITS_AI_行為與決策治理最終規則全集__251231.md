# TAITS_AI_行為與決策治理最終規則全集__251231.md
---
---
...
- 本 整合段落 為 Only-Add；不改寫 AI_GOV 既有任何條文。  
...
# 【B】AI_GOV｜整合段落 2025-12-28-E（Only-Add）
---
...
```text
ref_doc: TAITS_AI_行為與決策治理最終規則全集__251231.md
ref_doc_key: AI_GOV
...
- canonical_filename（Index 裁決檔名）：`TAITS_AI_行為與決策治理最終規則全集__251231.md`
- canonical_doc_key（Index 裁決識別碼）：`AI_GOV`
- 版本狀態：ACTIVE（Freeze v1.0；Only-Add）

### A2. 本專案目錄中的實體檔案（Physical Artifact）
- physical_filename（目前專案內實際檔名）：`TAITS_AI_行為與決策治理最終規則全集__251231__ADDENDUM_20251228H_FINAL.md`
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。

### A3. 引用合法性最小規則（不新增制度，只固定寫法）
- 任何跨文件引用本文件時，必須使用：`doc_key=AI_GOV` + `canonical_filename=TAITS_AI_行為與決策治理最終規則全集__251231.md`。  
