...
- 本 整合段落 為 Only-Add；不改寫任何既有正文與 UI 流程。  
...
- physical_filename（目前專案內實際檔名）：`TAITS_使用者介面與人機決策規範（UI_SPEC）__251231__ADDENDUM_20251227_FINAL.md`
...
- 本文件 `doc_key` 既有標示為 `UI_SPEC`；本 整合段落 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
