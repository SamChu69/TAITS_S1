...
- 本 整合段落 為 Only-Add；不改寫任何既有正文與操作步驟。  
...
- physical_filename（目前專案內實際檔名）：`TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__251231__ADDENDUM_20251227_FINAL.md`
...
- 本文件 `doc_key` 既有標示為 `BEGINNER_GUIDE`；本 整合段落 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
