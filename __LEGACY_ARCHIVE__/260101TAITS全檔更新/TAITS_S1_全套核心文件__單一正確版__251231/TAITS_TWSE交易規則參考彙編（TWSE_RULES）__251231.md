...
本 整合段落 僅處理「引用端」歧義，不改寫本文件對官方制度來源、Rulebook Snapshot、Trigger Map 的任何語義：

- 本文件仍可使用「DATA_UNIVERSE」作為概念名詞（Data Universe／資料宇宙）或閱讀別名（alias）
- 但本 整合段落 將其 **明確否認為 doc_key**，並規定引用端的唯一合法 doc_key
...
```text
ref_doc_key = DATA_SOURCES
ref_section = <例如：整合段落 0 / 整合段落 0.1 / Part X / 整合段落 Y>
ref_notes = 若片段內含「DATA_UNIVERSE」字樣，依 DATA_SOURCES 整合段落 0/0.1 視為 alias，不具法律效力
...
- 其法律效果由本 整合段落 限定為「概念別名（alias）」用途
...
- physical_filename（目前專案內實際檔名）：`TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251231__ADDENDUM_20251227_FINAL.md`
...
- 本文件 `doc_key` 既有標示為 `TWSE_RULES`；本 整合段落 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
