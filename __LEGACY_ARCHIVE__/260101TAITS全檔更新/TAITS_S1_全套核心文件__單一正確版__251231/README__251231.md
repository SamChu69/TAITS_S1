...
- 本 整合段落 為 Only-Add；不改寫 README 既有導覽內容。  
...
### 5.1 A+（最高母法）
- `AI_GOV`：TAITS_AI_行為與決策治理最終規則全集__251231（A+）
...
- physical_filename（目前專案內實際檔名）：`TAITS_專案總覽與使用說明（README）__251231__ADDENDUM_20251227_FINAL.md`
...
- 本文件 `doc_key` 既有標示為 `README`；本 整合段落 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
