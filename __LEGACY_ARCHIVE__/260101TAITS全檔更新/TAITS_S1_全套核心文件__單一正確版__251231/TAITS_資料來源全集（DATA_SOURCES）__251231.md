...
- 任何出現 `doc_key：DATA_SOURCES` 之舊標示，均屬「Metadata Discrepancy」的歷史殘留標記；其法律效力被本 整合段落 0 否認
...
本 整合段落 0 屬於「METADATA_FIX（中繼資料修補）」型變更：
...
## 0.1 本 整合段落 的適用範圍（Hard Boundary）

本 整合段落 只處理下列「引用/摘錄/拷貝」場景（不改寫資料治理主體內容）：

- 文件被切片摘錄到其他文件（例如 整合段落/Part 被單獨引用）
...
- 在 Evidence/Audit/UI Trace 中，把 `DATA_UNIVERSE` 當成 ACTIVE 文件引用（除非 DOCUMENT_INDEX 另行新增並裁決為 ACTIVE；Freeze v1.0 期間不得以此 整合段落 變相新增）
...
ref_section = <例如：Part 3 §32 / 整合段落 B §B.7 / 主文 §2.3>
ref_excerpt_hash = <可選：若已對摘錄做 hash>
ref_notes = 若摘錄內含「doc_key：DATA_SOURCES」字樣，依 整合段落 0/0.1 視為 alias，不具法律效力
```
...
本 整合段落 允許以 **Only-Add** 方式補上缺失之結尾 fence：
- 不改寫 Mermaid 內容
- 不刪改後續章節文字
...
（整合段落 A｜DATA_UNIVERSE 對位 MASTER_CANON｜Freeze v1.0｜Only-Add）
...
- 若本附錄任一描述與主文（Part 1–4）或 整合段落 A 產生張力：
  - 一律以主文與 整合段落 A 為準
  - 本附錄該段落自動失效，不需宣告
...
- `redaction_policy_ref`：脫敏規則（見 整合段落 C）
...
- `audit_checks_ref`：稽核檢核表引用（可引用 整合段落 A 的 A.6）
...
（整合段落 B｜DATA_UNIVERSE 工件模板 × 欄位字典 × Reason Codes｜Freeze v1.0｜Only-Add）
...
（整合段落 C｜敏感資料脫敏與最小揭露規範｜Freeze v1.0｜Only-Add）

---

# 【D】DATA_SOURCES｜整合段落 0.2B（Only-Add）
---

## 0.2B.1 既有 Mermaid fence 格式錯誤之法律定位
- 本文件既有多處 ```mermaid 區塊因 fence 關閉位置錯誤，造成圖表行被當作一般文字。
- 依 Only-Add：不得回頭改寫既有段落；因此本 整合段落 以重新附錄方式提供等價語義之可渲染版本。
- 任何引用 Mermaid 圖表時：以本 整合段落 之附錄版本為準（僅限渲染修補，不變更流程語義）。

---

## 0.2B.2 整合段落｜Mermaid 可渲染版本（對應原章節 12 / 15 / 17 / 20）
...
- physical_filename（目前專案內實際檔名）：`TAITS_資料來源全集（DATA_SOURCES）__251231__ADDENDUM_0_2C_FINAL.md`
