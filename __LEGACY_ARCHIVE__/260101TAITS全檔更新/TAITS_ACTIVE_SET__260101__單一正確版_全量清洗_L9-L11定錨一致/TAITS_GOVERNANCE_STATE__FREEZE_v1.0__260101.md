# TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260101
...
## 3.X Freeze 例外條款：Human Sovereignty（HFI Override）｜Only-Add

Freeze v1.0 為預設保守狀態，用於防止未授權之語義漂移與結構性破壞；但 Freeze 不得凌駕 HUMAN SOVEREIGNTY。

當 `DOCUMENT_INDEX` 所定義之 **Valid HFI（Human Final Instruction）** 存在時：

- Freeze 之限制不得阻擋 HFI `scope` 範圍內之更新、覆寫、重排版或全量替換。
- 系統必須轉為「放行＋強制稽核承接」：`VERSION_AUDIT` 留痕、`HASH_MANIFEST` 對帳、`CHANGELOG` 生成。
...
（TAITS_GOVERNANCE_STATE__FREEZE_v1.0__260101｜ACTIVE）
