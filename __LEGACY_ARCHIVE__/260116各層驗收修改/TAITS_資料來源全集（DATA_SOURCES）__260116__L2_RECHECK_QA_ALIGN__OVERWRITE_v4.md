...
- 若未滿足任一條：視為違反 L2 禁止項 → **必須輸出 QAFail 並 STOP（不得進入 L3）**。
  - 允許：在不變更既有工件語義之前提下，於 **L1/L2 範圍內重新取得/重新正規化並重跑**（屬同層重試，不構成跳層或層間回寫）。
  - 必須：重試亦需保留 `reason_codes[]`、更新 `hash_manifest_ref` 與 `active_version_map_ref`，並保留失敗工件之引用以供 L11 回放。
...
- `trust_level`：NORMAL/LOW（Canonical；對齊 MASTER_CANON。若沿用 legacy HIGH/MEDIUM/LOW，必須在同一工件內提供映射並於下游只讀取映射後值）
- `compliance_eligible`：true/false（第三方通常為 false）
- `licensing_notes_ref`：降級原因＋替代來源鏈＋授權/限制註記（Canonical；對齊 MASTER_CANON；不得省略）
- `notes_ref`：Legacy alias（禁止作新輸出欄位；若於舊介面/日誌出現，必須提供對應至 `licensing_notes_ref` 的映射以避免混讀）
---
...
- `licensing_notes_ref`（必填：降級原因＋替代來源鏈＋授權/限制註記；不得省略）
...
- `source_id`
- `subcategory_id`
- `fetch_window`  # start/end；若不適用仍需以 null 明示（對齊 ARCH_FLOW=7.1）
- `captured_at`   # 取得完成時間戳（不得以資料內時間戳替代）
- `raw_payload_ref`
- `provenance_ref`
- `hash_manifest_ref`
- `quality_report_ref`
- `active_version_map_ref`  # 對齊 E2E Trace／回放一致性（不得缺漏）
...
- `trust_level`             # NORMAL/LOW；若 source_tier_used=FALLBACK 必須為 LOW（對齊 22.3）
- `compliance_eligible`     # true/false；若 source_tier_used=FALLBACK 必須為 false（對齊 22.3）
- `failover_event_ref`      # 可選：若觸發 fallback，建議提供以利稽核與回放（對齊 ARCH_FLOW=7.1）

#### 23.1.1 Failure Output：SourceFail（L1 事件/工件｜對齊 MASTER_CANON）
> L1 取數若出現 FAIL/RETURN/BLOCK 條件，必須輸出 SourceFail 並留痕；不得靜默重試到成功才記錄。
- `reason_code`
- `source_id`
- `subcategory_id`
- `attempted_window`
...
- scope_files_output: TAITS_資料來源全集（DATA_SOURCES）__260116__L2_RECHECK_QA_ALIGN__OVERWRITE_v4.md
- scope_mode: LAYER_QA_MODE（L2：Imputation 失敗處置語義對齊（QAFail→STOP），消除與 MASTER_CANON/ARCH_FLOW/FULL_ARCH 之不一致；不改寫 MASTER_CANON 主幹語義）
- version_date: 2026-01-16（Asia/Taipei）

## B. Changelog（變更清單｜非正文）
1) **正文去重收斂**：移除重複的文件頭/定位段落與多重分隔線，保留單一一致版本。
2) **正文乾淨化**：修復占位式語句、補齊 Mermaid 區塊結束標記並修正章節標題格式，使可長期維護與可引用。
3) **稽核四件套固定**：保留 Scope／Changelog／Hash Manifest／Audit Hand-off 於檔尾（非正文）。
4) **FINAL_QA_NORMALIZE**：移除可能殘留之標籤式非正文行（若存在），並依 Hash Manifest 規則重新計算 body_sha256，確保稽核指紋可重現。
5) **L2_LAYER_QA_FIX**：補強 L2（Normalization）之可回放輸出契約：新增 `canonical_data_ref`、補值追溯欄位（`value_origin_map_ref`/`imputation_ref`）與對應品質旗標 `IMPUTED_VALUES_PRESENT`，用以落地 MASTER_CANON L2「禁止隱性補值」。
6) **L1_LAYER_QA_FIX**：補強 L1（DataIngested）輸出契約欄位集合（fetch_window、active_version_map_ref、source_tier_used/licensing_notes_ref、trust_level、compliance_eligible、failover_event_ref），以消除 L1 契約漂移與混讀。
7) **L1_QA_PATCH**：統一 FALLBACK 降級註記欄位為 `licensing_notes_ref`（移除 `notes_ref` 命名漂移），避免 L1 DataIngested 契約混讀。
8) **DOC_KEY_ONLY_BODY_HEADER**：移除正文文件頭中的『版本日期／基線日期』行，避免以日期作為正文識別；版本辨識僅存在於檔名；日期/時間僅允許於檔尾稽核區塊承接。
9) **HASH_REFRESH**：依 Hash Manifest 規則重新計算 body_sha256，確保稽核指紋可重現。
10) **MERMAID_FENCE_FIX**：補齊 Mermaid 區塊的結束標記（```），避免後續章節被誤判為程式碼區塊而造成文件渲染與引用定位錯誤。
11) **SOURCEFAIL_SCHEMA**：補齊 L1 Failure Output（SourceFail）之欄位級契約（對齊 MASTER_CANON），將 FAIL/RETURN/BLOCK 事件工件化，避免僅有敘述性 playbook 而缺可機器化輸出。
12) **L1_DEGRADE_OUTPUT_CANON_ALIGN**：修正第 10.2 節降級輸出欄位之命名與枚舉漂移：以 `licensing_notes_ref` 取代 `notes_ref` 作 Canonical；並將 `trust_level` Canonical 固定為 NORMAL/LOW（legacy HIGH/MEDIUM/LOW 必須映射），避免下游混讀與裁決誤用。
13) **L2_RECHECK_QA_ALIGN**：修正 L2（Imputation）不符可追溯條件之處置語義：由『RETURN/BLOCK 依下游需求』收斂為『QAFail→STOP（不得進入 L3）』，並明確允許同層重試（L1/L2 範圍），以消除工程實作與 Canonical（MASTER_CANON）之混讀風險。

## C. Hash Manifest（指紋清單｜非正文）
- hash_scope: 正文區（自檔首至『# 檔尾稽核區塊（Audit Section｜非正文）』之前，不含稽核區塊）
- body_sha256: 1cd3af2b3898f8cad0ee5ca734185a24f71a69af99cb2607668191d332671b77
