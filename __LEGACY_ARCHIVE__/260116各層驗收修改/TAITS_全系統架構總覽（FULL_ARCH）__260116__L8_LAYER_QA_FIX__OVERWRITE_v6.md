...
### 1) 變更清單（Changelog）
- 2026-01-16：L8_LAYER_QA_FIX_RECHECK（Markdown bullet 修正：硬禁條列符號格式化；不改寫任何制度語義；重算 BODY_SHA256。）
- 2026-01-16：L8_LAYER_QA_RECHECK（補強 L8 Hard 禁止事項清單，使 FULL_ARCH 與 doc_key=MASTER_CANON L8 禁止輸出一致；維持 Only-Add；重算 BODY_SHA256。）
- 2026-01-16：L8_LAYER_QA_FIX（補強 L8 `StrategyProposed` 介面契約：新增 Canonical 最小 Proposal 工件鍵 refs、入場硬條件（L7 PASS token + Regime 非 UNCERTAIN/STOP）、與禁止輸出清單之對齊聲明；不改寫 doc_key=MASTER_CANON 主幹語義；重算 BODY_SHA256。）
- 2026-01-14：L2_LAYER_QA_FIX（補齊 `DataNormalized`（L2）必備欄位，對齊 DATA_SOURCES=23.2；明確 quality_flags legacy alias 映射規則；維持既有禁止事項；重算 BODY_SHA256。）
- 2026-01-14：L1_LAYER_QA_FIX：對齊 L1 `DataIngested` 介面契約（補齊 `subcategory_id` / `captured_at` / `quality_report_ref` / `hash_manifest_ref`），並明確禁止靜默補值與第三方冒充官方；不改寫 MASTER_CANON 主幹語義。
- 新增：E2E Trace Manifest 區塊之『非再定義』邊界聲明，避免與 MASTER_CANON 混讀。
- 更新：基線日期／版本日期對齊 2026-01-11（Asia/Taipei），以避免跨檔新舊混讀。
- 2026-01-13：L6_LAYER_QA_FIX：補強 L6 `RegimeDetermined` 之狀態碼/原因碼/信心報告尺度與多證據限制（Only-Add），對齊 MASTER_CANON「不得由單一訊號決定 Regime」與「Regime 不明確可 STOP」之原則；同步重算正文指紋。
- 2026-01-13：L5_LAYER_QA_FIX：修正 L5 `EvidenceAssembled` 輸入欄位描述，避免對 doc_key=MASTER_CANON L5 之輸入定義造成特別化；同步重算正文指紋。
- 2026-01-13：L3_REQA_FIX：補強 L3 `SnapshotCreated` 必備欄位與 Event Bus 訊息契約加入 `version_ref`，使其與 Contract 共同硬性欄位及 E2E Trace Manifest 一致。
- 2026-01-13：L3_LAYER_QA_FIX：補強跨模組共同硬性欄位之 `version_ref` 定義，對齊 MASTER_CANON L3 對「版本引用」之要求，避免 Gate/ UI 端缺欄位而發生混讀。
...
- 2026-01-11：Final QA 正文化覆蓋版。處理重點包含：移除非正文痕跡、去除箭頭符號與治理位階字母標籤、收斂 Canonical Flow 引用口徑為 doc_key=MASTER_CANON、以及重算正文指紋。
