...
### 1) 變更清單（Changelog）
- 2026-01-16：L1-L11_FULL_CHAIN_QA_ALIGN（索引自指向與 ACTIVE 檔名口徑對齊）：修正 doc_key=DOCUMENT_INDEX 第 5 章 Authoritative Index，使『doc_key → ACTIVE 檔名』與目前專案最新覆蓋版一致（含 doc_key=DOCUMENT_INDEX 自指向、MASTER_ARCH/ARCH_FLOW/DATA_SOURCES/STRATEGY_* 等檔名口徑對齊）；同步更新本檔留痕與指紋；不改寫裁決序位與 doc_key=MASTER_CANON 主幹語義。
- 2026-01-16：L8_RECHECK（FULL_ARCH Markdown 條列修正之留痕與 ACTIVE 指向更新）：更新 DOCUMENT_INDEX 與 FULL_ARCH 交付檔名，確保工程落地不受格式錯誤影響；不改寫 Canonical 主幹語義。
- 2026-01-16：RECHECK（L8 Proposal｜FULL_ARCH 禁止清單對齊）：補強 FULL_ARCH 之 L8（StrategyProposed）Hard 禁止事項，使其與 MASTER_CANON L8 禁止輸出一致，避免工程以 FULL_ARCH 為接口規格時發生越權輸出（價格/數量/買賣方向等）。
  - 變更範圍（doc_key）：FULL_ARCH、DOCUMENT_INDEX、VERSION_AUDIT
  - 舊→新（交付檔名）：
    - FULL_ARCH：__260116__L8_LAYER_QA_FIX__OVERWRITE_v4 → __260116__L8_LAYER_QA_FIX__OVERWRITE_v5
    - DOCUMENT_INDEX：__260116__L8_ACTIVATION__OVERWRITE_v5 → __260116__L8_ACTIVATION__OVERWRITE_v6
    - VERSION_AUDIT：__260116__L8_ACTIVATION__OVERWRITE_v5 → __260116__L8_ACTIVATION__OVERWRITE_v6
  - 理由：屬接口契約補強（Only-Add），不改寫 MASTER_CANON 主幹語義；僅消除 L8 越權輸出與稽核缺口風險。
- 2026-01-16：ACTIVATION（L8 Layer QA Fix 生效留痕）：依 doc_key=DOCUMENT_INDEX 裁決口徑，啟用 FULL_ARCH 之 L8 Proposal 介面契約補強版（__260116__L8_LAYER_QA_FIX__OVERWRITE_v4），以對齊 doc_key=MASTER_CANON 之 L8 最小 Proposal 工件鍵 refs 與入場硬條件（Risk PASS token + Regime 非 UNCERTAIN/STOP）。
  - 變更範圍（doc_key）：FULL_ARCH、DOCUMENT_INDEX、VERSION_AUDIT
  - 舊→新（交付檔名）：
    - FULL_ARCH：__260114__L2_LAYER_QA_FIX__OVERWRITE_v3 → __260116__L8_LAYER_QA_FIX__OVERWRITE_v4
    - DOCUMENT_INDEX：__260116__L6_RECHECK__OVERWRITE_v4 → __260116__L8_ACTIVATION__OVERWRITE_v5
    - VERSION_AUDIT：__260116__L6_RECHECK__OVERWRITE_v4 → __260116__L8_ACTIVATION__OVERWRITE_v5
  - 理由：解除 L8（Strategy Proposal）工程落地之欄位缺口與跨文件混讀風險；不改寫 Canonical 主幹語義，僅做介面契約補強＋ACTIVE 指向更新＋可追溯留痕。
- 2026-01-16：RECHECK（L6 Recheck｜索引自指向與 UI_SPEC 版本日期一致性修正）：
  - 變更範圍（doc_key）：DOCUMENT_INDEX、VERSION_AUDIT、UI_SPEC
  - 舊→新（交付檔名）：
    - DOCUMENT_INDEX：__260116__L6_RECHECK__OVERWRITE_v3 → __260116__L6_RECHECK__OVERWRITE_v4
    - VERSION_AUDIT：__260116__L6_RECHECK__OVERWRITE_v3 → __260116__L6_RECHECK__OVERWRITE_v4
    - UI_SPEC：__260116__L6_RECHECK_UI_MAP__OVERWRITE_v1 → __260116__L6_RECHECK_UI_MAP__OVERWRITE_v2
...
- 2026-01-16：ACTIVATION（L6 Layer QA Fix 生效留痕）：依 DOCUMENT_INDEX 口徑，啟用 MASTER_CANON 之 L6（Regime）欄位級契約補強版，消除 Regime 契約過短造成之工程漂移/混讀風險。
  - 變更範圍（doc_key）：MASTER_CANON、DOCUMENT_INDEX、VERSION_AUDIT
  - 舊→新（交付檔名）：
    - MASTER_CANON：__260114__L5_LAYER_QA_RECHECK__OVERWRITE_v3 → __260116__L6_LAYER_QA_FIX__OVERWRITE
    - DOCUMENT_INDEX：__260114__L5_RECHECK__OVERWRITE_v2 → __260116__L6_ACTIVATION__OVERWRITE
    - VERSION_AUDIT：__260114__L5_RECHECK__OVERWRITE_v3 → __260116__L6_ACTIVATION__OVERWRITE
...
- 2026-01-16：RECHECK（L6 Recheck｜DOCUMENT_INDEX 版本日期對齊修正）：修正 DOCUMENT_INDEX 中 MASTER_CANON（__260116__L6_LAYER_QA_FIX__OVERWRITE）之『版本日期』欄位，對齊 MASTER_CANON 文件頭版本日期（2026-01-16），避免 ACTIVE 清單之版本日期誤導與混讀風險。
  - 變更範圍（doc_key）：DOCUMENT_INDEX、VERSION_AUDIT
  - 舊→新（交付檔名）：
    - DOCUMENT_INDEX：__260116__L6_ACTIVATION__OVERWRITE → __260116__L6_RECHECK__OVERWRITE_v2
    - VERSION_AUDIT：__260116__L6_ACTIVATION__OVERWRITE → __260116__L6_RECHECK__OVERWRITE_v2
  - 理由：僅修正索引列之版本日期欄位（不改寫裁決序位、不改寫 Canonical 主幹語義），確保『ACTIVE 指向』與『版本日期』同源一致，降低治理稽核歧義。
- 2026-01-16：RECHECK（L6 UI 對位｜Regime 狀態碼映射）：補強 UI_SPEC 的 Regime Panel，明確宣告 UI 僅做呈現映射：Canonical `regime_decision`（OK/UNCERTAIN/STOP）→ UI `regime_status`（CONFIRMED/UNCERTAIN/STOP），以消除 FULL_ARCH/UI_SPEC 與 MASTER_CANON 之狀態碼名詞差異造成的工程漂移。
  - 變更範圍（doc_key）：UI_SPEC、DOCUMENT_INDEX、VERSION_AUDIT
  - 舊→新（交付檔名）：
    - UI_SPEC：__260113__FINAL_QA__NORMALIZE → __260116__L6_RECHECK_UI_MAP__OVERWRITE_v1
    - DOCUMENT_INDEX：__260116__L6_RECHECK__OVERWRITE_v2 → __260116__L6_RECHECK__OVERWRITE_v3
    - VERSION_AUDIT：__260116__L6_RECHECK__OVERWRITE_v2 → __260116__L6_RECHECK__OVERWRITE_v3
  - 理由：屬 UI 呈現契約補強（非 Canonical 重新定義），不改寫 MASTER_CANON 主幹語義；僅明確化『欄位名與狀態碼』之跨文件對位規則，降低 L6→L10 人機介面混讀風險。
- 2026-01-14：RECHECK（L5 Evidence EC 口徑對齊與 L6 重複標頭修正）：補強 MASTER_CANON 之 L5 輸出欄位（completeness_level/decision_context_type/required_set_version/missing_required_items/quality_grade_summary）與 EC 對應之 RETURN/BLOCK 規則，並移除重複 L6 標頭；同步更新 DOCUMENT_INDEX 指向最新覆蓋版檔名；本檔留痕。
  - 變更範圍（doc_key）：MASTER_CANON、DOCUMENT_INDEX、VERSION_AUDIT
  - 舊→新（交付檔名）：
    - MASTER_CANON：TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__260114__L5_LAYER_QA_RECHECK__OVERWRITE_v2.md → TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__260114__L5_LAYER_QA_RECHECK__OVERWRITE_v3.md
    - DOCUMENT_INDEX：TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260114__L5_ACTIVATION__OVERWRITE.md → TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260114__L5_RECHECK__OVERWRITE_v2.md
    - VERSION_AUDIT：TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260114__L5_RECHECK__OVERWRITE_v2.md → TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260114__L5_RECHECK__OVERWRITE_v3.md
- 2026-01-14：ACTIVATION（L5 Evidence 契約補強生效留痕）：依 DOCUMENT_INDEX 裁決口徑，啟用 __260114__ 覆蓋版作為 ACTIVE。
  - 變更範圍（doc_key）：MASTER_CANON、DOCUMENT_INDEX、VERSION_AUDIT
  - 舊→新（交付檔名）：
    - MASTER_CANON：__260114__L4_LAYER_QA_FIX__OVERWRITE → __260114__L5_LAYER_QA_FIX__OVERWRITE
    - DOCUMENT_INDEX：__260114__L4_ACTIVATION__OVERWRITE → __260114__L5_ACTIVATION__OVERWRITE
    - VERSION_AUDIT：__260114__L4_ACTIVATION__OVERWRITE → __260114__L5_ACTIVATION__OVERWRITE
...
- 2026-01-14：ACTIVATION（L4 Layer QA Fix 生效留痕）：依 DOCUMENT_INDEX 裁決口徑，啟用 MASTER_CANON 之 __260114__L4_LAYER_QA_FIX__OVERWRITE 作為 L4（Analysis）欄位級契約之唯一正文來源。
  - 變更範圍（doc_key）：MASTER_CANON、DOCUMENT_INDEX
  - 舊→新（交付檔名）：
    - MASTER_CANON：__260114__L3_LAYER_QA_FIX__OVERWRITE → __260114__L4_LAYER_QA_FIX__OVERWRITE
    - DOCUMENT_INDEX：__260114__L3_ACTIVATION__OVERWRITE → __260114__L4_ACTIVATION__OVERWRITE
  - 理由：消除 L4 契約不足導致下位文件自行補定義之漂移風險；不改寫 Canonical 主幹語義，屬契約補強＋治理指向更新＋可追溯留痕。
- 2026-01-14：ACTIVATION（L3 Layer QA Fix 生效留痕）：依 DOCUMENT_INDEX 裁決口徑，啟用 MASTER_CANON 之 __260114__L3_LAYER_QA_FIX__OVERWRITE 作為 ACTIVE。
  - 變更範圍（doc_key）：MASTER_CANON、DOCUMENT_INDEX
  - 舊→新（交付檔名）：MASTER_CANON：__260114__L2_LAYER_QA_FIX__OVERWRITE → __260114__L3_LAYER_QA_FIX__OVERWRITE
  - 理由：補強 L3（Snapshot & State）欄位級 I/O 契約、狀態轉移與 Hard No 禁止事項，以消除工程落地與稽核回放的口徑漂移；不改寫 Canonical 主幹語義。
- 2026-01-14：ACTIVATION（L1/L2 Layer QA Fix 生效留痕）：依 DOCUMENT_INDEX 裁決口徑，啟用 __260114__ 覆蓋版作為 ACTIVE 交付檔名口徑。
  - 變更範圍（doc_key）：MASTER_CANON、FULL_ARCH、ARCH_FLOW、DATA_SOURCES、DOCUMENT_INDEX
  - 舊→新（交付檔名）：
    - MASTER_CANON：__260113__FINAL_QA__NORMALIZE → __260114__L2_LAYER_QA_FIX__OVERWRITE
    - FULL_ARCH：__260113__FINAL_QA__NORMALIZE → __260114__L2_LAYER_QA_FIX__OVERWRITE_v3
    - ARCH_FLOW：__260113__FINAL_QA__NORMALIZE → __260114__L1_QA__OVERWRITE_v2
    - DATA_SOURCES：__260113__FINAL_QA__NORMALIZE → __260114__L1_LAYER_QA_FIX__OVERWRITE
    - DOCUMENT_INDEX：__260113__FINAL_QA__NORMALIZE → __260114__L2_ACTIVATION__OVERWRITE
...
- 更新：版本日期與基線日期統一為 2026-01-11（Asia/Taipei）。
- 固定：檔尾稽核四件套為唯一非正文留痕。
