# TAITS_使用者介面與人機決策規範（UI_SPEC）__260102
...
```text
〔TAITS UI Trace｜FREEZE 狀態（以 GOVERNANCE_STATE 為準）｜最大完備＋累積式更新〕
...
## 21. U4. 最終宣告（FREEZE 狀態（以 GOVERNANCE_STATE 為準））
...
## 29. G5. 最終宣告（FREEZE 狀態（以 GOVERNANCE_STATE 為準））
...
- scope_files_modified: TAITS_使用者介面與人機決策規範（UI_SPEC）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）

## B. Changelog（變更清單）
1) 口徑統一：將檔頭與正文中「Only-Add／只可新增／只增不減／不得覆寫／不可覆寫」等舊口徑，統一改為「最大完備＋累積式更新」；允許融合更新／覆寫修正／重排版；禁止摘要縮水；未被新版本明確取代之有效內容必保留並持續累積；被取代者由留痕承接。  
2) 重新排版與依序排列：保留全部既有有效內容，僅做章節整併、去除重複標頭混讀風險，並將章節重排為 0..N 連續編號；同步修正章內子標題編號以利穩定引用。  
3) 語義定錨一致化：於檔頭新增 L9/L10/L11 與 UI 分層定錨，確保介面規範不再產生「L11 可下單/可批准」或「AI 可越權」之混讀。  

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: 34c1bdaff5587bc0e021ebaaf4415616a2b9ab5177bd9cb53235f0b9794255de

## D. Audit Hand-off（裁決承接）
- change_id: UI-FUSION-260102-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=UI_SPEC 文件融合更新）
- downstream_requirements:
  - 若 DOCUMENT_INDEX 的 UI_SPEC 指向仍為 __260101，需更新為 __260102 以維持新對話載入一致性。  
  - 後續若 README／BEGINNER_GUIDE／DATA_SOURCES 等仍含舊口徑，需沿用同一交付規格逐檔一致化，以避免入口文件帶歪 UI 對層級的解讀。  
