# TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260102
...
### D2.2 建議固定引用標頭（可直接貼用）
```text
〔TAITS Deploy/Ops 引用標頭｜FREEZE 狀態（以 GOVERNANCE_STATE 為準）｜最大完備＋累積式更新〕
...
## 7. D5. 最終宣告（FREEZE 狀態（以 GOVERNANCE_STATE 為準））
...
DOCUMENT_INDEX 可用且未被 FREEZE 狀態
...
5. 停線與凍結（Stop-the-Line & FREEZE 狀態）
...
上位裁決：AI_GOV（A+）＋DOCUMENT_INDEX（A+｜Authoritative Index）＋MASTER_ARCH（A）＋MASTER_CANON（A）  
目的：在 FREEZE 狀態（以 GOVERNANCE_STATE 為準） 下，新增「Repo 落盤/封存」的營運規範（不改任何治理裁決），以降低同一 doc_key 多版本並存造成的“選錯檔”風險，並確保工具鏈與人工載入的 latest 行為一致、可回放、可稽核。
...
taits_repo_landing_manifest:
  ...
  governance_state: "FREEZE 狀態（以 GOVERNANCE_STATE 為準）"
  doc_key: "<DOC_KEY>"
...
## 32. G5. 最終宣告（FREEZE 狀態（以 GOVERNANCE_STATE 為準））
...
- scope_files_modified: TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__260102.md
- scope_mode: FILE UPDATE MODE（融合更新／整合重排版／語義定錨一致化）
- version_date: 2026-01-02（Asia/Taipei）

## B. Changelog（變更清單）
1) 口徑統一：將檔頭與正文中「Only-Add/只可新增/不可覆寫」等口徑，統一改為「最大完備＋累積式更新」，並保持禁止摘要縮水、未取代內容必保留、取代內容以留痕承接。  
2) 重新排版：將原章節依既有章節序與原始出現順序重排為 0..N 連續編號，並同步修正章內子標題編號以利穩定引用。  
3) 語義定錨一致化：檔頭補入 L9/L10/L11 單一口徑定錨，避免運維文件對層級角色產生混讀。  

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: 2042c65e1bae192e842288768af65a2e56dfc2d4bb44c9d4c2c6a964ff34e885

## D. Audit Hand-off（裁決承接）
- change_id: DO-FUSION-260102-0001
- authority_basis: HFI（人類最高決策者明確命令｜scope=DEPLOY_OPS 文件融合更新）
- downstream_requirements:
  - 若 DOCUMENT_INDEX 仍引用 DEPLOY_OPS 舊版檔名（__260101），需更新為 __260102 以維持新對話載入一致性。  
  - UI_SPEC／DATA_SOURCES／README／BEGINNER_GUIDE 等仍含舊式「只能加」口徑者，應沿用同一交付規格逐檔一致化。  
