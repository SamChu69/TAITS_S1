# TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__260102.md
---
...
- B2. 新對話啟動最小規格（Minimum Load Set｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
- B3. 引用合法性最小格式（Minimum Legal Citation Format）
  - B3.1 強制欄位（缺一視為未引用）
  - B3.2 可直接貼用的引用標頭
- B4. 裁決順序字串之法律定位（Mnemonic ≠ Override Rule）
- B5. 最終宣告（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
- 治理補強（已整合為正文）
...
- 有效 HFI 存在時：Freeze/最大完備＋累積式更新（原 Only-Add 口徑已淘汰）/Gate 不得阻擋 scope 範圍內之更新/覆寫/重排版；並必須同步產生稽核承接（VERSION_AUDIT 留痕、HASH_MANIFEST、CHANGELOG）。
...
3) MASTER_ARCH（A）  
4) MASTER_CANON（A）  
5) GOVERNANCE_STATE（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 狀態宣告；若列入 Index）
...
### B3.2 可直接貼用的引用標頭
```text
〔TAITS 新手引用標頭｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）｜最大完備＋累積式更新（原 Only-Add 口徑已淘汰）〕
ref_file = <完整檔名>
ref_doc_key = <DOC_KEY>
...
版本狀態：ACTIVE（行為治理文件，最大完備＋累積式更新（原 Only-Add 口徑已淘汰） 演進）
版本日期：2026-01-01
變更原則：最大完備＋累積式更新（原 Only-Add 口徑已淘汰）（可融合更新/覆寫修正（原「只可新增」口徑已淘汰），不可刪減或弱化既有約束）
...
- 版本狀態：ACTIVE（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）；最大完備＋累積式更新（原 Only-Add 口徑已淘汰））
...
## A. Scope（適用範圍）
- doc_key：BEGINNER_GUIDE
- 檔案：TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__260102.md
- 適用狀態：ACTIVE
- 適用時間：2026-01-02（Asia/Taipei）
- 本次作業：融合更新（口徑統一：Only-Add → 最大完備＋累積式更新）、重排版（新增目錄依序排列）、稽核留痕分離（Scope/Changelog/Hash/Audit Hand-off）。

## B. Changelog（變更清單）
1) 口徑統一：將全文殘留之 Only-Add／只可新增／只增不減／只能加／不得覆寫等描述，統一改寫為「最大完備＋累積式更新」治理口徑（允許融合更新/覆寫修正/重排版；禁止摘要縮水；未被新內容明確取代者必保留；被取代者可移除但須留痕承接）。
2) 混讀防護：將 Freeze/ FREEZE 等標記改為「以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）」的表述，避免新舊狀態混讀。
3) 重排版：新增「目錄（依序排列）」以提升可讀性與定位效率；正文內容不減少。
4) 稽核留痕：新增本檔 Scope／Changelog／Hash Manifest／Audit Hand-off，並與正文分離。

## C. Hash Manifest（指紋清單）
- file_name: TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__260102.md
- hash_algo: sha256
- hash_value_sha256: 742b2ab8dce9ce452cfac9c836cae9a8f468406ae3da8a7d9281b2fe60621a12

## D. Audit Hand-off（裁決承接）
- 裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（如有衝突以此序為準）。
- 承接聲明：本檔為操作引導（治理等級 C），不得覆寫 A+/A/B 級治理裁決；所有裁決以其上位文件為準。
