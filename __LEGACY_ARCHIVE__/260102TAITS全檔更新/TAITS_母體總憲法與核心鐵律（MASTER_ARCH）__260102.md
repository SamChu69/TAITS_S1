...
- 版本狀態：ACTIVE（Freeze v1.0；最大完備＋累積式更新）
...
> 本段落為本次「融合更新」之留痕：用於版本可追溯、L11 回放與 VERSION_AUDIT 落帳承接。  
> 本段落不構成新增治理規則；正文裁決力以本文件正文與上位 A+（AI_GOV／DOCUMENT_INDEX）為準。  
> 留痕分離目的：避免新舊混讀；正文維持「單一正確版本」，歷史承接以稽核鏈保存。

## A. Scope（適用範圍）
- doc_key：MASTER_ARCH
- 文件名稱：TAITS_母體總憲法與核心鐵律（MASTER_ARCH）
- 版本日期：2026-01-02（Asia/Taipei）
- 變更類型：融合更新／覆寫修正／重排版（Single Correct 正文收斂）
- 融合來源：
  - TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260102.md（作為主體底稿）
  - TAITS_S1_母體總憲法與核心鐵律（MASTER_ARCH）__251231.md（抽取「L9–L11 細項規格」與「本地運算環境基線」等有效資訊）
  - TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260101.md（交叉校對用；不作為主體版本）

## B. Changelog（變更清單）
1) 將「變更原則」統一對齊 `DOCUMENT_INDEX` §1.4：改為「最大完備＋累積式更新」，允許融合更新／覆寫修正／重排版；禁止摘要化縮水；未被新正文明確更新之有效內容一律保留並持續累積。  
2) 補全 L9/L10/L11 最終語義定錨：移除「…」占位，補入可操作的必備欄位與輸出規格（tracking_id、report_version、authorization_envelope 等），並再次明示 **L11 非下單層**。  
3) 移除重複之「L9–L11 定錨」拷貝段落，改為重導向（redirect）指向唯一有效定義（§1.2），以避免新舊混讀。  
4) 保留並移入「工程參考附錄」：本地運算環境基線資訊（明示其不具治理裁決力，正式制度以 LOCAL_ENV 為準）。  
5) 更新稽核指紋（Hash Manifest）與裁決承接（Audit Hand-off）欄位。

## C. Hash Manifest（指紋清單）
- hash_alg：sha256
- body_scope：BODY_ONLY（不含本 Audit Section）
- body_sha256：bbb3ec6c89d7bda312ad393354a0509c02ecb7e3eada390a34cf5717d77a3c1c

### C1. Source File Hashes（來源檔指紋）
| source_file | sha256 |
|---|---|
| TAITS_S1_母體總憲法與核心鐵律（MASTER_ARCH）__251231.md | fc2f166b9975efe22edf826ebab7cebe3b49e9f0045d1d4650e9c30b1d1e08a1 |
| TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260102.md | a79661e99425e4b56ae6eb660e7fe5241b32ee77eaab7a440873c509c31a8460 |
| TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260101.md | 63a378d62d5cf9ac0a6ccb8895393f226d18c05b6311dcf6268c8f4362c35bbf |

## D. Audit Hand-off（裁決承接）
- change_id：MA-FUSION-260102-0002
- authority_basis：HFI（人類最高決策者明確命令｜scope=MASTER_ARCH｜action=融合更新/覆寫/重排版｜intent=單一正確正文＋最大完備）
- downstream_requirements（必做）
  1) 將本次變更承接落入 `VERSION_AUDIT`（doc_key=VERSION_AUDIT）：新增一筆變更帳（含 change_id、scope、原因、影響面、回滾策略、body_sha256）。  
  2) 若 `DOCUMENT_INDEX` §5 中之 MASTER_ARCH 條目尚未指向本檔（__260102.md），必須更新索引表之檔名/版本日期（索引裁決以 DOCUMENT_INDEX 為準）。  
  3) L11 回放鏈：保存「融合來源檔」與「本檔 body_sha256」對應關係，確保可回放比對（diff 可生成但不得混入正文）。

- rationale：以「最大完備＋累積式更新」承接歷史有效資訊並收斂為單一正確正文；補全 L9–L11 的可操作輸出規格，消除占位與重複拷貝造成的新舊混讀風險。
