# TAITS_專案總覽與使用說明（README）__260102.md
doc_key：README  
...
## R2. 新對話載入最小規格（Minimum Load Set｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
...
建議固定引用標頭（可直接貼用）：
```text
〔TAITS 引用標頭｜以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）｜最大完備＋累積式更新（原 Only-Add 口徑已淘汰）〕
ref_file = <完整檔名>
ref_doc_key = <DOC_KEY>
...
- 允許 **多 Agent** 與 **多策略宇宙** 長期演進（最大完備＋累積式更新（原 Only-Add 口徑已淘汰））
...
1) **最大完備＋累積式更新（原 Only-Add 口徑已淘汰）**：內容可融合更新/覆寫修正（原「只可新增」口徑已淘汰），不可刪減/覆寫/弱化  
...
3) **最大完備＋累積式更新（原 Only-Add 口徑已淘汰）**：不允許刪減或覆寫既有制度語義  
...
- 版本狀態：ACTIVE（Freeze v1.0；最大完備＋累積式更新（原 Only-Add 口徑已淘汰））
...
## A. Scope（適用範圍）
- doc_key：README
- 檔案：TAITS_專案總覽與使用說明（README）__260102.md
- 適用狀態：ACTIVE
- 適用時間：2026-01-02（Asia/Taipei）
- 本次作業：Only-Add 口徑統一 → 最大完備＋累積式更新；重排版（依序排列）；補齊稽核留痕章節（Scope/Changelog/Hash/Audit Hand-off）。

## B. Changelog（變更清單）
1) 口徑統一：移除/改寫 README 內所有 Only-Add/只可新增/只增不減/不得覆寫等口徑，統一為「最大完備＋累積式更新」治理哲學。
2) 歷史標記處理：凡原文出現 Freeze 字樣，改為「以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記）」以避免新舊混讀。
3) 新增本檔稽核留痕：補齊 Scope／Changelog／Hash Manifest／Audit Hand-off，且與正文分離。

## C. Hash Manifest（指紋清單）
- file_name: TAITS_專案總覽與使用說明（README）__260102.md
- hash_algo: sha256
- hash_value_sha256: 078e65434491e2f7c0a004493f240fddd8215774fd9938e3f744db508f221005

## D. Audit Hand-off（裁決承接）
- 裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（如有衝突以此序為準）。
- 承接聲明：本檔僅為入口導覽（治理等級 C），不具上位裁決權；所有治理裁決以 A+/A 級文件為準。
- L9/L10/L11 定錨：本檔不得改寫其定義；如需調整，必須回到 MASTER_CANON 與 AI_GOV 進行單一正確正文融合更新並留痕。
