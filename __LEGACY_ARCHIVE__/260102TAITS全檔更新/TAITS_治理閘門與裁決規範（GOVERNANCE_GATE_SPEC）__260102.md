...
- 本文件實體檔名（本次正文版）：`TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__260102.md`
...
## B. Changelog（變更清單）
1) 融合舊版（S1/251231）與新版（260102）內容：保留全部有效資訊，移除會造成新舊混讀之補丁式段落與舊標記（Addendum/Appendix/Freeze/Only-Add 註記），以單一正文呈現。  
2) 將「只能加／Only-Add」口徑統一改為「最大完備＋累積式更新」：允許融合更新／覆寫修正／重排版以形成單一正確正文；禁止摘要縮水；未被新版本明確更新之有效內容必保留並持續累積；被明確取代者由稽核留痕承接。  
3) 重新排版、依序排列：以「文件定位→語義定錨→Gate 模型→Flow 定位→邊界→衝突裁決→引用/可回放→規則全集→原因碼→Two-Gate→審計→UI→流程圖→演進規則→環境基線→終局語句」建立單一路徑閱讀順序。  
4) 修復原檔中段落順序不連續、內容重複與缺段（例如 Two-Gate 協作段落、Mermaid 節點缺漏、原因碼缺項、引用格式分散等），並統一為可稽核可引用之正文。  
5) 將工程端「本地運算環境基線」納入正文之「參照」章節，明確標示其不構成治理強制，並宣告若與 LOCAL_ENV 衝突以 LOCAL_ENV 裁決為準。

## C. Hash Manifest（指紋清單）
- hash_alg: sha256
- scope: BODY_ONLY（不含本 Audit Section）
- hash_value_sha256: d1e9ee0b1c1a86216be0ab2917c22ef29e63fa3a9caafec87cf5fae9e8a866fc

## D. Audit Hand-off（裁決承接）
- change_id: GGS-FUSION-260102-0002
...
- downstream_requirements:
  ...
  - 若其他文件仍存在「Only-Add 口徑」「附錄/補丁混入正文」「順位字串被誤讀為覆蓋規則」或「UI 遮蔽 Gate/風控資訊」等情形，需依同一模式逐檔融合更新以完成全局一致化。
