# TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260102__融合更新完整版
...
- 上位裁決序列：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON →（AI 行為規範受 AI_GOV 約束；衝突裁決依 DOCUMENT_INDEX §6）
...
- A6. 最終宣告（以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
...
### A2.1 「不定義資料來源（由 DATA_UNIVERSE 管理）」之唯一合法解讀
本文件主文中若出現上述或同義語句，以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 下唯一合法解讀為：
...
- 版本狀態：ACTIVE（以 GOVERNANCE_STATE 現況為準；若見 Freeze 字樣，視為歷史標記）
...
flowchart TB
  ...
  L9 --> L10[L10 UI Human Decision]
  L10 --> L7[L7 Risk/Compliance Veto]
  L7 -->|PASS Token| L11[L11 Audit Replay / Trace]
  L7 -->|VETO| V[VETO + reason_codes + audit]
```
...
（最大完備＋累積式更新 · STRATEGY_UNIVERSE 專用 · 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記））
...
- 策略狀態（Strategy Status）：DRAFT / REVIEW / ACTIVE / DEPRECATED / ARCHIVED
  （以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） 建議：新增一律先 DRAFT → 經治理審查後才可標 ACTIVE）
...
（Part 8｜新增策略條目治理模板 · 以 GOVERNANCE_STATE 現況為準（若見 Freeze 字樣，視為歷史標記） · 最大完備＋累積式更新 完）
...
- doc_key：STRATEGY_UNIVERSE
- 版本日期：2026-01-02（Asia/Taipei）
- 融合更新目標：形成「單一正確正文版」（Single Correct Body），並將稽核留痕與正文分離（本章）。
- 融合來源（inputs）：
  1) `TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260101.md`
  2) `TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260102.md`
- 產出（output）：
  - `TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260102__融合更新完整版.md`

## B. Changelog（變更清單）

1) **正文去重與重排版**：移除重複片段與重覆「治理補強」段落，改以單一結構呈現；章節改為一致編號與清楚邊界。  
2) **語義定錨一致化**：全面對齊 S1 定錨（L9＝投資報告；L10＝人類裁決；L11＝全鏈路稽核回放、非下單層）。  
   - 修正原文中可能引發混讀之表述（例如：以 L11 直接承擔「下單／執行指令」之措辭），改為：策略層不得生成可執行委託；交易執行僅能在 L10 裁決後由外部執行模組承接，並由《EXECUTION_CONTROL》規範控制與留痕。  
3) **DATA_UNIVERSE → DATA_SOURCES 封口**：保留 DATA_UNIVERSE 作為「概念別名」之歷史可讀性，但治理引用一律回歸 `doc_key=DATA_SOURCES`，並提供最小可稽核引用模板（A3、附錄 G）。  
4) **補齊治理身份與引用合法性**：新增 A7–A9（Canonical Identity / Physical Artifact / 引用最小規則），避免跨文件引用時出現「找得到檔案但身份不合法」之 Gate 風險。  
5) **附錄整理**：  
   - 附錄 Z：集中 MASTER_CANON 對位補強，並修正層級與標題階層以避免目錄混讀。  
   - 附錄 G：集中跨文件一致口徑（Index/Label/Alias/引用格式）並提供可直接貼用模板。  

## C. Hash Manifest（指紋清單）

### C1. Inputs（來源檔案 SHA-256）

- `TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260101.md`  
  - sha256: `a1287f74bb8255726d28601951afef0c9246e3232f96f8572757458b1ab0c0ae`
- `TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260102.md`  
  - sha256: `643ac61a1c64e3e4e8ee5a843b4d54b5b95840238e6498f7d751909108dd7a85`

### C2. Output（產出 SHA-256）

- `output_body_sha256`（僅計算「正文＋附錄」；不含本 Audit 章節）：`11ffd64f9b65b70cec0b105e916efbef799510f73165e6f3459618e9e4641c5c`
- `output_full_sha256_excluding_hashline`（全檔；但**排除本行**後計算）：`7dad4a6ea4114c05af6fcab35e0e038fa12b87b514a0c6ea92bfd3d5b3d5618e`

> 註：此設計用於避免「Hash 寫入檔案本身導致自我變動」問題；驗證時請先移除本行再計算 SHA-256。

## D. Audit Hand-off（裁決承接）

1) **VERSION_AUDIT 登錄要求**：本次融合更新屬「正文整併＋語義定錨一致化」，需在 `doc_key=VERSION_AUDIT` 中新增一筆紀錄，至少包含：  
   - ref_doc_key=STRATEGY_UNIVERSE  
   - ref_version=260102  
   - ref_file=`TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260102__融合更新完整版.md`  
   - ref_change_type=merge_reformat_semantic_anchor  
   - ref_notes=（填入 B 節 Changelog 要點）  
2) **跨文件引用承接**：任何文件引用 STRATEGY_UNIVERSE 時，必須同時帶：`doc_key=STRATEGY_UNIVERSE` + `canonical_filename=TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__251219.md`（見 A7–A9）。  
3) **相依文件提醒（不等於要求修改）**：若相依文件仍存在「L11=下單層」等舊措辭，應以本檔 S1 定錨為準，並在其下一次融合更新時一併消歧。
