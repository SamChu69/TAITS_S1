...
## 附錄 Z｜Canonical Addendum：L9a–L11 人機分工與全紀錄原則補充
（Only-Add · Semantic Lock）

### 補充性質聲明
- 適用文件：MASTER_CANON
- 補充類型：Canonical Flow 語義補強（Addendum）
...
# Addendum 2025-12-28｜Only-Add：Trace ID / Evidence Chain 欄位貫穿規範（Ops-First Correlation Contract）× Freeze v1.0

> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219.md（doc_key：DEPLOY_OPS）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：AI_GOV（A+）＋DOCUMENT_INDEX（A+｜Authoritative Index）＋MASTER_ARCH（A）＋MASTER_CANON（A）  
> 平行對齊：LOCAL_ENV｜Addendum 2025-12-27（Evidence Chain Template）／UI_SPEC｜Addendum 2025-12-27（UI Trace 最小引用欄位）／VERSION_AUDIT｜Addendum 2025-12-28（Ledger 收斂）  
> 稽核對位：VERSION_AUDIT｜Appendix A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0020`）  
...
## Appendix T｜Trace & Evidence Correlation Contract（Freeze v1.0）
...
taits_ops_header:
  ...
  index_gate_ref:
    ...
    ref_doc_key: "DOCUMENT_INDEX"
    ref_version_date: "__251220"
    ref_section: "<Authoritative Index / Addendum 映射表章節>"
...
# Addendum 2025-12-28-B｜Only-Add：Repo 落盤規則（LATEST/ARCHIVE）× Artifact Lifecycle × 選檔風險控制（Freeze v1.0）

> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219.md（doc_key：DEPLOY_OPS）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：AI_GOV（A+）＋DOCUMENT_INDEX（A+｜Authoritative Index）＋MASTER_ARCH（A）＋MASTER_CANON（A）  
> 平行對齊：DOCUMENT_INDEX｜Latest Mapping（machine-readable）Addendum 2025-12-28-D/F／VERSION_AUDIT｜METADATA_FIX Ledger／LOCAL_ENV｜Evidence Chain／UI_SPEC｜Trace 最小欄位  
> 稽核對位：VERSION_AUDIT:VA-METADATA_FIX-20251227-0024（需可在 Ledger 查得）  
> 目的：在 Freeze v1.0 下，新增「Repo 落盤/封存」的營運規範（不改任何治理裁決），以降低同一 doc_key 多版本並存造成的“選錯檔”風險，並確保工具鏈與人工載入的 latest 行為一致、可回放、可稽核。

---

## B1. 適用範圍與硬邊界（Hard Boundary）

本 Addendum 僅規範「檔案落盤與封存」的**營運操作**：
...
taits_repo_landing_manifest:
  ...
  latest_mapping_source: "DOCUMENT_INDEX machine-readable mapping (Addendum 2025-12-28-D/F)"
  archive_policy:
...
### A2. 本專案目錄中的實體檔案（Physical Artifact）
- physical_filename（目前專案內實際檔名）：`TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219__ADDENDUM_20251228_FINAL (1).md`
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。
...
# Addendum 0｜Only-Add：METADATA_FIX（DATA_SOURCES × DATA_UNIVERSE 名詞對齊與引用合法性鎖定｜Freeze v1.0）
...
- 任何出現 `doc_key：DATA_UNIVERSE` 之舊標示，均屬「Metadata Discrepancy」的歷史殘留標記；其法律效力被本 Addendum 0 否認
...
本 Addendum 0 屬於「METADATA_FIX（中繼資料修補）」型變更：
...
# Addendum 0.1｜Only-Add：防誤用告示（DATA_UNIVERSE 別名鎖定）× 引用模板 × 摘錄安全標頭（Freeze v1.0）
...
## 0.1 本 Addendum 的適用範圍（Hard Boundary）

本 Addendum 只處理下列「引用/摘錄/拷貝」場景（不改寫資料治理主體內容）：

- 文件被切片摘錄到其他文件（例如 Appendix/Part 被單獨引用）
...
凡引用本文件（不論主文或任何 Part/Appendix 片段）時：
...
- 在 Evidence/Audit/UI Trace 中，把 `DATA_UNIVERSE` 當成 ACTIVE 文件引用（除非 DOCUMENT_INDEX 另行新增並裁決為 ACTIVE；Freeze v1.0 期間不得以此 Addendum 變相新增）

---

## 0.3 摘錄安全標頭（Excerpt Safety Header｜可直接貼用）

> 用途：任何從本文件複製出去的片段（尤其是 Part/Appendix），必須在片段最前方加上本標頭，以防誤用。
...
ref_section = <例如：Part 3 §32 / Appendix B §B.7 / 主文 §2.3>
ref_excerpt_hash = <可選：若已對摘錄做 hash>
ref_notes = 若摘錄內含「doc_key：DATA_UNIVERSE」字樣，依 Addendum 0/0.1 視為 alias，不具法律效力
```
...
本 Addendum 允許以 **Only-Add** 方式補上缺失之結尾 fence：
- 不改寫 Mermaid 內容
- 不刪改後續章節文字
- 僅補上必要的結尾 fence，以恢復正確的 Markdown 區塊邊界

（Addendum 0.1｜Only-Add｜Freeze v1.0 完）
...
# 【D】DATA_SOURCES｜Addendum 0.2B（Only-Add）
> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_資料來源全集（DATA_SOURCES）__251219.md（doc_key：DATA_SOURCES）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON  
...
- 依 Only-Add：不得回頭改寫既有段落；因此本 Addendum 以重新附錄方式提供等價語義之可渲染版本。
- 任何引用 Mermaid 圖表時：以本 Addendum 之附錄版本為準（僅限渲染修補，不變更流程語義）。

---

## 0.2B.2 Appendix｜Mermaid 可渲染版本（對應原章節 12 / 15 / 17 / 20）

### Appendix M12｜資料來源治理流程（L1-L2）— Renderable
...
### Appendix M15｜Source Category × Fallback Hierarchy — Renderable
...
### Appendix M17｜Data Quality Gate — Renderable
...
### Appendix M20｜Snapshot × Replay Data Ref Graph — Renderable
...
# Addendum 2025-12-28｜Only-Add：P0 全閉合（DATA_UNIVERSE 全量降格為 Alias × doc_key 封口 × 引用模板硬化｜Freeze v1.0）
...
（Addendum 2025-12-28｜Only-Add｜DATA_SOURCES｜P0 全閉合 完）
---
...
### A2. 本專案目錄中的實體檔案（Physical Artifact）
- physical_filename（目前專案內實際檔名）：`TAITS_資料來源全集（DATA_SOURCES）__251219__ADDENDUM_0_2C_FINAL.md`
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。
...
# Addendum 2025-12-27｜Only-Add：DATA_UNIVERSE 別名映射 × 引用合法性鎖定 × Rulebook/Trigger Map 之 doc_key 對位（Freeze v1.0）

> 補充性質：Only-Add（只可新增，不可刪減/覆寫/偷換制度語義）  
> 適用文件：TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251219.md（doc_key：TWSE_RULES）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：Index Gate（DOCUMENT_INDEX）＋治理等級覆蓋（A+ > A > B > C）＋衝突裁決程序（DOCUMENT_INDEX §6）＋AI_GOV（A+）最高約束  
> 口徑對齊：DOCUMENT_INDEX｜Addendum 2025-12-27（裁決順序字串統一 × 歧義消解）  
> Gate 對齊：GOVERNANCE_GATE_SPEC｜Addendum 2025-12-27（字串助記定位 × 雙軌裁決消歧）  
> 資料治理對位：DATA_SOURCES｜Addendum 0 / 0.1（DATA_UNIVERSE 僅為概念別名，不具 doc_key 法律效力）  
> 稽核對位：VERSION_AUDIT｜Appendix A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251227-0001`）  
> 目的：修補本文件檔頭「平行參照：… / DATA_UNIVERSE / …」可能造成的 doc_key 誤用風險；固定本文件在引用 DATA 層時的唯一合法 doc_key 口徑，避免 Gate 命中「引用非法」而 BLOCK。

---

## 0. 適用範圍（Hard Boundary）

本 Addendum 僅處理「引用端」歧義，不改寫本文件對官方制度來源、Rulebook Snapshot、Trigger Map 的任何語義：

- 本文件仍可使用「DATA_UNIVERSE」作為概念名詞（Data Universe／資料宇宙）或閱讀別名（alias）
- 但本 Addendum 將其 **明確否認為 doc_key**，並規定引用端的唯一合法 doc_key
...
ref_doc_key = DATA_SOURCES
ref_section = <例如：Addendum 0 / Addendum 0.1 / Part X / Appendix Y>
ref_notes = 若片段內含「DATA_UNIVERSE」字樣，依 DATA_SOURCES Addendum 0/0.1 視為 alias，不具法律效力
audit_anchor = VERSION_AUDIT:VA-METADATA_FIX-20251227-0001
```
...
- 其法律效果由本 Addendum 限定為「概念別名（alias）」用途
...
### A2. 本專案目錄中的實體檔案（Physical Artifact）
- physical_filename（目前專案內實際檔名）：`TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251219__ADDENDUM_20251227_FINAL.md`
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。
...
> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_專案總覽與使用說明（README）__251220.md（doc_key：README）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：AI_GOV（A+）＋DOCUMENT_INDEX（A+｜Authoritative Index）＋MASTER_ARCH（A）＋MASTER_CANON（A）  
...
# Addendum 2025-12-27｜Only-Add：Index Gate 載入優先 × 本文件法律地位限縮 × ACTIVE 清單「不固化」× 引用/稽核最小格式強化（Freeze v1.0）
