...
基線日期：2026-01-11（Asia/Taipei）  裁決序位（自高至低）：DOCUMENT_INDEX、MASTER_ARCH、AI_GOV
...
- 新增：基線日期 2026-01-11（Asia/Taipei），與全專案基線對齊。
- 2026-01-11｜Final QA｜完成正文正文化：去重收斂與結構重排；移除箭頭順位與治理字母標籤；統一策略層之引用口徑；重算指紋。
- 2026-01-13：L8_LAYER_QA_FIX（新增 Canonical 對齊聲明；補強 L8 最小提案工件鍵段落；將 direction_hint 明確標示為 Legacy 相容欄位且禁止輸出；重算 BODY_SHA256。）
- 2026-01-13：L8_LAYER_QA_FIX_v2（澄清 direction_hint 僅為語義標籤且永久禁止 long/short 等方向值；新增 intent_hint 為 Canonical 欄位並定義 direction_hint 為 Legacy alias；重算 BODY_SHA256。）

- 2026-01-13：CONSISTENCY_QA_FIX（修正 Canonical 對齊裁決序位表述以對齊 DOCUMENT_INDEX→MASTER_ARCH→AI_GOV；修正 L7 VETO 與 L11 稽核回放關係之描述；修正 Mermaid 流程圖使 L7 位於 L8 之前且治理 Gate 為 non-layer；重算 BODY_SHA256。）
