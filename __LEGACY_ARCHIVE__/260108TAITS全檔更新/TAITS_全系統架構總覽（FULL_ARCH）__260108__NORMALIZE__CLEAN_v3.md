...
### 2.1 0 Execution & Control Domain（執行控制域）
...
### 2.1 1 Infrastructure Domain（基礎設施域）
...
### 4.4 Agent 在架構中的定位（Agent Positioning｜嚴格對齊）
...
所屬 Domain（Data/State/Analysis/Evidence/Regime/Risk/Strategy/Gov/UI/Exec/Infra）
...
### 4.5 AI 在架構中的位置（AI Placement｜不得升格）
...
### 4.6 儲存與不可變更稽核（Storage & Immutable Audit）
#### 5.6.1 儲存分層（推薦最小集合）
...
### 6.1 Contract 的共同硬性欄位（所有跨模組輸入/輸出都必須）
...
### 7.1 0 Execution & Control Domain（L11）契約
...
### 12.6 Risk/Compliance 失效模式（最高敏感）
規則快照缺失 / 版本不可追溯
...
### 13.3 Trace / Audit 完整性（Version/Audit 必須做到）
...
### 13.4 Strategy 不可越權（Strategy ≠ Order）
...
### 14.2 允許的回饋（Feedback）型態（最大完備＋累積式更新）
...
### 15.1 Module Spec Template（每模組一份）
module_id
...
- BODY_SHA256：4cc5d9f8b91274c391e07aef17b1e10809a9e8dbd587ff34a5f157212df1b888
