...
- **Canonical Flow 主幹來源（唯一正文）**：L1–L11 定義以 doc_key=MASTER_CANON 為唯一正文來源；本檔僅宣告承接規則，禁止其他文件重複定義造成語義漂移。  
...
### S1-2｜L9–L11 最終語義承接（以 MASTER_CANON 為準）
- 定義來源：doc_key=MASTER_CANON（Canonical Flow）之 `### L9`／`### L10`／`### L11` 為唯一正文來源。
- 承接規則：其他文件不得另行重複定義或擴寫以致改寫主幹語義；如需補強語義，應在 MASTER_CANON 進行並由 VERSION_AUDIT 落帳承接。
...
| TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260108__NORMALIZE__CLEAN.md | DOCUMENT_INDEX | A+ | ACTIVE | 2026-01-08 | TAITS 全系統（TWSE / TAIFEX；Research / Backtest / Simulation / Paper / Live） |
...
- 2026-01-08：Normalize 收斂：移除文件頭補丁式敘述；將 §S1-2 之 L9–L11 定義改為指向 MASTER_CANON（避免跨文件重複定義漂移）；並同步更新 Authoritative Index 中已更新文件之檔名與版本日期；同步更新 Hash Manifest。- 修正：將 doc_key=DOCUMENT_INDEX 之「Authoritative Index（ACTIVE=21）」中，已完成 Final QA 清理版本之檔名統一指向現行覆蓋輸出（優先採用 __CLEAN_260107 版本；避免 __PATCH1 版本作為現行引用）。
...
- BODY_SHA256：8a58e7c50aecbb7356e3d2d6e6417336d817340971ac1713b2e449e6e2b6dc31
- LEGACY_AUDIT_SHA256：2cf04eefb79f6a9fe1da27d23ecc2e53bf3d5b48a1e0ed97fec48e12c72b9cb3
