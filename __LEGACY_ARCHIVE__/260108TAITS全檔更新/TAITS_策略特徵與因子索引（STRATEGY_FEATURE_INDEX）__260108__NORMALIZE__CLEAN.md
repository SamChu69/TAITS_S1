...
→ Strategy Proposal
→ Investment Report
→ Governance Gate（non-layer｜對 報告做檢核）
→ Human Decision
→ Execution Control（non-layer｜僅於 授權後可啟動）
→ Audit Replay
任何違反此路徑的行為，均屬治理違規：
...
Feature 只能用於：
1) 組成 Evidence Bundle 的元素
2) 協助 Regime 判定（作為狀態描述輸入，不是方向輸入）
3) 約束策略「適用性」與「可信度條件」
4) 提供審計/解釋/回放素材
Feature 永遠不能用於：
...
---
## 附錄A：Feature ID 命名規範與範例全集（最大完備）
本附錄A 為 Part 1 的 **最大完備＋累積式更新 擴充**，不改寫、不覆蓋任何既有語義。
...
把方向/行為/績效詞合法化
用「簡化」名義刪除版本段（VER）
## 附錄B：特徵治理檢核（Feature Governance Checklist）＋違規判定（最大完備）
本附錄B 為 Part 1/Part 2 的 **最大完備＋累積式更新 擴充**。
...
本附錄B 的作用就是把上述「展開」變成可審計規則，而不是靠猜。
---
...
- 本附錄提供「結構化模板」與「描述性特徵範例」：
 - ✅ 可用於 Feature Engine / Evidence Builder / UI Explain / Audit Replay
...
複製程式碼
flowchart TB
 [Snapshot Ready] --> [Feature Engine<br/>Compute FeatureValueRecords]
 --> P[ProvenanceMap + Hash + QualityFlags]
 P --> G{Feature Governance Gates<br/>Part3 Gate-1..6}
 G -->|FAIL| R[RETURN: Fix Feature Meta/Provenance/Replay]
 G -->|PASS| [Evidence Builder<br/>Assemble Evidence Bundle]
 --> [Regime Engine<br/>State Classification]
 --> [Risk & Compliance Gate<br/>PASS/VETO]
...
## A.0 本段定位與效力聲明（不可省略）
1. 本段為 **STRATEGY_FEATURE_INDEX 對位 MASTER_CANON 的補充指引**，用於確保「特徵（Feature）層」在 Canonical Flow 中之合法邊界、責任歸屬與稽核可追溯性；**不構成新 Layer**，亦 **不改寫 MASTER_CANON 的 –**。
...
- MASTER_CANON：定義 – 的唯一 Canonical Flow（流程裁決母法）。
- STRATEGY_FEATURE_INDEX：定義 （Feature）層之**語義邊界、計算輸出、品質旗標、來源映射（Provenance）、版本/雜湊（Hash）、可回放（Replay Determinism）**等工程與治理必備義務；並提供 Feature → Evidence → Regime 的合法流之規格化展開。
...
- 上游：本檔不定義 Snapshot 規則本體，但要求 Feature 計算必須能被 Snapshot 回放支持（Reproducible / Deterministic）。
- 本層：本檔定義 FeatureValueRecord、Provenance、QualityFlags、VersionRef、Hash 等最小稽核工件；缺任一項可在 Live/Paper 升級為風控否決（SYS-AUDIT / SYS-PROV）。
- 下游：本檔允許 Feature 作為 Evidence/Regime 的輸入，但 **不得讓 Feature 直接成為 PASS/VETO 或裁決依據**；裁決權仍屬 （RISK_COMPLIANCE）與 （Decision）。
...
## Y.0 附錄目的（不可省略）
本附錄提供兩件事：
1) **Canonical 對位**：把 Feature 層的輸出、品質、來源、版本、稽核工件，與 MASTER_CANON 的上下游責任邊界做「可驗證」對位。
...
---
## Y.1 Canonical Layer 對位表
本表為「工程落地檢核」用途；不改寫 MASTER_CANON 定義。
| Canonical Layer | 角色定位 | STRATEGY_FEATURE_INDEX 在此層的硬義務 | 缺失後果（治理語義） |
|---|---|---|---|
| Snapshot Ready | 可回放快照就緒 | Feature 計算須可回放；不得存在 lookahead；需能追溯 available_at（對延遲資料尤需） | Gate RETURN 或升級風控（視缺陷類型） |
| Feature | 特徵計算與語義契約 | 必輸出：FeatureValueRecord / ProvenanceMapRef / QualityFlags / VersionRef / Hash | Live/Paper 可升級否決（SYS-AUDIT / SYS-PROV） |
| Evidence | 證據組裝 | Feature 僅作為 Evidence Input；不得直接輸出買賣方向或「裁決」 | 語義違規（GOV-FTR-SEM-*） |
| Regime | 狀態分類 | Feature 允許作 Regime Input，但不得越權宣告可交易/不可交易 | 越權視同治理違規 |
| Risk & Compliance | 最高否決權 | Feature 不得成為 PASS/VETO；不得縮減風控揭露與稽核工件 | 觸發 VETO/BLOCK |
---
...
FeatureRefBlock:
 ...
 - "Hash"
 ui_explain_binding:
 explain_level: "||" # 對位 UI_SPEC 顯示階層（僅接口，不越權 UI 規範）
 display_prohibition:
...
### 1) 變更清單（Changelog）
- 正常化：移除補丁式/對話式/歷史快照（含 Part/舊版 metadata）與跨文件治理附錄（Label/助記順位/跨文件一致口徑重複）。
- 收斂：整併 Part 2/Part 3 為「附錄A/附錄B」並維持最大完備內容；修正文中不必要的層級編號記號，避免在本檔重複層級定義。
- 排版：修正標題階層、清單與分隔線，移除省略符號占位。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256
- BODY_SHA256：ef19803ad3e27ecf6877038c3486a7d9f03728b592295d5b59e112a9f91bf3bb

### 3) 作用範圍（Scope）
- doc_key=STRATEGY_FEATURE_INDEX（本文正文；不含稽核區塊）。

### 4) 稽核交付（Audit Hand-off）
- 交付型態：可直接覆蓋之完整 Markdown（最大完備；正文乾淨）。
- 驗收要點：正文不得含 Label/助記順位/歷史快照；不得重複定義 Canonical Flow（L1–L11）。
