# TAITS_資料來源全集（DATA_SOURCES）__260108（正文乾淨｜最大完備）
...
編修原則：在不改寫制度語義之前提下，僅做正文去重收斂與結構重排；不得摘要縮水。
...
- 定義 TAITS 可使用之資料來源類型、權限/授權、可靠度/延遲/頻率、欄位與最小引用格式（與 VERSION_AUDIT/稽核回放層 留痕相容）。
...
編修原則：在不改寫制度語義之前提下，僅做正文去重收斂與結構重排；不得摘要縮水。
Canonical Flow：L1–稽核回放層 之定義以 doc_key=MASTER_CANON 為唯一正文來源；本文件僅規範資料來源與資料治理。
...
### 6.3 FALLBACK（第三方｜只作/交叉驗證，不得裁決制度）
...
- 影響方向在系統中**不寫死單向因果**：預設以「全球→區域→台灣」做環境先驗，但任何“方向性結論”必須由數據與事件交叉驗證支撐，並全留痕於 稽核回放層。
...
- scope_files_output: TAITS_資料來源全集（DATA_SOURCES）__260108__NORMALIZE__CLEAN.md
- scope_files_source: TAITS_資料來源全集（DATA_SOURCES）__260106__單一正確正文版__最終覆蓋版__覆蓋輸出__FINALQA_260106__CLEAN_260107.md
- scope_mode: FINAL QA MODE｜NORMALIZE ALL（正文去重收斂／結構重排／非正文稽核四件套固定）
- version_date: 2026-01-08（Asia/Taipei）

## B. Changelog（變更清單｜非正文）
1) **正文去重**：移除重複之「全局定錨/文件定位」段落，保留單一版本。
2) **語義口徑收斂**：移除將 L1–L11 另行特別化之敘述；僅保留「L1–L11 以 MASTER_CANON 為唯一正文來源」之承接句。
3) **職責收斂**：移除非屬資料來源治理之「補丁式治理段落與跨檔身份/引用助記」等章節，避免跨文件混讀。
4) **補丁語句清除**：移除對話/流程式語句（如、等），維持制度正文表述一致。
5) **版本日期更新**：版本日期統一為 2026-01-08（Asia/Taipei）。
6) **稽核四件套固定**：保留 A/B/C/D 四件套於檔尾（非正文），並同步更新指紋。

## C. Hash Manifest（指紋清單｜非正文）
- body_sha256: 428a46d8b38aa0542a74c4064de7a954760a7d3bae66d477f1bb7f3c58a17e70
