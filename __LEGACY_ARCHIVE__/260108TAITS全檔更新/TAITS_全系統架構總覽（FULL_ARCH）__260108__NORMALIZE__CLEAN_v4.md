...
### 事件匯流排（Event Bus）與訊息契約（Message Contracts）
...
### 儲存與不可變更稽核（Storage & Immutable Audit）
#### 儲存分層（推薦最小集合）
...
### Strategy & Research Domain（L8）契約
...
#### `GovernanceChecked`（治理閘門產物｜非 Canonical Layer）
**輸入**：Proposal + 全鏈路審計引用  
...
### Execution & Control Domain（L11）契約
...
#### `ReconciliationCompleted`（ExecutionControl-Post｜非 L11 層）
必備欄位：
...
- BODY_SHA256：27792032869f725130f8d691e25af1fa929aa3c06a28ee2e6f29563cb9e63ff1
### 3) 覆蓋範圍（Scope）
