...
- 治理裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV（最終裁決序位）
...
### 1) 變更清單（Changelog）
- 修正：文件頭之版本日期與基線日期更新為 2026-01-08（Asia/Taipei）。
- 修正：全文件「治理裁決序位」之表示法統一收斂為唯一箭頭鏈口徑：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV。
- 修正：移除正文重複的 L9/L10/L11 定錨段落；L9/L10/L11 以 Canonical Flow 正文段落為唯一版本。
- 修正：本檔若引用其他治理文件檔名，統一指向現行覆蓋輸出檔名（優先 __CLEAN_260107）。
- 修正：若原檔存在稽核區塊，已以 Legacy Snapshot 承接於檔尾稽核區塊，避免正文／稽核邊界混讀。
- 修正：文件頭（Header）之重複 doc_key/日期/裁決序位資訊已收斂為單一「文件頭」區塊，避免同檔雙表頭混讀。
- 修正：Legacy Audit Snapshot 內部標題已改為「Legacy Audit Snapshot」以避免與檔尾「稽核區塊」同名造成混讀；保留原文內容與結構。
- 保留：MASTER_CANON 內容全量承接，不做摘要縮水；僅做去混讀與引用一致化。


### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：0f6805bc04372aa3dfc0ef605d61805572571d9038e351c34fa7068444268840
- LEGACY_AUDIT_SHA256：4604c6765753d3b987495706e65cc9473c85e65f94dac8cf9ac9071951fbb7fa
...
- 文件頭重複去重（表頭收斂為單一 Header）
- Legacy Snapshot 標題去混讀（不影響正文裁決；僅降低讀者混讀風險）
...
### 5) Legacy Audit Snapshot（原稽核區塊原文｜非正文）

> # Legacy Audit Snapshot（原稽核區塊原文｜非正文）
> 
