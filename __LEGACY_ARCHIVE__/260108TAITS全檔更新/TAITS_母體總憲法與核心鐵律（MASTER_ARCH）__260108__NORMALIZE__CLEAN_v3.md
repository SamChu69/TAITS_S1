...
- 2026-01-08：NORMALIZE（正文乾淨／無重複／最大完備）
  ...
  - 移除跨文件語義重覆宣告（L9–L11 定義以 MASTER_CANON 為主幹；本檔保留治理鐵律與裁決規則）
  - 移除「治理補強」區塊中與正文重複之 0–13 章節（避免同檔雙版本混讀）
  - 承接並正式化「Snapshot 不裁決／Index Gate First」規則，納入 §12
  - 移除非必要之標籤/助記型補丁段落（避免正文污染）
  - 移除 Legacy/對話式留痕內容（僅保留稽核四件套）

### 2) 指紋清單（Hash Manifest）
- HASH_RULE：SHA-256（UTF-8，LF；Body=本檔「稽核區塊」之前全文，含結尾換行）
- BODY_SHA256：85f41a130a0527db869f682284ee007a3e9bd4d6f15730e69e697e857e977b4c
