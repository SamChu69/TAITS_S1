...
### 1) Changelog（變更清單）
- 2026-01-08｜NORMALIZE｜正文重排與格式修復：補齊文件頭之治理等級；修正章節層級/編號一致性；將表格與清單轉為標準 Markdown；移除正文中非 Markdown 片段（如「複製程式碼」提示）；統一以「L1–L11（MASTER_CANON）為唯一正文來源」承接，不在本檔改寫定義；維持最大完備（僅去重與結構重排）。

### 2) Hash Manifest（指紋清單）
- BODY_SHA256（UTF-8, LF）：05e7815c6dfd67f39d44defd484cc7e437560864ffa37797a267d26e6200dd80
