...
- 上位裁決與治理約束一律依裁決序位：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV。
- Canonical Flow（L1–L11）之層級定義與語義一律以 doc_key=MASTER_CANON 為唯一正文來源；本文件不得另行重複定義層級語義。
- 部署/營運不得自行裁決 ACTIVE 文件集合、doc_key、治理等級或「最新版本」指向；如需裁決，一律依 DOCUMENT_INDEX 的 Authoritative Index。
...
### 1) 變更清單（Changelog）
- 正常化：移除正文中「覆蓋輸出/去混讀/補丁式」描述與對話痕跡，收斂為乾淨正文。
- 正常化：移除正文中對 L9/L10/L11 的重複定義與特別化段落；層級語義改為僅承接 doc_key=MASTER_CANON（L1–L11）。
- 正常化：移除正文中「子級標籤/Label」「順位助記」「DATA_UNIVERSE alias」「Legacy Snapshot」等非部署/營運制度正文內容。
- 保留：部署/營運制度正文最大完備承接（不做摘要縮水）；僅進行去重、結構重排與格式修復。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：e57bd50b815d9f0a137f0c03b249e91959a74787b98a9913a6ea0d553821e33a

### 3) 適用範圍（Scope）
- doc_key：DEPLOY_OPS
- 變更類型：NORMALIZE（正文乾淨／去重收斂／結構重排／格式修復）
- 不變更事項：不新增治理層級定義；不改寫 MASTER_CANON 之層級語義；不新增投資/交易決策內容。
