...
2) **子級標籤**：凡出現 `S` / `B+` / `C+` 等字樣，一律視為顯示標籤（Label），不構成新的治理等級 bucket；其中 `S` 僅代表「Supreme Canon 顯示/完備度標籤」，`B+` / `C+` 僅代表「嚴格度/完備度子級標籤」。治理覆蓋規則仍固定為：**A+ > A > B > C**（以 DOCUMENT_INDEX 為準）。  
...
- 收斂：將「子級標籤（S/B+/C+）」之解讀收斂為 G0 單一條款；移除獨立 G2 章並重編號後續 G 章，避免正文重複與誤解為新治理等級。
### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：70019c8c02148cba6685e887cee9f1bcaa02ddab5bad6883f5c8a4a8b1c378fe
