...
- 風險與合規（Risk/Compliance）：
  - 在無人類明確命令時可否決（阻斷進入執行）。
  - 在有人類明確命令時，必須輸出完整風險揭露與替代方案，並以「強制揭露＋確認＋全紀錄」承接；不得以程序性方式卡死命令。
...
- 有效 HFI 存在時：本檔不得以程序性理由阻擋命令範圍內的合法行為，但必須同步輸出完整風險揭露、替代方案與可稽核留痕。
...
### 1) 變更清單（Changelog）
- NORMALIZE：移除正文中的補丁式語句、流程/狀態描述與非制度性段落。
- 去重收斂：合併重複之「全局定錨（S1）」為單一版本，並移除任何對層級之重複定義。
- 正文清理：移除「標籤/助記/順位字串法律定位」等非必要正文段落，避免混讀。
- 版本更新：版本日期與基線日期更新為 2026-01-08（Asia/Taipei）。
- 稽核同步：更新 Hash Manifest 以維持可追溯性。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SCOPE：全文正文（不含本稽核區塊）
- BODY_SHA256：e1d146b48d5c4db07f50601d11c215073117d0e690a483c33cf8d4784346182b

### 3) 適用範圍（Scope）
- 僅處理 doc_key=RISK_COMPLIANCE 之文本正常化（去重、重排、移除非制度正文）。
- 不變更本檔既有風險/合規否決權條款、理由碼、門檻規則與 Gate 結構之實質語義。

### 4) Audit Hand-off（裁決承接）
- change_id：RC-NORMALIZE-260108-0001
