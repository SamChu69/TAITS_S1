...
- 裁決序位：DOCUMENT_INDEX、MASTER_ARCH、AI_GOV（依序）
...
### 1) 變更清單（Changelog）
- NORMALIZE：正文正常化（去重、去補丁、移除歷史台帳與標籤/助記內容）。
- 更新：版本日期／基線日期統一為 2026-01-08（Asia/Taipei）。
- 固定：檔尾稽核四件套為唯一非正文留痕。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：7effd1875babf22112af39f6da64a16c0338f598efe7213ab1e74a91e1991533
...
- 裁決序位依 DOCUMENT_INDEX、MASTER_ARCH、AI_GOV（依序）承接。
