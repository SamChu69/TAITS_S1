...
本文件為 TAITS 之 **AI 行為與決策治理最高母法（A+）**，用於：

1) 鎖定 AI / Agent / LLM / 自動化工具在 TAITS 的合法角色邊界（AI 非決策主體）。
2) 規範 AI 之禁止事項、必做輸出、證據鏈要求、可追溯留痕與可回放稽核要求。
3) 固定並強制遵循核心鐵律：策略 ≠ 下單；Agent ≠ 策略；AI ≠ 唯一真理；Regime 高於策略；Risk / Compliance 可否決一切。

適用解釋與裁決邊界：
- 本文件 **不裁決** ACTIVE 文件集合與治理位階：一律以 doc_key=DOCUMENT_INDEX（A+｜Authoritative Index）為準。
- L1–L11（含 L9 / L10 / L11）之層級定義與 Canonical Flow：一律以 doc_key=MASTER_CANON 為唯一正文來源；本文件僅規範 AI 不得跳層、不得越權、不得偷換語義與繞過 L10 人類裁決邊界。
...
### 1.2 L9／L10／L11 之層級邊界（不重複定義）
- **唯一正文來源**：L1–L11（含 L9／L10／L11）之定義以 doc_key=MASTER_CANON（Canonical Flow）為唯一正文來源。
- **本檔規範範圍**：本檔僅規範 AI/Agent/LLM 之行為與輸出治理：不得跳層、不得越權；不得將「報告／稽核」語義偷換為「交易授權或執行」語義；不得以任何方式繞過 L10（人類裁決與交易授權）或 L11（稽核回放）之邊界。
...
## 12. 文件更新與融合（（已移除））｜最大完備＋累積式更新（Cumulative Update）

> 本節規範治理文件之更新、融合、重排版與錯誤修正；僅得在明確啟用 （已移除）（或等效治理授權）且完成留痕之情境下執行。
> 未啟用 （已移除） 時，AI 不得自行修改治理文件正文。
...
### 1) Changelog
- 2026-01-08｜NORMALIZE：移除檔首重複 S1 摘要與重複「0. 文件定位」段落；移除 文件融合更新模式／標籤式補丁敘述；保留 L 層定義唯一來源為 MASTER_CANON 之承接條款；重算 hash。
- 2026-01-08｜NORMALIZE｜正文正常化：移除檔首重複表頭與流程/補丁語句；移除正文之 Legacy Snapshot/引用範本；移除 §11.3（Label 與裁決順序字串之法律定位）；去除對話式措辭；更新索引引用欄位說明；重算 hash。
- 2026-01-08｜NORMALIZE：移除正文中重複之 L9–L11 定義與細項（改以 MASTER_CANON 為唯一正文來源），保留本檔僅作「不得跳層/不得越權/不得偷換語義」之治理規範。

### 2) 指紋清單（Hash Manifest）
- BODY_SHA256: 221acad4ab8c4b55ebaa8e0175add778eff5ba9b5a8923c1480cb1c7b4397335  (Body=稽核區塊前全文；不含本稽核區塊）
- hash_scope: BODY_ONLY（不含本稽核區塊）
- hash_rule: UTF-8 / LF / include trailing newline

### 3) Scope
- scope: BODY_ONLY
- excluded: Audit Section（本段以下全部內容）

### 4) Audit Hand-off
- prepared_by: TAITS Core System Engineer（AI）
- reviewed_by: Product Owner / Architecture Arbiter（Human）
