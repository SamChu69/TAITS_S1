...
- L9／L10／L11 之定義以 doc_key=MASTER_CANON（Canonical Flow）為唯一正文來源；本檔僅規範 AI 不得跳層、不得越權、不得把報告/稽核語義偷換為交易授權或執行。
...
4) 在 FILE UPDATE MODE（經明確啟用之文件融合更新模式）下，將「最大完備＋累積式更新」納入 AI 行為治理，避免摘要化縮水與新舊混讀
...
> 本節規範治理文件之更新、融合、重排版與錯誤修正；僅得在明確啟用 FILE UPDATE MODE（或等效治理授權）且完成留痕之情境下執行。
> 未啟用 FILE UPDATE MODE 時，AI 不得自行修改治理文件正文。
...
- BODY_SHA256: 1a714981b9711e5d9c06f1fe275df3b9503195e43a3bc086845ca21caf5eaa40
