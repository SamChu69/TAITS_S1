...
- 裁決序位（由高到低）：1) DOCUMENT_INDEX 2) MASTER_ARCH 3) AI_GOV 4) RISK_COMPLIANCE 5) GOVERNANCE_GATE_SPEC 6) EXECUTION_CONTROL
...
### 1) 變更清單（Changelog）
- NORMALIZE：移除正文中的補丁式語句、流程/狀態描述、歷史摘錄與重複章節。
- 去重收斂：移除跨檔重複之層級相關敘述，層級定義一律承接 doc_key=MASTER_CANON（Canonical Flow）。
- 邊界對齊：移除非制度性之標記/助記/檔名身份等正文污染來源，保留本檔作為「執行與控制」制度規範之純正文。
- 版本更新：基線日期與版本日期更新為 2026-01-08（Asia/Taipei）。
- 稽核同步：更新 Hash Manifest 以維持可追溯性。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SCOPE：全文正文（不含本稽核區塊）
- BODY_SHA256：c409c0f52edfa3623ff1a8713da5542f1f6d9bb93efed60eddd7503bb194337c

### 3) 適用範圍（Scope）
- 僅處理 doc_key=EXECUTION_CONTROL 之文本正常化（去重、重排、移除非制度正文）。
- 不變更本檔既有之執行控制條款、檢核項目、事件碼、回放包結構與 Kill Switch 原則之實質語義（僅做文字與結構正常化，以避免混讀）。

### 4) Audit Hand-off（裁決承接）
- change_id：EC-NORMALIZE-260108-0001
- authority_basis：DOCUMENT_INDEX；MASTER_ARCH；AI_GOV
- prepared_by：TAITS 文件正常化程序（FINAL QA MODE｜NORMALIZE）
