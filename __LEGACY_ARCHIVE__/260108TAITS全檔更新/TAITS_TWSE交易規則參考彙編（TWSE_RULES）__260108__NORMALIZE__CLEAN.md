...
### 1) 變更清單（Changelog）
- 2026-01-08：NORMALIZE（正文乾淨／去重收斂／章節重排）。移除補丁式與重複文件頭、移除 Alias/Label/順位助記與 Layer 特別化敘述；修正 Mermaid 區塊格式；保留規則彙編最大完備內容並補齊「制度更新策略／演進規則」為正常章節。

### 2) 指紋清單（Hash Manifest）
- BODY_SHA256（正文區塊）：2d8930c55af3d570601ba79f8907188bb6fb901b0c9cd96d4e49d3a552c1f173

### 3) 範圍聲明（Scope）
- 本次變更僅針對：正文去重收斂、結構重排、移除補丁式/混讀文字、格式修正（含 Mermaid）、更新文件頭日期。
