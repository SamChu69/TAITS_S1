...
> 稽核對位：VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0015`）  
...
- 本文件 `doc_key` 既有標示為 `VERSION_AUDIT`；本 整併更新 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
...
# 整併更新（2025-12-28）｜0023–0025）｜Freeze v1.0
...
> 本段為 附錄 A（METADATA_FIX Ledger）之續增；不回填、不改寫既有表格內容。
