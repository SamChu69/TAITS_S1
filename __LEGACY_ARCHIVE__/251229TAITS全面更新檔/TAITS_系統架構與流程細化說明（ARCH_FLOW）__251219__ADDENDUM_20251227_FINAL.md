...
本 整併更新 僅處理下列事項，**不改寫**本文件的架構模組定義、資料流、事件流、分層責任與工程規格內容：
...
> 稽核對位：VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0018`）  
...
- 本文件 `doc_key` 既有標示為 `ARCH_FLOW`；本 整併更新 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
