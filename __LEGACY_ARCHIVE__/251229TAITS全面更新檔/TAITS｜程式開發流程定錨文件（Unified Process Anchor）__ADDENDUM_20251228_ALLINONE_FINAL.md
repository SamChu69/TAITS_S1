...
# 附錄｜｜三個 GPT 續編必備模板（可直接複製建立）

## 附錄｜1｜TAITS_GPT_WORKLOG（模板）
```markdown

# TAITS_GPT_WORKLOG（唯一進度事實來源）

## 附錄｜2｜TAITS_GPT_ECR_LEDGER（模板）
```markdown
...
## 附錄｜3｜TAITS_GPT_CONTEXT_PACK（模板）
```markdown
...
- output_artifact：輸出產物路徑或檔名（例如：新增段落標題 / 整併更新 檔）
...
> 規則：Only-Add / Append-only（新增與追加；不覆寫歷史條目）  
...
> 規則：Only-Add（只可新增新的「最新續編包」區塊；保留舊包作歷史）  
...
> 稽核對位：VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0024`）  
...
- 若本次變更為新增（Only-Add）且尚無對應條目：必須於同一批次變更中，先在 VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger 追加條目，再以新增條目 ID 作為 `audit_anchor`。
