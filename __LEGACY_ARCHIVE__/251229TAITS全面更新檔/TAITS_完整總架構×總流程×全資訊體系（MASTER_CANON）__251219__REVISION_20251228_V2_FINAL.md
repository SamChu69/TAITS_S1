# 【2025-12-28｜STRUCTURAL REWRITE NOTICE｜Human Sovereign Command】
...
> 補充性質：結構性重整（Structural Rewrite；以導讀/索引/定位/章節重排為主）  
> 適用文件：MASTER_CANON（doc_key：MASTER_CANON）  
> 生效狀態：GOVERNANCE_STATE = UNFREEZE v1.1（timeboxed）  
> 變更邊界：不得改寫 Canonical Flow（L1–L11）之語義；不得改寫覆蓋規則；不得削弱 Risk/Compliance 最高否決權  
> 稽核對位：VERSION_AUDIT｜Appendix A｜METADATA_FIX Ledger（本次條目：`VA-METADATA_FIX-20251228-0026`）

本版目的：解決「讀者在 MASTER_CANON 內搜尋 L9–L11 時，混淆主文 L9–L11 與整併 Z（L9a–L11 人機分工/全紀錄補充）」所造成之誤讀；並提供**可快速定位**的導航層（不影響任何裁決語義）。

---

## 0. 快速定位（Reader Map｜不要混用）

### 0.1 L9 / L10 / L11（主文：Canonical Flow 逐層定義）
- 位置：**第 2 章｜TAITS Canonical Flow（L1–L11）**  
- 性質：Canonical Flow 的逐層 I/O/禁止條款（Normative）

### 0.2 L9a / L10 Authority / L11 Audit & Maintenance（整併於主文）
- 位置：**整併 Z｜Canonical 整併補強：L9a–L11 人機分工與全紀錄原則補充**  
- 性質：Only-Add Semantic Lock（補強邊界；避免敘事層混入裁決/執行）

### 0.3 對位文件（下位治理責任）
- L9（Gate）：`GOVERNANCE_GATE_SPEC` / `DOCUMENT_INDEX`  
- L10（Human）：`UI_SPEC` / `EXECUTION_CONTROL`  
- L11（Execution）：`EXECUTION_CONTROL` / `DEPLOY_OPS` / `VERSION_AUDIT`

> 解讀硬規則：  
> - **主文 L9–L11** 定義「層級職責」；  
> - **整併 Z** 定義「人話敘事（L9a）不得取代裁決／按鈕權（L10）不得外包給 AI／稽核維修（L11）不得被策略或敘事層滲透」。  
> 兩者不可互換、不可混用。

---

## 1. 本版改動摘要（非裁決性）
- 僅新增：導讀/導航/定位（Reader Map），使 L9–L11 與整併 Z 的閱讀路徑清晰化。  
- 不改寫：任何既有正文條款、任何既有 Only-Add Addendum 條款。  

---

# 【原文正文（Verbatim）｜以下內容保持不動｜Index/Audit 以本段為主】
