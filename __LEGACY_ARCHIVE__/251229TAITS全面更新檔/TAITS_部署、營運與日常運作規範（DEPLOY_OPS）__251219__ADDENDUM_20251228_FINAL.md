...
> 平行對齊：DOCUMENT_INDEX｜Latest Mapping（machine-readable）整併更新 2025-12-28-D/F／VERSION_AUDIT｜METADATA_FIX Ledger／LOCAL_ENV｜Evidence Chain／UI_SPEC｜Trace 最小欄位  
...
> 稽核對位：VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0020`）  
...
- 本文件 `doc_key` 既有標示為 `DEPLOY_OPS`；本 整併更新 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
