...
# Addendum 2025-12-27-B｜Only-Add：B+/C+ 子級標籤定義 × 覆蓋規則對位 × 表格排序/裁決一致化（Freeze v1.0）

> 補充性質：Only-Add（只可新增，不可刪減、覆寫、偷換既有語義）  
> 適用文件：TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220.md（doc_key：DOCUMENT_INDEX）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位約束：AI_GOV（A+）＋MASTER_ARCH（A）＋MASTER_CANON（A）  
> 依據正文：§3（治理等級與裁決力）、§6（文件衝突裁決程序）、§7（引用合法性）、§9（Index Change Ledger）  
> 目的：針對本文件表格中已出現之「B+ / C+」標示提供唯一合法解釋口徑，確保：  
> 1) 不引入新的治理層級；2) 不改寫既有覆蓋規則（A+ > A > B > C）；3) 使 Gate / Audit / 新對話載入可一致判讀、可回放。

---

## 0. 適用範圍與硬邊界（Hard Boundary）

### 0.1 適用
本 Addendum 僅針對 DOCUMENT_INDEX 表格欄位中可能出現的治理等級標示：`B+`、`C+`，提供「子級標籤（sub-grade tag）」之定義與解讀規則。

### 0.2 不適用（禁止越權）
本 Addendum **不得**被用於：
- 建立新的治理等級層級（例如：S、B++、A-、D…）  
- 重排或弱化覆蓋規則（A+ > A > B > C）  
- 以「+」標籤宣稱可越權裁決或覆蓋上位文件  
- 將任何文件升格/降格為不同 bucket（例如：把 B 變成 A）而不走版本/稽核流程

---

## 1. 定義：B+/C+ 的法律定位（Tag ≠ New Grade）

### 1.1 統一裁決：B+/C+ 皆為「子級標籤」，其裁決 bucket 仍為 B/C
- `B+`：表示「B 類文件中的較高成熟度/較強規格性」之**標籤**  
  - **裁決 bucket = B**（仍受 A+ / A 覆蓋；仍覆蓋 C）  
- `C+`：表示「C 類文件中的較高成熟度/較強操作性」之**標籤**  
  - **裁決 bucket = C**（仍受 A+ / A / B 覆蓋）

> 統一原則：任何覆蓋/衝突裁決仍依正文 §3 的 **A+ > A > B > C**；  
> `+` 只影響「同 bucket 內的排序/閱讀提示」，不影響跨 bucket 的覆蓋關係。

---

## 2. 覆蓋規則對位（Override Rule Mapping）
...
### 2.2 同 bucket 內的「+」處理（僅作排序/提示）
在「同 bucket（B 或 C）」內，若兩份文件同時可適用且未被上位 bucket 明確覆蓋：
- `B+` 在閱讀/指引排序上可優先於 `B`  
- `C+` 在閱讀/指引排序上可優先於 `C`  
但裁決仍必須依正文 §6 的程序完成（不可跳步），且不得以此規則替代稽核留痕。

---

## 3. 表格排序與人機讀法（Index Presentation Rule）

為避免不同工具/人員以不同方式排序造成歧義，本文件對「顯示排序」提供固定規則（Only-Add）：

### 3.1 顯示排序（由高到低）
A+ → A → B+ → B → C+ → C

### 3.2 機器判讀（bucket 化）
任何自動化判讀（Gate/Parser/索引器）**不得**直接把 `B+` 當作新 bucket；必須先做 bucket 化：
...
## 4. 衝突裁決程序中的使用方式（Procedure Safe）

在正文 §6 的衝突裁決程序中，若遇到：
- `B+` vs `B`：不得直接宣告 `B+` 必勝；必須回到 §6 的逐步裁決（含引用合法性、同級衝突、保守停止等）。  
- `C+` vs `C`：同上。

**允許的唯一用途：**在完成 §6 裁決後，若仍需「選擇一份作為實作指引文件」且不涉及覆蓋權力，`+` 可作為「優先閱讀提示」。

---

## 5. 變更與稽核（Index Change Ledger 對位）

凡將某文件的等級標示在同 bucket 內做「B ↔ B+」或「C ↔ C+」之調整：
- 仍屬治理中繼資料之變更（Metadata Change）  
- 必須在 VERSION_AUDIT / Index Change Ledger 留痕（Append-only）  
- 不得在 Freeze v1.0 期間以覆寫既有條目方式處理

---

## 6. 對外固定用語（可貼用模板）

未來任何文件若需要描述自身等級為 `B+ / C+`，建議使用以下固定表述，以避免被誤讀為新層級：

- 「治理等級：B+（顯示標籤；bucket= B；覆蓋規則仍依 A+ > A > B > C）」  
- 「治理等級：C+（顯示標籤；bucket= C；覆蓋規則仍依 A+ > A > B > C）」

（Addendum 2025-12-27-B｜Only-Add｜Freeze v1.0 完）
