...
變更原則：（只可新增，不可刪減/覆寫/弱化人類主權/否決可視化/審計義務） 
核心鐵律：UI 是人與系統唯一交界面；風險揭露優先於績效；否決不可被模糊化

---
---

# 2025-12-27｜：UI Trace 引用最小欄位強制 × Index Gate 唯一裁決 × Snapshot 清單不裁決（Freeze v1.0）
...
> 平行對齊： 
> - TAITS_PROJECT_PROMPT｜ 2025-12-27（Index Gate First／ACTIVE 不固化／引用最小格式） 
> - README｜ 2025-12-27（清單/數量快照化；治理裁決回到 DOCUMENT_INDEX） 
> - VERSION_AUDIT｜ 2025-12-27（METADATA_FIX Ledger 批次補登） 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0011`） 
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0006`） 
...
- 本文件 `doc_key` 既有標示為 `UI_SPEC`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
