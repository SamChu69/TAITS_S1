...
變更原則：（允許補強導覽、補充流程入口、補充引用索引；禁止刪減既有治理宣告與邊界） 

---
---

# 2025-12-27｜：README「文件數量/清單」快照化 × Index Gate（DOCUMENT_INDEX）唯一裁決來源 × 新對話載入最小規格（Freeze v1.0）
...
> 平行對齊：TAITS_PROJECT_PROMPT｜ 2025-12-27（Index Gate 載入優先 × ACTIVE 不固化 × 引用最小格式） 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0010`） 
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0009`） 
...
- 本文件 `doc_key` 既有標示為 `README`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
