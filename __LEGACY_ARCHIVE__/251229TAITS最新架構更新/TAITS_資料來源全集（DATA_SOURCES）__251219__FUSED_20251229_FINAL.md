...
變更原則：（只可新增，不可刪減/覆寫/偷換資料語義；不得用第三方裁決制度） 
核心鐵律：官方資料優先；多層Fallback；Provenance 可追溯；Snapshot 可回放；資料品質可審計；缺證據不得裁決

---

# 0｜：METADATA_FIX（DATA_SOURCES × DATA_UNIVERSE 名詞對齊與引用合法性鎖定｜Freeze v1.0）
...
- 任何出現 `doc_key：DATA_UNIVERSE` 之舊標示，均屬「Metadata Discrepancy」的歷史殘留標記；其法律效力被本 0 否認
...
## 0.4 版本與稽核要求
本 0 屬於「METADATA_FIX（中繼資料修補）」型變更：
...
# 0.1｜：防誤用告示（DATA_UNIVERSE 別名鎖定）× 引用模板 × 摘錄安全標頭（Freeze v1.0）
...
> 口徑對齊：DOCUMENT_INDEX｜ 2025-12-27（裁決順序字串統一 × 歧義消解） 
> Gate 對齊：GOVERNANCE_GATE_SPEC｜ 2025-12-27（字串助記定位 × 雙軌裁決消歧） 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251227-0001`） 
> 目的：在不改寫既有主文/附錄之情況下，提供「引用端（人/工具/AI）」的硬規則與可貼用模板，避免因摘錄或複製附錄片段而誤用 `DATA_UNIVERSE` 作為 doc_key，導致 Gate 命中「引用非法」而 BLOCK。

---

## 0.1 本 的適用範圍（Hard Boundary）

本 只處理下列「引用/摘錄/拷貝」場景（不改寫資料治理主體內容）：

- 文件被切片摘錄到其他文件（例如 /Part 被單獨引用）
...
- 在 Evidence/Audit/UI Trace 中，把 `DATA_UNIVERSE` 當成 ACTIVE 文件引用（除非 DOCUMENT_INDEX 另行新增並裁決為 ACTIVE；Freeze v1.0 期間不得以此 變相新增）

---

## 0.3 摘錄安全標頭（Excerpt Safety Header｜可直接貼用）

> 用途：任何從本文件複製出去的片段（尤其是 Part/），必須在片段最前方加上本標頭，以防誤用。

請在摘錄片段最前方，加入下列標頭（不得改寫語義；可原封貼上）：

```text
〔引用安全標頭｜TAITS｜Freeze v1.0｜〕
...
ref_section = <例如：Part 3 §32 / B §B.7 / 主文 §2.3>
ref_excerpt_hash = <可選：若已對摘錄做 hash>
ref_notes = 若摘錄內含「doc_key：DATA_UNIVERSE」字樣，依 0/0.1 視為 alias，不具法律效力
```
...
- 不得以「模型推測」補完缺失引用；缺就標記缺，並返回上游補齊（ 的流程精神）

---

## 0.6 格式性修補宣告（Mermaid Fence Patch｜）

本文件若存在 Mermaid 區塊缺失結尾 fence（```），將造成後段章節被誤判為 code block，影響引用定位與稽核可讀性。

本 允許以 **** 方式補上缺失之結尾 fence：
- 不改寫 Mermaid 內容
- 不刪改後續章節文字
...
> 【文件閱讀與 Canonical 對位指引】
>
> 本文件主體內容定義 TAITS DATA_SOURCES（DATA_UNIVERSE）之資料來源分類、
> 來源優先序與基本資料治理規範。
> 自文件最末之「 A｜DATA_UNIVERSE 對位 MASTER_CANON 補充附錄」起，
...
- 確保資料層（L1–L2）輸入可被：
 ...
 - 稽核（Auditable）
 - 追責（Traceable）
- 支援 TAITS 以台灣市場（TWSE/TPEX/TAIFEX）為主軸的長期演進
...
> TAITS 將每個資料來源定義為一筆「可版本化」的 Source Record。
...
> 注意：此處是「架構骨架」。具體欄位細表可在後續 擴充成 `schemas/` 文件； 
> 但本骨架不可刪減，避免後續偷換語義。
...
上位約束：MASTER_ARCH / DOCUMENT_INDEX / MASTER_CANON / VERSION_AUDIT 
平行參照：FULL_ARCH / ARCH_FLOW / RISK_COMPLIANCE / TWSE_RULES / EXECUTION_CONTROL / UI_SPEC / DEPLOY_OPS / LOCAL_ENV 
變更原則：（只可新增，不可刪減/覆寫/偷換語義；不得用第三方裁決制度） 
...
> Flags 是「客觀檢測結果」，不得帶方向性語句。 可新增，不可刪除既有。
...
上位約束：MASTER_ARCH / DOCUMENT_INDEX / MASTER_CANON / VERSION_AUDIT 
平行參照：FULL_ARCH / ARCH_FLOW / RISK_COMPLIANCE / TWSE_RULES / EXECUTION_CONTROL / UI_SPEC / DEPLOY_OPS / LOCAL_ENV 
變更原則：（只可新增，不可刪減/覆寫/偷換語義；不得以第三方裁決制度） 
...
# A｜：DATA_UNIVERSE 對位 MASTER_CANON 之附錄（Freeze v1.0）
...
## A.5 Freeze v1.0 下的 新增規則模板（DATA_UNIVERSE 專用）
...
（ A｜DATA_UNIVERSE 對位 MASTER_CANON｜Freeze v1.0｜）

# B｜：DATA_UNIVERSE 工件模板 × 欄位字典 × Reason Codes（Freeze v1.0）

> 補充性質：（只可新增，不可刪減、覆寫、偷換既有語義） 
> 適用文件：TAITS_資料來源全集（DATA_UNIVERSE）__251219（doc_key：DATA_UNIVERSE） 
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0 
> 上位裁決：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON 
> 目的：補齊 DATA_UNIVERSE 在工程落地（L1/L2/L3）與稽核回放（VERSION_AUDIT / Replay）所需之「最小可檢查工件模板」、欄位字典與標準化 Reason Codes；不新增流程、不改權限、不改既有 Part 1–4 與 A 之語義。
...
> 規則： 可增加欄位、可加嚴檢核；不可刪減既有必填、不可放寬合規限制。
...
- `audit_checks_ref`：稽核檢核表引用（可引用 A 的 A.6）

### B.3.2 Subcategory Record（建議欄位｜）
...
## B.8 Quality Flags（QF_*）建議標準化字典（ 可增不可減）
...
## B.9 Reason Codes（RC_*）建議標準化字典（ 可增不可減）
...
# C｜：敏感資料（BROKER_API/帳務/個資）脫敏與最小揭露規範（Freeze v1.0）
...
- 依 ：不得回頭改寫既有段落；因此本 以重新附錄方式提供等價語義之可渲染版本。
- 任何引用 Mermaid 圖表時：以本 之附錄版本為準（僅限渲染修補，不變更流程語義）。

---

## 0.2B.2 ｜Mermaid 可渲染版本（對應原章節 12 / 15 / 17 / 20）

### M12｜資料來源治理流程（L1-L2）— Renderable
...
### M15｜Source Category × Fallback Hierarchy — Renderable
...
### M17｜Data Quality Gate — Renderable
...
### M20｜Snapshot × Replay Data Ref Graph — Renderable
...
# 2025-12-28｜：P0 全閉合（DATA_UNIVERSE 全量降格為 Alias × doc_key 封口 × 引用模板硬化｜Freeze v1.0）
...
> 目的：徹底封口本文件內「DATA_UNIVERSE 被誤用為 doc_key／引用鍵」之風險；維持既有正文與既有附錄原封不動，僅追加硬規則與工具鏈解析提示。
...
（ 2025-12-28｜｜DATA_SOURCES｜P0 全閉合 完）
---

## 2025-12-28｜：doc_key 別名（Legacy Alias）保留宣告（Freeze v1.0）
...
> 補充性質：（只可新增，不可刪減、覆寫、偷換既有語義） 
> 適用文件：TAITS_資料來源全集（DATA_SOURCES）__251219__ADDENDUM_0_2C_FINAL.md（doc_key：DATA_SOURCES） 
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0 
> 上位裁決序列：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON（AI 行為規範仍受 AI_GOV 最高約束；衝突裁決流程依 DOCUMENT_INDEX §6） 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0019`） 
