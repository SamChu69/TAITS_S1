...
變更原則：（本文件生效期間，所有受約束文件僅可新增，不可刪減／覆寫／偷換語義） 
核心裁決：FREEZE=ON（Freeze v1.0）｜任何衝突以本文件裁決為準 

---
---

# 2025-12-27｜：GOVERNANCE_STATE 內「文件清單/數量」快照化 × Index Gate（DOCUMENT_INDEX）唯一裁決 × 狀態檔引用最小格式（Freeze v1.0）
...
> 平行對齊：README｜ 2025-12-27／PROJECT_PROMPT｜ 2025-12-27／VERSION_AUDIT｜ 2025-12-27 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0016`） 
...
### S2.2 可直接貼用的引用標頭
```text
〔TAITS GOVERNANCE_STATE 引用標頭｜Freeze v1.0｜〕
...
Freeze v1.0 期間，任何文件變更（即便是 ）必須同時滿足：
...
# 【C】GOVERNANCE_STATE｜ 2025-12-28-E
> 補充性質：（只可新增，不可刪減／覆寫／偷換既有語義） 
...
# 2025-12-28｜：P0 全閉合（狀態檔引用模板作廢 × 新模板固定 × Alias 封口｜Freeze v1.0）
...
> 目的：閉合本文件內部「舊引用模板仍使用 GOVERNANCE_STATE」與「doc_key 唯一裁決為 GOVERNANCE_STATE_FREEZE_V1」之自相矛盾；以 方式宣告舊模板作廢（保留歷史），並提供唯一合法新模板。

---

## E3. 舊引用模板作廢宣告（已作廢｜保留但不得再用）

### E3.1 作廢對象（S2-OLD）
凡本文件內或衍生文件中，引用模板出現下列任一形式：
- `ref_doc_key = GOVERNANCE_STATE`
- `doc_key：GOVERNANCE_STATE`（用於引用欄位）
均屬 **S2-OLD（已作廢 Template）**。
...
（ 2025-12-28｜｜GOVERNANCE_STATE_FREEZE_V1｜P0 全閉合 完）
---

# 2025-12-28｜：S2 Template Hard Banner（防誤抄封口｜Freeze v1.0）
...
（ 2025-12-28｜｜GOVERNANCE_STATE_FREEZE_V1｜S2 Template Hard Banner 完）
---
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0002`） 
...
- 本文件 `doc_key` 既有標示為 `GOVERNANCE_STATE_FREEZE_V1`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
