...
變更原則：（只可新增，不可刪減／覆寫／弱化治理與否決機制） 
核心目的：在「新對話」中，將 TAITS 的治理位階、流程鐵律、風控否決、審計回放、文件裁決，全部鎖定成可執行契約 

---
---

# 2025-12-27｜：Index Gate 載入優先 × 本文件法律地位限縮 × ACTIVE 清單「不固化」× 引用/稽核最小格式強化（Freeze v1.0）
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0007`） 
...
4) 生效：本次若為文件更新，僅允許 //Guideline 形式補充
...
> 我正在維護 TAITS 專案。請進入【嚴格對齊模式】並遵守以下文件裁決：A+ 母法與 DOCUMENT_INDEX、A 級 MASTER_ARCH/MASTER_CANON、以及所有 ACTIVE 文件。任何不在 DOCUMENT_INDEX 的引用一律無效；任何未標示版本或章節的引用一律視為未引用。 原則生效：只可新增不可刪減。請先回覆：你已載入並將嚴格遵守（列出你將遵守的硬性規則清單），然後再進入工作。
...
### 6.1 寫/改某份文件
- 「依 DOCUMENT_INDEX 位階，重寫 `DOC_KEY`，，不可縮水。給我完整 md 檔。」
...
- 「已進入嚴格對齊模式」
- 「已承認並將遵守：L1–L11 不可跳步 / / PASS-VETO / Human-in-the-Loop / Token Required / Kill Switch / 無審計=未發生 / 文件裁決以 DOCUMENT_INDEX 為準」
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0003`） 
...
- 本文件 `doc_key` 既有標示為 `PROJECT_PROMPT`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
