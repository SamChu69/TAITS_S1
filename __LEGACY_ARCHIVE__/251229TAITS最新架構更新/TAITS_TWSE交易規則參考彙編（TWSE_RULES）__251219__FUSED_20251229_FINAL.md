...
變更原則：（只可新增，不可刪減/覆寫/偷換制度語義；制度裁決一律以官方最新為準） 
...
# 2025-12-27｜：DATA_UNIVERSE 別名映射 × 引用合法性鎖定 × Rulebook/Trigger Map 之 doc_key 對位（Freeze v1.0）

> 補充性質：（只可新增，不可刪減/覆寫/偷換制度語義） 
...
> 資料治理對位：DATA_SOURCES｜ 0 / 0.1（DATA_UNIVERSE 僅為概念別名，不具 doc_key 法律效力） 
...
本 僅處理「引用端」歧義，不改寫本文件對官方制度來源、Rulebook Snapshot、Trigger Map 的任何語義：

- 本文件仍可使用「DATA_UNIVERSE」作為概念名詞（Data Universe／資料宇宙）或閱讀別名（alias）
- 但本 將其 **明確否認為 doc_key**，並規定引用端的唯一合法 doc_key
...
ref_notes = 若片段內含「DATA_UNIVERSE」字樣，依 DATA_SOURCES 0/0.1 視為 alias，不具法律效力
audit_anchor = VERSION_AUDIT:VA-METADATA_FIX-20251227-0001
```

---

## 3. 檔頭「平行參照」之兼容解釋（，不改原文）

本文件檔頭「平行參照：… / DATA_UNIVERSE / …」屬歷史殘留的概念引用：
- 在 Freeze v1.0 下不覆寫原文
- 其法律效果由本 限定為「概念別名（alias）」用途
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0004`） 
...
- 本文件 `doc_key` 既有標示為 `TWSE_RULES`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
