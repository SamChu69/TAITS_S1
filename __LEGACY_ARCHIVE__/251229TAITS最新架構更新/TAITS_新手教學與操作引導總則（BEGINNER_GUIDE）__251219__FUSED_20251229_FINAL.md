...
# 2025-12-27｜：新手入口 Snapshot 不裁決 × Index Gate（DOCUMENT_INDEX）唯一裁決 × 新對話啟動最小引用格式（Freeze v1.0）
...
> 平行對齊：README｜ 2025-12-27／PROJECT_PROMPT｜ 2025-12-27／UI_SPEC｜ 2025-12-27／DEPLOY_OPS｜ 2025-12-27 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0013`） 
> 目的：在 Freeze v1.0 期間，修補新手入口最常見的誤用：把「文件數量/清單」當裁決依據。自本 起，BEGINNER_GUIDE 內所有文件清單/數量一律視為 Snapshot（導覽用途），ACTIVE/doc_key/治理等級一律以 DOCUMENT_INDEX 表格裁決為準；並提供新對話啟動的最小引用格式，使新手操作不會從入口繞過治理封口。
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0011`） 
...
- 本文件 `doc_key` 既有標示為 `BEGINNER_GUIDE`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
