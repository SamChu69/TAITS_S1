...
# 2025-12-27｜：營運/部署證據鏈最小欄位強制 × Evidence Chain 對齊 LOCAL_ENV × Index Gate 唯一裁決（Freeze v1.0）
...
> 平行對齊： 
> - LOCAL_ENV｜ 2025-12-27（Evidence Chain Template） 
> - UI_SPEC｜ 2025-12-27（UI Trace 引用最小欄位） 
> - PROJECT_PROMPT｜ 2025-12-27（Index Gate First／ACTIVE 不固化／引用最小格式） 
> - VERSION_AUDIT｜ 2025-12-27（METADATA_FIX Ledger 批次補登） 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0012`） 
...
### D2.2 建議固定引用標頭（可直接貼用）
```text
〔TAITS Deploy/Ops 引用標頭｜Freeze v1.0｜〕
...
# 2025-12-28｜：Trace ID / Evidence Chain 欄位貫穿規範（Ops-First Correlation Contract）× Freeze v1.0

> 補充性質：（只可新增，不可刪減／覆寫／偷換既有語義） 
> 適用文件：TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219.md（doc_key：DEPLOY_OPS） 
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0 
> 上位裁決：AI_GOV（A+）＋DOCUMENT_INDEX（A+｜Authoritative Index）＋MASTER_ARCH（A）＋MASTER_CANON（A） 
> 平行對齊：LOCAL_ENV｜ 2025-12-27（Evidence Chain Template）／UI_SPEC｜ 2025-12-27（UI Trace 最小引用欄位）／VERSION_AUDIT｜ 2025-12-28（Ledger 收斂） 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（建議新增條目：`VA-METADATA_FIX-20251227-0020`） 
...
## T｜Trace & Evidence Correlation Contract（Freeze v1.0）
...
taits_ops_header:
 ...
 ref_section: "<Authoritative Index / 映射表章節>"
...
# 2025-12-28-B｜：Repo 落盤規則（LATEST/ARCHIVE）× Artifact Lifecycle × 選檔風險控制（Freeze v1.0）

> 補充性質：（只可新增，不可刪減／覆寫／偷換既有語義） 
> 適用文件：TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219.md（doc_key：DEPLOY_OPS） 
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0 
> 上位裁決：AI_GOV（A+）＋DOCUMENT_INDEX（A+｜Authoritative Index）＋MASTER_ARCH（A）＋MASTER_CANON（A） 
> 平行對齊：DOCUMENT_INDEX｜Latest Mapping（machine-readable） 2025-12-28-D/F／VERSION_AUDIT｜METADATA_FIX Ledger／LOCAL_ENV｜Evidence Chain／UI_SPEC｜Trace 最小欄位 
...
當產出新 addendum（Freeze v1.0 下必為 ）時：
...
taits_repo_landing_manifest:
 ...
 latest_mapping_source: "DOCUMENT_INDEX machine-readable mapping ( 2025-12-28-D/F)"
 archive_policy:
...
> 補充性質：（只可新增，不可刪減、覆寫、偷換既有語義） 
> 適用文件：TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219__ADDENDUM_20251228_FINAL.md（doc_key：DEPLOY_OPS） 
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0 
> 上位裁決序列：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON（AI 行為規範仍受 AI_GOV 最高約束；衝突裁決流程依 DOCUMENT_INDEX §6） 
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0020`） 
...
- 本文件 `doc_key` 既有標示為 `DEPLOY_OPS`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
