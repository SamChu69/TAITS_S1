...
變更原則：（只可新增，不可刪減/覆寫/弱化：金鑰隔離、可回放稽核、最小權限、可回滾/可停機） 
...
# 2025-12-28｜：Evidence Chain Template（LOCAL_ENV）對齊 DEPLOY_OPS D3 × 回放必需最小結構（Freeze v1.0）
...
> 平行對齊：DEPLOY_OPS｜ 2025-12-27（D3：Evidence Chain 最小結構） 
...
> 稽核對位：VERSION_AUDIT｜ A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0012`） 
...
- 本文件 `doc_key` 既有標示為 `LOCAL_ENV`；本 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
