...
###（原 Appendix A｜RISK_COMPLIANCE × MASTER_CANON 對位補充（Only-Add））
...
###（原 Appendix B｜對位 MASTER_CANON 附錄 Z：人機分工邊界對 L7 的限制（Only-Add））
...
###（原 Appendix C｜L7 全紀錄原則：Risk Gate 工件鏈與 Replay 最小集（Only-Add））
...
###（原 Appendix D｜L7→L10→L11 交接契約（Handoff Contract）與 UI 呈現補強（Only-Add））
