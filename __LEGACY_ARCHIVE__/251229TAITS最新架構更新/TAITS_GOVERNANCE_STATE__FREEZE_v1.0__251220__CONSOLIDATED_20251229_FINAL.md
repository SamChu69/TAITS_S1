...
###（原 Addendum 2025-12-27｜Only-Add：GOVERNANCE_STATE 內「文件清單/數量」快照化 × Index Gate（DOCUMENT_INDEX）唯一裁決 × 狀態檔引用最小格式（Freeze v1.0））
...
###（原 Addendum 2025-12-28｜Only-Add：P0 全閉合（狀態檔引用模板作廢 × 新模板固定 × Alias 封口｜Freeze v1.0））
...
###（原 Addendum 2025-12-28｜Only-Add：S2 Template Hard Banner（防誤抄封口｜Freeze v1.0））
