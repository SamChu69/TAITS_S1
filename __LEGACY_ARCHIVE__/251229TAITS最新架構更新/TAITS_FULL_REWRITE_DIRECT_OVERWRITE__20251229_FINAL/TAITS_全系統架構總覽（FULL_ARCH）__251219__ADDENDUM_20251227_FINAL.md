...
本 整併更新 僅處理「引用端歧義」與「欄位語句限縮解讀」，不改寫 FULL_ARCH 的系統模組、分層、資料流、事件流、Canonical Flow 對位或任何架構規格內容。
...
> 稽核對位：VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0007`）  
...
- 本文件 `doc_key` 既有標示為 `FULL_ARCH`；本 整併更新 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
