...
- 本 整併更新 僅作歧義消解與口徑統一，不改寫 Canonical Flow（L1–L11）與既有正文任何條款。  
...
> 稽核對位：VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0008`）  
...
- 本文件 `doc_key` 既有標示為 `MASTER_CANON`；本 整併更新 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
...
- 本 整併更新 為 Only-Add；不改寫本文件任何既有條款。  
- 本 整併更新 之解讀與適用，均以 DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON 為最終裁決順序；AI 行為仍以 AI_GOV 為最高約束。  
- 本 整併更新 目的僅在於消解引用歧義、避免 Gate 因 metadata/格式缺口而誤判，並確保全鏈路可稽核、可回放。

（整併更新 2025-12-28｜Only-Add｜Freeze v1.0 完）

---

# 整併更新（2025-12-28）｜0001（占位行「...」非規範性宣告）｜Freeze v1.0

> 補充性質：Only-Add（只可新增，不可刪減、覆寫、偷換既有語義）  
> 適用文件：TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__251219__ADDENDUM_20251227_FINAL.md（doc_key：MASTER_CANON）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決序列：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON（AI 行為規範仍受 AI_GOV 最高約束；衝突裁決流程依 DOCUMENT_INDEX）  
> 稽核對位：VERSION_AUDIT｜附錄 A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0023`）  
> 目的：消解本文件「整併更新 2025-12-28｜GLOBAL_ALIGNMENT_PATCH」中出現之單行占位字元（`...`）可能造成的裁決/稽核歧義；不改寫任何既有正文條款。
...
自本 整併更新 生效起，對本文件之任何讀取、稽核、回放、或 Gate 解析：
...
- Freeze v1.0 期間：**不得**以「刪除該行」方式改寫既有正文；僅以本 整併更新 形式完成裁決封口。  
...
# 附錄：L9／L9a／L10／L11 權限邊界與敘事層解讀裁決（Only-Add）

> 補充性質：Only-Add（只可新增，不可刪改既有語義）  
> 適用文件：MASTER_CANON（TAITS_完整總架構×總流程×全資訊體系）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 補充目的：釐清 L9（治理閘門）／L9a（人話敘事參考）／L10（人類裁決權）／L11（執行與控制）之不可混用邊界，避免閱讀或實作時將敘事層誤當裁決層、將治理層誤當執行層。

## A. 裁決宣告（Interpretation Ruling）

本附錄為 **解讀裁決與使用指引**，其功能為：
1) **鎖定邊界**：強制界定 L9／L9a／L10／L11 的權限與輸出型態。  
2) **防止混用**：禁止以 L9a 敘事文本取代 Evidence（L4–L5）或覆寫 Risk/Compliance（L7）與 Human Decision（L10）。  
3) **不改寫母文**：本附錄 **不改寫、不重排** 既有 L1–L11 定義；若本附錄措辭與既有正文產生表面差異，應以 **DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON** 的裁決序列與 MASTER_CANON 既有正文語義為準。

## B. 名詞最小定義（僅為解讀一致性）

- **Evidence Bundle（證據包）**：由 L5 組裝之可審計證據集合，須具來源脈絡（provenance）與可回指性。  
- **Narrative（敘事）**：以人類可理解方式重述 Evidence 與狀態，不新增裁決權，不產生交易授權。  
- **PASS Token（放行憑證）**：由 L7 產生，僅在 PASS 時存在；L11 啟動前置條件之一。  
- **Flow Trace（流程軌跡）**：治理檢核所需之過程證據（含引用、版本、關聯鍵），供 L9 使用。

## C. L9／L9a／L10／L11 不可混用規範

### C1. L9｜Governance Gate（流程治理閘門）
- **允許做什麼**：檢查流程完整性、文件一致性與引用合法性；裁決輸出為 **PASS / RETURN / STOP**。  
- **不得做什麼**：  
  1) 不得以「敘事文本」替代流程軌跡（Flow Trace）或 Evidence Bundle。  
  2) 不得跳過 L10 直接觸發 L11。  
  3) 不得產生交易授權、不得改寫風控結果、不得生成或偽造 PASS Token。

### C2. L9a｜Narrative Reference（人話敘事參考層）
- **定位**：L9a 僅為 **NON_DECISION_REFERENCE（非裁決參考）**。  
- **允許做什麼**：  
  1) 將 Evidence Bundle（L5）與 Regime State（L6）整理為人類可讀敘事。  
  2) 對每一項關鍵敘述提供回指（pointer）至 Evidence Bundle 的對應項（例如：evidence_id / feature_id / snapshot_ref）。  
- **不得做什麼（強制禁止）**：  
  1) 不得產生或暗示 **任何裁決結論**（包含 PASS/VETO、APPROVE/REJECT）。  
  2) 不得輸出可被系統解析為下單參數之內容（例如：qty、price、order_type、venue、time-in-force 等）。  
  3) 不得覆寫或改寫 L7 的 reason codes、VETO 結論或 PASS Token。  
  4) 不得被視為 Evidence（證據）；其僅能 **引用** Evidence，不得「自成證據」。  
- **若違反**：視為流程污染，L9 應裁決 **STOP** 或 **RETURN**（依治理閘門規範），並於稽核鏈註記「Narrative Misuse」。

### C3. L10｜Human Decision Authority（人類裁決權）
- **定位**：L10 為人類對「是否執行」之最終裁決入口，輸出為 **APPROVE / REJECT**。  
- **最低可讀性要求**（不新增制度、僅做解讀鎖定）：  
  1) 人類裁決必須能看到：L7 的結論（PASS/VETO + reason codes）與 L9 的治理裁決（PASS/RETURN/STOP）。  
  2) 若 REJECT，理由不得被 UI 模糊化；須能回指至 Evidence 或 Gate 結果（以便稽核）。  
- **不得做什麼**：AI 或自動化不得取代 L10 的裁決；任何「預設同意」或「無人裁決自動執行」皆視為違規流程。

### C4. L11｜Execution & Control（執行與控制）
- **啟動前置條件（最小集合）**：  
  1) L10 = APPROVE  
  2) L7 = PASS 且存在有效 PASS Token  
  3) L9 = PASS（或等價放行狀態）  
- **允許做什麼**：依 EXECUTION_CONTROL 執行下單、監控、可中止（Kill Switch）與回報（ACK/Fills），並完成審計與可回放引用。  
- **不得做什麼**：  
  1) 無 PASS Token 下單。  
  2) Kill Switch 不可用仍執行。  
  3) 缺 Pre/In/Post 任一審計物仍執行。  

## D. 關聯治理文件對位（引用鎖定，不新增規則）

本附錄之執行/稽核落點應對位至下列 ACTIVE 文件（依 DOCUMENT_INDEX 為準）：
- L7：RISK_COMPLIANCE  
- L9：GOVERNANCE_GATE_SPEC  
- L10：UI_SPEC  
- L11：EXECUTION_CONTROL  
- 版本與稽核：VERSION_AUDIT  
- 上位鐵律：MASTER_ARCH、AI_GOV

## E. 稽核鏈最小欄位（供回放對位）

為確保 L9a／L10／L11 可被稽核回放，任一決策事件（含 RETURN/STOP/REJECT）至少應具備：
- trace_id（流程追蹤代碼）
- evidence_bundle_ref（證據包引用）
- snapshot_ref（快照引用）
- regime_ref（Regime 識別引用）
- risk_token_ref（僅 PASS 時；VETO 則標示 null 並保留 reason codes）
- governance_gate_ref（L9 裁決引用）
- human_decision_ref（L10 裁決引用）
- execution_ref（僅 L11 啟動時）
