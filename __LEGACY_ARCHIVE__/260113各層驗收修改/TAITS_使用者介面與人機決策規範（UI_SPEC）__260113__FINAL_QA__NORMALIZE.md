...
### 1) 變更清單（Changelog）
- 2026-01-13：L10_LAYER_QA_FIX（補強 UI 輸出欄位：新增 `human_approve_signature` 作為 `decision_signature` 之執行層別名；明確 Canonical 對齊與不一致處置（Preflight FAIL → BLOCK + AUDIT）；維持 UI 既有制度語義不變；重算 BODY_SHA256。）
- 2026-01-13：L7_LAYER_QA_FIX：補強 Risk/Compliance 的 PASS/VETO 與 Governance 的 PASS/RETURN/BLOCK 之語義分離提示與 UI 行為鎖定規則；重算正文指紋。
- 2026-01-13：L6_LAYER_QA_FIX：補強 UI 的 Regime Panel 必備呈現欄位（對齊 FULL_ARCH L6 `RegimeDetermined`：status／confidence／allowed/blocked／supporting evidence refs／reason codes），避免 Regime 狀態在 UI 端口徑漂移；同步重算正文指紋。
...
- 2026-01-11：Final QA 正文化覆蓋版。正文移除非制度性助記符號（含箭頭與治理字母標籤之法律定位寫法），更新文件頭基線與版本日期；檔尾僅保留稽核四件套並更新正文指紋。
