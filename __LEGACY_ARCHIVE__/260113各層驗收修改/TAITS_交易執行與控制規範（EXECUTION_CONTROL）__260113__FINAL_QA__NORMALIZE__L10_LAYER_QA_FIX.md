...
### 1) 變更清單（Changelog）
- 2026-01-13：L10_LAYER_QA_FIX（補強 Execution I/O Contract：新增 `decision_signature` 欄位別名之接收與正規化規則（UI/L10 ↔ Execution）；明確不一致處置（Preflight FAIL → BLOCK + AUDIT）；維持既有執行控制條款語義不變；重算 BODY_SHA256。）
...
- 2026-01-11：正文整理（去重收斂、結構重排、口徑一致化），並更新指紋清單。
...
- change_id：EC-FINALQA-260111-0001
