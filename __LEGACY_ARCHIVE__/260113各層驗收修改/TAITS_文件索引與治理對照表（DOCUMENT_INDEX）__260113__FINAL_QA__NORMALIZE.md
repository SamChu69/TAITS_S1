...
|---|---|---|---|---|---|
| TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__260113__FINAL_QA__NORMALIZE.md | DOCUMENT_INDEX | 最高母法 | ACTIVE | 2026-01-11 | 治理有效文件清單（Authoritative Index）與衝突裁決程序之唯一權威索引 |
| TAITS_AI_行為與決策治理最終規則全集（AI_GOV）__260113__FINAL_QA__NORMALIZE.md | AI_GOV | 最高母法 | ACTIVE | 2026-01-11 | AI 行為邊界、越權禁止、裁決序位與可稽核輸出規範 |

### 5.2 母體／憲法級（主權鐵律、Canonical、治理狀態、最高否決權與執行控制）
| 文件名稱（檔名） | doc_key | 治理位階 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|
| TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__260113__FINAL_QA__NORMALIZE.md | MASTER_ARCH | 母體／憲法級 | ACTIVE | 2026-01-11 | 母體總憲法、核心鐵律、權責邊界與上位裁決原則 |
| TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__260113__FINAL_QA__NORMALIZE.md | MASTER_CANON | 母體／憲法級 | ACTIVE | 2026-01-11 | Canonical Flow（L1–L11）之唯一正文來源與全資訊體系定錨 |
...
| TAITS_全系統架構總覽（FULL_ARCH）__260113__FINAL_QA__NORMALIZE.md | FULL_ARCH | 治理／制度級 | ACTIVE | 2026-01-11 | 全系統模組總覽與交互關係（不重複定義 L1–L11 語義） |
| TAITS_系統架構與流程細化說明（ARCH_FLOW）__260113__FINAL_QA__NORMALIZE.md | ARCH_FLOW | 治理／制度級 | ACTIVE | 2026-01-11 | 架構與流程細化（工程與資料事件流落地細節） |
| TAITS_資料來源全集（DATA_SOURCES）__260113__FINAL_QA__NORMALIZE.md | DATA_SOURCES | 治理／制度級 | ACTIVE | 2026-01-11 | 資料來源全集、合法性、取得方式、欄位契約與品質治理 |
...
| TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__260113__FINAL_QA__NORMALIZE.md | STRATEGY_UNIVERSE | 治理／制度級 | ACTIVE | 2026-01-11 | 策略宇宙：策略分類、輸入輸出契約、啟用與禁用條件（策略不等於下單） |
| TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__260113__FINAL_QA__NORMALIZE.md | STRATEGY_FEATURE_INDEX | 治理／制度級 | ACTIVE | 2026-01-11 | 策略特徵與因子索引：定義、欄位、計算依賴與可追溯引用 |
| TAITS_使用者介面與人機決策規範（UI_SPEC）__260113__FINAL_QA__NORMALIZE.md | UI_SPEC | 治理／制度級 | ACTIVE | 2026-01-11 | UI 與人機裁決：L9 呈現、L10 人類裁決流程、L11 回放呈現與權限邊界 |
...
| TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__260113__FINAL_QA__NORMALIZE.md | VERSION_AUDIT | 治理／制度級 | ACTIVE | 2026-01-11 | 版本控管、封版、變更留痕、稽核資料結構、Hash Manifest 與交付格式 |
...
- BODY_SHA256：af86b4f2e6afa8bcf9a86bfddf31f8ba8cd4cbf008ad97ab5aabfc80386ae9f5
### 3) 適用範圍（Scope）
- scope_doc_key：DOCUMENT_INDEX
- scope_change_type：FINAL_QA_NORMALIZE＋LAYER_QA_FIX＋CONSISTENCY_QA_FIX（索引一致性收斂；HFI 消歧義 Only-Add；不改寫制度語義）
- scope_invariants：
