...
- 流程不可跳步（Canonical Flow 不可跳步；層級定義以 MASTER_CANON 為唯一正文來源）
...
- 分布於 Canonical Flow 各層（以檢核點形式存在）  
...
- `l9_integrity_check_ref`：L9 報告內嵌之 Governance Integrity Self-Check 結果引用（可選；僅供 Gate 加速定位缺口與候選補齊清單；不得視為 Gate 裁決，且不得取代 Gate 生成之 `return_required_items[]`）
- `version_ref`：doc/policy/model/rule 定版參照（不可缺）
...
- 可承接 `l9_integrity_check_ref` 作為提示輸入，但 Gate 必須獨立重算並產生最終 `return_required_items[]`；不得直接轉交而不驗證

### 3.1 Gate 負責
- 流程完整性裁決（No Skip）
- 文件引用合法性裁決（doc_key / ACTIVE / 唯一性）
- 證據完整性裁決（Evidence 可回放、可追溯）
- 版本一致性裁決（version_ref / policy_version / rule 定版參照）
...
- 治理位階覆蓋與衝突裁決程序，以 DOCUMENT_INDEX（Authoritative Index）為準。
...
2) **治理位階覆蓋**：套用 DOCUMENT_INDEX 定義之位階覆蓋規則（自最高母法至下位操作級，採自高至低之覆蓋裁決）。  
...
### 6.1 必備檢核：流程完整性（FLOW Integrity）
- 檢核：上游審計物齊全（`layer_artifacts_map_ref` 完整且可追溯）。
- 檢核：證據引用與 Regime 引用存在且合法（`evidence_bundle_ref`、`regime_state_ref` 可回放、可追溯）。
- 檢核：若曾 RETURN，必須看見補齊項目已完成之證據（`return_required_items` 對位）。
- 檢核：不同模式不得降低 Gate 覆蓋與審計密度（No Downgrade）。

裁決：
- 若流程或審計物缺口可補：RETURN（並列明 `return_required_items`）。
- 若缺口不可補、互相矛盾，或屬跳步／繞過：BLOCK（並輸出對應 reason_code）。

### 6.2 必備檢核：文件引用合法性（DOCUMENT_INDEX Compliance）
- 檢核：所有引用 `doc_key` 必須存在於 DOCUMENT_INDEX。
- 檢核：引用文件狀態必須為 ACTIVE。
- 檢核：`doc_key` 唯一性與版本引用一致性（不得含糊、不得多版混用）。
- 檢核：上位／平行參照不得互相矛盾（硬衝突必阻擋）。

裁決：
- 命中引用非法（對應 GOV-DOC-01／GOV-DOC-02／GOV-DOC-03）：BLOCK。
- 命中硬衝突（對應 GOV-DOC-04）：硬衝突 BLOCK；若為缺口且可補則 RETURN。

### 6.3 必備檢核：審計可回放（AUDIT／REPLAY）
- 檢核：`correlation_id`／`session_id` 完整。
- 檢核：hash 鏈（`input_hash`／`output_hash`）可驗證（不得事後補寫）。
- 檢核：回放包（Replay Bundle）可組裝，且指向不可變儲存。
- 檢核：審計物可讀、可取回、可回放。

裁決：
- `correlation_id`／`session_id` 缺口且可補：RETURN。
- 其餘審計／回放缺口（對應 GOV-AUD-*）：BLOCK（不得在裁決後補寫）。

### 6.4 必備檢核：版本一致性（VERSION Alignment）
- 檢核：所有輸出皆帶 `version_ref`（doc／policy／model／rule 定版參照）。
- 檢核：`policy_version` 可在 VERSION_AUDIT 回溯。
- 檢核：制度／規則定版參照可在 TWSE_RULES（及相關制度文件）或受治理認可之定版引用中找到。
- 檢核：同一 `correlation_id` 不可混用互斥版本參照。

裁決：
- `version_ref` 缺口且可補：RETURN。
- 版本不可追溯、定版缺失或互斥矛盾（對應 GOV-VER-*）：BLOCK。

### 6.5 必備檢核：UI 呈現義務（UI Must-Show）
- 檢核：Risk Gate 結果（PASS／VETO）與 reason_codes 必須可視化（不得遮蔽）。
- 檢核：`version_ref`／`correlation_id` 必須可視化。
- 檢核：RETURN 時補齊清單必須可視化且阻斷前進。
- 檢核：不得以績效圖表弱化風險揭露（風險揭露優先）。

裁決：
- 命中任一 UI 違規（對應 GOV-UI-*）：BLOCK（屬治理違規）。

### 6.6 必備檢核：安全與旁路防護（Bypass Prevention）
- 檢核：不得存在繞過 Risk 或 Gate 的旁路通道（暗通道）。
- 檢核：敏感資訊不得被寫入不允許位置（依 LOCAL_ENV）。
- 檢核：不得以效能或便利理由移除審計、回放、對帳鏈路。

裁決：
- 命中安全或旁路疑慮（對應 GOV-SAFE-*）：BLOCK。
...
### 7.1 FLOW（流程完整性）
- GOV-FLOW-01：跳步或未執行必要步驟（違反 MASTER_CANON 所定 Canonical Flow）
...
### 7.4 VER（版本一致性）
- GOV-VER-01：version_ref 缺漏（doc/policy/model/rule 定版參照）
...
- version_ref（doc/policy/model/rule 定版參照）
...
```mermaid
flowchart TB
  PROPOSE[Strategy Proposal<br/>(Non-binding)] --> REPORT[Investment Report Output<br/>(Report Artifact)]
  REPORT --> GATE[Governance Gate<br/>(non-layer governance check)]

  GATE -->|PASS| HUMAN[Human Decision & Trade Authorization<br/>(Only authorization entry)]
  GATE -->|RETURN| RETURN_NODE[Return to Required Step(s)<br/>+ Required Items List]
  GATE -->|BLOCK| STOP_BLOCK[STOP<br/>Preserve Evidence + Audit Artifact]

  RETURN_NODE --> FIX[Evidence/Regime/Artifacts Completion]
  FIX --> REPORT

  HUMAN -->|NO_ACTION| STOP_AUDIT[STOP + Audit]
  HUMAN -->|AUTHORIZED| EXEC[Execution Control<br/>(Module, not a layer)]
  EXEC --> LOG[Canonical Layer Audit/Replay Record<br/>(Non-authorization layer)]
```
...
### 1) 變更清單（Changelog）
- 2026-01-13：L9_LAYER_QA_FIX（新增 Gate 可選承接 `l9_integrity_check_ref` 之規範，並明確其不取代 Gate 裁決與 `return_required_items[]`；重算 BODY_SHA256。）
...
- 2026-01-11：FINAL_QA_NORMALIZE（基線日期與版本日期對齊；移除順位符號與標籤化位階字樣；收斂 Gate 規則表述並避免 Canonical Flow 被重複定義；修正 Mermaid 節點命名避免單字母標籤；稽核區塊固定為四件套；重算 BODY_SHA256。）

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：87c5e72f7f2295d811540c383343979bca99ba3cb8cb113580e3f61a3888020c
### 3) 適用範圍（Scope）
本次覆蓋版修正範圍僅限於 doc_key=GOVERNANCE_GATE_SPEC（正文整編與驗收）：
- 基線日期／版本日期對齊（2026-01-11）
- 正文乾淨化：移除順位符號與標籤化位階字樣；將對 Canonical Flow 的引用改為承接 MASTER_CANON 之單一口徑
- Gate 規則收斂：以 reason_code（GOV-*）域為主軸，保持原有裁決強度（PASS／BLOCK／RETURN）不被弱化
- 稽核區塊固定化：僅保留 Changelog／Hash Manifest／Scope／Audit Hand-off

### 4) 裁決承接（Audit Hand-off）
- 本檔為 doc_key=GOVERNANCE_GATE_SPEC 之現行可引用正文；引用時必須附帶 `ref_version_date` 與 `ref_section`（依本檔 EC1 最小引用標頭）。
- 上位裁決序位固定：DOCUMENT_INDEX、MASTER_ARCH、AI_GOV；本檔不得自設覆蓋規則或牴觸上位裁決序位。
- Canonical Flow（層級語義與邊界）之唯一正文來源為 doc_key=MASTER_CANON；本檔僅定義 Gate 制度，不對層級語義作再定義或特別化。
