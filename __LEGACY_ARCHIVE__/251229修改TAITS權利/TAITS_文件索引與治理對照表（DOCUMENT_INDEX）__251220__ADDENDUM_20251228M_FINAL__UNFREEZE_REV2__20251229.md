...
# Addendum 2025-12-29｜Only-Add：UNFREEZE v1.1（REV2）納管登錄｜人類主權置頂裁決

補充性質：Only-Add  
適用文件：DOCUMENT_INDEX（doc_key = DOCUMENT_INDEX）  
生效狀態：UNFREEZE v1.1（Timebox 內）  
授權來源：Human Sovereignty（人類最高決策者）  
時區：Asia/Taipei  

---

## 0. 置頂裁決（Human Sovereignty Override）

- 人類最高決策者之明確命令為最終裁決來源；任何文件內容不得否決或阻止該命令。  
- 無明確人類命令時，系統裁決序位依：DOCUMENT_INDEX → MASTER_ARCH → AI_GOV。  
...
- 檔名：TAITS_GOVERNANCE_STATE__UNFREEZE_v1.1__20251229_REV2_FINAL.md
- doc_key：GOVERNANCE_STATE_UNFREEZE_V1X
- 狀態：ACTIVE（於其 Timebox 內生效）

---

## 2. 裁決（Decision）

在 UNFREEZE v1.1（REV2）Timebox 內：

- 允許對「全部 ACTIVE 文件（以本 DOCUMENT_INDEX 為準）」進行結構性重編（去附錄化、正文整併重編、移除被最新內容取代之舊段落、章節重排與標題整併）。
- 上述重編行為**僅能在明確人類命令下啟動**；AI/Agent 不得自行啟動或擴張範圍。
- 仍要求可稽核：每份文件必須建立 per-file rewrite record 並於 VERSION_AUDIT 留痕。

Timebox 結束後：

- 必須依 TAITS_GOVERNANCE_STATE__UNFREEZE_v1.1__20251229_REV2_FINAL.md 之 Re-Freeze 條件完成再封版裁決與留痕；否則不得再進行結構性重編。
