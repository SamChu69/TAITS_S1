...
## VA-UNFREEZE-20251229-0002
- event_type：GOV_STATE_SWITCH
- from_state：FREEZE_v1.0
- to_state：UNFREEZE v1.1（REV2）
- effective_timebox：2025-12-29 00:00 ~ 2025-12-31 23:59（Asia/Taipei）
- scope：ALL_ACTIVE（以 DOCUMENT_INDEX 為準）
- authorization：Human Sovereignty（人類最高決策者）
- purpose：全量去附錄化＋正文整併重編（該刪就刪、該改就改）— 人類主權置頂（文件不得否決）
- governance_reference：
  - unfreeze_file：TAITS_GOVERNANCE_STATE__UNFREEZE_v1.1__20251229_REV2_FINAL.md
  - index_gate：DOCUMENT_INDEX
- required_outputs：
  - batch_id
  - audit_anchor（本批次起點）
  - per-file rewrite record（刪除/取代/整併對照）
  - updated DOCUMENT_INDEX pointers（ACTIVE/latest_artifact 對齊）
  - explicit human instruction record（指令摘要＋範圍）
