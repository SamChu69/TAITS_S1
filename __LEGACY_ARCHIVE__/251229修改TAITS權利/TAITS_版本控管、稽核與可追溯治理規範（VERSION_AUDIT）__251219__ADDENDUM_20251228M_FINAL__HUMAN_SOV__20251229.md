...
# Appendix A｜Ledger（Only-Add）

## VA-HUMAN_SOV-20251229-0001
- event_type：GOV_STATE_SUPREME_CLAUSE_ADD
- affected_doc_key：GOVERNANCE_STATE_FREEZE_V1
- affected_file：TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251220.md
- change_type：ONLY_ADD
- effective_state：FREEZE_v1.0（ACTIVE）
- authorization：Human Sovereignty（人類最高決策者）
- purpose：在 Freeze v1.0 下置頂宣告「人類最高決策權」；文件不得阻止明確命令；系統僅能要求留痕與可回放。
- cross_refs：
  - document_index_notice：Addendum 2025-12-29（HUMAN_SOV）
  - freeze_addendum：Addendum 2025-12-29｜Human Sovereignty Supreme Clause
- audit_requirements：
  - future_overwrite_or_rewrite_actions MUST record explicit instruction summary + scope + per-file rewrite record
  - preserve replayability (evidence chain)
