...
輸入：無
輸出：Raw Data（DataIngested）
禁止：
- 以非官方裁決制度
- 以單一來源作唯一依據（必須設計 fallback）
#### 層定義與邊界（What / Why / Not）
**What（做什麼）**
- 從「官方/授權」資料源取得原始資料（Raw Payload），並以**可追溯引用鍵**封裝為 L1 工件（DataIngested）。
- 對每次取得行為生成最小不可變留痕：來源、時間窗、取得時間、雜湊、來源證明（Provenance）與品質報告（Quality Report）。
**Why（為什麼）**
- 讓後續層（L2–L11）永遠能以「可回放的引用鍵」重建同一份輸入，避免黑箱資料與不可追溯推論。
**Not（不做什麼；硬邊界）**
- 不做清洗、正規化、欄位映射、缺值補齊、插值/推估（這些屬 L2 的責任）。
- 不做體制判定（Regime）、風險裁決、交易裁決或下單（分別屬 L6/L7/L10/L11 的職責）。
- 不得以第三方/新聞等非官方資料作為制度裁決依據；若因缺漏必須使用 FALLBACK，必須明確降級標記並禁止作裁決用途（見下述「風險/合規介面」）。
#### Inputs / Outputs（資料欄位層級）
**Inputs**
- 無（僅允許由排程/事件觸發「取數作業」；觸發器屬執行控制，不構成本層資料輸入）。
**Outputs：DataIngested（Raw Data Artifact）**
- `source_id`
- `subcategory_id`
- `fetch_window`（start/end；不適用仍需以 null 明示）
- `captured_at`（取得完成時間戳；不得以資料內時間戳替代）
- `raw_payload_ref`
- `provenance_ref`
- `hash_manifest_ref`
- `quality_report_ref`
...
- `licensing_notes_ref`（條件必填：非 PRIMARY 必填）
- `trust_level`（NORMAL/LOW；若 source_tier_used=FALLBACK 必須為 LOW）
- `compliance_eligible`（true/false；若 source_tier_used=FALLBACK 必須為 false）
- `failover_event_ref`（可選：若觸發 fallback 建議提供）
**Failure Outputs：SourceFail（事件/工件）**
- `reason_code` / `source_id` / `subcategory_id` / `attempted_window` / `error_ref` / `captured_at`
#### 依賴與上/下游契約（Contract）
**上游（Upstream）**
- 無資料上游；但必須依 doc_key=DATA_SOURCES 之 Source Registry/授權條款與子類別定義進行取數。
**下游（Downstream：L2）**
- L2 僅能以 `raw_payload_ref` 讀取原始資料，且必須同時持有 `provenance_ref`、`hash_manifest_ref`、`active_version_map_ref`。
- 任一引用鍵缺漏或雜湊驗證失敗：不得進入 L2；必須輸出 SourceFail 並留痕（不可靜默重試到成功才記錄）。
#### 風險/合規介面（L1 必備最小要求）
- 若 `source_tier_used=FALLBACK`：必須強制 `trust_level=LOW` 且 `compliance_eligible=false`，並提供 `licensing_notes_ref`（不得省略）。
- L1 不得做任何「合規 PASS/VETO」裁決；僅能提供「是否具備裁決資格」的最小旗標（`compliance_eligible`）供後續層使用。
#### 缺口與風險點（本層專屬）
- 若 DataIngested 工件未帶引用鍵（provenance/hash/version map），將造成後續層「不可回放」與「不可追溯」；此屬重大缺陷，必須在 L1 即阻斷。
- 若未明確標記 fallback 降級，將導致下游誤用非官方資料進行制度裁決；此屬合規風險，必須在 L1 即強制降級標記與留痕。
---
...
#### 層定義與邊界（What / Why / Not）
**What**
- 將 L1 之 Raw Data（DataIngested）進行「可稽核」的清洗與標準化（Normalization），輸出 Canonical Data（DataNormalized），使資料可比較、可回放、可跨來源對齊。
**Why**
- 避免後續層在不同來源/欄位命名/格式下產生不可重現的推論；L2 必須把「欄位語義」固定為可審計的 canonical schema。
- 將任何補值/插值/推估（若允許）顯性化、可追溯化，避免隱性補值造成證據鏈污染。
**Not**
- 不得偷換欄位語義（例：把成交量當成交額）。
- 不得靜默修補；任何補值必須留痕並可回放。
- 不得以模型猜測取代缺值（除非明確標記並可追溯）。
#### Inputs / Outputs（欄位級契約）
**輸入：Raw Data（DataIngested）**
- 必須可回指：`raw_payload_ref`、`provenance_ref`、`hash_manifest_ref`、`quality_report_ref`、`active_version_map_ref`。
- 必須可識別：`source_id`、`subcategory_id`、`fetch_window`、`captured_at`。
**輸出：Canonical Data（DataNormalized）**
必備欄位（欄位名固定；不得自造 alias）：
- `canonical_data_ref`（正規化後資料本體引用；可回放）
- `value_origin_map_ref`（值來源標記：OBSERVED/IMPUTED/DERIVED；即使無補值亦可為 minimal map）
- `imputation_ref`（若有）— 補值/插值/推估方法、範圍、理由、版本（不可省略）
- `canonical_schema_id`
- `subcategory_id`
- `field_map_ref`（原欄位對應標準欄位）
- `normalization_ruleset_version`
- `data_quality_flags[]`
- `quality_flags[]`（legacy alias；若出現在事件/日誌/舊介面，必須在同一工件內提供對應至 `data_quality_flags[]` 的映射，避免新舊混讀）
**失敗狀態**
- 失敗狀態：`QAFail`（停止；不得進入 L3）。失敗時必須產生可追溯之 error_ref 與 reason_code（工程落地依 ARCH_FLOW 與 Gate 規範）。
### L3｜Snapshot & State（狀態快照）
目的：建立「當下市場狀態」快照（Snapshot）與狀態快取（State Cache），**必須可回放（Replayable）**。
### 1) 層邊界（What / Not）
- What：將 L2 `DataNormalized`（Canonical Data）在「單一時間點」固化為可載入的 Snapshot，並建立與 Snapshot 綁定的 State Cache（供後續層一致讀取）。
- Not：
 - 不得在本層生成交易方向、建議、訊號（屬 L8–L10）
 - 不得在本層裁決 Regime（屬 L6）或做風控合規裁決（屬 L7）
### 2) Inputs / Outputs（欄位級契約）
輸入：Canonical Data（`DataNormalized`；由 L2 產出，必須可回放）
- 必備引用鍵（最小集合；承接 L2 `DataNormalized`）：
 - `canonical_data_ref`
 - `canonical_schema_id`
 - `subcategory_id`
 - `field_map_ref`
 - `normalization_ruleset_version`
 - `data_quality_flags[]`
 - `value_origin_map_ref`
 - `imputation_ref`（條件必備：若存在補值/插值/推估）
輸出：Snapshot + State Cache
A. `SnapshotCreated`（L3）
**輸入**：Canonical Data
**輸出**：Snapshot（可回放）
必備欄位：
- `snapshot_id`
...
- `version_ref`（版本定版參照；需與 active_version_map_ref 可互相驗證）
B. `StateCacheCreated`（L3）
**輸入**：Snapshot
**輸出**：State Cache（與 Snapshot 綁定之快取視圖；可回放）
必備欄位：
- `state_cache_id`
- `snapshot_id`
- `state_cache_store_ref`（必須落盤，不得只在記憶體）
- `state_cache_schema_id`
- `version_ref`（需與 Snapshot 同版）
### 3) 上下游契約（Contract）
- 進入條件：S02（L2）輸出狀態 `CanonicalReady`。
- 成功輸出：S03（L3）輸出狀態 `SnapshotReady`。
- 失敗：`SnapshotFail` → STOP（不得進入 L4）。
### 4) 禁止事項（Hard No）
- 只存在記憶體（Snapshot/State Cache 皆不得）
- 無 `version_ref`
- Snapshot 與 version map 不一致
---
## L4｜Analysis（分析層）
目的：將 Snapshot/State 轉為可解釋特徵（Indicators / Factors / Structures）。**只描述，不產出方向**。
### 1) 層邊界（What / Not）
- What：
 - 以 L3 產出的 Snapshot + State Cache 作為唯一輸入基準，計算可解釋之 Feature Set（指標/因子/統計特徵）與（若有）Structure Descriptors（結構描述：型態/纏論容器）。
 - 產出必須可回放：Feature Set 與 Structure Descriptors 皆須可被重建/重算或具備可載入引用。
- Not（硬邊界）：
 - 不得產生交易方向（不得輸出 BUY/SELL/target position 等方向性結論）
 - 不得直接輸出下單參數（例如：價格、數量、下單類型、停損停利、倉位目標等）
 - 不得繞過證據組裝（不得直接把特徵輸出送入 L6/L7；必須先經 L5 Evidence）
### 2) Inputs / Outputs（欄位級契約）
輸入：Snapshot/State（由 L3 產出；必須可回放）
- 必備引用鍵（最小集合；承接 L3 `SnapshotCreated` + `StateCacheCreated`）：
 - `snapshot_id`
 - `market_time`
 - `trading_session_state`
 - `snapshot_store_ref`
 - `version_ref`（必須與 active_version_map_ref 可互驗；缺漏視為不可稽核）
 - `state_cache_id`
 - `state_cache_store_ref`
輸出：Feature Set + Structure Descriptors（L4 產出；供 L5 Evidence 組裝）
- A. `FeaturesComputed`（L4）
 **輸入**：Snapshot/State
 **輸出**：Feature Set + Structure Descriptors
 必備欄位（最小集合）：
 - `feature_set_id`
 - `feature_schema_version`
 - `indicator_set_ref`（指標/因子集合引用；可回放或可重算）
 - `structure_descriptor_ref`（若有結構/型態/纏論結果，必須以「結構描述」形式輸出，不得以交易語義輸出）
 - `feature_quality_flags[]`（缺值/異常/降級/不可用等標記）
 E2E Trace（最小集合；供全鏈路稽核串接）：
 - `feature_set_ref`（可與 `indicator_set_ref` 等價或指向同一存放集合之 alias；工程實作需固定一致口徑）
 - `structure_descriptor_ref`
### 3) 上下游契約（Contract）
- 進入條件：S03（L3）輸出狀態 `SnapshotReady`（且 Snapshot/State Cache 均已落盤）。
- 成功輸出：S04（L4）輸出狀態 `FeatureReady`。
- 失敗處置：
 - `FeatureFail` → RETURN（回到 L3 或 L2 補齊資料/修復缺口；不得以靜默補值或不留痕方式繼續）
 - 若檢出越權輸出（方向性或下單參數）→ BLOCK（不得往下游傳遞；必須留痕並交由治理/Gate 處置）
### 4) 禁止事項（Hard No）
- 方向性輸出（BUY/SELL/target position 等）
- 下單參數輸出（價格/數量/倉位/委託類型/停損停利等）
- 以特徵輸出直接跳層進入 L6/L7（必須經 L5 Evidence）
- 未提供 `feature_quality_flags[]` 或缺 `version_ref` 仍標記為成功輸出
---
### L5｜Evidence（證據層）
目的：將 L4 產出的 Feature Set／Structure Descriptors「組裝成可稽核的 Evidence Bundle（證據包）」；使後續 L6（Regime）與 L7（Risk/Compliance）能以同一份可回放、可追溯的證據集合進行判讀／否決／限制。
硬邊界（Hard No）：
- 不裁決（不輸出 Regime 狀態／風險通過／策略方向／下單參數）
- 不允許以「摘要文字」取代 evidence_items（AI 只能輔助描述，不能成為唯一證據）
- 不允許 Evidence 缺 provenance／hash／引用鍵仍標記成功
- 不允許繞過本層：L6/L7 不得在「無 Evidence Bundle」狀態下運行（Fail-Closed）
Inputs / Outputs（欄位級契約）
輸入：Feature Set（L4 `FeaturesComputed`；必要時僅以 input_refs 引用 Snapshot/State；不構成 Canonical Layer 之重新定義）
- `feature_set_id`
- `feature_schema_version`
- `indicator_set_ref`（或 `feature_set_ref`；工程需固定口徑）
- `structure_descriptor_ref`（可為 null；若存在必須以「結構描述」輸出）
- `feature_quality_flags[]`
- `input_refs[]`（至少包含：`snapshot_id`、`snapshot_store_ref`、`version_ref`、`hash_manifest_ref` 之引用鍵）
輸出：Evidence Bundle（L5 `EvidenceAssembled`）
必備欄位：
- `evidence_bundle_id`
- `evidence_schema_version`
- `evidence_items[]`（每一項至少包含：`item_type`、`source_ref`、`captured_at`、`hash_ref`、`time_window`）
- `provenance_map_ref`（來源追溯映射：item→來源→授權/條款→hash）
- `conflict_flags[]`（衝突/不一致標記；本層不裁決）
- `completeness_score`（完整度分數/等級；僅度量，不裁決）
- `completeness_missing_items[]`（缺口清單：缺哪些 item_type／缺哪些來源／缺哪些時間窗）
- `decision_context_type`（Evidence 必備集合之用途類型；對齊 DATA_SOURCES §30.1）
- `required_set_version`（本次用途類型所採用之 Required Evidence Set 版本）
- `completeness_level`（EC-0~EC-4；**裁決口徑以 level 為準**；對齊 DATA_SOURCES §29）
- `completeness_score`（0~100；僅輔助度量，不得替代 level）
- `missing_required_items[]`（缺口清單；等價/別名：`completeness_missing_items[]`；對齊 DATA_SOURCES §29 欄位）
- `quality_grade_summary`（各子類別 QG 分布摘要；對齊 DATA_SOURCES §29 欄位）
- `evidence_bundle_ref`（可回放載入引用；不可僅存在記憶體）
- `hash_manifest_ref`（Evidence Bundle 之 hash 鏈根）
- `version_ref`（與 input_refs 之版本互驗；不可缺）
狀態契約（State Contract）
- 進入條件：S04 `FeatureReady`
- 成功輸出：S05 `EvidenceReady`
- 失敗處置：
 - `EvidenceFail` → RETURN（回到 L4 補齊特徵或品質旗標；不得靜默補值後繼續）
 - EC（Evidence Completeness）處置（對齊 DATA_SOURCES §29–§30；本層僅計算與標記，不形成裁決）：
 - `completeness_level=EC-4` 或 `EC-3`：可輸出 `EvidenceReady`（允許進入 L6/L7；但必須保留 `missing_required_items[]` 與不確定性揭露）
 - `completeness_level=EC-2` 或 `EC-1`：一律 `EvidenceInsufficient` → RETURN（回到 L4/L3/L2 依缺口回補；不得進入 L6/L7）
 - `completeness_level=EC-0`：系統級硬物缺失 → BLOCK（不得進入 L6/L7；必須留痕並交由治理/Gate）
 - 若檢出以下任一「稽核不可成立」情形 → BLOCK（不得進入 L6/L7；必須留痕並交由治理/Gate）：
 - `evidence_items[]` 為空或僅有摘要文字
 - `provenance_map_ref` 缺漏
 - `hash_manifest_ref` 缺漏或驗證失敗
 - `version_ref` 缺漏或與 input_refs 不一致
風險/合規介面（本層需提供、但不裁決）
- `conflict_flags[]`：提供後續層做否決/限制的依據（本層不裁決）
- `completeness_level` + `missing_required_items[]`（與 `completeness_score`/`completeness_missing_items[]` 同步）：提供後續層判斷「證據不足」之結構化訊號（本層不裁決；裁決口徑以 level 為準）
- `provenance_map_ref` + `hash_manifest_ref` + `version_ref`：提供全鏈路稽核回放與合規追溯之最小集合
禁止（重申）
- 沒 Evidence Bundle 進入 Regime/Risk
- Evidence 缺 provenance
- 只留摘要不留 item refs
- 以 AI 文本替代 evidence_items（AI 只能輔助描述，不能成為唯一證據）
---
### L6｜Regime（市場狀態判定）
目的：判定市場狀態（Regime），作為策略啟用之前置條件（Regime 高於策略）。
定位與邊界（What / Why / Not）
- What：將 L5 的 Evidence Bundle（證據包）轉為可稽核、可回放之 Regime State（市場狀態工件），供 L7（Risk/Compliance）與後續層一致引用。
- Why：若未固化 Regime，後續層可能以不同證據片段/不同假設自行判讀市場狀態，造成不可重現與跨層混讀。
- Not（明確禁止）：
 - Regime 由單一訊號決定（禁止 single-signal regime）。
 - Regime 不明確仍往下推進（允許 STOP；UNCERTAIN/STOP 必須明示）。
 - 以策略績效、AI 推薦或敘事文本覆寫 Regime。
 - 在本層輸出任何交易方向/下單參數（屬 L8–L11）。
輸入（Inputs｜欄位級契約）
- evidence_bundle_ref（L5）
- evidence_bundle_id（可追溯鍵）
- decision_context_type（用途類型；例：REGIME_DECISION）
- required_set_version（必備集合版本）
- completeness_level（EC-0~EC-4；裁決口徑以 level 為準）
- missing_required_items[]（缺口清單；若 EC<門檻則必須產出 UNCERTAIN/STOP）
- conflict_flags[]（衝突標記；本層不做合規否決，但必須揭露衝突）
- evidence_items[]（以 refs 引用即可；不得以摘要取代 item refs）
- provenance_map_ref / hash_manifest_ref（Evidence chain）
- as_of（資料截點；由 Evidence 推導或明示）
- version_ref（與 Evidence 互驗；不可缺）
輸出（Outputs｜Regime State 工件；欄位級契約）
事件：RegimeDetermined（L6）
- regime_state_id
- regime_state_ref（可載入引用；必須落盤，不得只在記憶體）
- regime_schema_version
- regime_axes[]（可多軸；每軸至少含 axis_name / axis_value / confidence）
 - 允許軸例：RISK_APPETITE（ON/OFF/NEUTRAL）、VOLATILITY（LOW/HIGH）、LIQUIDITY（NORMAL/STRESS）、EVENT_RISK（NONE/ELEVATED）
- regime_label（主標籤；由多軸聚合而成，禁止單一訊號直出）
- regime_confidence（0.0–1.0）
- regime_decision（ENUM）：OK / UNCERTAIN / STOP
- regime_reason_codes[]（可機器判讀；例：EVIDENCE_INCOMPLETE、EVIDENCE_CONFLICT、VOLATILITY_SPIKE、LIQUIDITY_STRESS、EVENT_RISK_ELEVATED）
- regime_constraints_ref（對後續層的「可用策略集合/限制」之引用；不得越權覆寫 L7；僅提供 regime-side 約束）
- invalidation_triggers[]（可觀測失效條件；需可回放驗證）
- evidence_bundle_ref（回指 L5；不得缺）
- citations_ref（指向證據項 refs 的引用表；不得以敘事替代）
- hash_manifest_ref（Regime 工件指紋；需可追溯）
- version_ref（與 Evidence/Active Version Map 互驗）
狀態契約（Contract｜狀態轉移）
- 進入條件：S05（L5）EvidenceReady
- 成功輸出：S06（L6）RegimeReady
- 特殊輸出：
 - 若 completeness_level 低於門檻或 missing_required_items[] 非空且影響裁決：regime_decision=UNCERTAIN 或 STOP（二者擇一必明示），並填入 reason_codes 與缺口清單。
 - 若 conflict_flags[] 重大衝突無法消解：regime_decision=UNCERTAIN（或 STOP），不得以敘事補完。
- 失敗處置：RegimeFail → RETURN（回 L5 補齊證據/修正衝突；不得靜默重試到成功才記錄）
風險/合規介面（Interface）
- L6 只做「市場狀態判定」，不做 Risk/Compliance 否決；但 L7 以 Regime State + Evidence 作為 PASS/VETO 的必要輸入。
- 下游強制約束：
 - 若 regime_decision=UNCERTAIN 或 STOP：不得進入 L8 生成策略提案（L8 已明示限制）；僅允許回補證據或停止流程。
---
...
### 1) 變更清單（Changelog）
- ：L6 Layer QA Fix：補強 L6（Regime）欄位級 Inputs/Outputs、狀態契約、Fail-Closed 與禁止事項；維持 Regime 高於策略且不得單一訊號決定之原語義；不改寫其他層語義。
- ：L5_LAYER_QA_RECHECK_FIX：補強 L5 Evidence Bundle 之 EC（completeness_level/required_set_version/decision_context_type）欄位與 RETURN/BLOCK 規則（對齊 DATA_SOURCES §29–§30），並移除重複 L6 標頭；不改寫 Canonical 主幹語義；重算 BODY_SHA256。
- ：L5_LAYER_QA_FIX：補強 L5（Evidence）欄位級 I/O 契約、Fail-Closed（EvidenceReady/Fail/BLOCK）與 Hard No（不得以摘要/AI 文本替代證據），對齊 FULL_ARCH 7.4 Evidence Domain 契約；不改寫 Canonical 主幹語義。
- ：L4_LAYER_QA_FIX：補強 L4（Analysis）之層邊界、欄位級 I/O 契約（FeaturesComputed）、上下游狀態契約（FeatureReady/FeatureFail）與 Hard No 禁止事項；僅屬契約明確化與稽核可回放要求，不改寫 Canonical 主幹語義。
- ：L3_LAYER_QA_FIX：補強 L3（Snapshot & State）之層邊界、欄位級 I/O 契約（SnapshotCreated/StateCacheCreated）、上下游狀態契約與 Hard No 禁止事項；僅屬契約明確化與稽核可回放要求，不改寫 Canonical 主幹語義。
- ：L2_LAYER_QA_FIX（補強 L2 之邊界、欄位級 I/O 契約、補值/語義偷換禁制與 QAFail 停止規則之文字化；對齊 DATA_SOURCES=23.2 與 FULL_ARCH=7.1.2；維持 Canonical 主幹語義不變；重算 BODY_SHA256。）
- ：L1_LAYER_QA_FIX（補強 L1 之邊界、I/O 欄位契約、承接規範與 fallback 降級標記；對齊 DATA_SOURCES=23.1/22.3 與 ARCH_FLOW=7.1；維持 Canonical 主幹語義不變；重算 BODY_SHA256。）
- ：L11_LAYER_QA_FIX（收斂 L11 之層級語義為『全層稽核回放與系統維運（非授權／非下單）』；明確交易執行為非 Layer 模組（EXECUTION_CONTROL）且僅能在 L10 授權後運作；補強 L11 最小 I/O 契約（refs）與 Replay Verification 錨定（VERSION_AUDIT/ARCH_FLOW）；修正 Canonical Boundary Declaration 避免 L9/L11 越權混讀；重算 BODY_SHA256。）
- ：L10_LAYER_QA_FIX（補強 L10 最低可機器化 Inputs/Outputs 契約；新增 HumanDecisionRecord_v1、AuthorizationEnvelope_v1；補齊 decision_signature ↔ human_approve_signature Canonical 對齊；明確 L10 與 Gate/Risk/Execution 硬切割；維持 Canonical 主幹語義不變；重算 BODY_SHA256。）
...
- ：FINAL_QA_NORMALIZE（基線日期與版本日期對齊；正文去重收斂並移除標籤化定位字樣；衝突裁決秩序文字化收斂並受上位裁決序位約束；稽核區塊固定為四件套；重算 BODY_SHA256。）
- ：L8_LAYER_QA_FIX（補強 L8 最小提案工件鍵與禁止輸出清單：價格/數量/買賣方向/可執行委託等；維持 Canonical 主幹語義不變；重算 BODY_SHA256。）
- ：L9_LAYER_QA_FIX（補強 L9 Report Artifact Schema 與 L9 Integrity Check Schema；明確 L9 自檢與主 Gate 的責任切割與 return_required_items 對位；維持 Canonical 主幹語義不變；重算 BODY_SHA256。）
### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256（UTF-8，LF）
- HASH_SCOPE：正文區（自檔首至『---\n## 稽核區塊』之前，不含稽核區塊）
- BODY_SHA256：0a0968cacf033b9913b0eb7c6bcdac0ec15f989c3819d4e9d94cd6f034be7f0f
### 3) 適用範圍（Scope）
本次覆蓋版修正範圍僅限於：
- L1（Data Source）～L5（Evidence）之層級定義補強、欄位級 I/O 契約、Fail-Closed/Return 規則之明文化（對齊 Changelog 所列各層 QA_FIX）。
- L8（Strategy Proposal）/ L9（Investment Report）/ L10（Human Decision）之「最小可回放工件鍵（refs）」「授權邊界」與「禁止越權」之條文澄清（不改寫 Canonical 主幹語義）。
- 稽核區塊之留痕一致性修正（Scope/Hand-off 用語一致化；不變更正文 hash 規則）。
不在本次修正範圍：
- 不新增/變更任何 Canonical Layer 之新增輸出型別（僅補強欄位級契約與失敗處置）。
- 不改寫裁決序位（DOCUMENT_INDEX → MASTER_ARCH → AI_GOV）與 Risk/Compliance 最高否決權之母法語義。
