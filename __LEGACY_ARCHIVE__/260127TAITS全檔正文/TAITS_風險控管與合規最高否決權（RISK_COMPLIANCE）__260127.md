...
### 1.3A HFI 承接 ≠ 執行（語義收斂｜全鏈路一致性硬規則）
> 本段為跨文件一致性收斂用之**硬邊界**：對齊 doc_key=DOCUMENT_INDEX 與 doc_key=AI_GOV 之「不得卡死承接」語義，同時維持本檔之「Risk/Compliance 最高否決權」語義不被削弱。

- **定義 1｜HFI 承接（Accept / Intake）**：指「接收命令、形成系統回應、產生必要稽核留痕」，不得被程序性理由阻斷（不得卡死承接）。
- **定義 2｜執行放行（Execution Allow）**：指「允許進入任何可能造成成交之行為（送單/改單/撤單/觸發）」；此權限**可且必須**受 Risk/Compliance 最高否決權約束。
- **定義 3｜否決對象（VETO Target）**：Risk/Compliance 的 VETO 指向「執行意圖與執行動作」，而非指向「承接與留痕」；因此 **HFI 不得被卡死承接，但可被 VETO 阻斷執行**。

#### 1.3A.1 HFI 三階段承接模型（不得混讀）
1) **RECEIVE（承接）**：收到 HFI → 必須產出 `hfi_receipt_ref`／`ui_trace_ref`／`audit_bundle_ref`（至少可追溯），並將命令內容封存至稽核鏈（依 doc_key=VERSION_AUDIT）。
2) **DISCLOSE（強制揭露）**：無論 PASS 或 VETO，均必須輸出完整風險揭露與替代方案；若 VETO，必須在 UI 明示 `veto_reason_codes[]` 與「禁止執行」之硬結果（對齊 doc_key=UI_SPEC 之呈現義務）。
3) **EXECUTION-GATE（執行閘門）**：
 - 若 Risk/Compliance=PASS：僅表示「可進入下一步授權/執行鏈路之必要條件之一已滿足」，仍需通過 Governance Gate 與 L10 人類授權，並接受 doc_key=EXECUTION_CONTROL 之 Preflight 檢核。
 - 若 Risk/Compliance=VETO：**必須阻斷任何執行動作**（Execution Allow = FALSE），即使有人類明確命令；但仍須完成 RECEIVE + DISCLOSE + 全紀錄（不得省略）。

#### 1.3A.2 狀態碼與事件落帳（最小集合）
- 當 HFI 存在且 Risk/Compliance=VETO 時，系統必須同時落帳：
 - `HFI_ACCEPTED`（承接完成；不可省略）
 - `EXECUTION_VETOED`（執行被否決；不可省略）
 - `veto_reason_codes[]`（原因碼）
 - `alternative_plans_ref`（替代方案引用）
 - `audit_bundle_ref`／`hash_manifest_ref`（可回放與不可抵賴）
- 禁止產生任何可被誤讀為放行或批准之語句；不得將 `HFI_ACCEPTED` 誤解為 `EXECUTION_ALLOWED`。

#### 1.3A.3 與其他治理構件之硬對齊（不可改寫）
- 與 doc_key=DOCUMENT_INDEX：一致—不得以程序性理由卡死承接；承接必留痕、可稽核。
- 與 doc_key=AI_GOV：一致—必須強制揭露、替代方案、全紀錄；不得以 AI 取代裁決主體。
- 與 doc_key=EXECUTION_CONTROL：一致—任何可能造成成交之行為均受 Preflight 與 Kill Switch 硬門檻約束；VETO 或授權缺件一律 BLOCK + AUDIT。
- 與 doc_key=MASTER_CANON：一致—L10 為唯一授權層；L11 為稽核回放層；本檔僅規範 Risk/Compliance 的否決、降級、隔離與放行條件，不改寫 Canonical Layer 定義。
...
- Evidence Bundle（L5）
- Regime State（L6）
- Strategy Proposal（L8：**非必備輸入**；若存在，僅作「可用性/風險映射」與「提案內容之風險約束一致性檢查」；**不得作為 Gate 前置條件**（不得形成『先策略後風控』之隱性路徑）；缺省時 L7 仍必須以 Evidence/Regime/Account/Rules 完成 PASS/VETO。）
- Account / Portfolio State（資金、倉位、未成交委託、曝險）
- Market Microstructure State（流動性、委買委賣深度、波動）
- 規則快照（TWSE/TAIFEX/券商限制/交易時段/停牌等）

> L7 裁決不得以「尚未生成 Strategy Proposal」作為延宕或拒絕裁決的理由；Strategy Proposal 僅能被視為『可選』的上下文參照，不得被升格為 L7 必要輸入。
### 3.3 Gate 輸出（Outputs）
...
（備註：RISK Gate 僅輸出 PASS/VETO；RETURN/BLOCK 屬 doc_key=GOVERNANCE_GATE_SPEC 之 Gate 裁決語義，UI 仍須承接其呈現與阻斷義務。）
...
- policy_version（可追溯）
- correlation_id（可稽核）
- 若 Governance Gate 為 RETURN：必須顯示「退回原因」與「需補齊清單」
...
### 1) Changelog（變更紀錄）
- ：L7_LAYER_QA_FIX_v3：全鏈路一致性修正：釐清 HFI『承接 ≠ 執行』之硬邊界；對齊 DOCUMENT_INDEX/AI_GOV 的不得卡死承接語義，同時維持 Risk/Compliance 對執行動作之最高否決權；補強狀態碼/落帳/對齊規則；重算正文指紋。
- ：L7_LAYER_QA_FIX_v2：澄清 RISK Gate 僅輸出 PASS/VETO；UI Must-Show 段落之 RETURN 係承接 Governance Gate 退回語義（避免混讀）；重算正文指紋。
- ：L7_LAYER_QA_FIX：釐清 L8 Strategy Proposal 為『非必備輸入』（僅可選參考，不得作 Gate 前置），避免循環依賴與混讀；不變更否決權或裁決邏輯；重算正文指紋。
- ：Final QA 正文化覆蓋版；去除箭頭順位與字母標籤等非正文助記，正文措辭正規化與結構重排（不縮減制度條文），並重建稽核四件套與重算指紋。

### 2) 指紋清單（Hash Manifest）
- BODY_SHA256：ac64af8c7a43e05641c85097b0d64f1e799c6c802072b9dd600fe50f25cc8b34
### 3) Scope（範圍）
- doc_key：RISK_COMPLIANCE
- baseline_date：（Asia/Taipei）
- version_date：（Asia/Taipei）
- scope_statement：本次覆蓋僅針對 L7（Risk/Compliance）之全鏈路一致性收斂：補強 HFI『承接 ≠ 執行』硬邊界（不得卡死承接，但可否決執行動作）；補強狀態碼/落帳與跨文件對齊（DOCUMENT_INDEX/AI_GOV/UI_SPEC/EXECUTION_CONTROL/MASTER_CANON）；不削弱既有制度裁決邏輯、否決權與約束強度。
