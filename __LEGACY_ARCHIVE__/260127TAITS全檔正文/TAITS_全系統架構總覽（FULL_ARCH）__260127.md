...
- 基線日期：[REDACTED_DATE]（Asia/Taipei）
- 版本日期：[REDACTED_DATE]（Asia/Taipei）
...
**典型子模組**：
- Regime Engine
- Regime Policy Map（Regime 對應允許策略類型映射）
...
```mermaid
flowchart TB
  D1[Data Domain<br/>L1-L2] --> STATE[State/Snapshot<br/>L3]
  STATE --> A1[Analysis Domain<br/>L4]
...
本節不重述 Canonical Flow 對 PASS／RETURN／VETO／STOP 等裁決狀態與行為的制度定義；一律以 doc_key=MASTER_CANON 為準。
本檔僅在系統架構層界定否決與阻斷的落地責任歸屬與通訊義務：
- 風險與合規裁決（含否決與退回）之制度與 reason_code 以 doc_key=RISK_COMPLIANCE 為準，由 Risk & Compliance Domain 落實並發佈事件。
- 治理閘門（Governance Gate）之裁決一致性檢核與缺口清單以 doc_key=GOVERNANCE_GATE_SPEC 為準，由 Governance Gate 模組落實並回報 UI。
- 執行控制之阻斷（例如：Token／通道健康／審計完整性缺失）以 doc_key=EXECUTION_CONTROL 為準，由 Execution Domain 落實並回報 UI。
- 任一否決、阻斷或退回事件，必須同時滿足：UI 可視化提示、可追溯引用鍵、以及稽核留痕（事件契約與 Trace 規格見 4.3 與第 7 章）。
...
- `module_id`：產出模組
- `timestamp_utc`
- `active_version_map_ref`：版本映射引用（缺失即阻斷（SYS-VERSION））
- `version_ref`：版本定版參照（doc/policy/model/rule；缺失需依 Gate 規範採 RETURN；不可追溯需採 BLOCK）
- `input_refs[]`：輸入引用（含 provenance）
- `output_refs[]`：輸出引用
- `hash_manifest_ref`：完整性校驗（缺失即阻斷（SYS-HASH/SYS-AUDIT））
...
必備欄位（除 6.1 共同硬性欄位外；並對齊 doc_key=DATA_SOURCES 之 L1 最小欄位集合）：
- `source_id`（資料源主鍵，需存在於 DATA_SOURCES（source registry））
- `subcategory_id`（資料子類別主鍵；用於綁定解析器/失效模式/降級策略；來源：DATA_SOURCES）
- `source_type`（TWSE/TAIFEX/MOPS/TDCC/券商/第三方等）
- `fetch_window`（start/end）
- `captured_at`（抓取完成時間戳；與 `timestamp_utc` 區分：captured_at 為資料抓取時點）
- `raw_payload_ref`
- `provenance_ref`（官方網址/端點/回應頭/時間戳/回應摘要或校驗）
- `quality_report_ref`（至少包含：延遲、缺漏、格式錯誤、值域檢查；來源：DATA_SOURCES）
- `hash_manifest_ref`（完整性校驗；亦屬 6.1 共同硬性欄位，為避免實作漏傳，此處再列為必備）

禁止：
- 靜默補值、改值或任何欄位語義變更（L1 不做 Normalization；若需正規化僅能在 L2）
- 把第三方資料假冒官方或授權來源（來源等級與授權條件以 DATA_SOURCES 為準）
...
- `canonical_schema_id`
- `subcategory_id`（避免跨子類別混讀；與 L1/L3/L4 之引用一致）
- `normalization_ruleset_version`
- `field_map_ref`（原欄位對應標準欄位）
- `data_quality_flags[]`（正式欄位名；跨模組/跨文件引用一律以 `data_quality_flags[]` 為準）
- `quality_flags[]`（legacy alias；與 `data_quality_flags[]` 同義。若出現在事件/日誌/舊介面，必須在同一工件內提供對應映射，避免新舊混讀）
...
**輸入**：Feature Set（必要時僅以 input_refs 引用 Snapshot/State；不構成 Canonical Layer 之重新定義）  
...
補強（Only-Add｜L6 Layer QA FIX）：狀態碼／原因碼／信心尺度（僅屬介面契約與稽核呈現；不構成 Canonical Flow 重新定義）

- 新增必備欄位（Regime State）：
  - `regime_status`：`CONFIRMED | UNCERTAIN | STOP`
  - `regime_supporting_evidence_refs[]`：支撐本次 Regime 的 Evidence Item refs（**最少 2 項**；不足 2 項不得輸出 `CONFIRMED`）
  - `uncertainty_reason_codes[]`：當 `regime_status=UNCERTAIN` 時必填（例：`EVIDENCE_INCOMPLETE`、`EVIDENCE_CONFLICT`、`DATA_STALE`、`MODEL_VERSION_MISMATCH`）
  - `stop_reason_codes[]`：當 `regime_status=STOP` 時必填（例：`NO_EVIDENCE`、`NO_PROVENANCE`、`TIME_DESYNC`、`SNAPSHOT_MISSING`、`POLICY_BLOCK`）

- `regime_confidence` 報告尺度（Reporting Convention）：
  - 範圍：`0.00–1.00`
  - 建議分段（僅供呈現與一致性；不構成裁決邏輯）：`HIGH (>=0.80)`／`MED (0.60–0.79)`／`LOW (0.40–0.59)`／`VERY_LOW (<0.40)`
  - 若 evidence 缺口或衝突導致信心不足：不得強行報告高信心；至少應降級為 `UNCERTAIN` 並填寫 `uncertainty_reason_codes[]`

- 單一訊號限制（對齊 MASTER_CANON 禁止「單一訊號決定 Regime」）：
  - `regime_supporting_evidence_refs[]` 不得只有 1 項；若僅 1 項，必須以 `UNCERTAIN` 或 `STOP` 表示，並在 reason codes 留痕。
...
對齊 Canonical（doc_key=MASTER_CANON）之最小可回放提案工件鍵（必備 refs；不得省略）：
- `strategy_run_id`
- `strategy_id` / `strategy_version`
- `parameter_set_ref`
- `evidence_bundle_ref`（L5）
- `regime_state_ref`（L6）
- `risk_pass_token_ref`（L7，PASS only；僅作引用憑證，不得取代下游驗證）
- `output_proposal_ref`（Strategy Proposal 主體引用；必落盤/可回放）
- `explanation_bundle_ref`（可解釋摘要＋依據 refs；不得取代證據）
- `hash_manifest_ref`（正文指紋）

入場硬條件（Fail-Closed）：
- 必須已取得 L7=PASS 且提供 `risk_pass_token_ref`。
- 必須已取得 L6 Regime State，且 `regime_decision` ≠ UNCERTAIN/STOP；若為 UNCERTAIN/STOP，L8 不得生成提案（僅允許 RETURN 或 STOP）。

補充說明（本段為介面契約補強；不重新定義 Canonical）：
- 本節既有之 `proposal_id` / `scenario_ref` / `assumptions_ref` / `limitations_ref` / `expected_behavior_ref` 等，屬「提案內容結構化欄位」，不得用以替代上述必備 refs。

硬禁：
- 不得產生下單指令
- 不得把特徵直接等同方向（特徵≠方向）
- 不得策略直連下單、呼叫任何券商／Broker API／Execution 模組
- 不得覆寫風控條件或推翻 L7 VETO（Risk/Compliance 最高否決權）
- 不得直接輸出任何可執行委託或等價欄位（包含但不限於：價格、數量、買賣方向、委託型別、帳戶、交易所/通路、立即可送單之指令碼）
- 不得以績效／勝率／AI 推薦對抗 Regime 或 Risk/Compliance 之否決與限制
...
> 重要聲明：本節為「E2E Trace 欄位命名範本／介面對位示意」，不構成對 Canonical Flow（L1–L11）之再定義。
> 若本節與 doc_key=MASTER_CANON 之任何語義、欄位口徑或層級契約不一致，一律以 MASTER_CANON 為準；本節僅用於工程落地之追溯清單命名。
...
- L3：`snapshot_store_ref` + `version_ref`
...
狀態：BLOCK（SYS-AUDIT）時，禁止 APPROVE

VETO/RETURN 狀態仍可按 APPROVE

狀態：嚴重違規（GOV-SCOPE）時，必須阻斷並稽核
...
### 層級不可跳步（L1–L11）
為符合 doc_key=MASTER_CANON 之「層級不可跳步」主幹原則，本系統在治理與介面層必須具備以下能力：
- doc_key=GOVERNANCE_GATE_SPEC：提供跳層檢測與缺口清單（缺口需可追溯至引用鍵）。
- doc_key=UI_SPEC：對 RETURN 之回補需求提供可操作導引（包含跳轉與提示），確保使用者可完成補齊後再進入後續裁決。
...
### 禁止「層間回寫」的具體定義
為符合 doc_key=MASTER_CANON 對不可變工件、可追溯回放與制度一致性的要求，執行結果回寫僅允許更新「帳戶狀態快照」與「稽核工件」。
為避免事後污染上游語義與證據鏈，以下內容一律禁止被回寫或覆寫：
- L4 的特徵定義（語義層）
- L5 的證據內容（不得改證據）
...
### 1) 變更清單（Changelog）
- [REDACTED_DATE]：L8_LAYER_QA_FIX_RECHECK（Markdown bullet 修正：硬禁條列符號格式化；不改寫任何制度語義；重算 BODY_SHA256。）
- [REDACTED_DATE]：L8_LAYER_QA_RECHECK（補強 L8 Hard 禁止事項清單，使 FULL_ARCH 與 doc_key=MASTER_CANON L8 禁止輸出一致；維持 Only-Add；重算 BODY_SHA256。）
- [REDACTED_DATE]：L8_LAYER_QA_FIX（補強 L8 `StrategyProposed` 介面契約：新增 Canonical 最小 Proposal 工件鍵 refs、入場硬條件（L7 PASS token + Regime 非 UNCERTAIN/STOP）、與禁止輸出清單之對齊聲明；不改寫 doc_key=MASTER_CANON 主幹語義；重算 BODY_SHA256。）
- [REDACTED_DATE]：L2_LAYER_QA_FIX（補齊 `DataNormalized`（L2）必備欄位，對齊 DATA_SOURCES=23.2；明確 quality_flags legacy alias 映射規則；維持既有禁止事項；重算 BODY_SHA256。）
- [REDACTED_DATE]：L1_LAYER_QA_FIX：對齊 L1 `DataIngested` 介面契約（補齊 `subcategory_id` / `captured_at` / `quality_report_ref` / `hash_manifest_ref`），並明確禁止靜默補值與第三方冒充官方；不改寫 MASTER_CANON 主幹語義。
- 新增：E2E Trace Manifest 區塊之『非再定義』邊界聲明，避免與 MASTER_CANON 混讀。
- 更新：基線日期／版本日期對齊 [REDACTED_DATE]（Asia/Taipei），以避免跨檔新舊混讀。
- [REDACTED_DATE]：L6_LAYER_QA_FIX：補強 L6 `RegimeDetermined` 之狀態碼/原因碼/信心報告尺度與多證據限制（Only-Add），對齊 MASTER_CANON「不得由單一訊號決定 Regime」與「Regime 不明確可 STOP」之原則；同步重算正文指紋。
- [REDACTED_DATE]：L5_LAYER_QA_FIX：修正 L5 `EvidenceAssembled` 輸入欄位描述，避免對 doc_key=MASTER_CANON L5 之輸入定義造成特別化；同步重算正文指紋。
- [REDACTED_DATE]：L3_REQA_FIX：補強 L3 `SnapshotCreated` 必備欄位與 Event Bus 訊息契約加入 `version_ref`，使其與 Contract 共同硬性欄位及 E2E Trace Manifest 一致。
- [REDACTED_DATE]：L3_LAYER_QA_FIX：補強跨模組共同硬性欄位之 `version_ref` 定義，對齊 MASTER_CANON L3 對「版本引用」之要求，避免 Gate/ UI 端缺欄位而發生混讀。
...
- [REDACTED_DATE]：Final QA 正文化覆蓋版。處理重點包含：移除非正文痕跡、去除箭頭符號與治理位階字母標籤、收斂 Canonical Flow 引用口徑為 doc_key=MASTER_CANON、以及重算正文指紋。
### 2) 指紋清單（Hash Manifest）
- HASH_SCOPE：正文區（自檔首至『## 稽核區塊』之前，不含稽核區塊）
- HASH_METHOD：SHA-256（UTF-8，LF）
- BODY_SHA256：a0be0e13cce869795df112c1b37957d874162a141cfe43bd840ab04f4f5cf32e
### 3) 覆蓋範圍（Scope）
- 本次覆蓋版修正範圍：僅調整 7.1.2 `DataNormalized`（L2）之必備欄位清單與 alias 映射說明；不影響其他章節語義。
- 本次覆蓋僅限「正文正常化」：去重收斂、章節結構重排與一致化、移除非正文痕跡；不新增個股/投資建議內容，不改寫 doc_key=MASTER_CANON 之 Canonical Flow 主幹語義。
- 本檔定位：全系統架構總覽。輸出僅限架構對位、資料流/事件流、介面契約、失效模式、交叉一致性檢核。

### 4) 稽核交接（Audit Hand-off）
- 交接狀態：READY_FOR_REVIEW
- 驗收責任：產品負責人／架構裁決者
- 驗收要點：正文乾淨、無重複、無混讀；Canonical Flow 不被本檔重複定義或特別化；稽核四件套齊全且 Hash 可重算一致。
