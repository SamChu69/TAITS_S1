...
## 文件頭（Document Header）
- doc_key：UI_SPEC
- 治理位階：治理制度級（使用者介面與人機決策規範）
- 裁決序位（自高至低）：DOCUMENT_INDEX、MASTER_ARCH、AI_GOV
- Canonical Flow：L1–L11 之定義與層級語義以 doc_key=MASTER_CANON 為唯一正文來源；本文件不重複定義層級，只規範 UI 與人機裁決之呈現、交互、留痕、與風控否決承接。
...
#### Regime Panel（Regime State）必備呈現欄位（Only-Add｜對齊 FULL_ARCH 7.5.1 `RegimeDetermined`）

> **Canonical 對位（L6 Regime 輸出）**：UI 層不得另創 Regime 裁決欄位；僅可做『呈現映射』。
> - Canonical 欄位：`regime_decision`（OK / UNCERTAIN / STOP）
> - UI 呈現欄位：`regime_status`（CONFIRMED / UNCERTAIN / STOP）
> - 映射規則（固定，不得改寫語義）：`OK → CONFIRMED`、`UNCERTAIN → UNCERTAIN`、`STOP → STOP`
> - 證據呈現：必須以 Canonical 的 `citations_ref`（或其展開後 refs）顯示支撐證據；不得以敘事替代。

UI 必須能完整呈現 L6 Regime State（僅呈現，不得自行推導/裁決）：

- 基本狀態
 - `regime_label`
 - `regime_status`：`CONFIRMED | UNCERTAIN | STOP`
 - `regime_confidence`（0.00–1.00；不得在證據缺口時呈現高信心）

- 適用性約束（Actions/Constraints）
 - `allowed_strategy_classes[]`
 - `blocked_strategy_classes[]`

- 證據支撐（Evidence Support）
 - `regime_supporting_evidence_refs[]`（至少顯示「項目數量」與可點選 refs）
 - `regime_change_log_ref`（最近變化與切換原因之引用）

- 不確定／停止原因（Reason Codes）
 - 若 `regime_status=UNCERTAIN`：顯示 `uncertainty_reason_codes[]`
 - 若 `regime_status=STOP`：顯示 `stop_reason_codes[]`，並在 UI 內明確提示「流程不得推進」（直到原因解除或人工裁決）

備註：
- UI 不得將 `uncertainty_reason_codes[]` 或 `stop_reason_codes[]` 重新解讀為交易指令或策略方向。
- UI 若需提供說明文字，必須以 refs（input/output/hash/version）為主，敘事僅作輔助，不得取代可回放證據引用。
...
#### 1.2.1 裁決語義分離（Risk 的 VETO vs Governance 的 RETURN）
- **Risk/Compliance（L7）**：輸出 **PASS / VETO**。VETO 為最高否決；只要存在 VETO，任何其他狀態（含 Governance RETURN）不得被解讀為放行或可執行。
- **Governance Gate（治理閘門）**：輸出 **PASS / RETURN / BLOCK**。RETURN 表示流程/資料/契約缺口需要補件；不表示風險放行。
- **UI 行為硬限制**：只要任一面板出現 **VETO / STOP / BLOCK**，UI 必須鎖死任何「核准／送出執行」路徑；僅允許導向『補件重跑』、『查看稽核』、『下載證據/快照』等非交易動作。
- **原因碼呈現規則**：`veto_reason_codes[]`、`uncertainty_reason_codes[]`、`stop_reason_codes[]` 必須以原碼＋對照說明呈現；UI 不得自行改寫為交易指令或情緒性措辭。
...
- `human_approve_signature`（欄位別名）：供執行層驗證之人類授權簽章；其內容必須與 `decision_signature` **完全相同**（位元級一致）。
 - Canonical 對齊：`decision_signature`（UI/L10） ↔ `human_approve_signature`（EXECUTION_CONTROL）
 - 任一不一致：視為 Preflight FAIL，必須 BLOCK + AUDIT（不得以 UI 文句或人工推定補完）。
...
- 提供「跳轉到缺口所在面板」功能（例如缺 Evidence 時，直接跳至 Evidence Panel）

## 8. UI Decision Trace（決策追溯）規範

### 8.1 Decision Trace 的定義
Decision Trace 是「UI 交互行為與決策結果」的不可否認記錄，用於：
- 稽核
- 回放
- 追責
- 故障排查（非唯一用途）
...
> 補充：UI 送往 Execution 的授權輸入，必須包含 `human_approve_signature`（=decision_signature）與 `risk_pass_token`，並附 `trace_id` 以供跨模組對帳；UI 不得自行生成或修補 execution 參數。
### 9.1 UI 與 Execution 的輸出物（必須）
UI 在 APPROVE 後必須生成：
...
### 1) 變更清單（Changelog）
- ：L10_LAYER_QA_FIX（補強 UI 輸出欄位：新增 `human_approve_signature` 作為 `decision_signature` 之執行層別名；明確 Canonical 對齊與不一致處置（Preflight FAIL → BLOCK + AUDIT）；維持 UI 既有制度語義不變；重算 BODY_SHA256。）
- ：L7_LAYER_QA_FIX：補強 Risk/Compliance 的 PASS/VETO 與 Governance 的 PASS/RETURN/BLOCK 之語義分離提示與 UI 行為鎖定規則；重算正文指紋。
- ：L6_LAYER_QA_FIX：補強 UI 的 Regime Panel 必備呈現欄位（對齊 FULL_ARCH L6 `RegimeDetermined`：status／confidence／allowed/blocked／supporting evidence refs／reason codes），避免 Regime 狀態在 UI 端口徑漂移；同步重算正文指紋。
...
- BODY_SHA256：92739593e2ad3fd55f9101a19a58b82467158c02250510b33dd69f32177c41ef
### 3) 適用範圍（Scope）
- non_scope：不變更 MASTER_CANON 之 Canonical Flow 語義；不新增任何交易/策略建議；不進行任何個股/標的分析；不改寫風險與合規否決鏈之制度語義。

### 4) 裁決承接（Audit Hand-off）
- 本檔作為 UI 規範（doc_key=UI_SPEC）之單一正確正文版；層級語義一律承接 MASTER_CANON。
- 後續如需新增 UI 欄位/頁面/流程，僅可於本檔新增並於 VERSION_AUDIT 留痕；不得以其他文件覆寫本檔之 UI 制度語義。
