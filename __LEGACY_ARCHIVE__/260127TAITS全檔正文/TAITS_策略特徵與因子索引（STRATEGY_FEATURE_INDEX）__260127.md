...
治理位階：治理制度級（策略特徵與因子索引）
版本狀態：ACTIVE（正文乾淨｜最大完備覆蓋版）
版本日期：（Asia/Taipei）
基線日期：（Asia/Taipei）
裁決序位（自高至低）：DOCUMENT_INDEX、MASTER_ARCH、AI_GOV
Canonical Flow（L1–L11）唯一正文來源：MASTER_CANON；本檔僅為 Feature/Factor 治理與索引，不重複分層定義。
...
複製程式碼
{
 ...
 "version_ref": {
 "doc_key": ["STRATEGY_FEATURE_INDEX@", "DATA_SOURCES@"],
...
## 附錄A：跨文件承接與引用對位（MASTER_CANON／STRATEGY_UNIVERSE）
本附錄用於說明本文件與 MASTER_CANON、STRATEGY_UNIVERSE 的分工、引用方式與接口對位，供治理、稽核與工程落地使用。
## A.0 本段定位與效力聲明（不可省略）
...
### 1) 變更清單（Changelog）
- 新增：Canonical Flow（L1–L11）唯一正文來源（MASTER_CANON）之邊界聲明，避免特徵/因子索引被誤讀為分層正文。
...
- ：正文去重收斂與排版一致化；移除占位符與作業語句；不改寫治理語義。

### 2) 指紋清單（Hash Manifest）
- HASH_METHOD：SHA-256
- BODY_SHA256：64b9ebee826864fcadee1afe509e62f4c184f6979e73d6359d1177dfb163e32e
### 3) 作用範圍（Scope）
