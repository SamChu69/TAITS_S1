...
|---|---|---|---|---|---|
| TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__L1-L11_FULL_CHAIN_QA_ALIGN__OVERWRITE_v8.md | DOCUMENT_INDEX | 最高母法 | ACTIVE | | 治理有效文件清單（Authoritative Index）與衝突裁決程序之唯一權威索引 |
| TAITS_AI_行為與決策治理最終規則全集（AI_GOV）__FINAL_QA__NORMALIZE.md | AI_GOV | 最高母法 | ACTIVE | | AI 行為邊界、越權禁止、裁決序位與可稽核輸出規範 |

### 5.2 母體／憲法級（主權鐵律、Canonical、治理狀態、最高否決權與執行控制）
| 文件名稱（檔名） | doc_key | 治理位階 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|
| TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__L1-L11_QA__OVERWRITE.md | MASTER_ARCH | 母體／憲法級 | ACTIVE | | 母體總憲法、核心鐵律、權責邊界與上位裁決原則 |
| TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__L6_LAYER_QA_FIX__OVERWRITE.md | MASTER_CANON | 母體／憲法級 | ACTIVE | | Canonical Flow（L1–L11）之唯一正文來源與全資訊體系定錨 |
| TAITS_GOVERNANCE_STATE__FREEZE_v1.0__FINAL_QA__NORMALIZE.md | GOVERNANCE_STATE | 母體／憲法級 | ACTIVE | | 治理狀態：封版、凍結、解凍之宣告、切換條件與承接要求 |
| TAITS_風險控管與合規最高否決權（RISK_COMPLIANCE）__FINAL_QA__NORMALIZE.md | RISK_COMPLIANCE | 母體／憲法級 | ACTIVE | | 風險與合規最高否決權、停機、降級、隔離與合規裁決 |
| TAITS_交易執行與控制規範（EXECUTION_CONTROL）__FINAL_QA__NORMALIZE.md | EXECUTION_CONTROL | 母體／憲法級 | ACTIVE | | 交易執行、下單前檢核、路由、成交回報、異常處理與緊急停機、復歸 |

### 5.3 治理／制度級（系統規格、資料、策略、介面、部署、環境、版本稽核）
| 文件名稱（檔名） | doc_key | 治理位階 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|
| TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__FINAL_QA__NORMALIZE.md | GOVERNANCE_GATE_SPEC | 治理／制度級 | ACTIVE | | 治理閘門 Gate 定義、裁決模型、通過、阻擋、退回條件與留痕要求 |
| TAITS_全系統架構總覽（FULL_ARCH）__L8_LAYER_QA_FIX__OVERWRITE_v6.md | FULL_ARCH | 治理／制度級 | ACTIVE | | 全系統模組總覽與交互關係（不重複定義 L1–L11 語義） |
| TAITS_系統架構與流程細化說明（ARCH_FLOW）__L4_BLOCKER_UNBLOCK__OVERWRITE_v4.md | ARCH_FLOW | 治理／制度級 | ACTIVE | | 架構與流程細化（工程與資料事件流落地細節） |
| TAITS_資料來源全集（DATA_SOURCES）__L2_RECHECK_QA_ALIGN__OVERWRITE_v4.md | DATA_SOURCES | 治理／制度級 | ACTIVE | | 資料來源全集、合法性、取得方式、欄位契約與品質治理 |
| TAITS_TWSE交易規則參考彙編（TWSE_RULES）__FINAL_QA__NORMALIZE.md | TWSE_RULES | 治理／制度級 | ACTIVE | | 台灣市場制度引用層（TWSE、TPEX、TAIFEX）之規則彙編與引用原則 |
| TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__L1-L11_QA__OVERWRITE.md | STRATEGY_UNIVERSE | 治理／制度級 | ACTIVE | | 策略宇宙：策略分類、輸入輸出契約、啟用與禁用條件（策略不等於下單） |
| TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__L1-L11_QA__OVERWRITE.md | STRATEGY_FEATURE_INDEX | 治理／制度級 | ACTIVE | | 策略特徵與因子索引：定義、欄位、計算依賴與可追溯引用 |
| TAITS_使用者介面與人機決策規範（UI_SPEC）__L6_RECHECK_UI_MAP__OVERWRITE_v2.md | UI_SPEC | 治理／制度級 | ACTIVE | | UI 與人機裁決：L9 呈現、L10 人類裁決流程、L11 回放呈現與權限邊界 |
| TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__FINAL_QA__NORMALIZE.md | DEPLOY_OPS | 治理／制度級 | ACTIVE | | 部署、營運、日常運作制度；Evidence Chain 與可回放、可稽核要求 |
| TAITS_本地執行與運算環境規範（LOCAL_ENV）__FINAL_QA__NORMALIZE.md | LOCAL_ENV | 治理／制度級 | ACTIVE | | 本地執行與運算環境規範：依賴、資安、隔離、資源限制與重現要求 |
| TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__L1-L11_FULL_CHAIN_QA_ALIGN__OVERWRITE_v8.md | VERSION_AUDIT | 治理／制度級 | ACTIVE | | 版本控管、封版、變更留痕、稽核資料結構、Hash Manifest 與交付格式 |

### 5.4 操作／教學／工程定錨級（僅操作說明，不具治理裁決）
| 文件名稱（檔名） | doc_key | 治理位階 | 版本狀態 | 版本日期 | 說明 |
|---|---|---|---|---|---|
| TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__FINAL_QA__NORMALIZE.md | BEGINNER_GUIDE | 操作／教學／工程定錨級 | ACTIVE | | 新手教學與操作引導（僅操作說明，不具治理裁決） |
| TAITS_專案總覽與快速開始（README）__FINAL_QA__NORMALIZE.md | README | 操作／教學／工程定錨級 | ACTIVE | | 專案總覽與快速開始（僅導覽與操作，不具治理裁決） |
| TAITS｜程式開發流程定錨文件（Unified Process Anchor）__FINAL_QA__NORMALIZE.md | UNIFIED_PROCESS_ANCHOR | 操作／教學／工程定錨級 | ACTIVE | | 工程流程定錨（Phase 0–5 與交付規格；僅工程作業，不具治理裁決） |
...
|---|---|---|---|---|
| TAITS_PROJECT_PROMPT__FINAL_QA__NORMALIZE.md | TAITS_PROJECT_PROMPT | 非治理文件（不在第 5 章） | | 新對話啟動與專案使用脈絡（不得作為任何治理裁決或 Gate 依據） |

---
## 稽核區塊（Audit Section｜非正文）

### 1) 變更清單（Changelog）
- ：L1-L11_FULL_CHAIN_QA_ALIGN：修正第 5 章（Authoritative Index）之交付檔名口徑，使『doc_key → ACTIVE 檔名』與目前專案最新覆蓋版一致（含 doc_key=DOCUMENT_INDEX 自指向、MASTER_ARCH/ARCH_FLOW/DATA_SOURCES/STRATEGY_* 等檔名口徑對齊）；不改寫裁決序位與 doc_key=MASTER_CANON 主幹語義。
- ：L8_RECHECK（FULL_ARCH v5→v6：修正 Markdown 條列格式；ACTIVE 指向更新；不改寫裁決序位與 Canonical 主幹語義。）
- ：L8_ACTIVATION：更新第 5.3 章 ACTIVE 指向：FULL_ARCH → __L8_LAYER_QA_FIX__OVERWRITE_v4（補強 L8 Proposal 介面契約與 Canonical 最小工件鍵 refs 對齊；避免工程漂移與混讀；不改寫 doc_key=MASTER_CANON 主幹語義）。
- ：L6_ACTIVATION：啟用 L6（Regime）欄位級契約補強版 MASTER_CANON（__L6_LAYER_QA_FIX）；同步更新 VERSION_AUDIT 檔名口徑，確保 ACTIVE 指向唯一且避免新舊混讀。
- ：INDEX_ALIGNMENT（L1/L2）：更新第 5 章 ACTIVE 指向以消除新舊混讀：ARCH_FLOW → __L2_LAYER_QA_FIX__OVERWRITE_v3；DATA_SOURCES → __L1_LAYER_QA_FIX__OVERWRITE_v3；並維持裁決序位與 Canonical 唯一正文來源不變。
- ：L5_ACTIVATION：更新 ACTIVE 指向，啟用 MASTER_CANON=__L5_LAYER_QA_FIX__OVERWRITE（Evidence 欄位級契約補強）；維持其餘 doc_key 不變，以消除新舊混讀。
- ：L4_ACTIVATION：更新 ACTIVE 指向：MASTER_CANON → __L4_LAYER_QA_FIX__OVERWRITE，以使 L4（Analysis）欄位級契約與 Hard No 規範具裁決力；其餘 ACTIVE 不變。
- ：L3_ACTIVATION：更新 ACTIVE 指向：MASTER_CANON → __L3_LAYER_QA_FIX__OVERWRITE（L3 Snapshot/State 契約補強），其餘 ACTIVE 不變。
- ：L2_ACTIVATION：更新第 5 章 ACTIVE 交付檔名口徑，將 MASTER_CANON/FULL_ARCH/ARCH_FLOW/DATA_SOURCES 指向 __ 覆蓋版（L1/L2 Layer QA Fix），以消除新舊混讀；並維持裁決序位與 Canonical 唯一正文來源不變。
- ：CONSISTENCY_QA_FIX：於第 0.2 節（HFI）新增「承接≠執行；VETO 對象為執行放行/執行動作」之消歧義條文，對齊 RISK_COMPLIANCE 與 AI_GOV；Only-Add，不改寫既有語義。
- ：LAYER_QA_FIX（L1）：補強「Canonical Flow 唯一正文來源」與「欄位契約/Provenance/品質治理之落地規格承載（DATA_SOURCES）」之索引式說明，以降低新舊口徑混讀風險；不改寫 Canonical Flow（MASTER_CANON）之制度語義。
- FINAL_QA_FIX：統一文件索引之交付檔名口徑為 __FINAL_QA__NORMALIZE（替換原 __ 參照），以確保 ACTIVE 集合可追溯性與版本集合一致。
...
- ：Final QA 正文化：正文去重收斂、治理位階文字化（移除字母標籤）、移除箭頭順位／排序符號、更新第 5 章交付檔名口徑與基線日期，並重算 Hash Manifest。
