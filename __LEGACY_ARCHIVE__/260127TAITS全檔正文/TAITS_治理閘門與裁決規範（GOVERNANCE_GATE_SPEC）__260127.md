...
### 1) 變更清單（Changelog）
-：L9_LAYER_QA_FIX（新增 Gate 可選承接 `l9_integrity_check_ref` 之規範，並明確其不取代 Gate 裁決與 `return_required_items[]`；重算 BODY_SHA256。）
...
-：FINAL_QA_NORMALIZE（基線日期與版本日期對齊；移除順位符號與標籤化位階字樣；收斂 Gate 規則表述並避免 Canonical Flow 被重複定義；修正 Mermaid 節點命名避免單字母標籤；稽核區塊固定為四件套；重算 BODY_SHA256。）
