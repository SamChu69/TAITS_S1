...
## 附錄承接（Only-Add 補丁全文之唯一載體）

為避免主線正文出現「重複紀錄」與「新舊口徑並存」之誤讀風險，凡屬下列性質之全文條款，均以獨立附錄檔承接（主線僅保留指引，不重複貼全文）：

- doc_key / 實體檔名（physical_filename）對齊宣告（A1–A3）
- GLOBAL_ALIGNMENT_PATCH（G0–G5）
- ERRATA_PATCH_0001（E1–E4）

**唯一承接檔：** `TAITS_MASTER_CANON__APPENDICES_20251228_FREEZE_v1.0.md`
