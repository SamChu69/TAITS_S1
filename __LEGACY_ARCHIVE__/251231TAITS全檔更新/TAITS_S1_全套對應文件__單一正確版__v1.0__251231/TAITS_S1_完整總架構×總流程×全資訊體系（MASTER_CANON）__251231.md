# TAITS_S1｜完整總架構×總流程×全資訊體系（MASTER_CANON）
...
---
## Canonical Flow（L1–L11）｜終局層級職責（S1）

> 本節為 TAITS 工程端的「層級職責定義」，用於避免任何 AI/Agent 對層級功能的自行改寫或縮減。
> 核心鐵律：策略 ≠ 下單；Regime 高於策略；Risk/Compliance 可否決一切；AI 只做輔助不做最終決策。

### L1｜資料收集（Data Acquisition）
- 目的：收集多源資料（市場/基本面/消息面/技術面/籌碼面/規則/內部資料）。
- 輸出：`raw_payload`（含來源、時間戳、版次、授權與可追溯 metadata）、`source_manifest`。

### L2｜資料清洗與標準化（Cleaning & Normalization）
- 目的：清洗、去噪、對齊時區/交易日、欄位標準化、缺失處理。
- 輸出：`normalized_dataset`、`data_quality_report`（缺失/異常/修補規則/可疑點）。

### L3｜特徵生成（Feature Engineering）
- 目的：由資料生成可比較、可回測、可審計的特徵/指標（數值計算以程式為主）。
- 輸出：`feature_store_snapshot`（含計算版本/參數）、`feature_explain`（人類可讀摘要）。

### L4｜多面向分析（Multi-Aspect Analysis）
- 目的：將消息/基本/技術/籌碼/事件/規則等面向拆解分析，形成可驗證論點。
- 輸出：`analysis_pack`（面向分解＋證據鏈）、`assumption_list`（假設與風險點）。

### L5｜證據包與引用（Evidence Pack）
- 目的：把所有論點所需的證據整理成可追溯的引用包，避免「憑感覺」。
- 輸出：`evidence_pack`（含引用、來源、時間戳、摘要、置信度）。

### L6｜Regime / Context（市場狀態與情境）
- 目的：判定市場/標的處於何種狀態（趨勢/震盪/高波動/事件期等），作為策略選擇上位條件。
- 輸出：`regime_label`、`regime_rationale`、`regime_confidence`、`regime_change_triggers`。

### L7｜風險與合規（Risk & Compliance）
- 目的：將風險與合規作為最高否決權，對策略候選與授權封套做硬性約束。
- 輸出：`risk_checklist`、`compliance_checklist`、`veto_or_constraints`（可否決/限制條件）。

### L8｜策略候選生成（Strategy Candidate Set）
- 目的：在 Regime 與風控合規的約束下產生策略候選（候選≠下單）。
- 輸出：`candidate_strategies`（含適用條件、預期行為、失效條件、風險映射）。
