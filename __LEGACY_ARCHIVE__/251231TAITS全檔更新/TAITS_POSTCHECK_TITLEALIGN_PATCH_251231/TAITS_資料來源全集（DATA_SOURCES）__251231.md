...
來源檔：TAITS_資料來源全集（DATA_SOURCES）__251219__ADDENDUM_0_2C_FINAL.md
-->

# TAITS_資料來源全集（DATA_SOURCES）__251231
doc_key：DATA_SOURCES  
治理等級：B（Data Sources Spec｜原標示：D（DataSources Universe｜官方優先 × 多層Fallback × 可追溯Provenance）  ）
...
> 適用文件：TAITS_資料來源全集（DATA_SOURCES）__251231.md  
...
- 任何出現 `doc_key：DATA_SOURCES` 之舊標示，均屬「Metadata Discrepancy」的歷史殘留標記；其法律效力被本 Addendum 0 否認
...
> 補充性質：Only-Add（只可新增，不可刪減、覆寫、偷換既有語義）  
> 適用文件：TAITS_資料來源全集（DATA_SOURCES）__251231.md（doc_key：DATA_SOURCES）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
...
source_doc_name = TAITS_資料來源全集（DATA_SOURCES）__251231.md
...
ref_notes = 若摘錄內含「doc_key：DATA_SOURCES」字樣，依 Addendum 0/0.1 視為 alias，不具法律效力
```
...
> 補充性質：Only-Add（只可新增，不可刪減、覆寫、偷換既有語義）  
> 適用文件：TAITS_資料來源全集（DATA_UNIVERSE）__251219（doc_key：DATA_SOURCES）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON  
...
```text
ref_doc = TAITS_資料來源全集（DATA_SOURCES）__251231.md
ref_doc_key = DATA_SOURCES
...
- canonical_filename（Index 裁決檔名）：`TAITS_資料來源全集（DATA_SOURCES）__251231.md`
...
- 任何跨文件引用本文件時，必須使用：`doc_key=DATA_SOURCES` + `canonical_filename=TAITS_資料來源全集（DATA_SOURCES）__251231.md`。  
...
> 稽核對位：VERSION_AUDIT｜Appendix A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0019`）  
