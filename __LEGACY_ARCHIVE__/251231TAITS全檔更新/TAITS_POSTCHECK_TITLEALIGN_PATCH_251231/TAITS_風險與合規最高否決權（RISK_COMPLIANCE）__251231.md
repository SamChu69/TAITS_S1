...
# TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231
...
> 適用文件：TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231.md（doc_key：RISK_COMPLIANCE）  
...
## 2.3 人類最高決策者命令之交互邊界（Only-Add｜Freeze v1.0）

> 補充性質：Only-Add（只可新增；不改寫既有否決權語義）  
> 目的：將「人類最高決策者之明確命令」與本文件之 **最高否決權** 關係固定成唯一可採用口徑，避免出現『命令凌駕否決』之歧義。  

### 2.3.1 統一裁決：否決權對「交易執行」不可被繞過
- **凡屬交易執行（Live / Paper 送單／委託）之鏈路**：Risk/Compliance 之 VETO 仍為硬性否決（VETO = NO-EXECUTE）。  
- 人類最高決策者可做的最高行為，限於：
  - (a) **停止系統 / 啟動 Kill Switch**（安全優先）
  - (b) **調整治理/風控參數**（必須留痕；並走 DOCUMENT_INDEX 定義之衝突裁決與 VERSION_AUDIT）
  - (c) **指定回到上游重做 Evidence/Regime**（要求補證據、補引用、補審計）

### 2.3.2 禁止誤讀：命令不可要求『跳過 VETO』直接下單
- 任何文字或流程若將「人類命令」解讀為可直接跳過本文件之否決判定，均屬越權誤讀（GOV-CONFLICT）→ 依 DOCUMENT_INDEX §6 保守處置（STOP/RETURN/BLOCK）。  

### 2.3.3 留痕要求（最小欄位）
凡涉及人類最高決策者命令且觸及風控/合規者，除 MASTER_ARCH Appendix 20251229 所列欄位外，至少追加：
- veto_snapshot = <本次 VETO 結果摘要與理由碼>  
- veto_ref = RISK_COMPLIANCE:<章節定位>  
- exec_ref = EXECUTION_CONTROL:<章節定位（PASS Token / Kill Switch）>  
...
timestamp = <ISO-8601>
doc_key = RISK_COMPLIANCE
doc_title = TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231.md
...
- canonical_filename（Index 裁決檔名）：`TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231.md`
...
- 任何跨文件引用本文件時，必須使用：`doc_key=RISK_COMPLIANCE` + `canonical_filename=TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231.md`。  
