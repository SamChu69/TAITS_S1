...
來源檔：TAITS_使用者介面與人機決策規範（UI_SPEC）__251219__ACTIVE_CLEAN_INLINEAPPENDIX_20251229.md
-->

# TAITS_使用者介面與人機決策規範（UI_SPEC）__251231
doc_key：UI_SPEC  
治理等級：B（UI Spec｜原標示：A（Human-in-the-Loop Interface Charter｜人類主權唯一入口）  ）
...
> 適用文件：TAITS_使用者介面與人機決策規範（UI_SPEC）__251231.md（doc_key：UI_SPEC）  
...
- canonical_filename（Index 裁決檔名）：`TAITS_使用者介面與人機決策規範（UI_SPEC）__251231.md`
...
- 任何跨文件引用本文件時，必須使用：`doc_key=UI_SPEC` + `canonical_filename=TAITS_使用者介面與人機決策規範（UI_SPEC）__251231.md`。  
...
## Appendix 20251229｜Only-Add：UI 層之「人類最高決策者明確命令」處置規範｜對齊 DOCUMENT_INDEX

補充性質：Only-Add（只可新增；用於消解 UI 端解讀歧義；不新增治理位階）  
適用文件：TAITS_使用者介面與人機決策規範（UI_SPEC）__251231.md（doc_key：UI_SPEC）  
