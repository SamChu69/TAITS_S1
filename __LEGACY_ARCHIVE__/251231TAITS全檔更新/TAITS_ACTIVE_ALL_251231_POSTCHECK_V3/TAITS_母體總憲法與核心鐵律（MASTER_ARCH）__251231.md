...
1) 當存在「人類最高決策者的明確命令」時：  
   ...
   - 【Only-Add｜解讀限縮】本處之「不得以程序性理由阻止」僅適用於**文件治理／版本裁決／工程部署等治理行為**之程序性阻擋（例如：引用格式、清單快照、AI 內規）。  
   - 本條款**不授權**繞過外部法規/交易所規則（TWSE/TAIFEX），亦**不授權**繞過 RISK_COMPLIANCE 之合規否決、或 EXECUTION_CONTROL 之執行安全機制（PASS Token / Kill Switch）。  
   - 凡命令涉及「實盤交易執行」且與上述硬機制產生張力：必須採保守處置（STOP/RETURN/BLOCK），並以 VERSION_AUDIT 留痕回放。  
