...
來源檔：TAITS_PROJECT_PROMPT__ADDENDUM_20251227_FINAL.md
-->

# TAITS_PROJECT_PROMPT｜TAITS 專案專屬啟動 Prompt（Project Lock Prompt）__251231
...
> 適用文件：TAITS_PROJECT_PROMPT__251231.md（doc_key：PROJECT_PROMPT）  
...
A+：
1) `TAITS_AI_行為與決策治理最終規則全集__251231.md`（doc_key：AI_GOV）

A / A+（裁決與母法）：
2) `TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251231.md`（doc_key：DOCUMENT_INDEX）
3) `TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__251231.md`（doc_key：MASTER_ARCH）
4) `TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__251231.md`（doc_key：MASTER_CANON）

B（系統治理主幹）：
5) `TAITS_全系統架構總覽（FULL_ARCH）__251231.md`（doc_key：FULL_ARCH）
6) `TAITS_系統架構與流程細化說明（ARCH_FLOW）__251231.md`（doc_key：ARCH_FLOW）
7) `TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231.md`（doc_key：RISK_COMPLIANCE）
8) `TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__251231.md`（doc_key：GOVERNANCE_GATE_SPEC）
9) `TAITS_交易執行與控制規範（EXECUTION_CONTROL）__251231.md`（doc_key：EXECUTION_CONTROL）
10) `TAITS_使用者介面與人機決策規範（UI_SPEC）__251231.md`（doc_key：UI_SPEC）
11) `TAITS_完_部署、TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251231.md`（doc_key：DEPLOY_OPS）
12) `TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md`（doc_key：LOCAL_ENV）
13) `TAITS_完_版本控管、TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251231.md`（doc_key：VERSION_AUDIT）
14) `TAITS_資料來源全集（DATA_SOURCES）__251231.md`（doc_key：DATA_SOURCES）
15) `TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251231.md`（doc_key：TWSE_RULES）
16) `TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__251231.md`（doc_key：STRATEGY_UNIVERSE）
17) `TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__251231.md`（doc_key：STRATEGY_FEATURE_INDEX）

C（操作與導覽）：
18) `TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__251231.md`（doc_key：BEGINNER_GUIDE）
19) `README｜TAITS README__251231.md`（doc_key：README）
