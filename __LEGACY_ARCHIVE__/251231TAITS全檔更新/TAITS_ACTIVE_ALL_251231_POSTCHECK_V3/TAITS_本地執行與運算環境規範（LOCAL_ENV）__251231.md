...
來源檔：TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md
-->

# TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231
doc_key：LOCAL_ENV  
...
> 適用文件：TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md（doc_key：LOCAL_ENV）  
...
- canonical_filename（Index 裁決檔名）：`TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md`
- canonical_doc_key（Index 裁決識別碼）：`LOCAL_ENV`
- 版本狀態：ACTIVE（Freeze v1.0；Only-Add）

### A2. 本專案目錄中的實體檔案（Physical Artifact）
- physical_filename（目前專案內實際檔名）：`TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md`
- 法律定位：實體檔名僅為「存放/分發/下載」之載體；治理裁決與引用身份一律以 **A1** 為準。

### A3. 引用合法性最小規則（不新增制度，只固定寫法）
- 任何跨文件引用本文件時，必須使用：`doc_key=LOCAL_ENV` + `canonical_filename=TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md`。  
