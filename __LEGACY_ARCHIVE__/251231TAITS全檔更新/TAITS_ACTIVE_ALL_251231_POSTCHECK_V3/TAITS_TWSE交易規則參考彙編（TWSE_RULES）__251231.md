...
來源檔：TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251219__ADDENDUM_20251227_FINAL.md
-->

# TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251231
doc_key：TWSE_RULES  
...
平行參照：FULL_ARCH / ARCH_FLOW / EXECUTION_CONTROL / VERSION_AUDIT / DATA_SOURCES（alias: DATA_UNIVERSE） / DEPLOY_OPS / UI_SPEC / LOCAL_ENV  
...
> 適用文件：TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251231.md（doc_key：TWSE_RULES）  
...
- `ref_doc_name = TAITS_資料來源全集（DATA_SOURCES）__251231.md`
...
- canonical_filename（Index 裁決檔名）：`TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251231.md`
...
- 任何跨文件引用本文件時，必須使用：`doc_key=TWSE_RULES` + `canonical_filename=TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251231.md`。  
...
> 補充性質：Only-Add（只可新增，不可刪減、覆寫、偷換既有語義）  
> 適用文件：TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251219__ADDENDUM_20251227_FINAL.md（doc_key：TWSE_RULES）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決序列：DOCUMENT_INDEX → MASTER_ARCH → MASTER_CANON（AI 行為規範仍受 AI_GOV 最高約束；衝突裁決流程依 DOCUMENT_INDEX §6）  
> 稽核對位：VERSION_AUDIT｜Appendix A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0004`）  
...
- 本文件 `doc_key` 既有標示為 `TWSE_RULES`；本 Addendum 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
