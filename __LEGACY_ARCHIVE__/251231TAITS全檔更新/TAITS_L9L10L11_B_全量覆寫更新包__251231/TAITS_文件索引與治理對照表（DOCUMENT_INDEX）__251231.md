...
| 文件名稱 | doc_key | 治理等級 | 版本狀態 | 說明 |
|---|---|---:|---|---|
| TAITS_GOVERNANCE_STATE__FREEZE_v1.1__251231.md | GOVERNANCE_STATE_FREEZE_V1 | A | ACTIVE | Freeze v1.1 生效：鎖定 L9–L11（B 路徑）語義；B/C 文件變更須可回放且不得語義縮減 |
| TAITS_GOVERNANCE_STATE__UNFREEZE_v1.1__251231.md | GOVERNANCE_STATE_UNFREEZE_V1_1 | A | ARCHIVED | 變更窗口票據（已關閉）：僅授權本次 L9–L11 語義全面重編之整併式覆寫（保留舊文於 DEPRECATED 附錄） |
| TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md | GOVERNANCE_STATE_FREEZE_V1 | A | ARCHIVED | 歷史：Freeze v1.0（本次改版後退役；僅供追溯） |

> 註：同一 doc_key 僅能有一份 ACTIVE（ACTIVE 唯一性）；狀態切換必須同步更新 VERSION_AUDIT 留痕。
