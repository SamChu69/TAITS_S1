...
## 【本地工作站基線（User Workstation Baseline）｜可回放｜不可腦補】（更新日：2025-12-31（Asia/Taipei））

> 本段用於固定「你目前的實體開發/操作環境」以避免後續 AI 腦補或遺漏。  
> 本段若需更新，必須走 VERSION_AUDIT 留痕（含原因與變更日期）。

- OS：Windows 11
- 記憶體（RAM）：32 GB
- 裝置：ASUS TUF Gaming FX504GE（以你提供的機型資訊為準）
- CPU：Intel Core i7-8750H
- GPU：NVIDIA GeForce GTX 1050 Ti（4GB）
- 本地推論建議（概略）：
  - 可跑「小/中型」本地模型做 L1/L2/L11 之摘要與索引，但不建議用於 L9/L10 的主稿推理。
  - 若要上本地推論，需優先確保：資料隔離、金鑰不落地、與可回放稽核（見本文件後續章節）。
