...
> 【狀態更新｜2025-12-31（Asia/Taipei）】本次已完成 L9–L11 語義「B 路徑（全面重編）」並鎖定：  
> - L9 = 投資報告層（可見、可追蹤、可版本化）  
> - L10 = 人類裁決與交易授權  
> - L11 = 工程稽核（全層回放、雙料輸出）  
> 現行治理狀態應以 `TAITS_GOVERNANCE_STATE__FREEZE_v1.1__251231.md`（ACTIVE）為準；Freeze v1.0 視為 ARCHIVED。
...
| VA-METADATA_FIX-20251228-0012 | 2025-12-28 | BATCH-20251228-GLOBAL_ALIGNMENT | LOCAL_ENV | `TAITS_本地執行與運算環境規範（LOCAL_ENV）__251219.md` | ONLY-ADD | GLOBAL_ALIGNMENT_PATCH（MASTER_CANON 對齊全域補丁） | DOCUMENT_INDEX / MASTER_ARCH / MASTER_CANON / GOVERNANCE_GATE_SPEC |
...
# Addendum 2025-12-31｜Only-Add：Bundle 251231 Physical Map × Hash Ledger 補登（Freeze v1.0）

> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219.md（doc_key：VERSION_AUDIT）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：DOCUMENT_INDEX（A+）→ MASTER_ARCH（A）→ MASTER_CANON（A）＋AI_GOV（A+）  
> 目的：將 2025-12-31（Bundle 251231）之「實體檔案映射（Physical Artifact Map）」與「sha256_12 hash ledger」以稽核條目形式補登，確保 repo 落盤檔案可被回放與追溯；本 Addendum **不改寫**既有 Ledger/章節內容。

---

## Appendix A｜METADATA_FIX Ledger（Append-Only）— Extension Batch 2025-12-31（P0-PHYSICAL-ARTIFACT-MAP）

| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0001 | 2025-12-31 | BATCH-20251231-P0-PHYSICAL-ARTIFACT-MAP | Physical Artifact Map × Hash Ledger（__251231 落盤對齊） | 將 repo 內 22 份實體檔（含 21 ACTIVE + 1 SUPPORT）以 mapping + sha256_12 補登；供 Gate/Loader/稽核回放使用。對應 DOCUMENT_INDEX｜Addendum 2025-12-31（N1–N4）。 | DOCUMENT_INDEX, VERSION_AUDIT |

---

## Appendix P0｜Bundle 251231｜Physical Artifact Hash Ledger（sha256_12）
...
# Addendum 2025-12-31｜Only-Add：P1｜Reference Alias Expansion 補登（Freeze v1.0）

> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219.md（doc_key：VERSION_AUDIT）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 上位裁決：DOCUMENT_INDEX（A+）→ MASTER_ARCH（A）→ MASTER_CANON（A）＋AI_GOV（A+）  
> 目的：補登 P1 之 legacy 引用解析（Reference Alias Expansion）事件，使 Gate/Loader/稽核回放在 Freeze v1.0 期間可穩定解析舊檔名/工程路徑/舊 addendum/樣板引用。

---

## Appendix A｜METADATA_FIX Ledger（Append-Only）— Extension Batch 2025-12-31（P1-REFERENCE-ALIAS-EXPANSION）

| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0002 | 2025-12-31 | BATCH-20251231-P1-REFERENCE-ALIAS-EXPANSION | Reference Alias Map（legacy refs → physical） | 補登 90 筆 legacy 引用檔名解析（含舊版版本戳、README.md、工程路徑、舊 addendum 檔名、樣板檔名）。對應 DOCUMENT_INDEX｜Addendum 2025-12-31｜P1（P1.1–P1.3）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
# Addendum 2025-12-31｜Only-Add：P1B｜Reference Alias Extension 補登（Freeze v1.0）
...
## Appendix A｜METADATA_FIX Ledger（Append-Only）— Extension Batch 2025-12-31（P1B-REFERENCE-ALIAS-EXTENSION）

| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0003 | 2025-12-31 | BATCH-20251231-P1B-REFERENCE-ALIAS-EXTENSION | Reference Alias Extension（v4 補登） | 補登 21 筆 legacy 引用解析（多為 addendum/inlineappendix/工程路徑）。對應 DOCUMENT_INDEX｜Addendum 2025-12-31｜P1B（P1B.1–P1B.3）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
# Addendum 2025-12-31｜Only-Add：P1C｜Canonical Ref Alias Map 補登（Freeze v1.0）
...
## Appendix A｜METADATA_FIX Ledger（Append-Only）— Extension Batch 2025-12-31（P1C-CANONICAL-REF-ALIAS）

| fix_id | date | batch_id | scope | note | touched_docs |
|---|---:|---|---|---|---|
| VA-METADATA_FIX-20251231-0004 | 2025-12-31 | BATCH-20251231-P1C-CANONICAL-REF-ALIAS | Canonical Ref Alias Map（v5 補登） | 補登 39 筆 canonical 引用解析（舊版版本戳、`canonical_filename=...`、README 標題版）。對應 DOCUMENT_INDEX｜Addendum 2025-12-31｜P1C（P1C.1–P1C.3）。 | DOCUMENT_INDEX, VERSION_AUDIT |
...
## Appendix｜L9L10L11_SEMANTIC_REBASE Ledger（Append-only）

> 本帳本用於記錄「L9–L11 語義全面重編（B 路徑）」之版本留痕。  
> 任何後續對 L9–L11 之調整，必須新增條目；禁止覆寫既有條目。

### VA-L9L10L11_SEMANTIC_REBASE-20251231-0001
- effective_time：2025-12-31（Asia/Taipei）
- change_type：SEMANTIC_REBASE（B 路徑：L9=投資報告 / L10=人類裁決 / L11=工程稽核）
- scope：全文件宇宙（22 ACTIVE 核心治理檔案）
- key_decisions：
  - L9 不再作 Governance Gate；L9=投資報告（含數據/圖形/進出建議/追蹤鍵）
  - L10 為唯一交易授權入口（模式：不動作/回測/模擬/半自動/全自動）
  - L11 純稽核回放（全層留痕 + 雙料輸出），不得作執行入口
- additional_fix：
  - LOCAL_ENV 新增「本地工作站基線（Win11 / RAM 32GB / i7-8750H / GTX1050Ti）」以防遺漏與腦補
- approver：HUMAN_SOVEREIGN（人類最高決策者）
- reviewer：SYSTEM_ARCHITECT_AI（TAITS 核心系統工程師／架構治理審核員）
- artifacts：
  - 新增治理狀態：FREEZE v1.1（ACTIVE）、UNFREEZE v1.1（ARCHIVED）
  - 22 份治理文件已統一插入 L9–L11 Binding 定義（含 DEPRECATED 舊口徑處置）
