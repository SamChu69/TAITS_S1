...
來源檔：TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ACTIVE_20251229.md
-->
...
> 凡涉及「交易執行／合規風險／資金風險」之命令，如與上述硬機制產生張力：依 DOCUMENT_INDEX §6 採保守處置（STOP/RETURN/BLOCK）並留 VERSION_AUDIT 可回放留痕。  
...
### 5.4.x 表格解讀規則（Only-Add｜避免 bucket 誤判）

1) 本節標題僅為編排標籤，不構成治理等級（bucket）裁決依據。  
2) 任何讀取端（Gate / UI / Audit / Agent / Tooling）一律以表格欄位「治理等級」作為唯一有效 bucket 判定。  
3) 若標題分桶與表內「治理等級」欄位出現張力：以「治理等級」欄位為準；並依 DOCUMENT_INDEX §6 採保守處置（必要時 STOP/RETURN/BLOCK）。

### 5.5 C（Operational / Guide）
| 文件名稱 | doc_key | 治理等級 | 版本狀態 | 說明 |
|---|---|---:|---|---|
| TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__251231.md | BEGINNER_GUIDE | C | ACTIVE | 新手操作引導；不得新增治理權力 |
| README__251231.md | README | C | ACTIVE | 專案總覽、使用方式、責任聲明；不得越權 |
...
> 補充性質：Only-Add（只可新增，不可刪減、覆寫、偷換既有語義）  
> 適用文件：TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251231.md  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
...
> 補充性質：Only-Add（只可新增，不可刪減、覆寫、偷換既有語義）  
> 適用文件：TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251231.md（doc_key：DOCUMENT_INDEX）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0  
> 對位文件：TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251231.md（doc_key：VERSION_AUDIT）  
...
> 補充性質：Only-Add  
> 生效狀態：Freeze v1.0  
> 對位文件：TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__251231.md（doc_key：GOVERNANCE_GATE_SPEC）  
...
|---|---|---|---|---|
| TAITS_PROJECT_PROMPT__251231.md | PROJECT_PROMPT | B（Project Prompt & Operating Contract） | ACTIVE | 入口治理契約；裁決仍回歸 DOCUMENT_INDEX / MASTER_ARCH / MASTER_CANON；僅供新對話啟動與對齊 |
...
# Addendum 2025-12-28｜Only-Add：ACTIVE 實體檔名映射表（Logical→Physical）× doc_key Alias 對照 × 稽核雜湊（Freeze v1.0）
...
|---|---|---|
| AI_GOV | TAITS_AI_行為與決策治理最終規則全集__251231.md | TAITS_AI_行為與決策治理最終規則全集__251217__ADDENDUM_20251228H_FINAL.md |
| MASTER_ARCH | TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__251231.md | TAITS_母體總憲法與核心鐵律（MASTER_ARCH）__251219__ADDENDUM_20251227_FINAL.md |
| MASTER_CANON | TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__251231.md | TAITS_完整總架構×總流程×全資訊體系（MASTER_CANON）__251219__ADDENDUM_20251227_FINAL.md |
| GOVERNANCE_STATE_FREEZE_V1 | TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md | TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251220__ADDENDUM_20251228K_FINAL.md |
| DOCUMENT_INDEX | TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251231.md | TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228M_FINAL.md |
| ARCH_FLOW | TAITS_系統架構與流程細化說明（ARCH_FLOW）__251231.md | TAITS_系統架構與流程細化說明（ARCH_FLOW）__251219__ADDENDUM_20251227_FINAL.md |
| FULL_ARCH | TAITS_全系統架構總覽（FULL_ARCH）__251231.md | TAITS_全系統架構總覽（FULL_ARCH）__251219__ADDENDUM_20251227_FINAL.md |
| RISK_COMPLIANCE | TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251231.md | TAITS_風險與合規最高否決權（RISK_COMPLIANCE）__251219__ADDENDUM_20251227_FINAL.md |
| EXECUTION_CONTROL | TAITS_交易執行與控制規範（EXECUTION_CONTROL）__251231.md | TAITS_交易執行與控制規範（EXECUTION_CONTROL）__251219__ADDENDUM_20251227_FINAL.md |
| UI_SPEC | TAITS_使用者介面與人機決策規範（UI_SPEC）__251231.md | TAITS_使用者介面與人機決策規範（UI_SPEC）__251219__ADDENDUM_20251227_FINAL.md |
| DEPLOY_OPS | TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251231.md | TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251219__ADDENDUM_20251228_FINAL (1).md |
| LOCAL_ENV | TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md | TAITS_本地執行與運算環境規範（LOCAL_ENV）__251231.md |
| DATA_SOURCES | TAITS_資料來源全集（DATA_SOURCES）__251231.md | TAITS_資料來源全集（DATA_SOURCES）__251219__ADDENDUM_0_2C_FINAL.md |
| TWSE_RULES | TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251231.md | TAITS_TWSE交易規則參考彙編（TWSE_RULES）__251219__ADDENDUM_20251227_FINAL.md |
| STRATEGY_UNIVERSE | TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__251231.md | TAITS_策略宇宙全集（STRATEGY_UNIVERSE）__251219__ADDENDUM_20251227_FINAL.md |
| STRATEGY_FEATURE_INDEX | TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__251231.md | TAITS_策略特徵與因子索引（STRATEGY_FEATURE_INDEX）__251219__ADDENDUM_20251227_FINAL.md |
| VERSION_AUDIT | TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251231.md | TAITS_版本控管、稽核與可追溯治理規範（VERSION_AUDIT）__251219__ADDENDUM_20251228M_FINAL.md |
| GOVERNANCE_GATE_SPEC | TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__251231.md | TAITS_治理閘門與裁決規範（GOVERNANCE_GATE_SPEC）__251219__ADDENDUM_20251227_FINAL.md |
| BEGINNER_GUIDE | TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__251231.md | TAITS_新手教學與操作引導總則（BEGINNER_GUIDE）__251219__ADDENDUM_20251227_FINAL.md |
| README | README__251231.md | TAITS_專案總覽與使用說明（README）__251220__ADDENDUM_20251227_FINAL.md |
...
- 本 Addendum 不修改既有 Index 任何條目之治理位階/裁決語義。  
...
- physical_filename（目前專案內實際檔名）：`TAITS_文件索引與治理對照表（DOCUMENT_INDEX）__251220__ADDENDUM_20251228M_FINAL.md`
