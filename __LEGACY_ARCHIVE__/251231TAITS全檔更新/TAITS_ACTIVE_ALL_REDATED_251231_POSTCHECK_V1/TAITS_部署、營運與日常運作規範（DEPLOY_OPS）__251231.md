...
# TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251231.md
doc_key：DEPLOY_OPS  
...
> 適用文件：TAITS_部署、營運與日常運作規範（DEPLOY_OPS）__251231.md（doc_key：DEPLOY_OPS）  
...
> 稽核對位：VERSION_AUDIT｜Appendix A｜METADATA_FIX Ledger（條目：`VA-METADATA_FIX-20251228-0020`）  
...
- 本文件 `doc_key` 既有標示為 `DEPLOY_OPS`；本 Addendum 仍以 Index Gate 口徑再次鎖定其引用端合法身份。
