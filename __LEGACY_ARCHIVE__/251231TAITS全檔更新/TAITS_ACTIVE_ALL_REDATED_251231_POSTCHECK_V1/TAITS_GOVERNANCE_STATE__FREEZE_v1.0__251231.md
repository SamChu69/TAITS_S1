...
- canonical_filename（Index 裁決檔名）：`TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md`  
...
- `ref_file = TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md`
...
canonical:
  doc_key: GOVERNANCE_STATE_FREEZE_V1
  filename: "TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md"
aliases:
...
## Appendix A 讀取警示｜Deprecated Template Hard Block（Only-Add）

1) 本檔唯一合法治理識別：`doc_key = GOVERNANCE_STATE_FREEZE_V1`（以本檔文件頭為準）。  
2) Appendix A 內之歷史文字（含 `ref_doc_key = GOVERNANCE_STATE` 等字串）僅作追溯留痕，不得作為任何引用端（Gate / Audit / UI / Tooling）之有效模板來源。  
3) 任何系統若在解析/掃描時於 Appendix A 抓取到舊 `ref_doc_key`：一律視為「引用非法或取樣來源錯誤」，必須回到主線 S2-NEW 模板重新生成引用欄位。
...
> 適用文件：TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md（doc_key：GOVERNANCE_STATE）  
...
> 補充性質：Only-Add（只可新增，不可刪減／覆寫／偷換既有語義）  
> 適用文件：TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md（doc_key：GOVERNANCE_STATE_FREEZE_V1）  
> 生效狀態：GOVERNANCE_STATE = Freeze v1.0（本文件即為狀態宣告載體）  
...
```text
ref_doc: TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md
ref_doc_key: GOVERNANCE_STATE_FREEZE_V1
...
- 任何跨文件引用本文件時，必須使用：`doc_key=GOVERNANCE_STATE_FREEZE_V1` + `canonical_filename=TAITS_GOVERNANCE_STATE__FREEZE_v1.0__251231.md`。  
