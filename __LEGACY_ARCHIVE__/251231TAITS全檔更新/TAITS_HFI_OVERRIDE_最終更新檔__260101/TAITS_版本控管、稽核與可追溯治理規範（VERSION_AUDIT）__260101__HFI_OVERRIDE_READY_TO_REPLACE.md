...
## HFI Ledger｜人類最高決策者明確命令留痕（Mandatory）｜Only-Add

每次人類最高決策者以 HFI 授權更新／覆寫／重排版／全量替換時，必須新增一筆 Ledger（單筆為不可分割最小單位）：
...
- `action`：
- `intent`：
- `risk_disclosure_ref`：（Risk/Compliance 輸出引用／文件定位）
- `acknowledgement`：（確認文字）
- `change_id`：（對應本次變更批次 ID）
- `affected_docs`：（doc_key 清單）
- `apply_order`：（套用順序）
- `hash_manifest_ref`：（HASH_MANIFEST 版本／檔名）
- `changelog_ref`：（CHANGELOG 版本／檔名）

> 備註：HFI Ledger 用於「可回放／可稽核／可追溯」之承接，不構成對 HFI 的否決。
