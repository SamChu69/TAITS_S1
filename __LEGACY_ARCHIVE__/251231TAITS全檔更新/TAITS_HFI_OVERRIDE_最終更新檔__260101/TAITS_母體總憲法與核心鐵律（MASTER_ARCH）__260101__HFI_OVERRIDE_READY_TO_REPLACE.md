...
### 1.X Interpretation Note｜Human Sovereignty 與 Supreme Veto 之關係（消歧義）｜Only-Add
本文件所稱 Risk & Compliance (Supreme Veto) 為「**系統內最高否決**」（適用於**無**人類最高決策者明確命令之情境）。

當人類最高決策者之 **Valid HFI（Human Final Instruction）** 存在時：
- Risk/Compliance 必須輸出完整風險揭露與反對理由；
- 其結果不得轉為阻止人類命令之否決；改以「**強制揭露＋雙重確認＋全紀錄**」承接。
