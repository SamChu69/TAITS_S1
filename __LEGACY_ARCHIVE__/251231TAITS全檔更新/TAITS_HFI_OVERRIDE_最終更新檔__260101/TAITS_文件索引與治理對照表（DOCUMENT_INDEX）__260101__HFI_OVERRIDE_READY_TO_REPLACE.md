...
## HUMAN SOVEREIGNTY｜明確命令之可執行工件（HFI：Human Final Instruction）｜Only-Add

### 1) 定義
為避免「主權宣告存在但無法執行」之治理失效，本系統將人類最高決策者之「明確命令」制度化為可機讀、可稽核、可回放之工件：**HFI（Human Final Instruction｜人類最終指令）**。

### 2) 生效條件（Valid HFI）
下列欄位齊備，即視為有效 HFI；任何 Gate／流程不得以程序性理由拒絕辨識：

- `hfi_id`：`HFI-YYYYMMDD-###`（Asia/Taipei）
- `authority_actor`：人類最高決策者識別（姓名／代號）
- `authority_time`：Asia/Taipei 時戳
- `scope`：`doc_key`／章節路徑／全檔（明確指定）
- `action`：`overwrite` / `rewrite` / `reformat` / `replace` / `publish_s1` / `publish_s2` / `unfreeze`（至少一項）
- `intent`：目的（例如：統一語義、修正錯誤定位、全面重排版）
- `acknowledgement`：明確承擔與確認（例如：我已閱讀風險揭露並確認執行）

### 3) 覆蓋規則（HFI Override Rule）
當 **Valid HFI** 存在時：

- Freeze／Only-Add／Gate 限制不得阻止 **scope 範圍內**之更新、覆寫、重排版或全量替換。
- Risk/Compliance 必須輸出風險揭露與反對理由，但不得將其結果升格為阻止 HFI 執行之否決；改以「**強制揭露＋雙重確認＋全紀錄**」承接。

### 4) 稽核承接（Mandatory Audit）
所有 HFI 執行必須同步產生並更新：

- `VERSION_AUDIT`：新增 HFI Ledger 記錄（含 change_id／scope／影響清單）
- `HASH_MANIFEST`：全檔指紋清單
- `CHANGELOG`：逐檔變更摘要

（稽核用於回放與追責，不構成對 HFI 的否決。）
...
2) 若引用檔名為 legacy（舊版版本戳、README.md、工程路徑、舊 addendum 檔名、樣板檔名等），則依本 Addendum 之 alias mapping 解析為 `resolve_to_physical_filename`。  
