...
👉 **TAITS_12B：回測流程與Pipeline規格（落地工程版）**
包含：

* 每日/每週/每月 pipeline 的步驟（資料→特徵→Regime→矩陣→回測→報告）
* 模組 I/O schema（可直接交給 Cursor 寫程式）
* 回測模式（單策略、策略族、整體系統）
* 實驗編號規範（Experiment ID）
* 失敗處理與重跑策略（Retry/Checkpoint）
* 產出檔案目錄結構（你 GitHub 直接照做）
